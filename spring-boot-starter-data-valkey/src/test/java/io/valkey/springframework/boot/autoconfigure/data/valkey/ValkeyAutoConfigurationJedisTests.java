...
class ValkeyAutoConfigurationJedisTests {
    ...
    void testValkeyConfigurationWithPool() {
        this.contextRunner
                .withPropertyValues(
                        "spring.data.valkey.host:foo",
                        "spring.data.valkey.jedis.pool.min-idle:1",
                        "spring.data.valkey.jedis.pool.max-idle:4",
                        "spring.data.valkey.jedis.pool.max-active:16",
                        "spring.data.valkey.jedis.pool.max-wait:2000",
                        "spring.data.valkey.jedis.pool.time-between-eviction-runs:30000")
                .withUserConfiguration(JedisDisableStartupConfiguration.class)
                .run(
                        (context) -> {
                            JedisConnectionFactory cf = context.getBean(JedisConnectionFactory.class);
                            assertThat(cf.getHostName()).isEqualTo("foo");
                            assertThat(cf.getPoolConfig())
                                    .satisfies(
                                            (poolConfig) -> {
    ...
    void testValkeyConfigurationDisabledPool() {
        this.contextRunner
                .withPropertyValues(
                        "spring.data.valkey.host:foo", "spring.data.valkey.jedis.pool.enabled:false")
                .run(
                        (context) -> {
                            JedisConnectionFactory cf = context.getBean(JedisConnectionFactory.class);
                            assertThat(cf.getHostName()).isEqualTo("foo");
        ...
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            ...
                            assertThat(cf.getClientConfiguration().getConnectTimeout().toMillis())
                                    .isEqualTo(1000);
                        });
    }
    ...
    void testValkeyConfigurationWithSentinel() {
        this.contextRunner
                ...
                .run(
                        (context) ->
                                assertThat(JedisConnectionFactoryCaptor.connectionFactory.isValkeySentinelAware())
                                        .isTrue());
    }
        ...
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            assertThat(JedisConnectionFactoryCaptor.connectionFactory.isValkeySentinelAware())
                                    .isTrue();
                            assertThat(getUserName(JedisConnectionFactoryCaptor.connectionFactory))
                                    .isEqualTo("user");
                            assertThat(JedisConnectionFactoryCaptor.connectionFactory.getPassword())
                                    .isEqualTo("password");
                        });
    }
    ...
    void testValkeyConfigurationWithCluster() {
        this.contextRunner
                ...
                .run(
                        (context) ->
                                assertThat(JedisConnectionFactoryCaptor.connectionFactory.isValkeyClusterAware())
                                        .isTrue());
    }
                ...
                .run(
                        (context) -> {
                            ...
                            assertThat(factory)
                                    ...
                                    .satisfies(
                                            (executor) ->
                                                    SimpleAsyncTaskExecutorAssert.assertThat(
