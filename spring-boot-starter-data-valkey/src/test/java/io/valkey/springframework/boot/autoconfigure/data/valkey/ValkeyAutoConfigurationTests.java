...
class ValkeyAutoConfigurationTests {
    ...
    void testDefaultValkeyConfiguration() {
        this.contextRunner.run(
                (context) -> {
                    ...
                    assertThat(context.getBean(ValkeyConnectionFactory.class))
                            .isInstanceOf(ValkeyGlideConnectionFactory.class);
                });
    }
    ...
    void testValkeyConfigurationWithCustomPoolSettings() {
        this.contextRunner
                .withPropertyValues("spring.data.valkey.valkeyglide.max-pool-size=16")
                .run(
                        (context) -> {
                            ValkeyGlideConnectionFactory cf = context.getBean(ValkeyGlideConnectionFactory.class);
    ...
    void testValkeyConfigurationWithCustomBean() {
        this.contextRunner
                .withUserConfiguration(ValkeyStandaloneConfig.class)
                .run(
                        (context) -> {
                            ValkeyGlideConnectionFactory cf = context.getBean(ValkeyGlideConnectionFactory.class);
                            assertThat(cf.getHostName()).isEqualTo("foo");
    ...
    // 	this.contextRunner.withPropertyValues("spring.data.valkey.host:foo",
    // "spring.data.valkey.client-name:spring-boot")
    // 		.run((context) -> {
    // 			ValkeyGlideConnectionFactory cf = context.getBean(ValkeyGlideConnectionFactory.class);
    // 			assertThat(cf.getHostName()).isEqualTo("foo");
                ...
                .run(
                        (context) -> {
                            ValkeyClusterConfiguration clusterConfiguration =
                                    context.getBean(ValkeyGlideConnectionFactory.class).getClusterConfiguration();
                ...
                .run(
                        (context) -> {
                            ValkeyGlideConnectionFactory connectionFactory =
                                    context.getBean(ValkeyGlideConnectionFactory.class);
        ...
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            ...
                            assertThat(properties.getValkeyGlide().getCluster().getRefresh().getPeriod())
                                    .hasSeconds(30);
                        });
    }
    ...
    void testValkeyConfigurationWithClusterAdaptiveRefresh() {
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            ValkeyProperties properties = context.getBean(ValkeyProperties.class);
                            assertThat(properties.getValkeyGlide().getCluster().getRefresh().isAdaptive())
                                    .isTrue();
                        });
    }
    ...
    void definesPropertiesBasedConnectionDetailsByDefault() {
        this.contextRunner.run(
                (context) -> assertThat(context).hasSingleBean(PropertiesValkeyConnectionDetails.class));
    }

    @Test
    void usesStandaloneFromCustomConnectionDetails() {
        this.contextRunner
                .withUserConfiguration(ConnectionDetailsStandaloneConfiguration.class)
                .run(
                        (context) -> {
                            assertThat(context)
    ...
    void usesClusterFromCustomConnectionDetails() {
        this.contextRunner
                .withUserConfiguration(ConnectionDetailsClusterConfiguration.class)
                .run(
                        (context) -> {
                            assertThat(context)
                                    .hasSingleBean(ValkeyConnectionDetails.class)
                            ...
                            assertThat(configuration.getClusterNodes())
                                    .containsExactly(
                                            new ValkeyNode("node-1", 12345), new ValkeyNode("node-2", 23456));
                        });
    }
    ...
    void shouldBindOpenTelemetrySamplePercentage() {
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            ValkeyProperties properties = context.getBean(ValkeyProperties.class);
                            assertThat(properties.getValkeyGlide().getOpenTelemetry().getSamplePercentage())
                                    .isEqualTo(30);
                        });
    }
    ...
    void shouldBindOpenTelemetryFlushIntervalMs() {
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            ValkeyProperties properties = context.getBean(ValkeyProperties.class);
                            assertThat(properties.getValkeyGlide().getOpenTelemetry().getFlushIntervalMs())
                                    .isEqualTo(3000L);
                        });
    }
    ...
    void shouldNotEnableOpenTelemetryByDefaultEvenIfOtherPropertiesSet() {
        this.contextRunner
                ...
                .run(
                        (context) -> {
                            ValkeyProperties properties = context.getBean(ValkeyProperties.class);
                            assertThat(properties.getValkeyGlide().getOpenTelemetry().isEnabled())
                                    .isEqualTo(false);
                        });
    }
