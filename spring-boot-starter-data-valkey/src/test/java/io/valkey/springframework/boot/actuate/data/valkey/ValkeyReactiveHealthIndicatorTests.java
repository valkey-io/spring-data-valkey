...
class ValkeyReactiveHealthIndicatorTests {
    ...
    void valkeyIsUp() {
        ...
        ValkeyReactiveHealthIndicator healthIndicator =
                createHealthIndicator(valkeyConnection, commands);
    ...
    void healthWhenClusterStateIsAbsentShouldBeUp() {
        ...
        ValkeyReactiveHealthIndicator healthIndicator =
                new ValkeyReactiveHealthIndicator(valkeyConnectionFactory);
        Mono<Health> health = healthIndicator.health();
        StepVerifier.create(health)
                .consumeNextWith(
                        (h) -> {
    ...
    void healthWhenClusterStateIsFailShouldBeDown() {
        ReactiveValkeyConnectionFactory valkeyConnectionFactory =
                createClusterConnectionFactory("fail");
        ValkeyReactiveHealthIndicator healthIndicator =
                new ValkeyReactiveHealthIndicator(valkeyConnectionFactory);
        Mono<Health> health = healthIndicator.health();
        StepVerifier.create(health)
    ...
    void valkeyCommandIsDown() {
        ReactiveServerCommands commands = mock(ReactiveServerCommands.class);
        given(commands.info("server"))
                .willReturn(Mono.error(new ValkeyConnectionFailureException("Connection failed")));
        ReactiveValkeyConnection valkeyConnection = mock(ReactiveValkeyConnection.class);
        given(valkeyConnection.closeLater()).willReturn(Mono.empty());
        ValkeyReactiveHealthIndicator healthIndicator =
                createHealthIndicator(valkeyConnection, commands);
    ...
    private ValkeyReactiveHealthIndicator createHealthIndicator(
            ReactiveValkeyConnection valkeyConnection, ReactiveServerCommands serverCommands) {
        ReactiveValkeyConnectionFactory valkeyConnectionFactory =
                mock(ReactiveValkeyConnectionFactory.class);
    ...
    private ReactiveValkeyConnectionFactory createClusterConnectionFactory(String state) {
        ...
        given(valkeyConnection.closeLater()).willReturn(Mono.empty());
        given(valkeyConnection.clusterGetClusterInfo())
                .willReturn(Mono.just(new ClusterInfo(clusterProperties)));
        ReactiveValkeyConnectionFactory valkeyConnectionFactory =
                mock(ReactiveValkeyConnectionFactory.class);
        given(valkeyConnectionFactory.getReactiveConnection()).willReturn(valkeyConnection);
