...
/**
 * Tests for {@link PropertiesValkeyConnectionDetails}.
 *
 * @author Scott Frederick
 * @author Moritz Halbritter
 */
class PropertiesValkeyConnectionDetailsTests {

    private ValkeyProperties properties;

    private PropertiesValkeyConnectionDetails connectionDetails;

    private DefaultSslBundleRegistry sslBundleRegistry;

    @BeforeEach
    void setUp() {
        this.properties = new ValkeyProperties();
        this.sslBundleRegistry = new DefaultSslBundleRegistry();
        this.connectionDetails =
                new PropertiesValkeyConnectionDetails(this.properties, this.sslBundleRegistry);
    }

    @Test
    void connectionIsConfiguredWithDefaults() {
        ValkeyConnectionDetails.Standalone standalone = this.connectionDetails.getStandalone();
    ...
    void credentialsAreConfiguredFromUrlWithUsernameAndPassword() {
        this.properties.setUrl("valkey://user:secret@example.com");
    ...
    void credentialsAreConfiguredFromUrlWithUsernameAndColon() {
        this.properties.setUrl("valkey://user:@example.com");
    ...
    void credentialsAreConfiguredFromUrlWithColonAndPassword() {
        this.properties.setUrl("valkey://:secret@example.com");
        this.properties.setUsername("notused");
        this.properties.setPassword("notused");
    ...
    void standaloneIsConfiguredFromUrlWithoutDatabase() {
        ...
        this.properties.setDatabase(5);
        PropertiesValkeyConnectionDetails connectionDetails =
                new PropertiesValkeyConnectionDetails(this.properties, null);
    ...
    void clusterIsConfigured() {
        ...
        assertThat(this.connectionDetails.getCluster().getNodes())
                .containsExactly(
                        new Node("localhost", 1111), new Node("127.0.0.1", 2222), new Node("[::1]", 3333));
    }

    @Test
    void sentinelIsConfigured() {
        ValkeyProperties.Sentinel sentinel = new ValkeyProperties.Sentinel();
        sentinel.setNodes(List.of("localhost:1111", "127.0.0.1:2222", "[::1]:3333"));
        this.properties.setSentinel(sentinel);
        this.properties.setDatabase(5);
        PropertiesValkeyConnectionDetails connectionDetails =
                new PropertiesValkeyConnectionDetails(this.properties, null);
        assertThat(connectionDetails.getSentinel().getNodes())
                .containsExactly(
                        new Node("localhost", 1111), new Node("127.0.0.1", 2222), new Node("[::1]", 3333));
