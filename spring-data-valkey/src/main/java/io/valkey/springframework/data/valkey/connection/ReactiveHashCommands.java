...
public interface ReactiveHashCommands {
    ...
    class HSetCommand extends KeyCommand {
        ...
        private HSetCommand(
                @Nullable ByteBuffer key, Map<ByteBuffer, ByteBuffer> keyValueMap, boolean upsert) {
        ...
        public HSetCommand ofField(ByteBuffer field) {
            ...
            return new HSetCommand(
                    getKey(), Collections.singletonMap(field, fieldValueMap.get(SINGLE_VALUE_KEY)), upsert);
        }
    ...
    default Mono<Boolean> hMSet(ByteBuffer key, Map<ByteBuffer, ByteBuffer> fieldValueMap) {
        ...
        return hSet(Mono.just(HSetCommand.fieldValues(fieldValueMap).forKey(key)))
                .next()
                .map(it -> true);
    }
    ...
    class HGetCommand extends HashFieldsCommand {
        ...
        /**
         * Applies the hash {@literal key}. Constructs a new command instance with all previously
    ...
    default Mono<ByteBuffer> hGet(ByteBuffer key, ByteBuffer field) {
        return hMGet(key, Collections.singletonList(field))
                .filter(it -> !it.contains(null))
                .flatMapIterable(Function.identity())
                .next();
    }

    /**
     * Get values for given {@literal fields} from hash at {@literal key}. Values are in the order of
    ...
    default Mono<Boolean> hDel(ByteBuffer key, ByteBuffer field) {
        ...
        return hDel(key, Collections.singletonList(field))
                .map(val -> val > 0 ? Boolean.TRUE : Boolean.FALSE);
    }
    ...
    class HRandFieldCommand extends KeyCommand {
        ...
        /**
         * Applies the {@literal count}. Constructs a new command instance with all previously
         * configured properties. If the provided {@code count} argument is positive, return a list of
         * distinct fields, capped either at {@code count} or the hash size. If {@code count} is
         * negative, the behavior changes and the command is allowed to return the same field multiple
         * times. In this case, the number of returned fields is the absolute value of the specified
         * count.
         *
         * @param count
         * @return a new {@link HRandFieldCommand} with {@literal key} applied.
    ...
    default Mono<ByteBuffer> hRandField(ByteBuffer key) {
        ...
        return hRandField(Mono.just(HRandFieldCommand.key(key).count(1)))
    ...
    default Mono<Map.Entry<ByteBuffer, ByteBuffer>> hRandFieldWithValues(ByteBuffer key) {
        ...
        return hRandFieldWithValues(Mono.just(HRandFieldCommand.key(key).count(1)))
                .flatMap(CommandResponse::getOutput)
                .next();
    }
    ...
    default Flux<ByteBuffer> hRandField(ByteBuffer key, long count) {
        ...
        return hRandField(Mono.just(HRandFieldCommand.key(key).count(count)))
                .flatMap(CommandResponse::getOutput);
    }
    ...
    default Flux<Map.Entry<ByteBuffer, ByteBuffer>> hRandFieldWithValues(ByteBuffer key, long count) {
        ...
        return hRandFieldWithValues(Mono.just(HRandFieldCommand.key(key).count(count)))
                .flatMap(CommandResponse::getOutput);
    }
     ...
     * @see <a href="https://valkey.io/commands/hrandfield">Valkey Documentation: HRANDFIELD</a>
     */
    Flux<CommandResponse<HRandFieldCommand, Flux<ByteBuffer>>> hRandField(
     ...
     * @see <a href="https://valkey.io/commands/hrandfield">Valkey Documentation: HRANDFIELD</a>
     */
    Flux<CommandResponse<HRandFieldCommand, Flux<Map.Entry<ByteBuffer, ByteBuffer>>>>
            hRandFieldWithValues(Publisher<HRandFieldCommand> commands);
     ...
     * @see <a href="https://valkey.io/commands/hgetall">Valkey Documentation: HGETALL</a>
     */
    Flux<CommandResponse<KeyCommand, Flux<Map.Entry<ByteBuffer, ByteBuffer>>>> hGetAll(
    ...
    default Flux<Map.Entry<ByteBuffer, ByteBuffer>> hScan(ByteBuffer key, ScanOptions options) {
        ...
        return hScan(Mono.just(KeyScanCommand.key(key).withOptions(options)))
     ...
     * @since 2.1
     */
    Flux<CommandResponse<KeyCommand, Flux<Map.Entry<ByteBuffer, ByteBuffer>>>> hScan(
    ...
    /**
     * Get the length of the value associated with {@code field}. If either the {@code key} or the
     * {@code field} do not exist, {@code 0} is emitted.
     *
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @return never {@literal null}.
    ...
    default Mono<Long> hStrLen(ByteBuffer key, ByteBuffer field) {
        ...
        return hStrLen(Mono.just(HStrLenCommand.lengthOf(field).from(key)))
                .next()
                .map(NumericResponse::getOutput);
    }
    ...
    class HashExpireCommand extends HashFieldsCommand {
         ...
         * @return new instance of {@link HashExpireCommand}.
         */
        public static HashExpireCommand expireAt(
                List<ByteBuffer> fields, Instant ttl, TimeUnit precision) {
    ...
    /**
     * Expire a given {@literal field} after a given {@link Duration} of time, measured in
     * milliseconds, has passed.
     *
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @param duration must not be {@literal null}.
     * @return a {@link Mono} emitting the expiration result - {@code 2} indicating the specific field
     *     is deleted already due to expiration, or provided expiry interval is 0; {@code 1}
     *     indicating expiration time is set/updated; {@code 0} indicating the expiration time is not
     *     set (a provided NX | XX | GT | LT condition is not met); {@code -2} indicating there is no
     *     such field;
     ...
     * @param fields must not be {@literal null}.
     * @param duration must not be {@literal null}.
     * @return a {@link Flux} emitting the expiration results one by one, {@code 2} indicating the
     ...
     * @see <a href="https://valkey.io/commands/hexpire">Valkey Documentation: HEXPIRE</a>
     */
    Flux<NumericResponse<HashExpireCommand, Long>> applyHashFieldExpiration(
    ...
    default Flux<Long> hpExpire(ByteBuffer key, Duration duration, List<ByteBuffer> fields) {
        ...
        return applyHashFieldExpiration(
                        Flux.just(
                                new HashExpireCommand(
                                        key,
                                        fields,
                                        Expiration.from(duration.toMillis(), TimeUnit.MILLISECONDS),
                                        ExpirationOptions.none())))
                .mapNotNull(NumericResponse::getOutput);
    }

    /**
     * Expire a given {@literal field} in a given {@link Instant} of time, indicated as an absolute <a
     * href="https://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> in seconds since Unix epoch
     *
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @param expireAt must not be {@literal null}.
     * @return a {@link Mono} emitting the expiration result - {@code 2} indicating the specific field
     *     is deleted already due to expiration, or provided expiry interval is in the past; {@code 1}
     *     indicating expiration time is set/updated; {@code 0} indicating the expiration time is not
     *     set (a provided NX | XX | GT | LT condition is not met); {@code -2} indicating there is no
     *     such field;
    ...
    /**
     * Expire a {@link List} of {@literal field} in a given {@link Instant} of time, indicated as an
     * absolute <a href="https://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> in seconds since
     * Unix epoch
     *
     * @param key must not be {@literal null}.
     * @param fields must not be {@literal null}.
     ...
     *     indicating there is no such field;
     * @see <a href="https://valkey.io/commands/hexpireat">Valkey Documentation: HEXPIREAT</a>
     * @since 3.5
     */
    default Flux<Long> hExpireAt(ByteBuffer key, Instant expireAt, List<ByteBuffer> fields) {
        ...
        return applyHashFieldExpiration(
                        Flux.just(HashExpireCommand.expireAt(fields, expireAt, TimeUnit.SECONDS).from(key)))
                .mapNotNull(NumericResponse::getOutput);
    }

    /**
     * Expire a given {@literal field} in a given {@link Instant} of time, indicated as an absolute <a
     * href="https://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> in milliseconds since Unix
     * epoch
     *
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
    ...
    /**
     * Expire a {@link List} of {@literal field} in a given {@link Instant} of time, indicated as an
     * absolute <a href="https://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> in milliseconds
     * since Unix epoch
     *
     * @param key must not be {@literal null}.
     * @param fields must not be {@literal null}.
    ...
    default Flux<Long> hpExpireAt(ByteBuffer key, Instant expireAt, List<ByteBuffer> fields) {
        ...
        return applyHashFieldExpiration(
                        Flux.just(
                                HashExpireCommand.expireAt(fields, expireAt, TimeUnit.MILLISECONDS).from(key)))
                .mapNotNull(NumericResponse::getOutput);
    }

    /**
     * Persist a given {@literal field} removing any associated expiration, measured as absolute <a
     * href="https://en.wikipedia.org/wiki/Unix_time">Unix timestamp</a> in seconds since Unix epoch
     *
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @return a {@link Mono} emitting the persist result - {@code 1} indicating expiration time is
     *     removed; {@code -1} field has no expiration time to be removed; {@code -2} indicating there
     *     is no such field;
     ...
     * @param key must not be {@literal null}.
     * @param fields must not be {@literal null}.
     * @return a {@link Flux} emitting the persisting results one by one - {@code 1} indicating
    ...
    default Flux<Long> hPersist(ByteBuffer key, List<ByteBuffer> fields) {
        return hPersist(Flux.just(new HashFieldsCommand(key, fields)))
                .mapNotNull(NumericResponse::getOutput);
    }
     ...
     * @return a {@link Flux} emitting the persisting results one by one - {@code 1} indicating
     *     expiration time is removed; {@code -1} field has no expiration time to be removed; {@code
     *     -2} indicating there is no such field; * @since 3.5
     * @see <a href="https://valkey.io/commands/hpersist">Valkey Documentation: HPERSIST</a>
     */
     ...
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @return a {@link Mono} emitting the TTL result - the time to live in seconds; or a negative
     *     value to signal an error. The command returns {@code -1} if the key exists but has no
     *     associated expiration time. The command returns {@code -2} if the key does not exist;
     ...
     * @param key must not be {@literal null}.
     * @param fields must not be {@literal null}.
     * @return a {@link Flux} emitting the TTL results one by one - the time to live in seconds; or a
     *     negative value to signal an error. The command returns {@code -1} if the key exists but has
     *     no associated expiration time. The command returns {@code -2} if the key does not exist;
     * @see <a href="https://valkey.io/commands/httl">Valkey Documentation: HTTL</a>
     * @since 3.5
     */
    default Flux<Long> hTtl(ByteBuffer key, List<ByteBuffer> fields) {
        return hTtl(Flux.just(new HashFieldsCommand(key, fields)))
                .mapNotNull(NumericResponse::getOutput);
    }
     ...
     *
     * @param commands must not be {@literal null}.
     * @return a {@link Flux} emitting the persisting results one by one - the time to live in
     ...
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @return a {@link Mono} emitting the TTL result - the time to live in milliseconds; or a
     *     negative value to signal an error. The command returns {@code -1} if the key exists but has
     *     no associated expiration time. The command returns {@code -2} if the key does not exist;
     ...
     * @param key must not be {@literal null}.
     * @param fields must not be {@literal null}.
     * @return a {@link Flux} emitting the TTL results one by one - the time to live in milliseconds;
     *     or a negative value to signal an error. The command returns {@code -1} if the key exists
     *     but has no associated expiration time. The command returns {@code -2} if the key does not
     *     exist;
     * @see <a href="https://valkey.io/commands/hpttl">Valkey Documentation: HPTTL</a>
     * @since 3.5
     */
    default Flux<Long> hpTtl(ByteBuffer key, List<ByteBuffer> fields) {
        return hpTtl(Flux.just(new HashFieldsCommand(key, fields)))
                .mapNotNull(NumericResponse::getOutput);
    }
