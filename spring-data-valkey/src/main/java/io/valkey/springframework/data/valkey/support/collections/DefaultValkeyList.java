...
/**
 * Default implementation for {@link ValkeyList}. Suitable for not just lists, but also queues (FIFO
 * ordering) or stacks (LIFO ordering) and deques (or double ended queues). Allows the maximum size
 * (or the cap) to be specified to prevent the list from over growing. Note that all write
 * operations will execute immediately, whether a cap is specified or not - the list will always
 * accept new items (trimming the tail after each insert in case of capped collections).
 *
 * @author Costin Leau
...
public class DefaultValkeyList<E> extends AbstractValkeyCollection<E> implements ValkeyList<E> {
     ...
     * @param operations {@link ValkeyOperations} used to retrieve values of the declared {@link E
     *     type} from this list.
     */
    ...
    /**
     * Constructs a new {@link DefaultValkeyList} instance constrained to the given {@link Integer max
     * size}.
     *
     * @param key Valkey key of this list.
     * @param operations {@link ValkeyOperations} used to retrieve values of the declared {@link E
    ...
    @Override
    public E moveFirstTo(
            ValkeyList<E> destination, ValkeyListCommands.Direction destinationPosition) {
        ...
        E result =
                listOps.move(
                        ValkeyListCommands.Direction.first(), destination.getKey(), destinationPosition);
    ...
    @Override
    public E moveFirstTo(
            ValkeyList<E> destination,
            ValkeyListCommands.Direction destinationPosition,
            long timeout,
            TimeUnit unit) {
        ...
        E result =
                listOps.move(
                        ValkeyListCommands.Direction.first(),
                        destination.getKey(),
                        destinationPosition,
                        timeout,
                        unit);
    ...
    public E moveLastTo(ValkeyList<E> destination, ValkeyListCommands.Direction destinationPosition) {
        ...
        E result =
                listOps.move(
                        ValkeyListCommands.Direction.last(), destination.getKey(), destinationPosition);
        potentiallyCap(destination);
        return result;
    }
    ...
    public E moveLastTo(
        ...
        E result =
                listOps.move(
                        ValkeyListCommands.Direction.last(),
                        destination.getKey(),
                        destinationPosition,
                        timeout,
                        unit);
    ...
    public void add(int index, E element) {
        ...
        throw new IllegalArgumentException(
                "Valkey supports insertion only at the beginning or the end of the list");
    }
    ...
    private class Itr implements Iterator<E> {
        ...
        /**
         * Index of the {@link E element} in this iteration to be returned by subsequent call to {@link
         * #next()}.
         */
        int cursor = 0;
    ...
    private class ListItr extends Itr implements ListIterator<E> {
        ...
        public void set(E element) {
            ...
            Assert.state(
                    this.lastReturnedElement != null, "next() or previous() must be called before set(:E)");
