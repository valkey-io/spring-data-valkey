...
/**
 * {@link ObjectMapper} based {@link HashMapper} implementation that allows flattening. Given an
 * entity {@code Person} with an {@code Address} like below the flattening will create individual
 * hash entries for all nested properties and resolve complex types into simple types, as far as
 * possible.
 *
 * <p>Flattening requires all property names to not interfere with JSON paths. Using dots or
 * brackets in map keys or as property names is not supported using flattening. The resulting hash
 * cannot be mapped back into an Object.
 *
 * <h3>Example</h3>
 *
 * <pre class="code">
...
public class Jackson2HashMapper implements HashMapper<Object, String, Object> {
    ...
    private static final boolean SOURCE_VERSION_PRESENT =
            ClassUtils.isPresent(
                    "javax.lang.model.SourceVersion", Jackson2HashMapper.class.getClassLoader());
     ...
     *     will be un/flattened using {@literal dot notation}, or whether to retain the hierarchical
     *     node structure created by Jackson.
     */
    public Jackson2HashMapper(boolean flatten) {
        ...
        this(
                new ObjectMapper() {
                    ...
                    protected TypeResolverBuilder<?> _constructDefaultTypeResolverBuilder(
                            DefaultTyping applicability, PolymorphicTypeValidator typeValidator) {
        ...
        this.typingMapper.activateDefaultTyping(
                this.typingMapper.getPolymorphicTypeValidator(), DefaultTyping.EVERYTHING, As.PROPERTY);
    ...
    public Object fromHash(Map<String, Object> hash) {
        ...
        try {
            if (this.flatten) {
                ...
                Object hashedObject =
                        this.typingMapper.reader().forType(Object.class).readValue(unflattenedHashedBytes);
        ...
        for (Entry<String, Object> entry : source.entrySet()) {
            ...
            if (keyParts.length == 1 && isNotIndexed(keyParts[0])) {
                ...
                if (result.containsKey(nonIndexedKeyName)) {
                    addValueToTypedListAtIndex(
                            (List<Object>) result.get(nonIndexedKeyName), index, entry.getValue());
                } else {
        ...
        for (String partial : treatSeparate) {
            ...
            if (isNonNestedIndexed(partial)) {
                ...
                if (result.containsKey(nonIndexPartial)) {
                    addValueToTypedListAtIndex(
                            (List<Object>) result.get(nonIndexPartial), index, doUnflatten(newSource));
                } else {
    ...
    private int getIndex(@NonNull String indexedValue) {
        return Integer.parseInt(
                indexedValue.substring(indexedValue.indexOf('[') + 1, indexedValue.length() - 1));
    }
    ...
    private void doFlatten(
            ...
            Iterator<Entry<String, JsonNode>> inputMap,
        ...
        if (element.isArray()) {
                    ...
                    if (next.isBinary()) {
                        ...
                        try {
                            ...
                            throw new IllegalStateException(
                                    "Cannot read binary value '%s'".formatted(propertyPrefix), ex);
                        }
            ...
            switch (element.getNodeType()) {
                ...
                default ->
                        resultMap.put(
                                propertyPrefix,
                                new DirectFieldAccessFallbackBeanWrapper(element).getPropertyValue("_value"));
            }
        }
    }
    ...
    private boolean mightBeJavaType(JsonNode node) {
        ...
        if (!SOURCE_VERSION_PRESENT) {
            return Arrays.asList("java.util.Date", "java.math.BigInteger", "java.math.BigDecimal")
                    .contains(textValue);
        }
    ...
    private void flattenCollection(
            String propertyPrefix, Iterator<JsonNode> list, Map<String, Object> resultMap) {
    ...
    private List<Object> createTypedListWithValue(int index, Object value) {
        ...
        List<Object> valueList =
                CollectionUtils.initializeList(new ArrayList<>(initialCapacity), initialCapacity);
        valueList.set(index, value);
    ...
    private static class HashMapperModule extends SimpleModule {
        ...
        HashMapperModule() {
            ...
            addSerializer(
                    java.util.Calendar.class, new UntypedSerializer<>(new CalendarToTimestampSerializer()));
