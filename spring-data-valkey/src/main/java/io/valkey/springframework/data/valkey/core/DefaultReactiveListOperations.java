...
class DefaultReactiveListOperations<K, V> implements ReactiveListOperations<K, V> {
    ...
    public Flux<V> range(K key, long start, long end) {
        ...
        return createFlux(
                connection -> connection.lRange(rawKey(key), start, end).map(this::readRequiredValue));
    }
    ...
    public Mono<V> move(K sourceKey, Direction from, K destinationKey, Direction to) {
        ...
        return createMono(
                connection ->
                        connection
                                .lMove(rawKey(sourceKey), rawKey(destinationKey), from, to)
                                .map(this::readRequiredValue));
    }

    @Override
    public Mono<V> move(
            K sourceKey, Direction from, K destinationKey, Direction to, Duration timeout) {
        ...
        return createMono(
                connection ->
                        connection
                                .bLMove(rawKey(sourceKey), rawKey(destinationKey), from, to, timeout)
                                .map(this::readRequiredValue));
    }
    ...
    public Mono<V> index(K key, long index) {
        ...
        return createMono(
                connection -> connection.lIndex(rawKey(key), index).map(this::readRequiredValue));
    }
    ...
    public Flux<V> leftPop(K key, long count) {
        ...
        return createFlux(
                listCommands -> listCommands.lPop(rawKey(key), count).map(this::readRequiredValue));
    }
    ...
    public Mono<V> leftPop(K key, Duration timeout) {
        ...
        Assert.isTrue(
                isZeroOrGreaterOneSecond(timeout),
                "Duration must be either zero or greater or equal to 1 second");

        return createMono(
                connection ->
                        connection
                                .blPop(Collections.singletonList(rawKey(key)), timeout)
    ...
    public Flux<V> rightPop(K key, long count) {
        ...
        return createFlux(
                listCommands -> listCommands.rPop(rawKey(key), count).map(this::readRequiredValue));
    }
    ...
    public Mono<V> rightPop(K key, Duration timeout) {
        ...
        return createMono(
                connection ->
                        connection
                                .brPop(Collections.singletonList(rawKey(key)), timeout)
                                .mapNotNull(popResult -> readValue(popResult.getValue())));
    }
    ...
    public Mono<V> rightPopAndLeftPush(K sourceKey, K destinationKey) {
        ...
        return createMono(
                connection ->
                        connection
                                .rPopLPush(rawKey(sourceKey), rawKey(destinationKey))
                                .map(this::readRequiredValue));
    }
    ...
    public Mono<V> rightPopAndLeftPush(K sourceKey, K destinationKey, Duration timeout) {
        ...
        return createMono(
                connection ->
                        connection
                                .bRPopLPush(rawKey(sourceKey), rawKey(destinationKey), timeout)
                                .map(this::readRequiredValue));
    }
    ...
    private boolean isZeroOrGreaterOneSecond(Duration timeout) {
        return timeout.isZero()
                || timeout.getNano() % TimeUnit.NANOSECONDS.convert(1, TimeUnit.SECONDS) == 0;
    }
