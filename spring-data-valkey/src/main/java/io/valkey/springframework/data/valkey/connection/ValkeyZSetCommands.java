...
public interface ValkeyZSetCommands {
    ...
    /**
     * {@link org.springframework.data.domain.Range} defines {@literal min} and {@literal max} values
     * to retrieve from a {@literal ZSET}.
     *
     * @author Christoph Strobl
     * @since 1.6
     * @deprecated since 3.0, use {@link org.springframework.data.domain.Range} or {@link #toRange()}
     *     instead.
     */
    @Deprecated
    class Range {
        ...
        /**
         * @return new {@link org.springframework.data.domain.Range} with {@literal min} and {@literal
         *     max} set to {@link Boundary#infinite()}.
         */
        ...
        public <T> org.springframework.data.domain.Range<T> toRange() {
            ...
            return (org.springframework.data.domain.Range<T>)
                    org.springframework.data.domain.Range.from(lower).to(upper);
        }

        private org.springframework.data.domain.Range.Bound<Object> toBound(
                @Nullable Boundary boundary) {
            ...
            return boundary.isIncluding()
                    ? org.springframework.data.domain.Range.Bound.inclusive(boundary.getValue())
     ...
     * @author Christoph Strobl
     * @since 1.6
     * @deprecated since 3.0, use {@link io.valkey.springframework.data.valkey.connection.Limit}
     *     instead.
     */
    @Deprecated
    ...
    /**
     * Add {@code value} to a sorted set at {@code key}, or update its {@code score} depending on the
     ...
     * @param start
     * @param end
     * @return empty {@link Set} when key does not exists or no members in range. {@literal null} when
    ...
    /**
     * Get set of {@link Tuple}s where score is between {@code Range#min} and {@code Range#max} from
     * sorted set.
     *
     * @param key must not be {@literal null}.
     ...
     */
    @Nullable
    default Set<Tuple> zRangeByScoreWithScores(
    ...
    default Set<byte[]> zRangeByScore(byte[] key, double min, double max, long offset, long count) {
        return zRangeByScore(
                ...
                new io.valkey.springframework.data.valkey.connection.Limit()
    ...
    /**
     * Get set of {@link Tuple}s in range from {@code Limit#offset} to {@code Limit#offset +
     * Limit#count} where score is between {@code Range#min} and {@code Range#max} from sorted set.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     ...
     */
    @Nullable
    default Set<byte[]> zRevRangeByScore(
            byte[] key, org.springframework.data.domain.Range<? extends Number> range) {
    ...
    default Set<Tuple> zRevRangeByScoreWithScores(byte[] key, double min, double max) {
        return zRevRangeByScoreWithScores(
                key, org.springframework.data.domain.Range.closed(min, max), Limit.unlimited());
    }
    ...
    /**
     * Get elements in range from {@code Limit#offset} to {@code Limit#offset + Limit#count} where
     * score is between {@code Range#min} and {@code Range#max} from sorted set ordered high -> low.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @param limit must not be {@literal null}.
     ...
     */
    @Nullable
    default Set<Tuple> zRevRangeByScoreWithScores(
            byte[] key, double min, double max, long offset, long count) {
    ...
    /**
     * Get set of {@link Tuple} where score is between {@code Range#min} and {@code Range#max} from
     * sorted set ordered from high to low.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
    ...
    /**
     * Get set of {@link Tuple} in range from {@code Limit#offset} to {@code Limit#count} where score
     * is between {@code Range#min} and {@code Range#max} from sorted set ordered high -> low.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @param limit must not be {@literal null}.
    ...
    /**
     * Count number of elements within sorted set with scores between {@code Range#min} and {@code
     * Range#max}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Remove elements with scores between {@code Range#min} and {@code Range#max} from sorted set
     * with {@code key}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     ...
     * @since 1.5
     * @see <a href="https://valkey.io/commands/zrangebyscore">Valkey Documentation: ZRANGEBYSCORE</a>
     * @deprecated since 3.0, use {@link #zRangeByScore(byte[],
     *     org.springframework.data.domain.Range)} instead.
     */
    @Nullable
    @Deprecated
     ...
     */
    @Nullable
    default Set<byte[]> zRangeByScore(
            byte[] key, org.springframework.data.domain.Range<? extends Number> range) {
    ...
    /**
     * Get elements in range from {@code Limit#count} to {@code Limit#offset} where score is between
     * {@code Range#min} and {@code Range#max} from sorted set.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @param limit must not be {@literal null}.
    ...
    /**
     * Get all the elements in {@link org.springframework.data.domain.Range} from the sorted set at
     * {@literal key} in lexicographical ordering.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
    ...
    /**
     * Get all the elements in {@link org.springframework.data.domain.Range} from the sorted set at
     * {@literal key} in lexicographical ordering. Result is limited via {@link Limit}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @param limit must not be {@literal null}.
    ...
    /**
     * Get all the elements in {@link org.springframework.data.domain.Range} from the sorted set at
     * {@literal key} in reversed lexicographical ordering.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
     ...
     */
    @Nullable
    default Set<byte[]> zRevRangeByLex(
            byte[] key, org.springframework.data.domain.Range<byte[]> range) {
    ...
    /**
     * Get all the elements in {@link org.springframework.data.domain.Range} from the sorted set at
     * {@literal key} in reversed lexicographical ordering. Result is limited via {@link Limit}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @param limit must not be {@literal null}.
    ...
    default Long zRangeStoreByLex(
            byte[] dstKey, byte[] srcKey, org.springframework.data.domain.Range<byte[]> range) {
    ...
    default Long zRangeStoreByScore(
            byte[] dstKey, byte[] srcKey, org.springframework.data.domain.Range<? extends Number> range) {
