 ...
 * content types, completely independent from the Valkey API.
 *
 * <p>Make sure to call {@link #afterPropertiesSet()} after setting all the parameters on the
 * adapter.
 *
 ...
 * becomes mandatory. This lenient behavior allows the adapter to be used uniformly across existing
 * listeners and message POJOs.
 *
 * <p>Modeled as much as possible after the JMS MessageListenerAdapter in Spring Framework.
 *
 * <p>By default, the content of incoming Valkey messages gets extracted before being passed into
 * the target listener method, to let the target method operate on message content types such as
 * String or byte array instead of the raw {@link Message}. Message type conversion is delegated to
 * a Spring Data {@link ValkeySerializer}. By default, the {@link JdkSerializationValkeySerializer}
 * will be used. (If you do not want such automatic message conversion taking place, then be sure to
 * set the {@link #setSerializer Serializer} to <code>null</code>.)
 *
 * <p>Find below some examples of method signatures compliant with this adapter class. This first
 * example handles all <code>Message</code> types and gets passed the contents of each <code>Message
 * </code> type as an argument.
 *
 * <pre class="code">
 ...
 * </pre>
 *
 * <p>In addition, the channel or pattern to which a message is sent can be passed in to the method
 * as a second argument of type String:
 *
 * <pre class="code">
 * public interface MessageContentsDelegate {
 ...
 * </pre>
 *
 * For further examples and discussion please do refer to the Spring Data reference documentation
 * which describes this class (and its attendant configuration) in detail. <b>Important:</b> Due to
 * the nature of messages, the default serializer used by the adapter is {@link
 * StringValkeySerializer}. If the messages are of a different type, change them accordingly through
 * {@link #setSerializer(ValkeySerializer)}.
 *
...
public class MessageListenerAdapter implements InitializingBean, MessageListener {
            ...
            for (Method m : methods) {
                ...
                Object[] args = //
                        types.length == 2 //
                                        ...
                                        && types[1].isInstance(arguments[1])
                                ? arguments
                                : message;
     ...
     *
     * <p>The default converter is a {@link StringValkeySerializer}.
     *
     * @param serializer
     */
     ...
     * @param extractedMessage the converted Valkey request message, to be passed into the listener
     *     method as argument
    ...
    protected void invokeListenerMethod(String methodName, Object[] arguments) {
        try {
            ...
            if (targetEx instanceof DataAccessException dataAccessException) {
                ...
                throw new ValkeyListenerExecutionFailedException(
                        "Listener method '%s' threw exception".formatted(methodName), targetEx);
            }
        } catch (Throwable ex) {
            throw new ValkeyListenerExecutionFailedException(
                    "Failed to invoke target method '%s' with arguments %s"
                            .formatted(methodName, ObjectUtils.nullSafeToString(arguments)),
                    ex);
        }
    }
