 ...
 * @since 1.7
 */
public class ValkeyRepositoryConfigurationExtension
    ...
    public void registerBeansForRoot(
            BeanDefinitionRegistry registry, RepositoryConfigurationSource configuration) {

        String valkeyTemplateRef = configuration.getAttribute("valkeyTemplateRef").get();

        if (!StringUtils.hasText(valkeyTemplateRef)) {
            throw new IllegalStateException(
                    "@EnableValkeyRepositories(valkeyTemplateRef = … ) must be configured to a non empty"
                            + " value");
        }

        // Mapping config

        String mappingConfigBeanName =
                BeanDefinitionReaderUtils.uniqueBeanName(VALKEY_MAPPING_CONFIG_BEAN_NAME, registry);
        String indexConfigurationBeanName =
                BeanDefinitionReaderUtils.uniqueBeanName("valkeyIndexConfiguration", registry);
        String keyspaceConfigurationBeanName =
                BeanDefinitionReaderUtils.uniqueBeanName("valkeyKeyspaceConfiguration", registry);

        registerIfNotAlreadyRegistered(
                () ->
                        BeanDefinitionBuilder.rootBeanDefinition(
                                        configuration.getRequiredAttribute("indexConfiguration", Class.class)) //
                                .setRole(BeanDefinition.ROLE_INFRASTRUCTURE) //
                                .getBeanDefinition(),
                registry,
                indexConfigurationBeanName,
                configuration.getSource());

        registerIfNotAlreadyRegistered(
                () ->
                        BeanDefinitionBuilder.rootBeanDefinition(
                                        configuration.getRequiredAttribute("keyspaceConfiguration", Class.class)) //
                                .setRole(BeanDefinition.ROLE_INFRASTRUCTURE) //
                                .getBeanDefinition(),
                registry,
                keyspaceConfigurationBeanName,
                configuration.getSource());

        registerIfNotAlreadyRegistered(
                () -> createMappingConfigBeanDef(indexConfigurationBeanName, keyspaceConfigurationBeanName),
                registry,
                mappingConfigBeanName,
                configuration.getSource());

        registerIfNotAlreadyRegistered(
                () -> createValkeyMappingContext(mappingConfigBeanName),
                registry,
        ...
        registerIfNotAlreadyRegistered(
                () -> new RootBeanDefinition(ValkeyCustomConversions.class),
                registry,
                VALKEY_CUSTOM_CONVERSIONS_BEAN_NAME,
                configuration.getSource());

        // Register referenceResolver
        registerIfNotAlreadyRegistered(
                () -> createValkeyReferenceResolverDefinition(valkeyTemplateRef),
                registry,
                VALKEY_REFERENCE_RESOLVER_BEAN_NAME,
                configuration.getSource());

        // Register converter
        registerIfNotAlreadyRegistered(
                () -> createValkeyConverterDefinition(),
                registry,
                VALKEY_CONVERTER_BEAN_NAME,
                configuration.getSource());

        registerIfNotAlreadyRegistered(
                () -> createValkeyKeyValueAdapter(configuration),
                registry,
                VALKEY_ADAPTER_BEAN_NAME,
                configuration.getSource());
    ...
    private static AbstractBeanDefinition createValkeyKeyValueAdapter(
            RepositoryConfigurationSource configuration) {

        BeanDefinitionBuilder builder =
                BeanDefinitionBuilder.rootBeanDefinition(ValkeyKeyValueAdapter.class) //
                        .addConstructorArgReference(
                                configuration.getRequiredAttribute("valkeyTemplateRef", String.class)) //
                        .addConstructorArgReference(VALKEY_CONVERTER_BEAN_NAME) //
                        .addPropertyValue(
                                "enableKeyspaceEvents",
                                configuration.getRequiredAttribute(
                                        "enableKeyspaceEvents", EnableKeyspaceEvents.class)) //
                        .addPropertyValue(
                                "keyspaceNotificationsConfigParameter",
                                configuration
                                        .getAttribute("keyspaceNotificationsConfigParameter", String.class)
                                        .orElse("")) //
                        .addPropertyValue(
                                "shadowCopy", configuration.getRequiredAttribute("shadowCopy", ShadowCopy.class));

        configuration
                .getAttribute("messageListenerContainerRef")
                .ifPresent(it -> builder.addPropertyReference("messageListenerContainer", it));

        return builder.getBeanDefinition();
    }

    private static AbstractBeanDefinition createValkeyReferenceResolverDefinition(
            String valkeyTemplateRef) {

        return BeanDefinitionBuilder.rootBeanDefinition(
                        "io.valkey.springframework.data.valkey.core.convert.ReferenceResolverImpl") //
    ...
    private static AbstractBeanDefinition createValkeyMappingContext(String mappingConfigRef) {
        ...
        return BeanDefinitionBuilder.rootBeanDefinition(ValkeyMappingContext.class) //
                .addConstructorArgReference(mappingConfigRef)
                .getBeanDefinition();
    }

    private static AbstractBeanDefinition createMappingConfigBeanDef(
            String indexConfigRef, String keyspaceConfigRef) {
