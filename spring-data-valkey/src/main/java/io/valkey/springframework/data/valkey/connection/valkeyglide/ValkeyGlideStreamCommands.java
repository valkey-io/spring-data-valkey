...
public class ValkeyGlideStreamCommands implements ValkeyStreamCommands {
        ...
        try {
            ...
            return connection.execute(
                    ...
                    (GlideString glideResult) ->
                            glideResult != null ? RecordId.of(glideResult.toString()) : null,
                    params.toArray());
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
        }
    ...
    public String xGroupCreate(
            byte[] key, String groupName, ReadOffset readOffset, boolean makeStream) {
        Assert.notNull(key, "Key must not be null!");
    ...
    public Boolean xGroupDestroy(byte[] key, String groupName) {
        ...
        try {
            return connection.execute(
                    "XGROUP", (Boolean glideResult) -> glideResult, "DESTROY", key, groupName);
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
        }
    }
        ...
        try {
            return connection.execute(
                    ...
                    (Map<?, ?> glideResult) -> {
                        ...
                        // first-entry and last-entry are special cases with nested structures and need to be
                        // converted to Map
        ...
        try {
            return connection.execute(
                    ...
                    (Object[] glideResult) -> {
                        ...
                        // XInfoGroups.fromList expects a list where each group is represented as a separate
                        // List
                        // Format: [group1List, group2List, ...] where each groupList is [key1, value1, key2,
                        // value2, ...]
    ...
    public PendingMessagesSummary xPending(byte[] key, String groupName) {
        ...
        try {
            return connection.execute(
                    "XPENDING",
                    (Object glideResult) ->
                            glideResult != null ? parsePendingMessagesSummary(glideResult) : null,
                    key,
                    groupName);
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
    ...
    public PendingMessages xPending(byte[] key, String groupName, XPendingOptions options) {
        ...
        try {
            ...
            return connection.execute(
                    "XPENDING",
                    (Object glideResult) ->
                            glideResult != null ? parsePendingMessages(glideResult, groupName) : null,
                    args.toArray());
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
        }
    ...
    public PendingMessages xPending(byte[] key, String groupName, String consumerName) {
        ...
        ValkeyStreamCommands.XPendingOptions options =
                ValkeyStreamCommands.XPendingOptions.range(Range.unbounded(), 100L).consumer(consumerName);
    ...
    public List<ByteRecord> xRange(byte[] key, Range<String> range, Limit limit) {
        ...
        try {
            ...
            if (range.getLowerBound().isBounded()) {
                ...
                lowerBound =
                        range.getLowerBound().isInclusive()
    ...
    public List<ByteRecord> xRevRange(byte[] key, Range<String> range, Limit limit) {
        ...
        try {
            ...
            // Handle upper bound with exclusive/inclusive semantics (note: xRevRange has reversed
            // argument order)
            Object upperBound;
            if (range.getUpperBound().isBounded()) {
                Object value = range.getUpperBound().getValue().orElse("+");
    ...
    private List<Object> buildXClaimArgs(
            byte[] key, String group, String newOwner, XClaimOptions options) {
        List<Object> args = new ArrayList<>();
        args.add(key);
        args.add(group);
    ...
    private List<ByteRecord> parseByteRecords(
            Map<?, ?> result, byte[] streamKey, boolean reverseOrder) {
        if (result == null) {
            return new ArrayList<>();
        }
    ...
    // This is a LLM-generated code and it's so hellish i cant even think to refactor it. I am ashemed
    // to show it here, but it works.
    private PendingMessagesSummary parsePendingMessagesSummary(Object result) {
        ...
        try {
            ...
            if (list.size() >= 4) {
                ...
                String lowestId =
                        convertResultToString(ValkeyGlideConverters.defaultFromGlideResult(list.get(1)));
                String highestId =
                        convertResultToString(ValkeyGlideConverters.defaultFromGlideResult(list.get(2)));
                ...
                if (consumersObj instanceof Object[]) {
                    ...
                    for (int i = 0; i < consumers.size(); i += 2) {
                        if (i + 1 < consumers.size()) {
                            Object consumerNameObj =
                                    ValkeyGlideConverters.defaultFromGlideResult(consumers.get(i));
            ...
            for (Object item : list) {
                ...
                if (convertedItem instanceof List) {
                    ...
                    if (messageData.size() >= 4) {
                        ...
                        Long idleTime =
                                ((Number) ValkeyGlideConverters.defaultFromGlideResult(messageData.get(2)))
                                        .longValue();
                        Long deliveryCount =
                                ((Number) ValkeyGlideConverters.defaultFromGlideResult(messageData.get(3)))
                                        .longValue();
