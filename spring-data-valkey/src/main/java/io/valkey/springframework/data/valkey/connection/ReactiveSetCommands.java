...
public interface ReactiveSetCommands {
    ...
    default Mono<Long> sAdd(ByteBuffer key, Collection<ByteBuffer> values) {
        ...
        return sAdd(Mono.just(SAddCommand.values(values).to(key)))
                .next()
    ...
    /**
     * Remove given {@literal value} from set at {@literal key} and return the number of removed
     * elements.
     *
     * @param key must not be {@literal null}.
     * @param value must not be {@literal null}.
    ...
    /**
     * Remove given {@literal values} from set at {@literal key} and return the number of removed
     * elements.
     *
     * @param key must not be {@literal null}.
     * @param values must not be {@literal null}.
    ...
    default Mono<Long> sRem(ByteBuffer key, Collection<ByteBuffer> values) {
        ...
        return sRem(Mono.just(SRemCommand.values(values).from(key)))
                .next()
                .map(NumericResponse::getOutput);
    }
    ...
    class SMoveCommand extends KeyCommand {
        ...
        private SMoveCommand(
                @Nullable ByteBuffer key, @Nullable ByteBuffer destination, ByteBuffer value) {
        ...
        /**
         * Applies the {@literal source} key. Constructs a new command instance with all previously
        ...
        /**
         * Applies the {@literal destination} key. Constructs a new command instance with all previously
         * configured properties.
         *
    ...
    default Mono<Boolean> sMove(ByteBuffer sourceKey, ByteBuffer destinationKey, ByteBuffer value) {
        ...
        return sMove(Mono.just(SMoveCommand.value(value).from(sourceKey).to(destinationKey)))
                .next()
                .map(BooleanResponse::getOutput);
    }

    /**
     * Move {@link SMoveCommand#getValue()} from {@link SMoveCommand#getKey()} to {@link
     * SMoveCommand#getDestination()}.
     *
     * @param commands must not be {@literal null}.
     * @return
     * @see <a href="https://valkey.io/commands/smove">Valkey Documentation: SMOVE</a>
    ...
    class SIsMemberCommand extends KeyCommand {
        ...
        /**
         * Applies the {@literal set} key. Constructs a new command instance with all previously
         * configured properties.
         *
    ...
    default Mono<Boolean> sIsMember(ByteBuffer key, ByteBuffer value) {
        ...
        return sIsMember(Mono.just(SIsMemberCommand.value(value).of(key)))
                .next()
                .map(BooleanResponse::getOutput);
    }
    ...
    default Mono<List<Boolean>> sMIsMember(ByteBuffer key, List<ByteBuffer> values) {
        ...
        return sMIsMember(Mono.just(SMIsMemberCommand.values(values).of(key)))
                .next()
                .map(MultiValueResponse::getOutput);
    }

    /**
     * Check if set at {@link SMIsMemberCommand#getKey()} contains {@link
     ...
     * @see <a href="https://valkey.io/commands/smismember">Valkey Documentation: SMISMEMBER</a>
     */
    Flux<MultiValueResponse<SMIsMemberCommand, Boolean>> sMIsMember(
    ...
    default Mono<Long> sInterStore(ByteBuffer destinationKey, Collection<ByteBuffer> keys) {
        ...
        return sInterStore(Mono.just(SInterStoreCommand.keys(keys).storeAt(destinationKey)))
                .next()
                .map(NumericResponse::getOutput);
    }
     ...
     * @see <a href="https://valkey.io/commands/sinterstore">Valkey Documentation: SINTERSTORE</a>
     */
    Flux<NumericResponse<SInterStoreCommand, Long>> sInterStore(
    ...
    default Mono<Long> sUnionStore(ByteBuffer destinationKey, Collection<ByteBuffer> keys) {
        ...
        return sUnionStore(Mono.just(SUnionStoreCommand.keys(keys).storeAt(destinationKey)))
                .next()
                .map(NumericResponse::getOutput);
    }
     ...
     * @see <a href="https://valkey.io/commands/sunionstore">Valkey Documentation: SUNIONSTORE</a>
     */
    Flux<NumericResponse<SUnionStoreCommand, Long>> sUnionStore(
    ...
    default Mono<Long> sDiffStore(ByteBuffer destinationKey, Collection<ByteBuffer> keys) {
        ...
        return sDiffStore(Mono.just(SDiffStoreCommand.keys(keys).storeAt(destinationKey)))
                .next()
                .map(NumericResponse::getOutput);
    }
    ...
    /**
     * Use a {@link Flux} to iterate over members in the set at {@code key}. The resulting {@link
     * Flux} acts as a cursor and issues {@code SSCAN} commands itself as long as the subscriber
     * signals demand.
     *
     * @param key must not be {@literal null}.
    ...
    /**
     * Use a {@link Flux} to iterate over members in the set at {@code key} given {@link ScanOptions}.
     * The resulting {@link Flux} acts as a cursor and issues {@code SSCAN} commands itself as long as
     * the subscriber signals demand.
     *
     * @param key must not be {@literal null}.
    ...
    default Flux<ByteBuffer> sScan(ByteBuffer key, ScanOptions options) {
        ...
        return sScan(Mono.just(KeyScanCommand.key(key).withOptions(options)))
    ...
    default Flux<ByteBuffer> sRandMember(ByteBuffer key, Long count) {
        ...
        return sRandMember(Mono.just(SRandMembersCommand.valueCount(count).from(key)))
                .flatMap(CommandResponse::getOutput);
    }

    /**
     * Get {@link SRandMembersCommand#getCount()} random elements from set at {@link
     * SRandMembersCommand#getKey()}.
     *
     * @param commands must not be {@literal null}.
     * @return
     * @see <a href="https://valkey.io/commands/srandmember">Valkey Documentation: SRANDMEMBER</a>
     */
    Flux<CommandResponse<SRandMembersCommand, Flux<ByteBuffer>>> sRandMember(
