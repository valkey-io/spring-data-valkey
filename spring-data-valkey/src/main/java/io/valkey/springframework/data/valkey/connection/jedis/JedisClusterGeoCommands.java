    ...
    public List<Point> geoPos(byte[] key, byte[]... members) {
        ...
        try {
            return JedisConverters.geoCoordinateToPointConverter()
                    .convert(connection.getCluster().geopos(key, members));
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
    ...
    public GeoResults<GeoLocation<byte[]>> geoRadiusByMember(
        ...
        try {
            return JedisConverters.geoRadiusResponseToGeoResultsConverter(radius.getMetric())
                    .convert(
                            connection.getCluster().georadiusByMember(key, member, radius.getValue(), geoUnit));
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
        ...
        try {
            return JedisConverters.geoRadiusResponseToGeoResultsConverter(radius.getMetric())
                    .convert(
                            connection
                                    .getCluster()
                                    .georadiusByMember(key, member, radius.getValue(), geoUnit, geoRadiusParam));
