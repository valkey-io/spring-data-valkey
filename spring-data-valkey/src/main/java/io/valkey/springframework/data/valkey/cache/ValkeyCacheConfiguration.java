...
/**
 * Immutable {@link ValkeyCacheConfiguration} used to customize {@link ValkeyCache} behavior, such
 * as caching {@literal null} values, computing cache key prefixes and handling binary
 * serialization.
 *
 * <p>Start with {@link ValkeyCacheConfiguration#defaultCacheConfig()} and customize {@link
 * ValkeyCache} behavior using the builder methods, such as {@link #entryTtl(Duration)}, {@link
 * #serializeKeysWith(SerializationPair)} and {@link #serializeValuesWith(SerializationPair)}.
 *
...
public class ValkeyCacheConfiguration {
     ...
     *   <dt>key expiration
     *   <dd>eternal
     *   <dt>cache null values
     *   <dd>yes
     *   <dt>prefix cache keys
     *   <dd>yes
     *   <dt>default prefix
     *   <dd>[the actual cache name]
     *   <dt>key serializer
     *   <dd>{@link io.valkey.springframework.data.valkey.serializer.StringValkeySerializer}
     *   <dt>value serializer
     *   <dd>{@link io.valkey.springframework.data.valkey.serializer.JdkSerializationValkeySerializer}
     *   <dt>conversion service
     *   <dd>{@link DefaultFormattingConversionService} with {@link
     *       #registerDefaultConverters(ConverterRegistry) default} cache key converters
     * </dl>
     *
     ...
     * </dl>
     *
     * @param classLoader the {@link ClassLoader} used for deserialization by the {@link
    ...
    public static ValkeyCacheConfiguration defaultCacheConfig(@Nullable ClassLoader classLoader) {
        ...
        return new ValkeyCacheConfiguration(
                TtlFunction.persistent(),
    ...
    private ValkeyCacheConfiguration(
            TtlFunction ttlFunction,
            Boolean cacheNullValues,
            Boolean enableTimeToIdle,
            Boolean usePrefix,
    ...
    /**
     * Use the given {@link CacheKeyPrefix} to compute the prefix for the actual Valkey {@literal key}
     * given the {@literal cache name} as function input.
     *
     ...
     * <strong>NOTE</strong> any {@link org.springframework.cache.Cache#put(Object, Object)} operation
     * involving {@literal null} value will error. Nothing will be written to Valkey, nothing will be
     * removed. An already existing key will still be there afterwards with the very same value as
     * before.
     *
     * @return new {@link ValkeyCacheConfiguration}.
     */
     ...
     * <strong>NOTE</strong>: {@link Cache#clear()} might result in unintended removal of {@literal
     * key}s in Valkey. Make sure to use a dedicated Valkey instance when disabling prefixes.
     *
     * @return new {@link ValkeyCacheConfiguration}.
     */
     ...
     *
     * <p>Enabling this option applies the same {@link #getTtlFunction() TTL expiration policy} to
     * {@link Cache} read operations as it does for {@link Cache} write operations. In effect, this
     * will invoke the Valkey {@literal GETEX} command in place of {@literal GET}.
     *
     * <p>Valkey does not support the concept of {@literal TTI}, only {@literal TTL}. However, if
     * {@literal TTL} expiration is applied to all {@link Cache} operations, both read and write
     * alike, and {@link Cache} operations passed with expiration are used consistently across the
     * application, then in effect, an application can achieve {@literal TTI} expiration-like
     * behavior.
     *
     * <p>Requires Valkey 6.2.0 or newer.
     *
     ...
     *
     * @param ttlFunction the {@link TtlFunction} to compute the time to live for cache entries, must
     *     not be {@literal null}.
     * @return new {@link ValkeyCacheConfiguration}.
     * @since 3.2
     */
     ...
     * @return new {@link ValkeyCacheConfiguration}.
     */
    public ValkeyCacheConfiguration serializeKeysWith(
            SerializationPair<String> keySerializationPair) {
     ...
     *
     * <p>Use {@link #enableTimeToIdle()} to opt-in and enable {@literal time-to-idle (TTI)
     * expiration} for caching.
     *
     * @return {@literal true} if {@literal time-to-idle (TTI) expiration} was configured and enabled
     *     for caching. Defaults to {@literal false}.
     * @see <a href="https://valkey.io/commands/getex/">GETEX</a>
     * @since 3.2.0
     */
    ...
    /**
     * @return {@literal true} if cache keys need to be prefixed with the {@link
     *     #getKeyPrefixFor(String)} if present or the default which resolves to {@link
     *     Cache#getName()}.
     */
    ...
    /**
     * @return The {@link ConversionService} used for cache key to {@link String} conversion. Never
     *     {@literal null}.
     */
    ...
    /**
     * Returns a computed {@link Duration TTL expiration timeout} based on cache entry key/value if a
     * {@link TtlFunction} was confiugred using {@link #entryTtl(TtlFunction)}.
     *
     * <p>Otherwise, returns the user-provided, fixed {@link Duration} if {@link #entryTtl(Duration)}
    ...
    /**
     * Gets the {@link TtlFunction} used to compute a cache key {@literal time-to-live (TTL)
     * expiration}.
     *
     * @return the {@link TtlFunction} used to compute expiration time (TTL) for cache entries; never
     *     {@literal null}.
     */
     ...
     *
     * @param cacheKeyConverter {@link Converter} used to convert a {@literal cache key} into a {@link
     *     String}.
     * @throws IllegalStateException if {@link #getConversionService()} does not allow {@link
     ...
     *
     * @param registryConsumer {@link Consumer} used to register a {@link Converter} with the
     *     configured {@link ConverterRegistry}; never {@literal null}.
     * @throws IllegalStateException if {@link #getConversionService()} does not allow {@link
     *     Converter} registration.
    ...
    public void configureKeyConverters(Consumer<ConverterRegistry> registryConsumer) {
        ...
        if (!(getConversionService() instanceof ConverterRegistry)) {
            throw new IllegalStateException(
                    ("'%s' returned by getConversionService() does not allow Converter registration; Please"
                                    + " make sure to provide a ConversionService that implements ConverterRegistry")
     ...
     *
     * <p>The following converters get registered:
     *
     * <ul>
     *   <li>{@link String} to {@code byte byte[]} using UTF-8 encoding.
     *   <li>{@link SimpleKey} to {@link String}
     * </ul>
     *
     * @param registry {@link ConverterRegistry} in which the {@link Converter key converters} are
     *     registered; must not be {@literal null}.
     * @see org.springframework.core.convert.converter.ConverterRegistry
     */
    public static void registerDefaultConverters(ConverterRegistry registry) {
        ...
        registry.addConverter(
                String.class, byte[].class, source -> source.getBytes(StandardCharsets.UTF_8));
