...
/**
 * Utility to create implementation objects for {@code Bound…Operations} so that bound key
 * interfaces can be implemented automatically by translating interface calls to actual {@code
...
class BoundOperationsProxyFactory {
    ...
    /**
     * Create a proxy object that implements {@link Class boundOperationsInterface} using the given
     * {@code key} and {@link DataType}. Calls to {@code Bound…Operations} methods are bridged by
     * forwarding these either to the {@code operationsTarget} or a default implementation.
     *
    ...
    public <T> T createProxy(
            Class<T> boundOperationsInterface,
            Object key,
            DataType type,
            ValkeyOperations<?, ?> operations,
            Function<ValkeyOperations<?, ?>, Object> operationsTargetFunction) {

        DefaultBoundKeyOperations delegate =
                new DefaultBoundKeyOperations(type, key, (ValkeyOperations) operations);
        ...
        proxyFactory.addAdvice(
                new BoundOperationsMethodInterceptor(
                        key, operations, boundOperationsInterface, operationsTarget, delegate));
    ...
    Method lookupRequiredMethod(Method method, Class<?> targetClass, boolean considerKeyArgument) {
        ...
        if (target == null) {
            throw new IllegalArgumentException(
                    "Cannot lookup target method for %s in class %s; This appears to be a bug"
    ...
    /**
     * {@link MethodInterceptor} to delegate proxy calls to either {@link ValkeyOperations}, {@code
     * key}, {@link DefaultBoundKeyOperations} or the {@code operationsTarget} such as {@link
     * ValueOperations}.
     */
    class BoundOperationsMethodInterceptor implements MethodInterceptor {
        ...
        public BoundOperationsMethodInterceptor(
                Object key,
                ValkeyOperations<?, ?> operations,
                Class<?> boundOperationsInterface,
                Object operationsTarget,
                DefaultBoundKeyOperations delegate) {
        ...
        public Object invoke(MethodInvocation invocation) throws Throwable {
            ...
            return switch (method.getName()) {
                ...
                default ->
                        method.getDeclaringClass() == boundOperationsInterface
        ...
        private Object doInvoke(
                MethodInvocation invocation, Method method, Object target, boolean considerKeyArgument) {
            ...
            if (isStreamRead(method)) {
                ...
                args[args.length - 1] =
                        new StreamOffset[] {
                            StreamOffset.create(
                                    delegate.getKey(),
                                    (ReadOffset) invocationArguments[invocationArguments.length - 1])
                        };
            } else if (backingMethod.getParameterCount() > 0
                    && backingMethod.getParameterTypes()[0].equals(Object.class)) {
