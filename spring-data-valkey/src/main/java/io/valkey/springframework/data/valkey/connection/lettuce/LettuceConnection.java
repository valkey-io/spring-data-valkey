...
/**
 * {@code ValkeyConnection} implementation on top of <a
 * href="https://github.com/mp911de/lettuce">Lettuce</a> Valkey client.
 *
 * <p>While the underlying Lettuce {@literal RedisClient} and {@literal StatefulRedisConnection}
 * instances used by {@link LettuceConnection} are Thread-safe, this class itself is not
 * Thread-safe. Therefore, instances of {@link LettuceConnection} should not be shared across
 * multiple Threads when executing Valkey commands and other operations. If optimal performance is
 * required by your application(s), then we recommend direct access to the low-level, API provided
 * by the underlying Lettuce client library (driver), where such Thread-safety guarantees can be
...
public class LettuceConnection extends AbstractValkeyConnection {
    ...
    private static class LettuceTransactionResultConverter<T> extends TransactionResultConverter<T> {
        ...
        public LettuceTransactionResultConverter(
    ...
    private final LettuceHyperLogLogCommands hyperLogLogCommands =
            new LettuceHyperLogLogCommands(this);
     ...
     *     LettuceConnection}s. Will not be used for transactions or blocking operations
     * @param timeout The connection timeout (in milliseconds)
     ...
     *
     * @param sharedConnection A native connection that is shared with other {@link
     *     LettuceConnection}s. Should not be used for transactions or blocking operations.
     * @param timeout The connection timeout (in milliseconds)
     * @param client The {@link RedisClient} to use when making pub/sub connections.
     * @param defaultDbIndex The db index to use along with {@link RedisClient} when establishing a
    ...
    public LettuceConnection(
            @Nullable StatefulRedisConnection<byte[], byte[]> sharedConnection,
            long timeout,
            @Nullable AbstractRedisClient client,
            int defaultDbIndex) {
    ...
    public LettuceConnection(
        ...
        this(
                (StatefulConnection<byte[], byte[]>) sharedConnection,
                connectionProvider,
                timeout,
                defaultDbIndex);
    }
     ...
     */
    LettuceConnection(
            @Nullable StatefulConnection<byte[], byte[]> sharedConnection,
            LettuceConnectionProvider connectionProvider,
            long timeout,
            int defaultDbIndex) {
    ...
    public Object execute(
            String command, @Nullable CommandOutput commandOutputTypeHint, byte[]... args) {
        ...
        CommandOutput expectedOutput =
                commandOutputTypeHint != null ? commandOutputTypeHint : typeHints.getTypeHint(commandType);
        ...
        return invoke()
                .just(
                        ...
                        valkeyCommand.getType(),
                        valkeyCommand.getOutput(),
    ...
    RedisClusterAsyncCommands<byte[], byte[]> getAsyncConnection() {
        ...
        if (sharedConnection != null) {
            ...
            if (sharedConnection
                    instanceof StatefulRedisClusterConnection<byte[], byte[]> statefulClusterConnection) {
    ...
    /**
     * Obtain a {@link LettuceInvoker} to call Lettuce methods using the default {@link
     * #getAsyncConnection() connection}.
     *
    ...
    /**
     * Obtain a {@link LettuceInvoker} to call Lettuce methods using the given {@link
     * RedisClusterAsyncCommands connection}.
     *
    ...
    /**
     * Obtain a {@link LettuceInvoker} to call Lettuce methods returning a status response using the
     * default {@link #getAsyncConnection() connection}. Status responses are not included in
     * transactional and pipeline results.
     *
     * @return the {@link LettuceInvoker}.
     * @since 2.5
     */
    ...
    private LettuceInvoker doInvoke(
            RedisClusterAsyncCommands<byte[], byte[]> connection, boolean statusCommand) {

        if (isPipelined()) {

            return new LettuceInvoker(
                    connection,
                    (future, converter, nullDefault) -> {
                        try {
                            if (statusCommand) {
    ...
    <T, R> LettuceResult<T, R> newLettuceResult(
            Future<T> resultHolder, Converter<T, R> converter, Supplier<R> defaultValue) {
    ...
    public RedisClusterAsyncCommands<byte[], byte[]> getNativeConnection() {
        ...
        return isAlive(subscription)
                ? subscription.getNativeConnection().async()
                : getAsyncConnection();
    }
        ...
        try {
            ...
            boolean done =
                    LettuceFutures.awaitAll(
                            timeout, TimeUnit.MILLISECONDS, futures.toArray(new RedisFuture[0]));
            ...
            if (done) {
                for (LettuceResult<?, ?> result : ppline) {
                    ...
                    if (resultHolder.isCompletedExceptionally()) {
                        ...
                        try {
                            results.add(
                                    result.conversionRequired() ? result.convert(result.get()) : result.get());
    ...
    public List<Object> exec() {
        ...
        try {
            Converter<Exception, DataAccessException> exceptionConverter =
                    this::convertLettuceAccessException;

            if (isPipelined()) {
                RedisFuture<TransactionResult> exec = getAsyncDedicatedValkeyCommands().exec();

                LettuceTransactionResultConverter resultConverter =
                        new LettuceTransactionResultConverter(new LinkedList<>(txResults), exceptionConverter);
            ...
            List<Object> results =
                    LettuceConverters.transactionResultUnwrapper().convert(transactionResult);
    ...
    public void watch(byte[]... keys) {
        ...
        if (isQueueing()) {
            throw new InvalidDataAccessApiUsageException(
                    "WATCH is not supported when a transaction is active");
        }
    ...
    <T> T failsafeReadScanValues(
            List<?> source, @SuppressWarnings("rawtypes") @Nullable Converter converter) {
     ...
     * If false, results of {@link #closePipeline()} and {@link #exec()} will be of the type returned
     * by the Lettuce driver
     *
    ...
    protected RedisClusterCommands<byte[], byte[]> getConnection() {
        ...
        if (asyncSharedConnection != null) {
            if (asyncSharedConnection
                    instanceof StatefulRedisConnection<byte[], byte[]> statefulConnection) {
    ...
    RedisClusterCommands<byte[], byte[]> getDedicatedConnection() {
        ...
        throw new IllegalStateException(
                "%s is not a supported connection type".formatted(connection.getClass().getName()));
    }
    ...
    private StatefulRedisSentinelConnection<String, String> getConnection(ValkeyNode sentinel) {
        return ((TargetAware) getConnectionProvider())
                .getConnection(StatefulRedisSentinelConnection.class, getValkeyURI(sentinel));
    }
    ...
    private static void potentiallySelectDatabase(
            StatefulConnection<byte[], byte[]> connection, int dbIndex) {
    ...
    private void validateCommand(ProtocolKeyword command, @Nullable byte[]... args) {
        ...
        if (!ValkeyCommand.UNKNOWN.equals(valkeyCommand) && valkeyCommand.requiresArguments()) {
            try {
                ...
                throw new InvalidDataAccessApiUsageException(
                        "Validation failed for %s command".formatted(command), ex);
            }
        }
    }
    ...
    static class TypeHints {
        ...
        private static final Map<ProtocolKeyword, Class<? extends CommandOutput>>
                COMMAND_OUTPUT_TYPE_MAPPING = new HashMap<>();

        @SuppressWarnings("rawtypes") //
        private static final Map<Class<?>, Constructor<CommandOutput>> CONSTRUCTORS =
        ...
        /**
         * Returns the {@link CommandOutput} mapped for given {@link CommandType} or {@link
         * ByteArrayOutput} as default.
         *
        ...
        /**
         * Returns the {@link CommandOutput} mapped for given {@link CommandType} given {@link
         * CommandOutput} as default.
         *
         * @param type {@link ProtocolKeyword} used to lookup the type hint.
         * @return the {@link CommandOutput} mapped for given {@link CommandType} given {@link
         *     CommandOutput} as default.
         */
        @SuppressWarnings("rawtypes")
        public CommandOutput getTypeHint(@Nullable ProtocolKeyword type, CommandOutput defaultType) {
            ...
            CommandOutput<?, ?, ?> outputType =
                    instantiateCommandOutput(COMMAND_OUTPUT_TYPE_MAPPING.get(type));
            ...
            if (constructor == null) {
                constructor =
                        (Constructor<CommandOutput>)
                                ClassUtils.getConstructorIfAvailable(type, RedisCodec.class);
    ...
    /**
     * Strategy interface to control pipelining flush behavior. Lettuce writes (flushes) each command
     * individually to the Valkey connection. Flushing behavior can be customized to optimize for
     * performance. Flushing can be either stateless or stateful. An example for stateful flushing is
     * size-based (buffer) flushing to flush after a configured number of commands.
     *
    ...
    public interface PipeliningFlushPolicy {
        ...
        /**
         * Return a policy to buffer commands and to flush once reaching the configured {@code
         * bufferSize}. The buffer is recurring so a buffer size of e.g. {@code 2} will flush after 2,
         * 4, 6, … commands.
         *
         * @param bufferSize the number of commands to buffer before flushing. Must be greater than
         *     zero.
         * @return a policy to flush buffered commands to the Valkey connection once the configured
         *     number of commands was issued.
         */
