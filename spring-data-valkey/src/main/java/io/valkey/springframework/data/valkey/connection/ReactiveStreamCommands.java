...
public interface ReactiveStreamCommands {
    ...
    class AcknowledgeCommand extends KeyCommand {
        ...
        private AcknowledgeCommand(
                @Nullable ByteBuffer key, @Nullable String group, List<RecordId> recordIds) {
        ...
        /**
         * Applies the {@literal recordIds}. Constructs a new command instance with all previously
        ...
        /**
         * Applies the {@literal group}. Constructs a new command instance with all previously
         * configured properties.
         *
    ...
    default Mono<Long> xAck(ByteBuffer key, String group, String... recordIds) {
        ...
        return xAck(Mono.just(AcknowledgeCommand.stream(key).inGroup(group).forRecords(recordIds)))
                .next()
    ...
    class AddStreamRecord extends KeyCommand {
        ...
        private AddStreamRecord(
                ByteBufferRecord record,
        ...
        /**
         * Applies the Geo set {@literal key}. Constructs a new command instance with all previously
         * configured properties.
         *
         * @param key must not be {@literal null}.
        ...
        public AddStreamRecord to(ByteBuffer key) {
            return new AddStreamRecord(
                    record.withStreamKey(key), maxlen, nomkstream, approximateTrimming, minId);
        }
        ...
        /**
         * Apply {@code MINID} trimming strategy, that evicts entries with IDs lower than the one
         * specified.
         *
    ...
    default Mono<RecordId> xAdd(ByteBufferRecord record, XAddOptions xAddOptions) {
        ...
        AddStreamRecord addStreamRecord =
                AddStreamRecord.of(record)
     ...
     *
     * @param commands must not be {@literal null}.
     * @return {@link Flux} emitting the {@link RecordId} on by for the given {@link AddStreamRecord}
     *     commands.
     * @see <a href="https://valkey.io/commands/xadd">Valkey Documentation: XADD</a>
     */
     ...
     * @since 2.3
     */
    default Flux<RecordId> xClaimJustId(
            ByteBuffer key, String group, String newOwner, XClaimOptions options) {

        return xClaimJustId(Mono.just(new XClaimCommand(key, group, newOwner, options)))
                .next()
     ...
     * @since 2.3
     */
    Flux<CommandResponse<XClaimCommand, Flux<RecordId>>> xClaimJustId(
            Publisher<XClaimCommand> commands);
     ...
     * @since 2.3
     */
    default Flux<ByteBufferRecord> xClaim(
            ByteBuffer key, String group, String newOwner, Duration minIdleTime, RecordId... recordIds) {
    ...
    default Flux<ByteBufferRecord> xClaim(
        ...
        return xClaim(Mono.just(new XClaimCommand(key, group, newOwner, options)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
     ...
     * @since 2.3
     */
    Flux<CommandResponse<XClaimCommand, Flux<ByteBufferRecord>>> xClaim(
            Publisher<XClaimCommand> commands);
    ...
    class XClaimCommand extends KeyCommand {
        ...
        private XClaimCommand(
                @Nullable ByteBuffer key, String groupName, String newOwner, XClaimOptions options) {
    ...
    default Mono<Long> xDel(ByteBuffer key, String... recordIds) {
        ...
        return xDel(Mono.just(DeleteCommand.stream(key).records(recordIds)))
                .next()
     ...
     * @return {@link Mono} emitting a summary of pending messages within the given {@literal consumer
     *     group}.
    ...
    default Mono<PendingMessagesSummary> xPending(ByteBuffer key, String groupName) {
        ...
        return xPendingSummary(Mono.just(PendingRecordsCommand.pending(key, groupName)))
                .next()
                .map(CommandResponse::getOutput);
    }
     ...
     *
     * @param commands must not be {@literal null}..
     * @return {@link Flux} emitting a summary of pending messages within the given {@literal consumer
     *     group} one by one.
     * @see <a href="https://valkey.io/commands/xpending">Valkey Documentation: xpending</a>
     * @since 2.3
     */
     ...
     * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
     * @param groupName the name of the {@literal consumer group}. Must not be {@literal null}.
     * @param consumerName the consumer to fetch {@link PendingMessages} for. Must not be {@literal
     *     null}.
     * @return {@link Mono} emitting pending messages for the given {@link Consumer}.
     * @see <a href="https://valkey.io/commands/xpending">Valkey Documentation: xpending</a>
     * @since 2.3
    ...
    default Mono<PendingMessages> xPending(ByteBuffer key, String groupName, String consumerName) {
        return xPending(Mono.just(PendingRecordsCommand.pending(key, groupName).consumer(consumerName)))
                .next()
                .map(CommandResponse::getOutput);
    }
     ...
     * Range} within a {@literal consumer group}.
     *
     * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
     * @param groupName the name of the {@literal consumer group}. Must not be {@literal null}.
     ...
     * @return {@link Mono} emitting pending messages for the given {@literal consumer group}.
     *     transaction.
     * @see <a href="https://valkey.io/commands/xpending">Valkey Documentation: xpending</a>
     * @since 2.3
     */
    default Mono<PendingMessages> xPending(
            ByteBuffer key, String groupName, Range<?> range, Long count) {
        return xPending(Mono.just(PendingRecordsCommand.pending(key, groupName).range(range, count)))
                .next()
                .map(CommandResponse::getOutput);
    }
    ...
    /**
     * Obtain detailed information about pending {@link PendingMessage messages} for a given {@link
     * Range} and {@link Consumer} within a {@literal consumer group}.
     *
     * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
     ...
     */
    default Mono<PendingMessages> xPending(
            ByteBuffer key, Consumer consumer, Range<?> range, Long count) {
        return xPending(key, consumer.getGroup(), consumer.getName(), range, count);
    }

    /**
     * Obtain detailed information about pending {@link PendingMessage messages} for a given {@link
     * Range} and {@literal consumer} within a {@literal consumer group}.
     *
     * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
     * @param groupName the name of the {@literal consumer group}. Must not be {@literal null}.
     ...
     */
    default Mono<PendingMessages> xPending(
            ByteBuffer key, String groupName, String consumerName, Range<?> range, Long count) {
        return xPending(
                        Mono.just(
                                PendingRecordsCommand.pending(key, groupName)
     ...
     * @since 2.3
     */
    Flux<CommandResponse<PendingRecordsCommand, PendingMessages>> xPending(
            Publisher<PendingRecordsCommand> commands);
    ...
    class RangeCommand extends KeyCommand {
        ...
        /**
         * Applies a {@link Range}. Constructs a new command instance with all previously configured
         * properties.
         *
         * @param range must not be {@literal null}.
        ...
        /**
         * Applies a {@code Limit}. Constructs a new command instance with all previously configured
         * properties.
         *
         * @param count
    ...
    default Flux<ByteBufferRecord> xRange(ByteBuffer key, Range<String> range, Limit limit) {
        ...
        return xRange(Mono.just(RangeCommand.stream(key).within(range).limit(limit)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
    ...
    class ReadCommand {
         ...
         */
        public ReadCommand(
                List<StreamOffset<ByteBuffer>> streamOffsets,
                @Nullable StreamReadOptions readOptions,
        ...
        public static ReadCommand from(StreamOffset<ByteBuffer> streamOffset) {
            ...
            return new ReadCommand(
                    Collections.singletonList(streamOffset), StreamReadOptions.empty(), null);
        }
        ...
        /**
         * Applies a {@link Consumer}. Constructs a new command instance with all previously configured
         * properties.
         *
        ...
        /**
         * Applies the given {@link StreamReadOptions}. Constructs a new command instance with all
     ...
     * @see <a href="https://valkey.io/commands/xread">Valkey Documentation: XREAD</a>
     */
    default Flux<ByteBufferRecord> xRead(
            StreamReadOptions readOptions, StreamOffset<ByteBuffer>... streams) {

        Assert.notNull(readOptions, "StreamReadOptions must not be null");
        Assert.notNull(streams, "StreamOffsets must not be null");

        return read(Mono.just(ReadCommand.from(streams).withOptions(readOptions)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
    ...
    default Flux<XInfoGroup> xInfoGroups(ByteBuffer key) {
        return xInfoGroups(Mono.just(XInfoCommand.of(key)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
     ...
     * @since 2.3
     */
    Flux<CommandResponse<XInfoCommand, Flux<XInfoGroup>>> xInfoGroups(
            Publisher<XInfoCommand> commands);
    ...
    default Flux<XInfoConsumer> xInfoConsumers(ByteBuffer key, String groupName) {
        return xInfoConsumers(Mono.just(XInfoCommand.of(key).consumersIn(groupName)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
     ...
     * @since 2.3
     */
    Flux<CommandResponse<XInfoCommand, Flux<XInfoConsumer>>> xInfoConsumers(
            Publisher<XInfoCommand> commands);
    ...
    class GroupCommand extends KeyCommand {
        ...
        public GroupCommand(
                ...
                GroupCommandAction action,
                @Nullable String groupName,
                @Nullable String consumerName,
                @Nullable ReadOffset offset,
                boolean mkStream) {
        ...
        public static GroupCommand deleteConsumer(Consumer consumer) {
            return new GroupCommand(
                    null, GroupCommandAction.DELETE_CONSUMER, consumer.getGroup(), consumer.getName(), null);
        }
    ...
    default Mono<String> xGroupCreate(ByteBuffer key, String groupName, ReadOffset readOffset) {
        ...
        return xGroup(Mono.just(GroupCommand.createGroup(groupName).forStream(key).at(readOffset)))
                .next()
                .map(CommandResponse::getOutput);
    }
     ...
     * @since 2.3
     */
    default Mono<String> xGroupCreate(
            ByteBuffer key, String groupName, ReadOffset readOffset, boolean mkStream) {
        return xGroup(
                        Mono.just(
                                GroupCommand.createGroup(groupName)
                                        .forStream(key)
                                        .at(readOffset)
                                        .makeStream(mkStream)))
                .next()
     ...
     * @see <a href="https://valkey.io/commands/xreadgroup">Valkey Documentation: XREADGROUP</a>
     */
    default Flux<ByteBufferRecord> xReadGroup(
            Consumer consumer, StreamOffset<ByteBuffer>... streams) {
     ...
     */
    default Flux<ByteBufferRecord> xReadGroup(
            Consumer consumer, StreamReadOptions readOptions, StreamOffset<ByteBuffer>... streams) {

        Assert.notNull(consumer, "Consumer must not be null");
        Assert.notNull(streams, "StreamOffsets must not be null");
        Assert.notNull(streams, "StreamOffsets must not be null");

        return read(Mono.just(ReadCommand.from(streams).withOptions(readOptions).as(consumer)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
    ...
    default Flux<ByteBufferRecord> xRevRange(ByteBuffer key, Range<String> range, Limit limit) {
        ...
        return xRevRange(Mono.just(RangeCommand.stream(key).within(range).limit(limit)))
                .next()
                .flatMapMany(CommandResponse::getOutput);
    }
     ...
     * @see <a href="https://valkey.io/commands/xrevrange">Valkey Documentation: XREVRANGE</a>
     */
    Flux<CommandResponse<RangeCommand, Flux<ByteBufferRecord>>> xRevRange(
            Publisher<RangeCommand> commands);
    ...
    class TrimCommand extends KeyCommand {
        ...
        /**
         * Applies the numeric {@literal count}. Constructs a new command instance with all previously
         * configured properties.
         *
         * @param count
        ...
        /**
         * Applies approximate trimming. Constructs a new command instance with all previously
         * configured properties.
         *
        ...
        /**
         * Applies {@code approximateTrimming}. Constructs a new command instance with all previously
         * configured properties.
         *
    ...
    default Mono<Long> xTrim(ByteBuffer key, long count, boolean approximateTrimming) {
        ...
        return xTrim(Mono.just(TrimCommand.stream(key).to(count).approximate(approximateTrimming)))
                .next()
                .map(NumericResponse::getOutput);
    }
