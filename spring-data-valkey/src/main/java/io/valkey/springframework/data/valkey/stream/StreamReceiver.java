 ...
 *
 * <p>Once created, a {@link StreamReceiver} can subscribe to a Valkey Stream and consume incoming
 * {@link Record records}. Consider a {@link Flux} of {@link Record} infinite. Cancelling the {@link
 * org.reactivestreams.Subscription} terminates eventually background polling. Records are converted
 * using {@link SerializationPair key and value serializers} to support various serialization
 * strategies. <br>
 ...
 *       io.valkey.springframework.data.valkey.core.ReactiveStreamOperations#acknowledge(Object,
 ...
 * initial {@link ReadOffset}, {@link StreamReceiver} applies an individual strategy to obtain the
 * next {@link ReadOffset}: <br>
 * <strong>Standalone</strong>
 *
 * <ul>
 *   <li>{@link ReadOffset#from(String)} Offset using a particular record Id: Start with the given
 *       offset and use the last seen {@link Record#getId() record Id}.
 *   <li>{@link ReadOffset#lastConsumed()} Last consumed: Start with the latest offset ({@code $})
 *       and use the last seen {@link Record#getId() record Id}.
 ...
 *   <li>{@link ReadOffset#from(String)} Offset using a particular record Id: Start with the given
 *       offset and use the last seen {@link Record#getId() record Id}.
 *   <li>{@link ReadOffset#lastConsumed()} Last consumed: Start with the last consumed record by the
 *       consumer ({@code >}) and use the last consumed record by the consumer ({@code >}) for
 *       subsequent reads.
 *   <li>{@link ReadOffset#latest()} Last consumed: Start with the latest offset ({@code $}) and use
 *       latest offset ({@code $}) for subsequent reads.
 * </ul>
 *
 * <strong>Note: Using {@link ReadOffset#latest()} bears the chance of dropped records as records
 * can arrive in the time during polling is suspended. Use recordId's as offset or {@link
 * ReadOffset#lastConsumed()} to minimize the chance of record loss.</strong>
 *
 * <p>{@link StreamReceiver} propagates errors during stream reads and deserialization as terminal
 * error signal by default. Configuring a {@link StreamReceiverOptions#getResumeFunction() resume
 * function} allows conditional resumption by dropping the record or by propagating the error to
 * terminate the subscription.
 *
 * <p>See the following example code how to use {@link StreamReceiver}:
 *
 * <pre class="code">
 ...
 * @see io.valkey.springframework.data.valkey.core.ReactiveStreamOperations
...
public interface StreamReceiver<K, V extends Record<K, ?>> {
    ...
    /**
     * Create a new {@link StreamReceiver} using {@link StringValkeySerializer string serializers}
     * given {@link ReactiveValkeyConnectionFactory}.
     *
    ...
    static StreamReceiver<String, MapRecord<String, String, String>> create(
        ...
        SerializationPair<String> serializationPair =
                SerializationPair.fromSerializer(StringValkeySerializer.UTF_8);
        return create(
                connectionFactory, StreamReceiverOptions.builder().serializer(serializationPair).build());
    }

    /**
     * Create a new {@link StreamReceiver} given {@link ReactiveValkeyConnectionFactory} and {@link
     ...
     * @return the new {@link StreamReceiver}.
     */
    static <K, V extends Record<K, ?>> StreamReceiver<K, V> create(
            ReactiveValkeyConnectionFactory connectionFactory, StreamReceiverOptions<K, V> options) {
    ...
    /**
     * Starts a Valkey Stream consumer that consumes {@link Record records} from the {@link
     * StreamOffset stream}. Records are consumed from Valkey and delivered on the returned {@link
     * Flux} when requests are made on the Flux. The receiver is closed when the returned {@link Flux}
     * terminates.
     *
     * <p>Every record must be acknowledged using {@link
     * io.valkey.springframework.data.valkey.connection.ReactiveStreamCommands#xAck(ByteBuffer,
     * String, String...)}
     *
     ...
     * terminates.
     *
     * <p>Every record is acknowledged when received.
     *
     ...
     *
     * <p>Every record must be acknowledged using {@link
     * io.valkey.springframework.data.valkey.core.ReactiveStreamOperations#acknowledge(Object, String,
    ...
    class StreamReceiverOptions<K, V extends Record<K, ?>> {
        ...
        private StreamReceiverOptions(
                Duration pollTimeout,
                @Nullable Integer batchSize,
                Function<? super Throwable, ? extends Publisher<Void>> resumeFunction,
                SerializationPair<K> keySerializer,
                SerializationPair<Object> hashKeySerializer,
                SerializationPair<Object> hashValueSerializer,
         ...
         */
        public static StreamReceiverOptionsBuilder<String, MapRecord<String, String, String>>
                builder() {

            SerializationPair<String> serializer =
                    SerializationPair.fromSerializer(StringValkeySerializer.UTF_8);
        ...
        public static <T> StreamReceiverOptionsBuilder<String, ObjectRecord<String, T>> builder(
            ...
            return new StreamReceiverOptionsBuilder<>()
                    .keySerializer(serializer)
                    .hashKeySerializer(raw)
                    .hashValueSerializer(raw)
    ...
    class StreamReceiverOptionsBuilder<K, V extends Record<K, ?>> {
        ...
        /**
         * Configure a resume {@link Function} to resume the main sequence when polling the stream
         * fails. The function can either resume by suppressing the error or fail the main sequence by
         * emitting the error to stop receiving. Receive errors (Valkey errors, Serialization failures)
         * stop receiving by default.
         *
         ...
         */
        @SuppressWarnings({"unchecked", "rawtypes"})
        public <NV> StreamReceiverOptionsBuilder<K, ObjectRecord<K, NV>> targetType(
                Class<NV> targetType) {
         ...
         */
        @SuppressWarnings({"unchecked", "rawtypes"})
        public <NV> StreamReceiverOptionsBuilder<K, ObjectRecord<K, NV>> objectMapper(
