...
public interface ReactiveZSetOperations<K, V> {
    ...
    /**
     * Store all elements at {@code dstKey} with lexicographical ordering from {@literal ZSET} at
     * {@code srcKey} with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     ...
     * at {@code srcKey} with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param srcKey must not be {@literal null}.
     * @param dstKey must not be {@literal null}.
     ...
     * ZSET} at {@code srcKey} with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param srcKey must not be {@literal null}.
     * @param dstKey must not be {@literal null}.
    ...
    /**
     * Store all elements at {@code dstKey} with ordering by score from {@literal ZSET} at {@code
     * srcKey} with a score between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param srcKey must not be {@literal null}.
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Store {@literal n} elements at {@code dstKey}, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with ordering by score from {@literal ZSET} at {@code
     * srcKey} with a score between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param srcKey must not be {@literal null}.
     * @param dstKey must not be {@literal null}.
    ...
    /**
     * Store all elements at {@code dstKey} with reverse ordering by score from {@literal ZSET} at
     * {@code srcKey} with a score between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param srcKey must not be {@literal null}.
     * @param dstKey must not be {@literal null}.
    ...
    /**
     * Use a {@link Flux} to iterate over entries in the sorted set at {@code key}. The resulting
     * {@link Flux} acts as a cursor and issues {@code ZSCAN} commands itself as long as the
     * subscriber signals demand.
     *
     * @param key must not be {@literal null}.
     * @return the {@link Flux} emitting the {@literal values} one by one or an {@link Flux#empty()
    ...
    /**
     * Use a {@link Flux} to iterate over entries in the sorted set at {@code key} given {@link
    ...
    /**
     * Count number of elements within sorted set with a value between {@link Range#getLowerBound()}
     * and {@link Range#getUpperBound()} applying lexicographical ordering.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}
     * @return
     ...
     *
     * @param key must not be {@literal null}.
     * @param timeout maximal duration to wait until an entry in the list at {@code key} is available.
     *     Must be either {@link Duration#ZERO} or greater {@literal 1 second}, must not be {@literal
     *     null}. A timeout of zero can be used to wait indefinitely. Durations between zero and one
     ...
     * @see <a href="https://valkey.io/commands/zinter">Valkey Documentation: ZINTER</a>
     */
    default Flux<TypedTuple<V>> intersectWithScores(
            K key, Collection<K> otherKeys, Aggregate aggregate) {
        return intersectWithScores(
                key, otherKeys, aggregate, Weights.fromSetCount(1 + otherKeys.size()));
    }
     ...
     * @see <a href="https://valkey.io/commands/zinter">Valkey Documentation: ZINTER</a>
     */
    Flux<TypedTuple<V>> intersectWithScores(
     ...
     * @see <a href="https://valkey.io/commands/zinterstore">Valkey Documentation: ZINTERSTORE</a>
     */
    default Mono<Long> intersectAndStore(
            K key, Collection<K> otherKeys, K destKey, Aggregate aggregate) {
        return intersectAndStore(
                key, otherKeys, destKey, aggregate, Weights.fromSetCount(1 + otherKeys.size()));
    }
     ...
     * @see <a href="https://valkey.io/commands/zunion">Valkey Documentation: ZUNION</a>
     */
    Flux<TypedTuple<V>> unionWithScores(
            K key, Collection<K> otherKeys, Aggregate aggregate, Weights weights);
     ...
     * @see <a href="https://valkey.io/commands/zunionstore">Valkey Documentation: ZUNIONSTORE</a>
     */
    Mono<Long> unionAndStore(
            K key, Collection<K> otherKeys, K destKey, Aggregate aggregate, Weights weights);

    /**
     * Get all elements with lexicographical ordering from {@literal ZSET} at {@code key} with a value
     ...
     * starting at {@link Limit#getOffset()} with lexicographical ordering from {@literal ZSET} at
     * {@code key} with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param key must not be {@literal null}
     * @param range must not be {@literal null}.
    ...
    /**
     * Get all elements with reverse lexicographical ordering from {@literal ZSET} at {@code key} with
     * a value between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     ...
     * Get all elements {@literal n} elements, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with reverse lexicographical ordering from {@literal
     * ZSET} at {@code key} with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param key must not be {@literal null}
     * @param range must not be {@literal null}.
