...
/**
 * Atomic double backed by Valkey. Uses Valkey atomic increment/decrement and watch/multi/exec
...
public class ValkeyAtomicDouble extends Number implements Serializable, BoundKeyOperations<String> {
    ...
    /**
     * Constructs a new {@link ValkeyAtomicDouble} instance. Uses the value existing in Valkey or
     * {@code 0} if none is found.
     *
    ...
    /**
     * Constructs a new {@link ValkeyAtomicDouble} instance with a {@code initialValue} that
     ...
     */
    public ValkeyAtomicDouble(
            String valkeyCounter, ValkeyConnectionFactory factory, double initialValue) {
        this(valkeyCounter, factory, Double.valueOf(initialValue));
    }

    private ValkeyAtomicDouble(
            String valkeyCounter, ValkeyConnectionFactory factory, @Nullable Double initialValue) {
    ...
    /**
     * Constructs a new {@link ValkeyAtomicDouble} instance. Uses the value existing in Valkey or 0 if
     * none is found.
     *
     * @param valkeyCounter Valkey key of this counter.
    ...
    /**
     * Constructs a new {@link ValkeyAtomicDouble} instance with a {@code initialValue} that
     * overwrites the existing value at {@code valkeyCounter}.
     *
     * <p>Note: You need to configure the given {@code template} with appropriate {@link
     * ValkeySerializer} for the key and value.
     *
     * <p>As an alternative one could use the {@link #ValkeyAtomicDouble(String,
     * ValkeyConnectionFactory, Double)} constructor which uses appropriate default serializers.
     *
     * @param valkeyCounter Valkey key of this counter.
     * @param template the template
     ...
     */
    public ValkeyAtomicDouble(
            String valkeyCounter, ValkeyOperations<String, Double> template, double initialValue) {
        this(valkeyCounter, template, Double.valueOf(initialValue));
    }

    private ValkeyAtomicDouble(
            String valkeyCounter,
            ValkeyOperations<String, Double> template,
        ...
        Assert.notNull(
                template.getValueSerializer(), "a valid value serializer in template is required");
    ...
    /**
     * Atomically set the value to the given updated value if the current value {@code ==} the
     * expected value.
     *
     * @param expect the expected value.
     * @param update the new value.
     * @return {@literal true} if successful. {@literal false} indicates that the actual value was not
    ...
    /**
     * Atomically update the current value using the given {@link DoubleUnaryOperator update
     * function}.
     *
     * @param updateFunction the function which calculates the value to set. Should be a pure function
     *     (no side effects), because it will be applied several times if update attempts fail due to
     *     concurrent calls. Must not be {@literal null}.
     * @return the previous value.
     * @since 2.2
     */
    ...
    /**
     * Atomically update the current value using the given {@link DoubleBinaryOperator accumulator
     * function}. The new value is calculated by applying the accumulator function to the current
     * value and the given {@code updateValue}.
     *
     * @param updateValue the value which will be passed into the accumulator function.
     * @param accumulatorFunction the function which calculates the value to set. Should be a pure
     *     function (no side effects), because it will be applied several times if update attempts
     *     fail due to concurrent calls. Must not be {@literal null}.
     * @return the previous value.
     * @since 2.2
     */
