...
/**
 * Container providing asynchronous behaviour for Valkey message listeners. Handles the low level
 * details of listening, converting and message dispatching.
 *
 * <p>As opposed to the low level Valkey (one connection per subscription), the container uses only
 * one connection that is 'multiplexed' for all registered listeners, the message dispatch being
 * done through the {@link #setTaskExecutor(Executor) task executor}. It is recommended to configure
 * the task executor (and subscription executor when using a blocking Valkey connector) instead of
 * using the default {@link SimpleAsyncTaskExecutor} for reuse of thread pools.
 *
 * <p>The container uses a single Valkey connection in a lazy fashion (the connection is used only
 * if at least one listener is configured). Listeners can be registered eagerly before {@link
 * #start() starting} the container to subscribe to all registered topics upon startup. Listeners
 * are guaranteed to be subscribed after the {@link #start()} method returns.
 *
 * <p>Subscriptions are retried gracefully using {@link BackOff} that can be configured through
 * {@link #setRecoveryInterval(long)} until reaching the maximum number of attempts. Listener errors
 * are handled through a {@link ErrorHandler} if configured.
 *
 * <p>This class can be used concurrently after initializing the container with {@link
 * #afterPropertiesSet()} and {@link #start()} allowing concurrent calls to {@link
 * #addMessageListener} and {@link #removeMessageListener} without external synchronization.
 *
 * <p>{@link MessageListener Listeners} that wish to receive subscription/unsubscription callbacks
 * in response to subscribe/unsubscribe commands can implement {@link SubscriptionListener}.
 *
 * @author Costin Leau
 ...
 */
public class ValkeyMessageListenerContainer
    ...
    public static final String DEFAULT_THREAD_NAME_PREFIX =
            ClassUtils.getShortName(ValkeyMessageListenerContainer.class) + "-";
    ...
    private BackOff backOff =
            new FixedBackOff(DEFAULT_RECOVERY_INTERVAL, FixedBackOff.UNLIMITED_ATTEMPTS);
    ...
    // Lookup maps; to avoid creation of hashes for each message, the maps use raw byte arrays
    // (wrapped to respect
    ...
    /**
     * Set an ErrorHandler to be invoked in case of any uncaught exceptions thrown while processing a
     * Message. By default, there will be <b>no</b> ErrorHandler so that error-level logging is the
     * only result.
     */
    ...
    /**
     * Sets the task execution used for subscribing to Valkey channels. By default, if no executor is
     * set, the {@link #setTaskExecutor(Executor)} will be used. In some cases, this might be
     * undesired as the listening to the connection is a long-running task.
     *
     * <p>Note: This implementation uses at most one long-running thread (depending on whether there
     * are any listeners registered or not) and up to two threads during the initial registration.
     *
    ...
    /**
     * Sets the task executor used for running the message listeners when messages are received. If no
     * task executor is set, an instance of {@link SimpleAsyncTaskExecutor} will be used by default.
    ...
    /**
     * Sets the serializer for converting the {@link Topic}s into low-level channels and patterns. By
     * default, {@link StringValkeySerializer} is used.
     *
    ...
    /**
     * Specify the max time to wait for subscription registrations, in <strong>milliseconds</strong>
     * The default is {@code 2000ms}, that is, 2 second. The timeout applies for awaiting the
     * subscription registration. Note that subscriptions can be created asynchronously and an expired
     * timeout does not cancel the timeout.
     *
     ...
     *
     * <p>Note: it's possible to call this method while the container is running forcing a
     * reinitialization of the container. Note however that this might cause some messages to be lost
     * (while the container reinitializes) - hence calling this method at runtime is considered
     * advanced usage.
     *
    ...
    protected TaskExecutor createDefaultTaskExecutor() {
        String threadNamePrefix =
                this.beanName != null ? this.beanName + "-" : DEFAULT_THREAD_NAME_PREFIX;
    ...
    /**
     * Startup the container and subscribe to topics if {@link MessageListener listeners} were
     * registered prior to starting the container.
     *
     * <p>This method is a potentially blocking method that blocks until a previous {@link #stop()} is
     * finished and until all previously registered listeners are successfully subscribed.
     *
     * <p>Multiple calls to this method are ignored if the container is already running. Concurrent
     * calls to this method are synchronized until the container is started up.
     *
     * @see #setRecoveryInterval(long)
    ...
    private void lazyListen() {
        ...
        CompletableFuture<Void> futureToAwait =
                state.isPrepareListening()
                        ? containerListenFuture
    ...
    private boolean doSubscribe(BackOffExecution backOffExecution) {
        ...
        CompletableFuture<Void> listenFuture =
                getRequiredSubscriber()
                        .initialize(
                                backOffExecution,
                                patternMapping.keySet().stream()
                                        .map(ByteArrayWrapper::getArray)
                                        .collect(Collectors.toList()),
                                channelMapping.keySet().stream()
                                        .map(ByteArrayWrapper::getArray)
                                        .collect(Collectors.toList()));
    ...
    /**
     * Stop the message listener container and cancel any subscriptions if the container is {@link
     * #isListening() listening}. Stopping releases any allocated connections.
     *
     * <p>This method is a potentially blocking method that blocks until a previous {@link #start()}
     * is finished and until the connection is closed if the container was listening.
     *
     * <p>Multiple calls to this method are ignored if the container was already stopped. Concurrent
     * calls to this method are synchronized until the container is stopped.
     */
    @Override
    public void stop() {
        stop(() -> {});
    ...
    /**
     * Removes a message listener from the given topics. If the container is running, the listener
     * stops receiving (matching) messages as soon as possible.
     *
     * <p>Note that this method obeys the Valkey (p)unsubscribe semantics - meaning an empty/null
     * collection will remove listener from all channels.
     *
     * @param listener message listener.
     * @param topics message listener topics.
     */
    public void removeMessageListener(
            @Nullable MessageListener listener, Collection<? extends Topic> topics) {
        removeListener(listener, topics);
    }

    /**
     * Removes a message listener from the given topic. If the container is running, the listener
     * stops receiving (matching) messages as soon as possible.
     *
     * <p>Note that this method obeys the Valkey (p)unsubscribe semantics - meaning an empty/null
     * collection will remove listener from all channels.
    ...
    private void initMapping(Map<? extends MessageListener, Collection<? extends Topic>> listeners) {
        ...
        if (!CollectionUtils.isEmpty(listeners)) {
            for (Map.Entry<? extends MessageListener, Collection<? extends Topic>> entry :
                    listeners.entrySet()) {
    ...
    private void addListener(MessageListener listener, Collection<? extends Topic> topics) {
        ...
        for (Topic topic : topics) {
            ...
            if (topic instanceof ChannelTopic) {
                Collection<MessageListener> collection =
                        resolveMessageListeners(this.channelMapping, serializedTopic);
                collection.add(listener);
                channels.add(serializedTopic.getArray());
                logTrace(
                        () -> "Adding listener '%s' on channel '%s'".formatted(listener, topic.getTopic()));
            } else if (topic instanceof PatternTopic) {
                Collection<MessageListener> collection =
                        resolveMessageListeners(this.patternMapping, serializedTopic);
                collection.add(listener);
                patterns.add(serializedTopic.getArray());
                logTrace(
                        () -> "Adding listener '%s' for pattern '%s'".formatted(listener, topic.getTopic()));
            } else {
        ...
        if (isRunning()) {
            ...
            if (wasListening) {
                ...
                getRequiredSubscriber()
                        .addSynchronization(
                                new SynchronizingMessageListener.SubscriptionSynchronization(
    ...
    private void removeListener(
        ...
        for (Topic topic : topics) {
            ...
            if (topic instanceof ChannelTopic) {
                ...
                logTrace(
                        () -> "Removing listener '%s' from channel '%s'".formatted(listener, topic.getTopic()));
            } else if (topic instanceof PatternTopic) {
                remove(listener, topic, holder, patternMapping, patternsToRemove);
                logTrace(
                        () -> "Removing listener '%s' from pattern '%s'".formatted(listener, topic.getTopic()));
            }
        }
        ...
        // check the current listening state
            ...
            getRequiredSubscriber()
                    .unsubscribeChannel(channelsToRemove.toArray(new byte[channelsToRemove.size()][]));
            getRequiredSubscriber()
                    .unsubscribePattern(patternsToRemove.toArray(new byte[patternsToRemove.size()][]));
        }
    }
    ...
    private void remove(
        ...
        Collection<MessageListener> listenersToRemove =
                (listener == null) ? new ArrayList<>(listeners) : Collections.singletonList(listener);
    ...
    private Subscriber createSubscriber(
            ValkeyConnectionFactory connectionFactory, Executor executor) {
        return ConnectionUtils.isAsync(connectionFactory)
                ? new Subscriber(connectionFactory)
     ...
     *
     * <p>The default implementation logs the exception at error level. This can be overridden in
     * subclasses.
     *
    ...
    /**
     * Handle subscription task exception. Will attempt to restart the subscription if the Exception
     * is a connection failure (for example, Valkey was restarted).
     *
     * @param cause Throwable exception
     */
    protected void handleSubscriptionException(
            CompletableFuture<Void> future, BackOffExecution backOffExecution, Throwable cause) {
        ...
        if (cause instanceof ValkeyConnectionFailureException && isRunning()) {
            ...
            BackOffExecution loggingBackOffExecution =
                    () -> {
                        ...
                        if (recoveryInterval != BackOffExecution.STOP) {
                            logger.error(
                                    "Connection failure occurred: %s; Restarting subscription task after %s ms"
                                            .formatted(cause, recoveryInterval),
                                    cause);
                        }
            ...
            Runnable recoveryFunction =
                    () -> {
                        CompletableFuture<Void> lazyListen =
                                lazyListen(new RecoveryBackoffExecution(backOffExecution));
                        lazyListen
                                .whenComplete(propagate(future))
                                .thenRun(
                                        () -> {
            ...
            future.completeExceptionally(
                    new IllegalStateException("Subscription attempts exceeded", cause));
            return;
        }

        if (isRunning()) { // log only if the container is still running to prevent close errors from
            // logging
            logger.error("SubscriptionTask aborted with exception:", cause);
        }
    ...
    private void dispatchSubscriptionNotification(
    ...
    private void dispatchMessage(
            Collection<MessageListener> listeners, Message message, @Nullable byte[] pattern) {
    ...
    private Subscriber getRequiredSubscriber() {
        ...
        Assert.state(
                ...
                "Subscriber not created; Configure ValkeyConnectionFactory to create a Subscriber. Make"
                        + " sure that afterPropertiesSet() has been called");
     ...
     * @param delegate
     */
    record RecoveryAfterSubscriptionBackoffExecution(BackOffExecution delegate)
    ...
    /**
     * Represents an operation that accepts three input arguments {@link SubscriptionListener}, {@code
     * channel or pattern}, and {@code count} and returns no result.
     */
    ...
    static class State {
        ...
        /**
         * Prepare listening after {@link #notListening()}. Next states are either {@link
         * #notListening()} upon failure or {@link #listening()}.
         */
        ...
        /**
         * Active listening state after {@link #prepareListening()}. Next is {@link
         * #prepareUnsubscribe()}.
         */
    ...
    private class DispatchMessageListener implements MessageListener, SubscriptionListener {
        ...
        public void onChannelSubscribed(byte[] channel, long count) {
            dispatchSubscriptionNotification(
                    channelMapping.getOrDefault(new ByteArrayWrapper(channel), Collections.emptyList()),
                    channel,
                    count,
        ...
        public void onPatternSubscribed(byte[] pattern, long count) {
            dispatchSubscriptionNotification(
                    patternMapping.getOrDefault(new ByteArrayWrapper(pattern), Collections.emptyList()),
                    pattern,
                    count,
    ...
    /**
     * Topic subscriber controller. Keeps track of the actual Valkey connection and provides entry
     * points to initially subscribe to Valkey topics and update subscriptions (add/remove).
     *
     * <p>Actual subscription notifications are routed through {@link DispatchMessageListener} to
     * multicast events to the actual listeners without blocking the event loop.
     *
     * @author Mark Paluch
     * @since 2.7
     */
    class Subscriber {
        ...
        private final SynchronizingMessageListener synchronizingMessageListener =
                new SynchronizingMessageListener(delegateListener, delegateListener);
         ...
         * @return a future that is completed either successfully after establishing all subscriptions
         *     or exceptionally after an error or when running out of {@link BackOffExecution#STOP
         *     retries}.
         */
        public CompletableFuture<Void> initialize(
                BackOffExecution backOffExecution,
                Collection<byte[]> patterns,
                        ...
                        try {
                            ...
                            if (connection.isSubscribed()) {
                                ...
                                initFuture.completeExceptionally(
                                        new IllegalStateException(
                                                "Retrieved connection is already subscribed; aborting listening"));
                            ...
                            try {
                                eventuallyPerformSubscription(
                                        connection, backOffExecution, initFuture, patterns, channels);
                            } catch (Throwable t) {
                                handleSubscriptionException(
                                        initFuture,
                                        nextBackoffExecution(backOffExecution, connection.isSubscribed()),
                                        t);
                            }
        ...
        void eventuallyPerformSubscription(
                ValkeyConnection connection,
                BackOffExecution backOffExecution,
                CompletableFuture<Void> subscriptionDone,
                Collection<byte[]> patterns,
                Collection<byte[]> channels) {
         ...
         */
        void doSubscribe(
                ValkeyConnection connection, Collection<byte[]> patterns, Collection<byte[]> channels) {
    ...
    class BlockingSubscriber extends Subscriber {
        ...
        void doUnsubscribe(ValkeyConnection connection) {
            closeSubscription(
                    connection); // connection will be closed after exiting the doSubscribe method
        }

        @Override
        protected void eventuallyPerformSubscription(
                ValkeyConnection connection,
                BackOffExecution backOffExecution,
                CompletableFuture<Void> subscriptionDone,
                Collection<byte[]> patterns,
                        ...
                        new SynchronizingMessageListener.SubscriptionSynchronization(
                                ...
                                () -> {
                                    try {
                                        ...
                                        handleSubscriptionException(
                                                subscriptionDone, nextBackoffExecution(backOffExecution, true), ex);
                                    }
                                }));
            } else {
            ...
            addSynchronization(
                    new SynchronizingMessageListener.SubscriptionSynchronization(
                            patterns, channels, () -> subscriptionDone.complete(null)));
