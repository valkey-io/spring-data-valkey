...
public class Expiration {
     ...
     * @param timeUnit {@link TimeUnit} used to measure the expiration period; must not be {@literal
     *     null}.
    ...
    /**
     * Creates new {@link Expiration} with the provided {@link TimeUnit}. Greater units than {@link
     * TimeUnit#SECONDS} are converted to {@link TimeUnit#SECONDS}. Units smaller than {@link
     * TimeUnit#MILLISECONDS} are converted to {@link TimeUnit#MILLISECONDS} and can lose precision
     * since {@link TimeUnit#MILLISECONDS} is the smallest granularity supported by Valkey.
     *
     * @param expirationTime {@link Long length of time} for the {@link Expiration}.
     * @param timeUnit {@link TimeUnit} used to measure the {@link Long expiration time}; can be
     *     {@literal null}. Defaulted to {@link TimeUnit#SECONDS}
     * @return a new {@link Expiration} configured with the given {@link Long length of time} in
     *     {@link TimeUnit}.
     */
    public static Expiration from(long expirationTime, @Nullable TimeUnit timeUnit) {
        ...
        return timeUnit != null
                ? new Expiration(timeUnit.toSeconds(expirationTime), TimeUnit.SECONDS)
     ...
     *
     * <p>Durations with at least {@literal seconds} resolution uses {@link TimeUnit#SECONDS}. {@link
     * Duration Durations} in {@literal milliseconds} use {@link TimeUnit#MILLISECONDS}.
     *
    ...
    /**
     * Obtain an {@link Expiration} that indicates to keep the existing one, e.g. when sending a
     * {@code SET} command.
     *
     * <p><strong>NOTE: </strong>Please follow the documentation for the individual commands to see if
     * keeping the existing TTL is applicable.
     *
     ...
     *
     * @param expirationTime {@link Long length of time} for expiration. Defaulted to {@link
     *     TimeUnit#SECONDS}.
     ...
     *
     * @param targetTimeUnit {@link TimeUnit} used to convert the {@link #getExpirationTime()}
     *     expiration time}; must not be {@literal null}.
     * @return the {@link #getExpirationTime() expiration time} converted into the given, desired
     *     {@link TimeUnit}.
    ...
    /**
     * @return {@literal true} if {@link Expiration} is set to a specified Unix time at which the key
     *     will expire.
     * @since 2.6
     */
    ...
    public boolean equals(Object obj) {
        ...
        return this.getTimeUnit().toMillis(getExpirationTime())
                == that.getTimeUnit().toMillis(that.getExpirationTime());
    }
