...
import static io.valkey.springframework.data.valkey.connection.ReactiveListCommands.*;
...
public interface ReactiveListOperations<K, V> {
    ...
    default Mono<V> move(MoveFrom<K> from, MoveTo<K> to) {
        ...
        return move(
                from.key,
                Direction.valueOf(from.direction.name()),
                to.key,
                Direction.valueOf(to.direction.name()));
    }
     ...
     *
     * @param key must not be {@literal null}.
     * @param count {@link Long count} of the number of elements to remove from the left-side of the
     *     Valkey list.
     ...
     *
     * @param key must not be {@literal null}.
     * @param count {@link Long count} of the number of elements to remove from the right-side of the
     *     Valkey list.
     * @return a {@link Flux} containing the elements removed from the Valkey list.
     * @since 3.2
     * @see <a href="https://valkey.io/commands/rpop">Valkey Documentation: RPOP</a>
     ...
     * @param sourceKey must not be {@literal null}.
     * @param destinationKey must not be {@literal null}.
     * @param timeout maximal duration to wait until an entry in the list at {@code sourceKey} is
     *     available. Must be either {@link Duration#ZERO} or greater {@literal 1 second}, must not be
     *     {@literal null}. A timeout of zero can be used to wait indefinitely. Durations between zero
     *     and one second are not supported.
     * @return
