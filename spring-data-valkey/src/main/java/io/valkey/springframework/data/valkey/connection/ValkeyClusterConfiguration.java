...
/**
 * Configuration class used to set up a {@link ValkeyConnection} via {@link ValkeyConnectionFactory}
 * for connecting to <a href="https://valkey.io/topics/cluster-spec">Valkey Cluster</a>. Useful when
 * setting up a highly available Valkey environment.
 *
...
public class ValkeyClusterConfiguration implements ValkeyConfiguration, ClusterConfiguration {
    ...
    private static final String VALKEY_CLUSTER_MAX_REDIRECTS_CONFIG_PROPERTY =
            "spring.valkey.cluster.max-redirects";
    ...
    /**
     * Creates a new {@link ValkeyClusterConfiguration} for given {@link String hostPort}
     * combinations.
     *
     * <pre class="code">
    ...
    private void initialize(PropertySource<?> propertySource) {
        ...
        if (propertySource.containsProperty(VALKEY_CLUSTER_MAX_REDIRECTS_CONFIG_PROPERTY)) {
            ...
            Object clusterMaxRedirects =
                    propertySource.getProperty(VALKEY_CLUSTER_MAX_REDIRECTS_CONFIG_PROPERTY);
            this.maxRedirects =
                    NumberUtils.parseNumber(String.valueOf(clusterMaxRedirects), Integer.class);
        }
    }
     ...
     *
     * @param propertySource must not be {@literal null}.
     * @return a new {@link ValkeyClusterConfiguration} configured from the given {@link
     *     PropertySource}.
     * @since 3.3
     */
    ...
    private static Map<String, Object> asMap(Collection<String> clusterHostAndPorts, int redirects) {
        ...
        Assert.noNullElements(
                clusterHostAndPorts, "ClusterHostAndPorts must not contain null elements");

        Map<String, Object> map = new HashMap<>();

        map.put(
                VALKEY_CLUSTER_NODES_CONFIG_PROPERTY,
                StringUtils.collectionToCommaDelimitedString(clusterHostAndPorts));
