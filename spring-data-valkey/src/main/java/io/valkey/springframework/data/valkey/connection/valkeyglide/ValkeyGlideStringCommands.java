...
public class ValkeyGlideStringCommands implements ValkeyStringCommands {
    ...
    public Boolean setNX(byte[] key, byte[] value) {
        ...
        try {
            return connection.execute(
                    "SETNX", (Long glideResult) -> glideResult != null ? glideResult != 0 : null, key, value);
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
        }
    }
    ...
    public List<byte[]> mGet(byte[]... keys) {
        ...
        try {
            ...
            return connection.execute(
                    "MGET", (Object[] glideResult) -> ValkeyGlideConverters.toBytesList(glideResult), args);
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
        }
    }
    ...
    public List<Long> bitField(byte[] key, BitFieldSubCommands subCommands) {
        ...
        try {
            ...
            for (BitFieldSubCommands.BitFieldSubCommand subCommand : subCommands.getSubCommands()) {
                if (subCommand instanceof BitFieldSubCommands.BitFieldGet) {
                    ...
                    BitFieldSubCommands.BitFieldIncrBy incrBy =
                            (BitFieldSubCommands.BitFieldIncrBy) subCommand;
            ...
            return connection.execute(
                    "BITFIELD", (Object[] glideResult) -> toBitFieldLongsList(glideResult), args.toArray());
        } catch (Exception ex) {
            throw new ValkeyGlideExceptionConverter().convert(ex);
        }
    }
    ...
    /**
     * Dedicated mapper for BITFIELD command results that handles null values. BITFIELD is unique in
     * that it can return NULL elements in the array when OVERFLOW FAIL is used and an
