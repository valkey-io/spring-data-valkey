...
class LettuceReactiveValkeyClusterConnection extends LettuceReactiveValkeyConnection
    ...
    /**
     * Creates new {@link LettuceReactiveValkeyClusterConnection} given {@link
     * LettuceConnectionProvider} and {@link RedisClusterClient}.
     *
     * @param connectionProvider must not be {@literal null}.
     * @param client must not be {@literal null}.
     * @throws IllegalArgumentException when {@code client} is {@literal null}.
     */
    LettuceReactiveValkeyClusterConnection(
    ...
    /**
     * Creates new {@link LettuceReactiveValkeyClusterConnection} given a shared {@link
     * StatefulConnection connection}, {@link LettuceConnectionProvider} and {@link
     * RedisClusterClient}.
     *
                ...
                .flatMapMany(
                        nodeToUse ->
                                execute(
                                        nodeToUse,
                                        cmd ->
                                                cmd.clusterSlaves(nodeToUse.getId()) //
                ...
                .flatMap(
                        node -> {
                            return Mono.just(node)
                                    .zipWith(
                                            execute(node, cmd -> cmd.clusterSlaves(node.getId())) //
                                                    .collectList() //
    ...
    @Override
    public Mono<Void> clusterDeleteSlotsInRange(
            ValkeyClusterNode node, ValkeyClusterNode.SlotRange range) {
    ...
    public Mono<Void> clusterForget(ValkeyClusterNode node) {
        ...
        return Flux.fromStream(
                ...
                .flatMap(actualNode -> execute(node, cmd -> cmd.clusterForget(nodeToRemove.getId())))
                .then();
    }
    ...
    public Mono<Void> clusterMeet(ValkeyClusterNode node) {
        ...
        Assert.isTrue(
                node.getPort() != null && node.getPort() > 0,
                "Node to meet cluster must have a port greater 0");

        return clusterGetNodes()
                .flatMap(
                        actualNode -> execute(node, cmd -> cmd.clusterMeet(node.getHost(), node.getPort())))
                .then();
    }
    ...
    protected Mono<RedisReactiveCommands<ByteBuffer, ByteBuffer>> getCommands(ValkeyNode node) {
        ...
        if (StringUtils.hasText(node.getId())) {
            return getConnection()
                    .cast(StatefulRedisClusterConnection.class)
                    .flatMap(
                            it -> {
        ...
        return getConnection()
                .flatMap(
                        it ->
                                Mono.fromCompletionStage(it.getConnectionAsync(node.getHost(), node.getPort()))
                                        .map(StatefulRedisConnection::reactive));
    }

    /**
     * Lookup a {@link ValkeyClusterNode} by using either ids {@link ValkeyClusterNode#getId() node
     * id} or host and port to obtain the full node details from the underlying {@link
