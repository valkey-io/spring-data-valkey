 ...
 * {@link ValkeyConnectionFactory} for connecting to <a
 * href="https://valkey.io/topics/sentinel">Valkey Sentinel(s)</a>. Useful when setting up a highly
 * available Valkey environment.
 *
...
public class ValkeySentinelConfiguration implements ValkeyConfiguration, SentinelConfiguration {
    ...
    private static final String VALKEY_SENTINEL_MASTER_CONFIG_PROPERTY =
            "spring.valkey.sentinel.master";
    private static final String VALKEY_SENTINEL_NODES_CONFIG_PROPERTY =
            "spring.valkey.sentinel.nodes";
    private static final String VALKEY_SENTINEL_USERNAME_CONFIG_PROPERTY =
            "spring.valkey.sentinel.username";
    private static final String VALKEY_SENTINEL_PASSWORD_CONFIG_PROPERTY =
            "spring.valkey.sentinel.password";
    private static final String VALKEY_SENTINEL_DATA_NODE_USERNAME_CONFIG_PROPERTY =
            "spring.valkey.sentinel.dataNode.username";
    private static final String VALKEY_SENTINEL_DATA_NODE_PASSWORD_CONFIG_PROPERTY =
            "spring.valkey.sentinel.dataNode.password";
    private static final String VALKEY_SENTINEL_DATA_NODE_DATABASE_CONFIG_PROPERTY =
            "spring.valkey.sentinel.dataNode.database";
    ...
    /**
     * Creates a new {@link ValkeySentinelConfiguration} for given {@link String hostPort}
     * combinations.
     *
     * <pre class="code">
    ...
    /**
     * Creates a new {@link ValkeySentinelConfiguration} looking up configuration values from the
     * given {@link PropertySource}.
     *
     * <pre class="code">
     ...
     * @since 1.5
     * @deprecated since 3.3, use {@link ValkeySentinelConfiguration#of(PropertySource)} instead. This
     *     constructor will be made private in the next major release.
     */
    ...
    public ValkeySentinelConfiguration(PropertySource<?> propertySource) {
        ...
        if (propertySource.containsProperty(VALKEY_SENTINEL_MASTER_CONFIG_PROPERTY)) {
            String sentinelMaster =
                    String.valueOf(propertySource.getProperty(VALKEY_SENTINEL_MASTER_CONFIG_PROPERTY));
        ...
        if (propertySource.containsProperty(VALKEY_SENTINEL_NODES_CONFIG_PROPERTY)) {
            String sentinelNodes =
                    String.valueOf(propertySource.getProperty(VALKEY_SENTINEL_NODES_CONFIG_PROPERTY));
        ...
        if (propertySource.containsProperty(VALKEY_SENTINEL_PASSWORD_CONFIG_PROPERTY)) {
            String sentinelPassword =
                    String.valueOf(propertySource.getProperty(VALKEY_SENTINEL_PASSWORD_CONFIG_PROPERTY));
        ...
        if (propertySource.containsProperty(VALKEY_SENTINEL_USERNAME_CONFIG_PROPERTY)) {
            String sentinelUsername =
                    String.valueOf(propertySource.getProperty(VALKEY_SENTINEL_USERNAME_CONFIG_PROPERTY));
        ...
        if (propertySource.containsProperty(VALKEY_SENTINEL_DATA_NODE_USERNAME_CONFIG_PROPERTY)) {
            String dataNodeUsername =
                    String.valueOf(
                            propertySource.getProperty(VALKEY_SENTINEL_DATA_NODE_USERNAME_CONFIG_PROPERTY));
            this.setUsername(dataNodeUsername);
        }

        if (propertySource.containsProperty(VALKEY_SENTINEL_DATA_NODE_PASSWORD_CONFIG_PROPERTY)) {
            String dataNodePassword =
                    String.valueOf(
                            propertySource.getProperty(VALKEY_SENTINEL_DATA_NODE_PASSWORD_CONFIG_PROPERTY));
        ...
        if (propertySource.containsProperty(VALKEY_SENTINEL_DATA_NODE_DATABASE_CONFIG_PROPERTY)) {
            String databaseSource =
                    String.valueOf(
                            propertySource.getProperty(VALKEY_SENTINEL_DATA_NODE_DATABASE_CONFIG_PROPERTY));
            int database;
            try {
                database = Integer.parseInt(databaseSource);
            } catch (NumberFormatException ex) {
                throw new IllegalArgumentException(
                        "Invalid DB index '%s'; integer required".formatted(databaseSource));
            }
     ...
     *
     * @param propertySource must not be {@literal null}.
     * @return a new {@link ValkeySentinelConfiguration} configured from the given {@link
     *     PropertySource}.
     * @since 3.3
     */
    ...
    public void setDatabase(int index) {
        ...
        Assert.isTrue(
                index >= 0, "Invalid DB index '%d'; non-negative index required".formatted(index));
    ...
    private static Map<String, Object> asMap(String master, Set<String> sentinelHostAndPorts) {
        ...
        Assert.noNullElements(
                sentinelHostAndPorts, "ClusterHostAndPorts must not contain null elements");

        Map<String, Object> map = new HashMap<>();

        map.put(VALKEY_SENTINEL_MASTER_CONFIG_PROPERTY, master);
        map.put(
                VALKEY_SENTINEL_NODES_CONFIG_PROPERTY,
