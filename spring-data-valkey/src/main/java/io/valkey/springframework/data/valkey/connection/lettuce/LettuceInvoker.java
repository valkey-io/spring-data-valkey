...
/**
 * Utility for functional invocation of {@link RedisClusterAsyncCommands Lettuce methods}. Typically
 * used to express the method call as method reference and passing method arguments through one of
 * the {@code just} or {@code from} methods.
 *
 ...
 * </pre>
 *
 * <p>The actual translation from {@link RedisFuture} is delegated to {@link Synchronizer} which can
 * either await completion or record the future along {@link Converter} for further processing.
 *
 * @author Mark Paluch
...
class LettuceInvoker {
    ...
    <R> R just(ConnectionFunction0<R> function) {
        ...
        return synchronizer.invoke(
                () -> function.apply(connection), Converters.identityConverter(), () -> null);
    }
    ...
    <R, T1, T2, T3> SingleInvocationSpec<R> from(
            ConnectionFunction3<T1, T2, T3, R> function, T1 t1, T2 t2, T3 t3) {
    ...
    <R extends Collection<E>, E, T1> ManyInvocationSpec<E> fromMany(
            ConnectionFunction1<T1, R> function, T1 t1) {
    ...
    <R extends Collection<E>, E, T1, T2> ManyInvocationSpec<E> fromMany(
            ConnectionFunction2<T1, T2, R> function, T1 t1, T2 t2) {
    ...
    interface ConnectionFunction4<T1, T2, T3, T4, R> {
         ...
         */
        RedisFuture<R> apply(
                RedisClusterAsyncCommands<byte[], byte[]> connection, T1 t1, T2 t2, T3 t3, T4 t4);
    }
    ...
    interface ConnectionFunction5<T1, T2, T3, T4, T5, R> {
         ...
         */
        RedisFuture<R> apply(
                RedisClusterAsyncCommands<byte[], byte[]> connection, T1 t1, T2 t2, T3 t3, T4 t4, T5 t5);
    }
    ...
    static class DefaultManyInvocationSpec<S> implements ManyInvocationSpec<S> {
        ...
        public DefaultManyInvocationSpec(
                Supplier<RedisFuture<? extends Collection<S>>> parent, Synchronizer synchronizer) {
    ...
    interface Synchronizer {
        ...
        default <I, T> T invoke(
                Supplier<RedisFuture<I>> futureSupplier,
        ...
        @Nullable
        Object doInvoke(
                Supplier<RedisFuture<Object>> futureSupplier,
                Converter<Object, Object> converter,
                Supplier<Object> nullDefault);
    }
}
