...
import io.valkey.springframework.data.valkey.connection.stream.StreamInfo;
...
class JedisStreamCommands implements ValkeyStreamCommands {
    ...
    public Long xAck(byte[] key, String group, RecordId... recordIds) {
        ...
        return connection
                ...
                .just(
                        Jedis::xack,
                        PipelineBinaryCommands::xack,
                        key,
                        JedisConverters.toBytes(group),
    ...
    public List<RecordId> xClaimJustId(
        ...
        return connection
                ...
                .fromMany(
                        Jedis::xclaimJustId,
                        ResponseCommands::xclaimJustId,
                        key,
                        JedisConverters.toBytes(group),
                        JedisConverters.toBytes(newOwner),
                        options.getMinIdleTime().toMillis(),
                        params,
    ...
    public String xGroupCreate(
        ...
        return connection
                ...
                .just(
                        Jedis::xgroupCreate,
                        PipelineBinaryCommands::xgroupCreate,
                        key,
                        JedisConverters.toBytes(groupName),
                        JedisConverters.toBytes(readOffset.getOffset()),
                        mkStream);
    }
    ...
    public Boolean xGroupDelConsumer(byte[] key, Consumer consumer) {
        ...
        return connection
                .invoke()
                .from(
                        Jedis::xgroupDelConsumer,
                        PipelineBinaryCommands::xgroupDelConsumer,
                        key,
                        JedisConverters.toBytes(consumer.getGroup()),
                        JedisConverters.toBytes(consumer.getName()))
                .get(r -> r > 0);
    }
    ...
    public Boolean xGroupDestroy(byte[] key, String groupName) {
        ...
        return connection
                ...
                .from(
                        Jedis::xgroupDestroy,
                        PipelineBinaryCommands::xgroupDestroy,
                        key,
                        JedisConverters.toBytes(groupName))
                .get(r -> r > 0);
    }
    ...
    public StreamInfo.XInfoStream xInfo(byte[] key) {
        ...
        return connection
                .invoke()
                .from(Jedis::xinfoStream, ResponseCommands::xinfoStream, key)
                .get(
                        it -> {
                            redis.clients.jedis.resps.StreamInfo streamInfo =
                                    BuilderFactory.STREAM_INFO.build(it);
                            return StreamInfo.XInfoStream.fromList(
                                    StreamConverters.mapToList(streamInfo.getStreamInfo()));
                        });
    }
    ...
    public StreamInfo.XInfoGroups xInfoGroups(byte[] key) {
        ...
        return connection
                .invoke()
                .from(Jedis::xinfoGroups, StreamPipelineBinaryCommands::xinfoGroups, key)
                .get(
                        it -> {
                            List<StreamGroupInfo> streamGroupInfos =
                                    BuilderFactory.STREAM_GROUP_INFO_LIST.build(it);
                            List<Object> sources = new ArrayList<>();
                            streamGroupInfos.forEach(
                                    streamGroupInfo ->
                                            sources.add(StreamConverters.mapToList(streamGroupInfo.getGroupInfo())));
    ...
    public StreamInfo.XInfoConsumers xInfoConsumers(byte[] key, String groupName) {
        ...
        return connection
                .invoke()
                .from(
                        Jedis::xinfoConsumers,
                        ResponseCommands::xinfoConsumers,
                        key,
                        JedisConverters.toBytes(groupName))
                .get(
                        it -> {
                            List<StreamConsumersInfo> streamConsumersInfos =
                                    BuilderFactory.STREAM_CONSUMERS_INFO_LIST.build(it);
                            List<Object> sources = new ArrayList<>();
                            streamConsumersInfos.forEach(
                                    streamConsumersInfo ->
                                            sources.add(
                                                    StreamConverters.mapToList(streamConsumersInfo.getConsumerInfo())));
    ...
    public PendingMessagesSummary xPending(byte[] key, String groupName) {
        ...
        return connection
                ...
                .from(
                        Jedis::xpending,
                        PipelineBinaryCommands::xpending,
                        key,
                        JedisConverters.toBytes(groupName))
    ...
    public PendingMessages xPending(byte[] key, String groupName, XPendingOptions options) {
        ...
        return connection
                ...
                .from(
                        Jedis::xpending,
                        ResponseCommands::xpending,
                        key,
                        JedisConverters.toBytes(groupName),
                        xPendingParams)
                .get(
                        r ->
                                StreamConverters.toPendingMessages(
                                        groupName, range, BuilderFactory.STREAM_PENDING_ENTRY_LIST.build(r)));
    }
    ...
    public List<ByteRecord> xRead(StreamReadOptions readOptions, StreamOffset<byte[]>... streams) {
        ...
        return connection
                ...
                .from(
                        Jedis::xread,
                        ResponseCommands::xread,
                        params,
                        StreamConverters.toStreamOffsets(streams))
    ...
    public Long xTrim(byte[] key, long count, boolean approximateTrimming) {
        ...
        return connection
                .invoke()
                .just(Jedis::xtrim, PipelineBinaryCommands::xtrim, key, count, approximateTrimming);
    }
}
