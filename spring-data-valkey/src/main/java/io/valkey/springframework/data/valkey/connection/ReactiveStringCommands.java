...
public interface ReactiveStringCommands {
    ...
    class SetCommand extends KeyCommand {
        ...
        /**
         * Applies the {@literal value}. Constructs a new command instance with all previously
        ...
        /**
         * Applies {@link Expiration}. Constructs a new command instance with all previously configured
         * properties.
         *
        ...
        /**
         * Applies {@link SetOption}. Constructs a new command instance with all previously configured
         * properties.
         *
     ...
     * @param key must not be {@literal null}.
     * @param value must not be {@literal null}.
     * @param expiration must not be {@literal null}. Use {@link Expiration#persistent()} for no
     *     expiration time or {@link Expiration#keepTtl()} to keep the existing.
     * @param option must not be {@literal null}.
     * @return
     * @see <a href="https://valkey.io/commands/set">Valkey Documentation: SET</a>
     */
    default Mono<Boolean> set(
            ByteBuffer key, ByteBuffer value, Expiration expiration, SetOption option) {

        Assert.notNull(key, "Key must not be null");
        Assert.notNull(value, "Value must not be null");

        return set(Mono.just(
                        SetCommand.set(key).value(value).withSetOption(option).expiring(expiration)))
                .next()
     ...
     * @return {@link Flux} of {@link BooleanResponse} holding the {@link SetCommand} along with the
     *     command result.
     * @see <a href="https://valkey.io/commands/set">Valkey Documentation: SET</a>
     */
    ...
    /**
     * Set {@literal value} for {@literal key} with {@literal expiration} and {@literal options}.
     * Return the old string stored at key, or empty if key did not exist. An error is returned and
     ...
     */
    @Nullable
    default Mono<ByteBuffer> setGet(
            ByteBuffer key, ByteBuffer value, Expiration expiration, SetOption option) {
        ...
        return setGet(
                        Mono.just(SetCommand.set(key).value(value).withSetOption(option).expiring(expiration)))
                .next()
                .map(CommandResponse::getOutput);
    }

    /**
     * Set each and every item separately by invoking {@link SetCommand}. Return the old string stored
     * at key, or empty if key did not exist. An error is returned and SET aborted if the value stored
     * at key is not a string.
     *
     * @param commands must not be {@literal null}.
     * @return {@link Flux} of {@link ByteBufferResponse} holding the {@link SetCommand} along with
     *     the command result.
     * @see <a href="https://valkey.io/commands/set">Valkey Documentation: SET</a>
     * @since 3.5
     */
    ...
    default Mono<ByteBuffer> get(ByteBuffer key) {
        ...
        return get(Mono.just(new KeyCommand(key)))
                .next()
                .filter(CommandResponse::isPresent)
                .map(CommandResponse::getOutput);
    }
     ...
     * @return {@link Flux} of {@link ByteBufferResponse} holding the {@literal key} to get along with
     *     the value retrieved.
     * @see <a href="https://valkey.io/commands/get">Valkey Documentation: GET</a>
     */
    ...
    default Mono<ByteBuffer> getDel(ByteBuffer key) {
        ...
        return getDel(Mono.just(new KeyCommand(key)))
                .next()
                .filter(CommandResponse::isPresent)
                .map(CommandResponse::getOutput);
    }
    ...
    default Mono<ByteBuffer> getEx(ByteBuffer key, Expiration expiration) {
        ...
        return getEx(Mono.just(GetExCommand.key(key).withExpiration(expiration)))
                .next()
                .filter(CommandResponse::isPresent)
                .map(CommandResponse::getOutput);
    }
    ...
    default Mono<ByteBuffer> getSet(ByteBuffer key, ByteBuffer value) {
        ...
        return getSet(Mono.just(SetCommand.set(key).value(value)))
                .next()
                .filter(CommandResponse::isPresent)
    ...
    /**
     * Get multiple values in one batch. Values are in the order of the requested keys. Absent field
     * values are represented using {@literal null} in the resulting {@link List}.
     *
     * @param keys must not be {@literal null}.
     * @return
    ...
    /**
     * Get multiple values at for {@literal keysets} in batches. Values are in the order of the
    ...
    default Mono<Boolean> setNX(ByteBuffer key, ByteBuffer value) {
        ...
        return setNX(Mono.just(SetCommand.set(key).value(value)))
                .next()
                .map(BooleanResponse::getOutput);
    }
    ...
    default Mono<Boolean> setEX(ByteBuffer key, ByteBuffer value, Expiration expireTimeout) {
        ...
        return setEX(Mono.just(SetCommand.set(key).value(value).expiring(expireTimeout)))
                .next()
                .map(BooleanResponse::getOutput);
    }
    ...
    default Mono<Boolean> pSetEX(ByteBuffer key, ByteBuffer value, Expiration expireTimeout) {
        ...
        return pSetEX(Mono.just(SetCommand.set(key).value(value).expiring(expireTimeout)))
                .next()
                .map(BooleanResponse::getOutput);
    }
    ...
    /**
     * Set multiple keys to multiple values using key-value pairs provided in {@literal keyValuePairs}
    ...
    default Mono<Boolean> mSetNX(Map<ByteBuffer, ByteBuffer> keyValuePairs) {
        ...
        return mSetNX(Mono.just(MSetCommand.mset(keyValuePairs)))
                .next()
                .map(BooleanResponse::getOutput);
    }

    /**
     * Set multiple keys to multiple values using key-value pairs provided in {@literal tuples} only
    ...
    class AppendCommand extends KeyCommand {
        ...
        /**
         * Applies the {@literal value} to append. Constructs a new command instance with all previously
         * configured properties.
         *
         * @param value must not be {@literal null}.
    ...
    default Mono<Long> append(ByteBuffer key, ByteBuffer value) {
        ...
        return append(Mono.just(AppendCommand.key(key).append(value)))
                .next()
    ...
    class SetRangeCommand extends KeyCommand {
        ...
        /**
         * Applies the {@literal index}. Constructs a new command instance with all previously
         * configured properties.
         *
         * @param index
    ...
    /**
     * Overwrite parts of {@literal key} starting at the specified {@literal offset} with given
     * {@literal value}.
     *
     * @param key must not be {@literal null}.
     * @param value must not be {@literal null}.
    ...
    default Mono<Long> setRange(ByteBuffer key, ByteBuffer value, long offset) {
        ...
        return setRange(Mono.just(SetRangeCommand.overwrite(key).withValue(value).atPosition(offset)))
                .next()
                .map(NumericResponse::getOutput);
    }

    /**
     * Overwrite parts of {@link SetRangeCommand#key} starting at the specified {@literal offset} with
     * given {@link SetRangeCommand#value}.
     *
     * @param commands must not be {@literal null}.
     * @return
     * @see <a href="https://valkey.io/commands/setrange">Valkey Documentation: SETRANGE</a>
    ...
    class GetBitCommand extends KeyCommand {
        ...
        /**
         * Applies the offset {@literal index}. Constructs a new command instance with all previously
         * configured properties.
         *
         * @param offset
    ...
    default Mono<Boolean> getBit(ByteBuffer key, long offset) {
        ...
        return getBit(Mono.just(GetBitCommand.bit(key).atOffset(offset)))
                .next()
                .map(BooleanResponse::getOutput);
    }
    ...
    class SetBitCommand extends KeyCommand {
        ...
        /**
         * Applies the {@literal bit}. Constructs a new command instance with all previously configured
         * properties.
         *
         * @param bit
         * @return a new {@link SetBitCommand} with {@literal offset} applied.
    ...
    /**
     * Sets the bit at {@literal offset} in value stored at {@literal key} and return the original
     * value.
     *
     * @param key must not be {@literal null}.
     * @param offset
    ...
    default Mono<Boolean> setBit(ByteBuffer key, long offset, boolean value) {
        ...
        return setBit(Mono.just(SetBitCommand.bit(key).atOffset(offset).to(value)))
                .next()
                .map(BooleanResponse::getOutput);
    }
    ...
    class BitCountCommand extends KeyCommand {
        ...
        /**
         * Applies the {@link Range}. Constructs a new command instance with all previously configured
         * properties.
         *
    ...
    default Mono<Long> bitCount(ByteBuffer key) {
        ...
        return bitCount(Mono.just(BitCountCommand.bitCount(key)))
                .next()
                .map(NumericResponse::getOutput);
    }

    /**
     * Count the number of set bits (population counting) of value stored at {@literal key} between
     * {@literal start} and {@literal end}.
     *
     * @param key must not be {@literal null}.
     * @param start
     * @param end
    ...
    class BitFieldCommand extends KeyCommand {
        ...
        /**
         * Applies the {@link BitFieldSubCommands}. Constructs a new command instance with all
    ...
    default Mono<List<Long>> bitField(ByteBuffer key, BitFieldSubCommands subCommands) {
        ...
        return bitField(Mono.just(BitFieldCommand.bitField(key).commands(subCommands)))
                .map(CommandResponse::getOutput)
                .next();
    }
    ...
    class BitOpCommand {
        ...
        private BitOpCommand(
                List<ByteBuffer> keys, BitOperation bitOp, @Nullable ByteBuffer destinationKey) {
        ...
        /**
         * Applies the operation to {@literal keys}. Constructs a new command instance with all
         * previously configured properties.
         *
         * @param keys must not be {@literal null}.
        ...
        /**
         * Applies the {@literal key} to store the result at. Constructs a new command instance with all
         * previously configured properties.
         *
     ...
     * @see <a href="https://valkey.io/commands/bitop">Valkey Documentation: BITOP</a>
     */
    default Mono<Long> bitOp(
            Collection<ByteBuffer> keys, BitOperation bitOp, ByteBuffer destination) {
    ...
    /**
     * Return the position of the first bit set to given {@code bit} in a string. {@link Range} start
     * and end can contain negative values in order to index <strong>bytes</strong> starting from the
     * end of the string, where {@literal -1} is the last byte, {@literal -2} is the penultimate.
     *
     * @param key the key holding the actual String.
     * @param bit the bit value to look for.
    ...
    default Mono<Long> bitPos(ByteBuffer key, boolean bit, Range<Long> range) {
        return bitPos(Mono.just(BitPosCommand.positionOf(bit).in(key).within(range)))
                .next()
                .map(NumericResponse::getOutput);
    }

    /**
     * Emmit the the position of the first bit set to given {@code bit} in a string. Get the length of
     * the value stored at {@literal key}.
     *
     * @param commands must not be {@literal null}.
