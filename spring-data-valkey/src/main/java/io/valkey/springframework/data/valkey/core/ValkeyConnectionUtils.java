...
/**
 * Helper class that provides static methods for obtaining {@link ValkeyConnection} from a {@link
 * ValkeyConnectionFactory}. Includes special support for Spring-managed transactional
 * ValkeyConnections, e.g. managed by {@link
 * org.springframework.transaction.support.AbstractPlatformTransactionManager}..
 *
 * <p>Used internally by Spring's {@link ValkeyTemplate}. Can also be used directly in application
 * code.
 *
 * @author Costin Leau
...
public abstract class ValkeyConnectionUtils {
    ...
    /**
     * Obtain a {@link ValkeyConnection} from the given {@link ValkeyConnectionFactory} and binds the
     * connection to the current thread to be used in closure-scope, if none is already bound.
     * Considers ongoing transactions by reusing the transaction-bound connection and allows reentrant
     * connection retrieval. Does not bind the connection to potentially ongoing transactions.
     *
     ...
     * connection to the current thread to be used in closure-scope, if none is already bound.
     * Considers ongoing transactions by reusing the transaction-bound connection and allows reentrant
     * connection retrieval. Binds also the connection to the ongoing transaction if no connection is
     * already bound if {@code transactionSupport} is enabled.
     *
     * @param factory connection factory.
     ...
     */
    public static ValkeyConnection bindConnection(
            ValkeyConnectionFactory factory, boolean transactionSupport) {
        return doGetConnection(factory, true, true, transactionSupport);
    }

    /**
     * Obtain a {@link ValkeyConnection} from the given {@link ValkeyConnectionFactory}. Is aware of
     * existing connections bound to the current transaction (when using a transaction manager) or the
     * current thread (when binding a connection to a closure-scope). Does not bind newly created
     * connections to ongoing transactions.
     *
     ...
     * Obtain a {@link ValkeyConnection} from the given {@link ValkeyConnectionFactory}. Is aware of
     * existing connections bound to the current transaction (when using a transaction manager) or the
     * current thread (when binding a connection to a closure-scope).
     *
     * @param factory connection factory for creating the connection.
     * @param transactionSupport whether transaction support is enabled.
     * @return an active Valkey connection with transaction management if requested.
     */
    public static ValkeyConnection getConnection(
            ValkeyConnectionFactory factory, boolean transactionSupport) {
        return doGetConnection(factory, true, false, transactionSupport);
    }

    /**
     * Actually obtain a {@link ValkeyConnection} from the given {@link ValkeyConnectionFactory}. Is
     * aware of existing connections bound to the current transaction (when using a transaction
     * manager) or the current thread (when binding a connection to a closure-scope). Will create a
     * new {@link ValkeyConnection} otherwise, if {@code allowCreate} is {@literal true}. This method
     * allows for re-entrance as {@link ValkeyConnectionHolder} keeps track of ref-count.
     *
     * @param factory connection factory for creating the connection.
     ...
     */
    public static ValkeyConnection doGetConnection(
        ...
        ValkeyConnectionHolder conHolder =
                (ValkeyConnectionHolder) TransactionSynchronizationManager.getResource(factory);
     ...
     * @return a Valkey Connection from the given {@link ValkeyConnectionFactory} (never {@literal
     *     null}).
    ...
    private static void potentiallyRegisterTransactionSynchronisation(
            ValkeyConnectionHolder connectionHolder, final ValkeyConnectionFactory factory) {
        ...
        if (!connectionHolder.isTransactionActive()) {
            ...
            TransactionSynchronizationManager.registerSynchronization(
                    new ValkeyTransactionSynchronizer(connectionHolder, connection, factory, readOnly));
        }
    }
    ...
    private static ValkeyConnection createConnectionSplittingProxy(
            ValkeyConnection connection, ValkeyConnectionFactory factory) {
    ...
    /**
     * Closes the given {@link ValkeyConnection}, created via the given factory if not managed
    ...
    public static void releaseConnection(
            @Nullable ValkeyConnection conn, ValkeyConnectionFactory factory) {
        if (conn == null) {
            return;
        }
    ...
    /**
     * Determine whether the given two ValkeyConnections are equal, asking the target {@link
     * ValkeyConnection} in case of a proxy. Used to detect equality even if the user passed in a raw
     * target Connection while the held one is a proxy.
     *
     * @param connectionHolder the {@link ValkeyConnectionHolder} for the held Connection (potentially
     *     a proxy)
     * @param passedInConnetion the {@link ValkeyConnection} passed-in by the user (potentially a
     *     target Connection without proxy)
     * @return whether the given Connections are equal
     * @see #getTargetConnection
     */
    private static boolean connectionEquals(
            ValkeyConnectionHolder connectionHolder, ValkeyConnection passedInConnetion) {
        ...
        return heldConnection.equals(passedInConnetion)
                || getTargetConnection(heldConnection).equals(passedInConnetion);
    }

    /**
     * Return the innermost target {@link ValkeyConnection} of the given {@link ValkeyConnection}. If
     * the given {@link ValkeyConnection} is a proxy, it will be unwrapped until a non-proxy {@link
     * ValkeyConnection} is found. Otherwise, the passed-in {@link ValkeyConnection} will be returned
     * as-is.
     *
    ...
    /**
     * Unbinds and closes the connection (if any) associated with the given factory from
     * closure-scope. Considers ongoing transactions so transaction-bound connections aren't closed
     * and reentrant closure-scope bound connections. Only the outer-most call to leads to releasing
    ...
    public static void unbindConnection(ValkeyConnectionFactory factory) {
        ...
        ValkeyConnectionHolder connectionHolder =
                (ValkeyConnectionHolder) TransactionSynchronizationManager.getResource(factory);
    ...
    /**
     * Return whether the given Valkey connection is transactional, that is, bound to the current
    ...
    public static boolean isConnectionTransactional(
            ValkeyConnection connection, ValkeyConnectionFactory connectionFactory) {

        Assert.notNull(connectionFactory, "No ValkeyConnectionFactory specified");

        ValkeyConnectionHolder connectionHolder =
                (ValkeyConnectionHolder) TransactionSynchronizationManager.getResource(connectionFactory);
    ...
    /**
     * A {@link TransactionSynchronization} that makes sure that the associated {@link
     * ValkeyConnection} is released after the transaction completes.
     *
     * @author Christoph Strobl
     * @author Thomas Darimont
     * @author Mark Paluch
    ...
    /**
     * {@link MethodInterceptor} that invokes read-only commands on a new {@link ValkeyConnection}
     * while read-write commands are queued on the bound connection.
     *
     * @author Christoph Strobl
     * @author Mark Paluch
     * @since 1.3
    ...
    static class ConnectionSplittingInterceptor implements MethodInterceptor {
        ...
        private ConnectionSplittingInterceptor(
                ValkeyConnectionFactory factory, Method commandInterfaceMethod) {
            this.factory = factory;
        ...
        public Object intercept(Object obj, Method method, Object[] args) throws Throwable {
            ...
            if (returnType.isInterface()
                    && returnType.getPackageName().equals("io.valkey.springframework.data.valkey.connection")
                    && returnTypeName.startsWith("Valkey")
                    && returnTypeName.endsWith("Commands")) {
    ...
    /**
     * Resource holder wrapping a {@link ValkeyConnection}. {@link ValkeyConnectionUtils} binds
     * instances of this class to the thread, for a specific {@link ValkeyConnectionFactory}.
     *
     * @author Christoph Strobl
     * @author Mark Paluch
     */
    private static class ValkeyConnectionHolder extends ResourceHolderSupport {
        ...
        /**
         * Create a new ValkeyConnectionHolder for the given Valkey Connection assuming that there is no
         * ongoing transaction.
         *
        ...
        /**
         * Override the existing {@link ValkeyConnection} handle with the given {@link
         * ValkeyConnection}. Reset the handle if given {@literal null}.
         *
         * <p>Used for releasing the Connection on suspend (with a {@literal null} argument) and setting
         * a fresh Connection on resume.
         */
    ...
    /**
     * Subinterface of {@link ValkeyConnection} to be implemented by {@link ValkeyConnection} proxies.
     * Allows access to the underlying target {@link ValkeyConnection}.
     *
    ...
    public interface ValkeyConnectionProxy extends ValkeyConnection, RawTargetAccess {
         ...
         *
         * <p>This will typically be the native driver {@link ValkeyConnection} or a wrapper from a
         * connection pool.
         *
