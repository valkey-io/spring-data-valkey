...
/**
 * Interface that specified a basic set of Valkey operations, implemented by {@link
 * ReactiveValkeyTemplate}. Not often used but a useful option for extensibility and testability (as
 * it can be easily mocked or stubbed).
 *
...
public interface ReactiveValkeyOperations<K, V> {
    ...
    /**
     * Executes the given action within a Valkey connection. Application exceptions thrown by the
     * action object get propagated to the caller (can only be unchecked) whenever possible. Valkey
     * exceptions are transformed into appropriate DAO ones. Allows for returning a result object,
     * that is a domain object or a collection of domain objects. Performs automatic
     * serialization/deserialization for the given objects to and from binary data suitable for the
     * Valkey storage. Note: Callback code is not supposed to handle transactions itself! Use an
     * appropriate transaction manager. Generally, callback code must not touch any Connection
     * lifecycle methods, like close, to let the template do its work.
     *
     * @param <T> return type
    ...
    /**
     * Executes the given action within a Valkey session using the same {@link
     * io.valkey.springframework.data.valkey.connection.ReactiveValkeyConnection}. Application
     * exceptions thrown by the action object get propagated to the caller (can only be unchecked)
     * whenever possible. Valkey exceptions are transformed into appropriate DAO ones. Allows for
     * returning a result object, that is a domain object or a collection of domain objects. Performs
     * automatic serialization/deserialization for the given objects to and from binary data suitable
     * for the Valkey storage. Note: Callback code is not supposed to handle transactions itself! Use
     * an appropriate transaction manager. Generally, callback code must not touch any Connection
     * lifecycle methods, like close, to let the template do its work.
     *
     * @param <T> return type
     * @param action callback object that specifies the Valkey action
    ...
    /**
     * Subscribe to the given Valkey {@code channels} and emit {@link Message messages} received for
     * those.
     *
     * <p>Note that this method allocates a new {@link
     * io.valkey.springframework.data.valkey.listener.ReactiveValkeyMessageListenerContainer} and uses
     * a dedicated connection, similar to other methods on this interface. Invoking this method
     * multiple times is an indication that you should use {@link
     * io.valkey.springframework.data.valkey.listener.ReactiveValkeyMessageListenerContainer}
     * directly.
     *
    ...
    /**
     * Subscribe to the Valkey channels matching the given {@code pattern} and emit {@link Message
    ...
    /**
     * Subscribe to the Valkey channels for the given {@link Topic topics} and emit {@link Message
     * messages} received for those.
     *
     * <p>Note that this method allocates a new {@link
     * io.valkey.springframework.data.valkey.listener.ReactiveValkeyMessageListenerContainer} and uses
     ...
     * @since 2.6
     * @see
     *     io.valkey.springframework.data.valkey.listener.ReactiveValkeyMessageListenerContainer#receiveLater(ChannelTopic...)
     */
    default Mono<Flux<? extends Message<String, V>>> listenToChannelLater(String... channels) {

        Assert.notNull(channels, "Channels must not be null");

        return listenToLater(
                Arrays.stream(channels).map(ChannelTopic::of).toArray(ChannelTopic[]::new));
    }

    /**
     * Subscribe to the Valkey channels matching the given {@code pattern} and emit {@link Message
     * messages} received for those. The {@link Mono} completes once the {@link Topic topic}
     ...
     * @since 2.6
     * @see
     *     io.valkey.springframework.data.valkey.listener.ReactiveValkeyMessageListenerContainer#receiveLater(PatternTopic...)
     */
    default Mono<Flux<? extends Message<String, V>>> listenToPatternLater(String... patterns) {

        Assert.notNull(patterns, "Patterns must not be null");
        return listenToLater(
                Arrays.stream(patterns).map(PatternTopic::of).toArray(PatternTopic[]::new));
    }
     ...
     * @since 2.6
     * @see
     *     io.valkey.springframework.data.valkey.listener.ReactiveValkeyMessageListenerContainer#receiveLater(Iterable,
     ...
     * <strong>IMPORTANT:</strong> It is recommended to use {@link #scan()} to iterate over the
     * keyspace as {@link #keys(Object)} is a non-interruptible and expensive Valkey operation.
     *
     ...
     *
     * @return the {@link Flux} emitting the {@literal keys} one by one or an {@link Flux#empty()
     *     empty flux} if none exist.
    ...
    /**
     * Delete given {@code keys}. This command buffers keys received from {@link Publisher} into
     * chunks of 128 keys to delete to reduce the number of issued {@code DEL} commands.
     *
    ...
    /**
     * Unlink the {@code key} from the keyspace. Unlike with {@link #delete(Object[])} the actual
     * memory reclaiming here happens asynchronously.
     *
     * @param key must not be {@literal null}.
     * @return The number of keys that were removed. {@literal null} when used in pipeline /
     *     transaction.
    ...
    /**
     * Unlink the {@code keys} from the keyspace. Unlike with {@link #delete(Publisher)} the actual
     * memory reclaiming here happens asynchronously. This command buffers keys received from {@link
     * Publisher} into chunks of 128 keys to delete to reduce the number of issued {@code UNLINK}
     * commands.
     *
     * @param keys must not be {@literal null}.
     * @return The number of keys that were removed. {@literal null} when used in pipeline /
     ...
     * @since 3.5
     */
    Mono<ExpireChanges.ExpiryChangeState> expire(
     ...
     *
     * @param key must not be {@literal null}.
     * @return the {@link Duration} of the associated key. {@link Duration#ZERO} if no timeout
     *     associated or empty {@link Mono} if the key does not exist.
     ...
     * @return result value of the script {@link Flux#empty()} if {@link ValkeyScript#getResultType()}
     *     is {@literal null}, likely indicating a throw-away status reply (i.e. "OK").
     */
     ...
     * @param resultReader The {@link ValkeyElementReader} to use for serializing the script return
     *     value
    ...
    <T> Flux<T> execute(
            ...
            List<K> keys,
            List<?> args,
            ValkeyElementWriter<?> argsWriter,
     ...
     * Returns geospatial specific operations interface.
     *
     * @param serializationContext serializers to be used with the returned operations, must not be
     *     {@literal null}.
     * @return geospatial specific operations.
     */
    <K, V> ReactiveGeoOperations<K, V> opsForGeo(
     ...
     * @return hash operations.
     */
    <K, HK, HV> ReactiveHashOperations<K, HK, HV> opsForHash(
    ...
    /**
     * Returns the operations performed on multisets using HyperLogLog given a {@link
     * ValkeySerializationContext}.
     *
     * @param serializationContext serializers to be used with the returned operations, must not be
     *     {@literal null}.
     * @return never {@literal null}.
     */
    <K, V> ReactiveHyperLogLogOperations<K, V> opsForHyperLogLog(
            ValkeySerializationContext<K, V> serializationContext);
     ...
     * @return list operations.
     */
    <K, V> ReactiveListOperations<K, V> opsForList(
            ValkeySerializationContext<K, V> serializationContext);
     ...
     * @return set operations.
     */
    <K, V> ReactiveSetOperations<K, V> opsForSet(
            ValkeySerializationContext<K, V> serializationContext);
     ...
     * @return value operations.
     */
    <K, V> ReactiveValueOperations<K, V> opsForValue(
            ValkeySerializationContext<K, V> serializationContext);
    ...
    /**
     * Returns the operations performed on zset values (also known as sorted sets) given a {@link
     * ValkeySerializationContext}.
     *
     * @param serializationContext serializers to be used with the returned operations, must not be
     *     {@literal null}.
     * @return zset operations.
     */
    <K, V> ReactiveZSetOperations<K, V> opsForZSet(
            ValkeySerializationContext<K, V> serializationContext);
