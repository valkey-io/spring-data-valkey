...
/**
 * Helper bean for registering {@link ValkeyListenerEndpoint} instances with a {@link ValkeyListenerEndpointRegistry}.
 * <p>
 ...
 * @since 4.1
 */
public class ValkeyListenerEndpointRegistrar implements BeanFactoryAware, InitializingBean {
	...
	private final List<ValkeyListenerEndpointDescriptor> valkeyListenerEndpointDescriptors = new ArrayList<>();

	private boolean startImmediately;

	/**
	 * Set the {@link ValkeyListenerEndpointRegistry} instance to use.
	 *
	 * @param endpointRegistry the {@link ValkeyListenerEndpointRegistry}
	 */
	...
	/**
	 * Return the {@link ValkeyListenerEndpointRegistry} instance for this registrar, may be {@code null}.
	 */
	public @Nullable ValkeyListenerEndpointRegistry getEndpointRegistry() {
	...
	List<ValkeyListenerEndpointDescriptor> getValkeyListenerEndpointDescriptors() {
		return valkeyListenerEndpointDescriptors;
	}
	...
	protected void registerAllEndpoints() {
		...
		Assert.state(this.endpointRegistry != null, "No ValkeyListenerEndpointRegistry set");

		for (ValkeyListenerEndpointDescriptor descriptor : this.valkeyListenerEndpointDescriptors) {
			if (descriptor.endpoint instanceof MethodValkeyListenerEndpoint methodEndpoint) {
	...
	/**
	 * Register a new {@link ValkeyListenerEndpoint} alongside the {@link ValkeyMessageListenerContainer}.
	 * <p>
	 * If the underlying {@link ValkeyListenerEndpointRegistry} is already active, the endpoint is registered and started
	 * immediately. Otherwise, it is queued and registered once the registry becomes active.
	 *
	 * @param endpoint the {@link ValkeyListenerEndpoint} to register
	 */
	public void registerEndpoint(ValkeyListenerEndpoint endpoint, ValkeyMessageListenerContainer container) {

		Assert.notNull(endpoint, "Endpoint must not be null");
		Assert.hasText(endpoint.getId(), "Endpoint id must be set");

		ValkeyListenerEndpointDescriptor descriptor = new ValkeyListenerEndpointDescriptor(endpoint, container);

		if (this.startImmediately) {
			Assert.state(this.endpointRegistry != null, "No ValkeyListenerEndpointRegistry set");

			if (endpoint instanceof MethodValkeyListenerEndpoint methodEndpoint) {
				methodEndpoint.setMessageHandlerMethodFactory(getMessageHandlerMethodFactory());
			}
			this.endpointRegistry.registerListener(endpoint, descriptor.container);
		} else {
			this.valkeyListenerEndpointDescriptors.add(descriptor);
		}
	}

	public void apply(List<ValkeyListenerConfigurer> configurers) {
		...
		ValkeyMessageConverters.Builder converterBuilder = ValkeyMessageConverters.builder();

		for (ValkeyListenerConfigurer configurer : configurers) {
			...
			configurer.configureValkeyListeners(this);
	 ...
	 */
	record ValkeyListenerEndpointDescriptor(ValkeyListenerEndpoint endpoint, ValkeyMessageListenerContainer container) {
		...
		private MessageHandlerMethodFactory createMessageHandlerMethodFactory() {
			...
			factory.setMessageConverter(Objects.requireNonNullElseGet(this.messageConverter,
					() -> {
						...
						MessageConverter converter = ValkeyMessageConverters.createMessageConverter();
