...
/**
 * {@link ClusterCommandExecutor} takes care of running commands across the known cluster nodes. By
 * providing an {@link AsyncTaskExecutor} the execution behavior can be configured.
 *
...
public class ClusterCommandExecutor implements DisposableBean {
     ...
     * @param resourceProvider must not be {@literal null}.
     * @param exceptionTranslation must not be {@literal null}.
     * @param executor the task executor to null, defaults to {@link SimpleAsyncTaskExecutor} if
     *     {@literal null}.
     */
    public ClusterCommandExecutor(
            ClusterTopologyProvider topologyProvider,
     ...
     */
    public <T> NodeResult<T> executeCommandOnArbitraryNode(
            ClusterCommandCallback<?, T> commandCallback) {
     ...
     */
    public <S, T> NodeResult<T> executeCommandOnSingleNode(
            ClusterCommandCallback<S, T> commandCallback, ValkeyClusterNode node) {

        return executeCommandOnSingleNode(commandCallback, node, 0);
    }

    private <S, T> NodeResult<T> executeCommandOnSingleNode(
            ClusterCommandCallback<S, T> commandCallback, ValkeyClusterNode node, int redirectCount) {
        ...
        try {
            ...
            if (translatedException instanceof ClusterRedirectException clusterRedirectException) {
                ...
                ValkeyClusterNode clusterNode =
                        topologyProvider.getTopology().lookup(targetHost, targetPort);
     ...
     * @return the resolved {@link ValkeyClusterNode node} from the {@link ClusterTopology topology};
     *     never {@literal null}.
     * @throws IllegalArgumentException in case the node could not be resolved to a topology-known
     *     node
     */
     ...
     */
    public <S, T> MultiNodeResult<T> executeCommandOnAllNodes(
            ClusterCommandCallback<S, T> commandCallback) {
    ...
    public <S, T> MultiNodeResult<T> executeCommandAsyncOnNodes(
            ClusterCommandCallback<S, T> commandCallback, Iterable<ValkeyClusterNode> nodes) {
        ...
        for (ValkeyClusterNode node : resolvedValkeyClusterNodes) {
            Callable<NodeResult<T>> nodeCommandExecution =
                    () -> executeCommandOnSingleNode(commandCallback, node);
    ...
    <T> MultiNodeResult<T> collectResults(Map<NodeExecution, Future<NodeResult<T>>> futures) {
        ...
        while (!futures.isEmpty()) {
            ...
            Iterator<Map.Entry<NodeExecution, Future<NodeResult<T>>>> entryIterator =
    ...
    /**
     * Run {@link MultiKeyClusterCommandCallback} with on a curated set of nodes serving one or more
     * keys.
     *
     * @param commandCallback must not be {@literal null}.
     * @return never {@literal null}.
     ...
     */
    public <S, T> MultiNodeResult<T> executeMultiKeyCommand(
            MultiKeyClusterCommandCallback<S, T> commandCallback, Iterable<byte[]> keys) {
        ...
        for (byte[] key : keys) {
            for (ValkeyClusterNode node : getClusterTopology().getKeyServingNodes(key)) {
                nodeKeyMap
                        .computeIfAbsent(node, val -> PositionalKeys.empty())
                        .append(PositionalKey.of(key, index++));
            }
        }
                ...
                for (PositionalKey key : entry.getValue()) {
                    futures.put(
                            ...
                            this.executor.submit(
                                    () ->
                                            executeMultiKeyCommandOnSingleNode(
                                                    commandCallback, entry.getKey(), key.getBytes())));
                }
            }
        }
    ...
    private <S, T> NodeResult<T> executeMultiKeyCommandOnSingleNode(
            MultiKeyClusterCommandCallback<S, T> commandCallback, ValkeyClusterNode node, byte[] key) {
    ...
    /**
     * Callback interface for Valkey 'low level' code using the cluster client directly. To be used
     * with {@link ClusterCommandExecutor} execution methods.
     *
     * @author Christoph Strobl
    ...
    /**
     * Callback interface for Valkey 'low level' code using the cluster client to execute multi key
     * commands.
     *
     * @author Christoph Strobl
     * @param <T> native driver connection
    ...
    /**
     * {@link NodeExecution} encapsulates the execution of a command on a specific node along with
     * arguments, such as keys, involved.
     *
     * @author Christoph Strobl
     * @author Mark Paluch
     * @since 1.7
    ...
    /**
     * {@link NodeResult} encapsulates the actual {@link T value} returned by a {@link
     * ClusterCommandCallback} on a given {@link ValkeyClusterNode}.
     *
    ...
    /**
     * {@link MultiNodeResult} holds all {@link NodeResult} of a command executed on multiple {@link
     * ValkeyClusterNode}.
     *
     * @author Christoph Strobl
     * @author Mark Paluch
    ...
    public static class MultiNodeResult<T> {
        ...
        public List<T> resultsAsListSortBy(byte[]... keys) {
            ...
            Map<PositionalKey, NodeResult<T>> result =
                    new TreeMap<>(new ResultByKeyPositionComparator(keys));

            result.putAll(positionalResults);

            return result.values().stream()
                    .map(tNodeResult -> tNodeResult.value)
    ...
    private static class PositionalKey {
        ...
        public boolean equals(@Nullable Object obj) {
            ...
            if (!(obj instanceof PositionalKey that)) return false;
    ...
    private class NodeExceptionCollector {
        ...
        public void addException(NodeExecution execution, Throwable throwable) {
            ...
            Throwable translated =
                    throwable instanceof Exception e ? convertToDataAccessException(e) : throwable;
