...
/**
 * Basic interface serialization and deserialization of Objects to byte arrays (binary data). It is
 * recommended that implementations are designed to handle {@literal null} objects/empty arrays on
 * serialization and deserialization side. Note that Valkey does not accept {@literal null} keys or
 * values but can return null replies (for non-existing keys).
 *
 * @author Mark Pollack
 * @author Costin Leau
...
public interface ValkeySerializer<T> {
    ...
    /**
     * Obtain a {@link ValkeySerializer} using java serialization. <strong>Note:</strong> Ensure that
     * your domain objects are actually {@link java.io.Serializable serializable}.
     *
     ...
     * <strong>Note:</strong> Ensure that your domain objects are actually {@link java.io.Serializable
     * serializable}.
     *
    ...
    /**
     * Obtain a {@link ValkeySerializer} that can read and write JSON using <a
    ...
    /**
     * Obtain a simple {@link java.lang.String} to {@code byte[]} (and back) serializer using {@link
     * java.nio.charset.StandardCharsets#UTF_8 UTF-8} as the default {@link java.nio.charset.Charset}.
     *
     * @return never {@literal null}.
     * @since 2.1
     */
