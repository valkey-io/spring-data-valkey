...
public abstract class ClusterSlotHashUtil {
     ...
     * Determines whether all keys will hash to the same slot.
     *
     * @param keys array of {@link ByteBuffer} objects containing the keys to evaluate; must not be
     *     {@literal null}.
     * @return a boolean value indicating whether all keys will hash to the same slot.
     ...
     *
     * @param key {@link String} containing the Valkey key to evaluate; must not be {@literal null} or
     *     {@literal empty}.
     * @return the computed slot based on the given key.
     * @throws IllegalArgumentException if the given {@link String key} is {@literal null} or
     *     {@literal empty}.
    ...
    /**
     * Generates a key that will map to the specified cluster slot. This method iterates through key
     * patterns until it finds one that matches the target slot.
     *
    ...
    public static String getKeyForSlot(int targetSlot) {
        ...
        Assert.isTrue(
                targetSlot >= 0 && targetSlot < SLOT_COUNT,
