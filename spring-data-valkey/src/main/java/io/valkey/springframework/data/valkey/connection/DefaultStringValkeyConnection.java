...
import io.valkey.springframework.data.valkey.connection.convert.MapConverter;
...
public class DefaultStringValkeyConnection
        implements StringValkeyConnection, DecoratedValkeyConnection {
    ...
    private SetConverter<StringTuple, Tuple> stringTupleToTuple =
            new SetConverter<>(new StringTupleConverter());
    private ListConverter<Tuple, StringTuple> tupleListToStringTuple =
    ...
    private Converter<GeoResults<GeoLocation<byte[]>>, GeoResults<GeoLocation<String>>>
            byteGeoResultsToStringGeoResults;
    private Converter<ByteRecord, StringRecord> byteMapRecordToStringMapRecordConverter =
            new Converter<ByteRecord, StringRecord>() {
    ...
    private ListConverter<ByteRecord, StringRecord> listByteMapRecordToStringMapRecordConverter =
    ...
    private Entry<String, String> convertEntry(Entry<byte[], byte[]> source) {
        return Converters.entryOf(
                bytesToString.convert(source.getKey()), bytesToString.convert(source.getValue()));
    }
    ...
    /**
     * Constructs a new <code>DefaultStringValkeyConnection</code> instance. Uses {@link
     * StringValkeySerializer} as underlying serializer.
     *
     ...
     */
    public DefaultStringValkeyConnection(
            ValkeyConnection connection, ValkeySerializer<String> serializer) {
    ...
    public byte[] bRPopLPush(int timeout, byte[] srcKey, byte[] dstKey) {
        return convertAndReturn(
                delegate.bRPopLPush(timeout, srcKey, dstKey), Converters.identityConverter());
    }
    ...
    public Boolean copy(byte[] sourceKey, byte[] targetKey, boolean replace) {
        return convertAndReturn(
                delegate.copy(sourceKey, targetKey, replace), Converters.identityConverter());
    }
    ...
    public Boolean expire(byte[] key, long seconds, ExpirationOptions.Condition condition) {
        return convertAndReturn(
                delegate.expire(key, seconds, condition), Converters.identityConverter());
    }

    @Override
    public Boolean expireAt(byte[] key, long unixTime, ExpirationOptions.Condition condition) {
        return convertAndReturn(
                delegate.expireAt(key, unixTime, condition), Converters.identityConverter());
    }
    ...
    public Long lInsert(byte[] key, Position where, byte[] pivot, byte[] value) {
        return convertAndReturn(
                delegate.lInsert(key, where, pivot, value), Converters.identityConverter());
    }

    @Override
    public byte[] lMove(byte[] sourceKey, byte[] destinationKey, Direction from, Direction to) {
        return convertAndReturn(
                delegate.lMove(sourceKey, destinationKey, from, to), Converters.identityConverter());
    }
    ...
    public byte[] bLMove(
        ...
        return convertAndReturn(
                delegate.bLMove(sourceKey, destinationKey, from, to, timeout),
    ...
    public String lMove(String sourceKey, String destinationKey, Direction from, Direction to) {
        return convertAndReturn(
                delegate.lMove(serialize(sourceKey), serialize(destinationKey), from, to), bytesToString);
    }

    @Override
    public String bLMove(
            String sourceKey, String destinationKey, Direction from, Direction to, double timeout) {
        return convertAndReturn(
                delegate.bLMove(serialize(sourceKey), serialize(destinationKey), from, to, timeout),
                bytesToString);
    }
    ...
    public List<Long> lPos(
        ...
        return convertAndReturn(
                delegate.lPos(key, element, rank, count), Converters.identityConverter());
    }
    ...
    public Boolean set(byte[] key, byte[] value, Expiration expiration, SetOption option) {
        return convertAndReturn(
                delegate.set(key, value, expiration, option), Converters.identityConverter());
    }

    @Override
    public byte[] setGet(byte[] key, byte[] value, Expiration expiration, SetOption option) {
        return convertAndReturn(
                delegate.setGet(key, value, expiration, option), Converters.identityConverter());
    }
    ...
    public Boolean pSetEx(byte[] key, long milliseconds, byte[] value) {
        return convertAndReturn(
                delegate.pSetEx(key, milliseconds, value), Converters.identityConverter());
    }
    ...
    public Double zIncrBy(byte[] key, double increment, byte[] value) {
        return convertAndReturn(
                delegate.zIncrBy(key, increment, value), Converters.identityConverter());
    }
    ...
    public Long zDiffStore(String destKey, String... sets) {
        return convertAndReturn(
                delegate.zDiffStore(serialize(destKey), serializeMulti(sets)),
                Converters.identityConverter());
    }
    ...
    public Set<Tuple> zInterWithScores(Aggregate aggregate, Weights weights, byte[]... sets) {
        return convertAndReturn(
                delegate.zInterWithScores(aggregate, weights, sets), Converters.identityConverter());
    }
    ...
    public Set<StringTuple> zInterWithScores(Aggregate aggregate, Weights weights, String... sets) {
        return convertAndReturn(
                delegate.zInterWithScores(aggregate, weights, serializeMulti(sets)), tupleToStringTuple);
    }
    ...
    public Long zInterStore(byte[] destKey, Aggregate aggregate, int[] weights, byte[]... sets) {
        return convertAndReturn(
                delegate.zInterStore(destKey, aggregate, Weights.of(weights), sets),
                Converters.identityConverter());
    }
    ...
    public Long zInterStore(byte[] destKey, Aggregate aggregate, Weights weights, byte[]... sets) {
        return convertAndReturn(
                delegate.zInterStore(destKey, aggregate, weights, sets), Converters.identityConverter());
    }
    ...
    public Set<byte[]> zRangeByScore(byte[] key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRangeByScore(key, min, max, offset, count), Converters.identityConverter());
    }
    ...
    public Set<byte[]> zRangeByScore(
        ...
        return convertAndReturn(
                delegate.zRangeByScore(key, range, limit), Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRangeByScoreWithScores(
            byte[] key, org.springframework.data.domain.Range<? extends Number> range) {
        return convertAndReturn(
                delegate.zRangeByScoreWithScores(key, range), Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRangeByScoreWithScores(
        ...
        return convertAndReturn(
                delegate.zRangeByScoreWithScores(key, min, max, offset, count),
                Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRangeByScoreWithScores(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeByScoreWithScores(key, range, limit), Converters.identityConverter());
    }

    @Override
    public Set<Tuple> zRangeByScoreWithScores(byte[] key, double min, double max) {
        return convertAndReturn(
                delegate.zRangeByScoreWithScores(key, min, max), Converters.identityConverter());
    }

    @Override
    public Set<Tuple> zRangeWithScores(byte[] key, long start, long end) {
        return convertAndReturn(
                delegate.zRangeWithScores(key, start, end), Converters.identityConverter());
    }

    @Override
    public Set<byte[]> zRevRangeByScore(byte[] key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRevRangeByScore(key, min, max, offset, count), Converters.identityConverter());
    }
    ...
    public Set<byte[]> zRevRangeByScore(byte[] key, double min, double max) {
        return convertAndReturn(
                delegate.zRevRangeByScore(key, min, max), Converters.identityConverter());
    }
    ...
    public Set<byte[]> zRevRangeByScore(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRevRangeByScore(key, range, limit), Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRevRangeByScoreWithScores(
            byte[] key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRevRangeByScoreWithScores(key, min, max, offset, count),
                Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRevRangeByScoreWithScores(
            byte[] key, org.springframework.data.domain.Range<? extends Number> range) {
        return convertAndReturn(
                delegate.zRevRangeByScoreWithScores(key, range), Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRevRangeByScoreWithScores(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRevRangeByScoreWithScores(key, range, limit), Converters.identityConverter());
    }

    @Override
    public Set<Tuple> zRevRangeByScoreWithScores(byte[] key, double min, double max) {
        return convertAndReturn(
                delegate.zRevRangeByScoreWithScores(key, min, max), Converters.identityConverter());
    }
    ...
    public Long zRemRangeByScore(byte[] key, double min, double max) {
        return convertAndReturn(
                delegate.zRemRangeByScore(key, min, max), Converters.identityConverter());
    }
    ...
    public Set<Tuple> zRevRangeWithScores(byte[] key, long start, long end) {
        return convertAndReturn(
                delegate.zRevRangeWithScores(key, start, end), Converters.identityConverter());
    }
    ...
    public Set<Tuple> zUnionWithScores(Aggregate aggregate, Weights weights, byte[]... sets) {
        return convertAndReturn(
                delegate.zUnionWithScores(aggregate, weights, sets), Converters.identityConverter());
    }
    ...
    public Set<StringTuple> zUnionWithScores(Aggregate aggregate, Weights weights, String... sets) {
        return convertAndReturn(
                delegate.zUnionWithScores(aggregate, weights, serializeMulti(sets)), tupleToStringTuple);
    }

    @Override
    public Long zUnionStore(byte[] destKey, Aggregate aggregate, Weights weights, byte[]... sets) {
        return convertAndReturn(
                delegate.zUnionStore(destKey, aggregate, weights, sets), Converters.identityConverter());
    }
    ...
    public Long zUnionStore(byte[] destKey, Aggregate aggregate, int[] weights, byte[]... sets) {
        return convertAndReturn(
                delegate.zUnionStore(destKey, aggregate, Weights.of(weights), sets),
                Converters.identityConverter());
    }
    ...
    public Boolean pExpire(byte[] key, long millis, ExpirationOptions.Condition condition) {
        return convertAndReturn(
                delegate.pExpire(key, millis, condition), Converters.identityConverter());
    }
    ...
    public Boolean pExpireAt(
        ...
        return convertAndReturn(
                delegate.pExpireAt(key, unixTimeInMillis, condition), Converters.identityConverter());
    }
    ...
    public <T> T eval(byte[] script, ReturnType returnType, int numKeys, byte[]... keysAndArgs) {
        return convertAndReturn(
                delegate.eval(script, returnType, numKeys, keysAndArgs), Converters.identityConverter());
    }
    ...
    public <T> T evalSha(
        ...
        return convertAndReturn(
                delegate.evalSha(scriptSha1, returnType, numKeys, keysAndArgs),
                Converters.identityConverter());
    }
    ...
    private org.springframework.data.domain.Range<byte[]> serialize(
    ...
    private org.springframework.data.domain.Range.Bound<byte[]> rawBound(
        ...
        return source
                ...
                .map(
                        it ->
                                source.isInclusive()
                                        ? org.springframework.data.domain.Range.Bound.inclusive(it)
    ...
    private GeoReference<byte[]> serialize(GeoReference<String> data) {
        return data instanceof GeoReference.GeoMemberReference
                ? GeoReference.fromMember(
                        serializer.serialize(((GeoMemberReference<String>) data).getMember()))
                : (GeoReference) data;
    }
    ...
    private StreamOffset<byte[]>[] serialize(StreamOffset<String>[] offsets) {
        ...
        return Arrays.stream(offsets)
                .map(it -> StreamOffset.create(serialize(it.getKey()), it.getOffset()))
    ...
    public String bRPopLPush(int timeout, String srcKey, String dstKey) {
        return convertAndReturn(
                delegate.bRPopLPush(timeout, serialize(srcKey), serialize(dstKey)), bytesToString);
    }
    ...
    public List<Entry<byte[], byte[]>> hRandFieldWithValues(byte[] key, long count) {
        return convertAndReturn(
                delegate.hRandFieldWithValues(key, count), Converters.identityConverter());
    }
    ...
    public Entry<String, String> hRandFieldWithValues(String key) {
        return convertAndReturn(
                delegate.hRandFieldWithValues(serialize(key)),
    ...
    public List<Entry<String, String>> hRandFieldWithValues(String key, long count) {
        return convertAndReturn(
                delegate.hRandFieldWithValues(serialize(key), count),
    ...
    public List<String> hMGet(String key, String... fields) {
        return convertAndReturn(
                delegate.hMGet(serialize(key), serializeMulti(fields)), byteListToStringList);
    }
    ...
    @Override
    public List<Long> lPos(
            String key, String element, @Nullable Integer rank, @Nullable Integer count) {
    ...
    public String rPopLPush(String srcKey, String dstKey) {
        return convertAndReturn(
                delegate.rPopLPush(serialize(srcKey), serialize(dstKey)), bytesToString);
    }
    ...
    public List<StringTuple> zRandMemberWithScores(String key, long count) {
        return convertAndReturn(
                delegate.zRandMemberWithScore(serialize(key), count), tupleListToStringTuple);
    }
    ...
    public Set<String> zRangeByScore(String key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRangeByScore(serialize(key), min, max, offset, count), byteSetToStringSet);
    }
    ...
    @Override
    public Set<StringTuple> zRangeByScoreWithScores(
            String key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRangeByScoreWithScores(serialize(key), min, max, offset, count),
                tupleToStringTuple);
    }
    ...
    public Set<StringTuple> zRangeByScoreWithScores(String key, double min, double max) {
        return convertAndReturn(
                delegate.zRangeByScoreWithScores(serialize(key), min, max), tupleToStringTuple);
    }

    @Override
    public Set<StringTuple> zRangeWithScores(String key, long start, long end) {
        return convertAndReturn(
                delegate.zRangeWithScores(serialize(key), start, end), tupleToStringTuple);
    }
    ...
    public Set<StringTuple> zRevRangeWithScores(String key, long start, long end) {
        return convertAndReturn(
                delegate.zRevRangeWithScores(serialize(key), start, end), tupleToStringTuple);
    }

    @Override
    public Set<String> zRevRangeByScore(String key, double min, double max) {
        return convertAndReturn(
                delegate.zRevRangeByScore(serialize(key), min, max), byteSetToStringSet);
    }

    @Override
    public Set<StringTuple> zRevRangeByScoreWithScores(String key, double min, double max) {
        return convertAndReturn(
                delegate.zRevRangeByScoreWithScores(serialize(key), min, max), tupleToStringTuple);
    }

    @Override
    public Set<String> zRevRangeByScore(String key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRevRangeByScore(serialize(key), min, max, offset, count), byteSetToStringSet);
    }

    @Override
    public Set<StringTuple> zRevRangeByScoreWithScores(
            String key, double min, double max, long offset, long count) {
        return convertAndReturn(
                delegate.zRevRangeByScoreWithScores(serialize(key), min, max, offset, count),
                tupleToStringTuple);
    }
    ...
    public Long geoAdd(byte[] key, Map<byte[], Point> memberCoordinateMap) {
        return convertAndReturn(
                delegate.geoAdd(key, memberCoordinateMap), Converters.identityConverter());
    }
    ...
    public Distance geoDist(byte[] key, byte[] member1, byte[] member2) {
        return convertAndReturn(
                delegate.geoDist(key, member1, member2), Converters.identityConverter());
    }
    ...
    public Distance geoDist(byte[] key, byte[] member1, byte[] member2, Metric metric) {
        return convertAndReturn(
                delegate.geoDist(key, member1, member2, metric), Converters.identityConverter());
    }
    ...
    public List<String> geoHash(String key, String... members) {
        return convertAndReturn(
                delegate.geoHash(serialize(key), serializeMulti(members)), Converters.identityConverter());
    }
    ...
    public GeoResults<GeoLocation<String>> geoRadius(String key, Circle within) {
        return convertAndReturn(
                delegate.geoRadius(serialize(key), within), byteGeoResultsToStringGeoResults);
    }

    @Override
    public GeoResults<GeoLocation<String>> geoRadius(
            String key, Circle within, GeoRadiusCommandArgs args) {
        return convertAndReturn(
                delegate.geoRadius(serialize(key), within, args), byteGeoResultsToStringGeoResults);
    }

    @Override
    public GeoResults<GeoLocation<String>> geoRadiusByMember(
    ...
    @Override
    public GeoResults<GeoLocation<String>> geoRadiusByMember(
            String key, String member, Distance radius) {

        return convertAndReturn(
                delegate.geoRadiusByMember(serialize(key), serialize(member), radius),
                byteGeoResultsToStringGeoResults);
    }

    @Override
    public GeoResults<GeoLocation<String>> geoRadiusByMember(
            String key, String member, Distance radius, GeoRadiusCommandArgs args) {

        return convertAndReturn(
                delegate.geoRadiusByMember(serialize(key), serialize(member), radius, args),
                byteGeoResultsToStringGeoResults);
    }
    ...
    public GeoResults<GeoLocation<byte[]>> geoRadiusByMember(
            byte[] key, byte[] member, double radius) {
        return geoRadiusByMember(key, member, new Distance(radius, DistanceUnit.METERS));
    }
    ...
    public GeoResults<GeoLocation<byte[]>> geoRadiusByMember(
        ...
        return convertAndReturn(
                delegate.geoRadiusByMember(key, member, radius), Converters.identityConverter());
    }
    ...
    public GeoResults<GeoLocation<byte[]>> geoRadiusByMember(
        ...
        return convertAndReturn(
                delegate.geoRadiusByMember(key, member, radius, args), Converters.identityConverter());
    }
    ...
    public GeoResults<GeoLocation<byte[]>> geoSearch(
        ...
        return convertAndReturn(
                delegate.geoSearch(key, reference, predicate, args), Converters.identityConverter());
    }
    ...
    public Long geoSearchStore(
        ...
        return convertAndReturn(
                delegate.geoSearchStore(destKey, key, reference, predicate, args),
                Converters.identityConverter());
    }

    @Override
    public GeoResults<GeoLocation<String>> geoSearch(
            String key, GeoReference<String> reference, GeoShape predicate, GeoSearchCommandArgs args) {
        return convertAndReturn(
                delegate.geoSearch(serialize(key), serialize(reference), predicate, args),
                byteGeoResultsToStringGeoResults);
    }
    ...
    public Long geoSearchStore(
            ...
            GeoSearchStoreCommandArgs args) {
        return convertAndReturn(
                delegate.geoSearchStore(
                        serialize(destKey), serialize(key), serialize(reference), predicate, args),
                Converters.identityConverter());
    }
    ...
    @Override
    public Boolean pExpireAt(
            String key, long unixTimeInMillis, ExpirationOptions.Condition condition) {
    ...
    /**
     * NOTE: This method will not deserialize Strings returned by Lua scripts, as they may not be
     * encoded with the same serializer used here. They will be returned as byte[]s
     */
     ...
     */
    public <T> T evalSha(
            String scriptSha1, ReturnType returnType, int numKeys, String... keysAndArgs) {
    ...
    public @Nullable List<Long> applyHashFieldExpiration(
            byte[] key,
    ...
    public @Nullable List<Long> applyExpiration(
            String key,
            io.valkey.springframework.data.valkey.core.types.Expiration expiration,
            ExpirationOptions options,
            String... fields) {
        return this.applyHashFieldExpiration(
                serialize(key), expiration, options, serializeMulti(fields));
    }

    @Override
    public List<Long> hExpire(
            String key, long seconds, ExpirationOptions.Condition condition, String... fields) {
        return hExpire(serialize(key), seconds, condition, serializeMulti(fields));
    }

    @Override
    public List<Long> hpExpire(
            String key, long millis, ExpirationOptions.Condition condition, String... fields) {
        return hpExpire(serialize(key), millis, condition, serializeMulti(fields));
    }

    @Override
    public List<Long> hExpireAt(
            String key, long unixTime, ExpirationOptions.Condition condition, String... fields) {
        return hExpireAt(serialize(key), unixTime, condition, serializeMulti(fields));
    }

    @Override
    public List<Long> hpExpireAt(
            String key, long unixTimeInMillis, ExpirationOptions.Condition condition, String... fields) {
    ...
    /*
     * @see io.valkey.springframework.data.valkey.connection.ValkeyServerCommands#getClientName()
     */
    @Override
    ...
    public Cursor<Entry<String, String>> hScan(String key, ScanOptions options) {
        ...
        return new ConvertingCursor<>(
                this.delegate.hScan(this.serialize(key), options), this::convertEntry);
    }
    ...
    public Set<byte[]> zRangeByLex(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeByLex(key, range, limit), Converters.identityConverter());
    }
    ...
    public Set<String> zRangeByLex(String key, org.springframework.data.domain.Range<String> range) {
        return zRangeByLex(
                key, range, io.valkey.springframework.data.valkey.connection.Limit.unlimited());
    }

    @Override
    public Set<String> zRangeByLex(
            String key,
            org.springframework.data.domain.Range<String> range,
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeByLex(serialize(key), serialize(range), limit), byteSetToStringSet);
    }
    ...
    public Set<byte[]> zRevRangeByLex(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRevRangeByLex(key, range, limit), Converters.identityConverter());
    }

    @Override
    public Set<String> zRevRangeByLex(
            String key,
            org.springframework.data.domain.Range<String> range,
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRevRangeByLex(serialize(key), serialize(range), limit), byteSetToStringSet);
    }
    ...
    public Long zRangeStoreByLex(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreByLex(dstKey, srcKey, range, limit), Converters.identityConverter());
    }
    ...
    public Long zRangeStoreByLex(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreByLex(serialize(dstKey), serialize(srcKey), serialize(range), limit),
                Converters.identityConverter());
    }
    ...
    public Long zRangeStoreRevByLex(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreRevByLex(dstKey, srcKey, range, limit), Converters.identityConverter());
    }
    ...
    public Long zRangeStoreRevByLex(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreRevByLex(serialize(dstKey), serialize(srcKey), serialize(range), limit),
                Converters.identityConverter());
    }
    ...
    public Long zRangeStoreByScore(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreByScore(dstKey, srcKey, range, limit), Converters.identityConverter());
    }
    ...
    public Long zRangeStoreByScore(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreByScore(serialize(dstKey), serialize(srcKey), range, limit),
                Converters.identityConverter());
    }
    ...
    public Long zRangeStoreRevByScore(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreRevByScore(dstKey, srcKey, range, limit),
                Converters.identityConverter());
    }
    ...
    public Long zRangeStoreRevByScore(
            ...
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.zRangeStoreRevByScore(serialize(dstKey), serialize(srcKey), range, limit),
                Converters.identityConverter());
    }
    ...
    public Long xAck(String key, String group, RecordId... recordIds) {
        return convertAndReturn(
                delegate.xAck(this.serialize(key), group, recordIds), Converters.identityConverter());
    }

    @Override
    public RecordId xAdd(StringRecord record, XAddOptions options) {
        return convertAndReturn(
                delegate.xAdd(record.serialize(serializer), options), Converters.identityConverter());
    }

    @Override
    public List<RecordId> xClaimJustId(
            String key, String group, String consumer, XClaimOptions options) {
        return convertAndReturn(
                delegate.xClaimJustId(serialize(key), group, consumer, options),
                Converters.identityConverter());
    }

    @Override
    public List<StringRecord> xClaim(
            String key, String group, String consumer, XClaimOptions options) {
        return convertAndReturn(
                delegate.xClaim(serialize(key), group, consumer, options),
    ...
    public Long xDel(String key, RecordId... recordIds) {
        return convertAndReturn(
                delegate.xDel(serialize(key), recordIds), Converters.identityConverter());
    }

    @Override
    public String xGroupCreate(String key, ReadOffset readOffset, String group) {
        return convertAndReturn(
                delegate.xGroupCreate(serialize(key), group, readOffset), Converters.identityConverter());
    }

    @Override
    public String xGroupCreate(String key, ReadOffset readOffset, String group, boolean mkStream) {
        return convertAndReturn(
                delegate.xGroupCreate(serialize(key), group, readOffset, mkStream),
                Converters.identityConverter());
    }
    ...
    public Boolean xGroupDelConsumer(String key, Consumer consumer) {
        return convertAndReturn(
                delegate.xGroupDelConsumer(serialize(key), consumer), Converters.identityConverter());
    }

    @Override
    public Boolean xGroupDestroy(String key, String group) {
        return convertAndReturn(
                delegate.xGroupDestroy(serialize(key), group), Converters.identityConverter());
    }
    ...
    public XInfoConsumers xInfoConsumers(String key, String groupName) {
        return convertAndReturn(
                delegate.xInfoConsumers(serialize(key), groupName), Converters.identityConverter());
    }
    ...
    public PendingMessagesSummary xPending(String key, String groupName) {
        return convertAndReturn(
                delegate.xPending(serialize(key), groupName), Converters.identityConverter());
    }

    @Override
    public PendingMessages xPending(
            String key,
            String groupName,
            String consumer,
            org.springframework.data.domain.Range<String> range,
            Long count) {
        return convertAndReturn(
                delegate.xPending(serialize(key), groupName, consumer, range, count),
                Converters.identityConverter());
    }
    ...
    public PendingMessages xPending(
            ...
            Long count) {
        return convertAndReturn(
                delegate.xPending(serialize(key), groupName, range, count), Converters.identityConverter());
    }

    @Override
    public PendingMessages xPending(String key, String groupName, XPendingOptions options) {
        return convertAndReturn(
                delegate.xPending(serialize(key), groupName, options), Converters.identityConverter());
    }

    @Override
    public List<StringRecord> xRange(
            String key,
            org.springframework.data.domain.Range<String> range,
            io.valkey.springframework.data.valkey.connection.Limit limit) {
        return convertAndReturn(
                delegate.xRange(serialize(key), range, limit), listByteMapRecordToStringMapRecordConverter);
    }

    @Override
    public List<StringRecord> xReadAsString(
            StreamReadOptions readOptions, StreamOffset<String>... streams) {
        return convertAndReturn(
                delegate.xRead(readOptions, serialize(streams)),
                listByteMapRecordToStringMapRecordConverter);
    }

    @Override
    public List<StringRecord> xReadGroupAsString(
            Consumer consumer, StreamReadOptions readOptions, StreamOffset<String>... streams) {

        return convertAndReturn(
                delegate.xReadGroup(consumer, readOptions, serialize(streams)),
                listByteMapRecordToStringMapRecordConverter);
    }

    @Override
    public List<StringRecord> xRevRange(
            String key,
            org.springframework.data.domain.Range<String> range,
            io.valkey.springframework.data.valkey.connection.Limit limit) {

        return convertAndReturn(
                delegate.xRevRange(serialize(key), range, limit),
                listByteMapRecordToStringMapRecordConverter);
    }
    ...
    public Long xTrim(String key, long count, boolean approximateTrimming) {
        return convertAndReturn(
                delegate.xTrim(serialize(key), count, approximateTrimming), Converters.identityConverter());
    }
    ...
    @Override
    public List<RecordId> xClaimJustId(
            byte[] key, String group, String newOwner, XClaimOptions options) {
    ...
    public String xGroupCreate(
            byte[] key, String groupName, ReadOffset readOffset, boolean mkStream) {
    ...
    public List<ByteRecord> xReadGroup(
            Consumer consumer, StreamReadOptions readOptions, StreamOffset<byte[]>... streams) {
        return delegate.xReadGroup(consumer, readOptions, streams);
    }

    @Override
    public List<ByteRecord> xRevRange(
            byte[] key,
            org.springframework.data.domain.Range<String> range,
            io.valkey.springframework.data.valkey.connection.Limit limit) {
     ...
     * {@link #closePipeline()} and {@link #exec()} will be of the type returned by the underlying
     * connection
     *
    ...
    private <T> T convertAndReturn(@Nullable Object value, Converter converter) {
        ...
        return value == null
                ? null
                : ObjectUtils.nullSafeEquals(converter, Converters.identityConverter())
                        ? (T) value
