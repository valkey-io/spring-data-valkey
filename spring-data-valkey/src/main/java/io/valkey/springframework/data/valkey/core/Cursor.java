...
/**
 * Cursor abstraction to scan over the keyspace or elements within a data structure using a variant
 * of a {@code SCAN} command.
 *
 * <p>Using a Java 8 {@link #stream() java.util.stream.Stream} allows to apply additional {@link
 * java.util.stream.Stream#filter(java.util.function.Predicate) filters} and {@link
 * java.util.stream.Stream#limit(long) limits} to the underlying {@link Cursor}.
 *
 * <p>Make sure to {@link CloseableIterator#close() close} the cursor when done as this allows
 * implementations to clean up any resources they need to keep open to iterate over elements (eg. by
...
public interface Cursor<T> extends CloseableIterator<T> {
    ...
    abstract class CursorId {
        ...
        private static final CursorId INITIAL =
                new CursorId() {
                    @Override
        ...
        /**
         * Returns whether the given {@code cursorId} represent an initial cursor identifier to indicate
         * an initial/finished cursor state.
         *
        ...
        /**
         * Returns whether the current cursor identifier represent an initial cursor identifier to
         * indicate an initial/finished cursor state.
         *
         * @return {@code true} if the cursorId represents an initial/finished state.
         */
