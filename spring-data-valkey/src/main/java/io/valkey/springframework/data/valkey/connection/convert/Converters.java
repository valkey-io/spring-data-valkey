...
import io.valkey.springframework.data.valkey.connection.ValkeyClusterNode.ValkeyClusterNodeBuilder;
...
public abstract class Converters {
     ...
     */
    public static <V>
            Converter<GeoResults<GeoLocation<byte[]>>, GeoResults<GeoLocation<V>>>
                    deserializingGeoResultsConverter(ValkeySerializer<V> serializer) {

        return new DeserializingGeoResultsConverter<>(serializer);
    }

    /**
     * {@link Converter} capable of converting Double into {@link Distance} using given {@link
     * Metric}.
     *
     * @param metric
     * @return
    ...
    /**
     * Converts array outputs with key-value sequences (such as produced by {@code CONFIG GET}) from a
     * {@link List} to {@link Properties}.
     *
     ...
     * {@code CONFIG GET}) from a {@link List} to {@link Properties}.
     *
    ...
    /**
     * Parse a rather generic Valkey response, such as a list of something into a meaningful structure
     * applying best effort conversion of {@code byte[]} and {@link ByteBuffer}.
     *
    ...
    /**
     * Parse a rather generic Valkey response, such as a list of something into a meaningful structure
     * applying best effort conversion of {@code byte[]} and {@link ByteBuffer} based on the {@literal
     * sourcePath} and a {@literal typeHintMap}
     *
     * @param source the source to parse
    ...
    public static Object parse(Object source, String sourcePath, Map<String, Class<?>> typeHintMap) {
        ...
        if (ClassUtils.isAssignable(List.class, targetType)
                && source instanceof List<?> sourceCollection) {
        ...
        public GeoResults<GeoLocation<V>> convert(GeoResults<GeoLocation<byte[]>> source) {
            ...
            for (GeoResult<GeoLocation<byte[]>> value : source.getContent()) {
                values.add(
                        new GeoResult<>(
                                new GeoLocation<>(
                                        serializer.deserialize(value.getContent().getName()),
                                        value.getContent().getPoint()),
                                value.getDistance()));
            }
    ...
    enum ClusterNodesConverter implements Converter<String, ValkeyClusterNode> {
         ...
         *
         * <ul>
         *   <li>{@code %s:%i} (Valkey 3)
         *   <li>{@code %s:%i@%i} (Valkey 4, with bus port)
         *   <li>{@code %s:%i@%i,%s} (Valkey 7, with announced hostname) The output of the {@code
         *       CLUSTER NODES } command is just a space-separated CSV string, where each line
         *       represents a node in the cluster. The following is an example of output on Valkey
         *       7.2.0. You can check the latest <a
         *       href="https://valkey.io/docs/latest/commands/cluster-nodes/">here</a>.
         *       <p>{@code <id> <ip:port@cport[,hostname]> <flags> <master> <ping-sent> <pong-recv>
        ...
        public ValkeyClusterNode convert(String source) {
            ...
            ValkeyClusterNodeBuilder nodeBuilder =
        ...
        private LinkState parseLinkState(String[] args) {
            ...
            return StringUtils.hasText(raw)
                    ? LinkState.valueOf(raw.toUpperCase())
