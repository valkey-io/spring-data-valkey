 ...
 * @since 1.7
 */
public class ValkeyQueryCreator
        extends AbstractQueryCreator<KeyValueQuery<ValkeyOperationChain>, ValkeyOperationChain> {
    ...
    private ValkeyOperationChain from(
            Part part, Iterator<Object> iterator, ValkeyOperationChain sink) {
        ...
        switch (part.getType()) {
            ...
            default ->
                    throw new IllegalArgumentException(
                            "%s is not supported for Valkey query derivation".formatted(part.getType()));
        }
    ...
    @Override
    protected ValkeyOperationChain and(
            Part part, ValkeyOperationChain base, Iterator<Object> iterator) {
    ...
    @Override
    protected KeyValueQuery<ValkeyOperationChain> complete(
            @Nullable ValkeyOperationChain criteria, Sort sort) {

        KeyValueQuery<ValkeyOperationChain> query = new KeyValueQuery<>(criteria);

        if (criteria != null
                && containsExactlyOne(criteria.getSismember())
    ...
    private NearPath getNearPath(Part part, Iterator<Object> iterator) {
        ...
        throw new InvalidDataAccessApiUsageException(
                "Expected to find a Circle or Point/Distance for geo query but was %s"
