...
/**
 * Marker interface for configuration classes related to Valkey connection setup. As the setup
 * scenarios are quite diverse instead of struggling with unifying those, {@link
 * ValkeyConfiguration} provides means to identify configurations for the individual purposes.
 *
...
public interface ValkeyConfiguration {
    ...
    /**
     * Get the configured database index if the current {@link ValkeyConfiguration} is {@link
     * #isDatabaseIndexAware(ValkeyConfiguration) database aware} or evaluate and return the value of
     * the given {@link Supplier}.
     *
     * @param other a {@code Supplier} whose result is returned if given {@link ValkeyConfiguration}
     *     is not {@link #isDatabaseIndexAware(ValkeyConfiguration) database aware}.
    ...
    /**
     * Get the configured {@link ValkeyPassword} if the current {@link ValkeyConfiguration} is {@link
     * #isAuthenticationAware(ValkeyConfiguration) password aware} or evaluate and return the value of
     * the given {@link Supplier}.
     *
     * @param other a {@code Supplier} whose result is returned if given {@link ValkeyConfiguration}
     *     is not {@link #isAuthenticationAware(ValkeyConfiguration) password aware}.
     * @return never {@literal null}.
     * @throws IllegalArgumentException if {@code other} is {@literal null}.
     */
    ...
    /**
     * @param configuration can be {@literal null}.
     * @return {@code true} if given {@link ValkeyConfiguration} is instance of {@link
     *     WithDatabaseIndex}.
     */
     ...
     * @throws IllegalArgumentException if {@code other} is {@literal null}.
     */
    static Integer getDatabaseOrElse(
            @Nullable ValkeyConfiguration configuration, Supplier<Integer> other) {

        Assert.notNull(other, "Other must not be null");
        return isDatabaseIndexAware(configuration)
                ? ((WithDatabaseIndex) configuration).getDatabase()
                : other.get();
    }
     ...
     */
    @Nullable
    static String getUsernameOrElse(
            @Nullable ValkeyConfiguration configuration, Supplier<String> other) {

        Assert.notNull(other, "Other must not be null");
        return isAuthenticationAware(configuration)
                ? ((WithAuthentication) configuration).getUsername()
                : other.get();
    }
     ...
     * @throws IllegalArgumentException if {@code other} is {@literal null}.
     */
    static ValkeyPassword getPasswordOrElse(
            @Nullable ValkeyConfiguration configuration, Supplier<ValkeyPassword> other) {

        Assert.notNull(other, "Other must not be null");
        return isAuthenticationAware(configuration)
                ? ((WithAuthentication) configuration).getPassword()
                : other.get();
    }
     ...
     * @param configuration can be {@literal null}.
     * @param other a {@code Supplier} whose result is returned if given {@link ValkeyConfiguration}
     *     is not {@link #isHostAndPortAware(ValkeyConfiguration) port aware}.
     * @return never {@literal null}.
     * @throws IllegalArgumentException if {@code other} is {@literal null}.
     * @since 2.1.6
     */
    static int getPortOrElse(@Nullable ValkeyConfiguration configuration, IntSupplier other) {
        ...
        Assert.notNull(other, "Other must not be null");
        return isHostAndPortAware(configuration)
                ? ((WithHostAndPort) configuration).getPort()
                : other.getAsInt();
    }
     ...
     * @param configuration can be {@literal null}.
     * @param other a {@code Supplier} whose result is returned if given {@link ValkeyConfiguration}
     *     is not {@link #isHostAndPortAware(ValkeyConfiguration) host aware}.
     * @return never {@literal null}.
     * @throws IllegalArgumentException if {@code other} is {@literal null}.
     * @since 2.1.6
     */
    static String getHostOrElse(@Nullable ValkeyConfiguration configuration, Supplier<String> other) {
        ...
        Assert.notNull(other, "Other must not be null");
        return isHostAndPortAware(configuration)
                ? ((WithHostAndPort) configuration).getHostName()
                : other.get();
    }

    /**
     * {@link ValkeyConfiguration} part suitable for configurations that may use authentication when
     * connecting.
     *
     * @author Christoph Strobl
     * @author Mark Paluch
    ...
    /**
     * {@link ValkeyConfiguration} part suitable for configurations that use host/port combinations
     * for connecting.
     *
     * @author Christoph Strobl
     * @since 2.1
    ...
    /**
     * {@link ValkeyConfiguration} part suitable for configurations that use native domain sockets for
     * connecting.
     *
     * @author Christoph Strobl
     * @since 2.1
    ...
    /**
     * Configuration interface suitable for single node valkey connections using local unix domain
     * socket.
     *
     * @author Christoph Strobl
     * @since 2.1
    ...
    interface SentinelConfiguration extends WithDatabaseIndex, WithPassword {
        ...
        /**
         * Create and set a {@link ValkeyPassword} to be used when authenticating with Valkey Sentinel
        ...
        /**
         * Create and set a {@link ValkeyPassword} to be used when authenticating with Valkey Sentinel
         * from the given {@link Character} sequence.
         *
         * @param password can be {@literal null}.
         * @since 2.2.2
         */
         ...
         * Can be set via {@link #setSentinelPassword(ValkeyPassword)} or {@link ValkeyPassword#none()}
