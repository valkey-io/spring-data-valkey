...
import io.valkey.springframework.data.valkey.connection.ReactiveStreamCommands.GroupCommand.GroupCommandAction;
...
class LettuceReactiveStreamCommands implements ReactiveStreamCommands {
    ...
    @Override
    public Flux<NumericResponse<AcknowledgeCommand, Long>> xAck(
            Publisher<AcknowledgeCommand> commands) {
    ...
    @Override
    public Flux<CommandResponse<AddStreamRecord, RecordId>> xAdd(
            Publisher<AddStreamRecord> commands) {
    ...
    @Override
    public Flux<CommandResponse<XClaimCommand, Flux<RecordId>>> xClaimJustId(
                        ...
                        Flux.from(commands)
                                .map(
                                        command -> {
                                            ...
                                            io.lettuce.core.Consumer<ByteBuffer> from =
                                                    io.lettuce.core.Consumer.from(
                                                            ByteUtils.getByteBuffer(command.getGroupName()),
                                                            ByteUtils.getByteBuffer(command.getNewOwner()));
                                            XClaimArgs args =
                                                    StreamConverters.toXClaimArgs(command.getOptions()).justid();
                                            Flux<RecordId> result =
                                                    cmd.xclaim(command.getKey(), from, args, ids)
                                                            .map(it -> RecordId.of(it.getId()));
                                            return new CommandResponse<>(command, result);
                                        }));
    }

    @Override
    public Flux<CommandResponse<XClaimCommand, Flux<ByteBufferRecord>>> xClaim(
            Publisher<XClaimCommand> commands) {
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            if (command.getAction().equals(GroupCommandAction.CREATE)) {
                                                ...
                                                StreamOffset offset =
                                                        StreamOffset.from(
                                                                command.getKey(), command.getReadOffset().getOffset());

                                                return cmd.xgroupCreate(
                                                                offset,
                                                                ByteUtils.getByteBuffer(command.getGroupName()),
                                            ...
                                            if (command.getAction().equals(GroupCommandAction.DESTROY)) {
                                                ...
                                                return cmd.xgroupDestroy(
                                                                command.getKey(), ByteUtils.getByteBuffer(command.getGroupName()))
                                                        .map(
                                                                it ->
                                                                        new CommandResponse<>(
                                                                                command, Boolean.TRUE.equals(it) ? "OK" : "Error"));
                                            }

                                            throw new IllegalArgumentException(
                                                    "Unknown group command " + command.getAction());
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.xlen(command.getKey())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.xpending(
                                                    ...
                                                    .map(
                                                            it -> {
                                                                return StreamConverters.toPendingMessagesInfo(
                                                                        command.getGroupName(), it);
                                                            })
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            io.lettuce.core.Range<String> range =
                                                    RangeConverter.toRangeWithDefault(command.getRange(), "-", "+");
                                            io.lettuce.core.Limit limit =
                                                    command.isLimited()
                                                            ? io.lettuce.core.Limit.from(command.getCount())
                                                            : io.lettuce.core.Limit.unlimited();

                                            Flux<PendingMessage> publisher =
                                                    command.hasConsumer()
                                                            ? cmd.xpending(
                                                                    command.getKey(),
                                                                    io.lettuce.core.Consumer.from(
                                            ...
                                            return publisher
                                                    ...
                                                    .map(
                                                            it -> {
                                                                return StreamConverters.toPendingMessages(
                                                                        command.getGroupName(), command.getRange(), it);
                                                            })
                                                    .map(value -> new CommandResponse<>(command, value));
                                        }));
    }
    ...
    @Override
    public Flux<CommandResponse<RangeCommand, Flux<ByteBufferRecord>>> xRange(
                        ...
                        Flux.from(commands)
                                .map(
                                        command -> {
                                            ...
                                            io.lettuce.core.Range<String> lettuceRange =
                                                    RangeConverter.toRange(command.getRange(), Function.identity());
                                            io.lettuce.core.Limit lettuceLimit =
                                                    LettuceConverters.toLimit(command.getLimit());

                                            return new CommandResponse<>(
                                                    command,
                                                    cmd.xrange(command.getKey(), lettuceRange, lettuceLimit)
                                                            .map(
                                                                    it ->
                                                                            StreamRecords.newRecord()
                                                                                    .in(it.getStream())
                ...
                .map(
                        command -> {
                            ...
                            if (readOptions.isBlocking()) {
                                return new CommandResponse<>(
                                        command, connection.executeDedicated(cmd -> doRead(command, readOptions, cmd)));
                            }

                            return new CommandResponse<>(
                                    command, connection.execute(cmd -> doRead(command, readOptions, cmd)));
                        });
    }

    private static Flux<ByteBufferRecord> doRead(
            ReadCommand command,
                        ...
                        Flux.from(commands)
                                .flatMap(
                                        command -> {
                                            ...
                                            return cmd.xinfoStream(command.getKey())
                                                    .collectList()
                                                    .map(XInfoStream::fromList)
                                                    .map(it -> new CommandResponse<>(command, it));
                                        }));
    }

    @Override
    public Flux<CommandResponse<XInfoCommand, Flux<XInfoGroup>>> xInfoGroups(
                        ...
                        Flux.from(commands)
                                .map(
                                        command -> {
                                            ...
                                            return new CommandResponse<>(
                                                    command,
                                                    cmd.xinfoGroups(command.getKey())
                                                            .map(it -> XInfoGroup.fromList((List<Object>) it)));
                                        }));
    }

    @Override
    public Flux<CommandResponse<XInfoCommand, Flux<XInfoConsumer>>> xInfoConsumers(
            Publisher<XInfoCommand> commands) {
                        ...
                        Flux.from(commands)
                                .map(
                                        command -> {
                                            ...
                                            return new CommandResponse<>(
                                                    command,
                                                    cmd.xinfoConsumers(command.getKey(), groupName)
                                                            .map(
                                                                    it ->
                                                                            new XInfoConsumer(
                                                                                    command.getGroupName(), (List<Object>) it)));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .map(
                                        command -> {
                                            ...
                                            return new CommandResponse<>(
                                                    command,
                                                    cmd.xrevrange(command.getKey(), lettuceRange, lettuceLimit)
                                                            .map(
                                                                    it ->
                                                                            StreamRecords.newRecord()
                                                                                    .in(it.getStream())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.xtrim(
                                                            command.getKey(), command.isApproximateTrimming(), command.getCount())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }
    ...
    private static <T> StreamOffset<T>[] toStreamOffsets(
            Collection<io.valkey.springframework.data.valkey.connection.stream.StreamOffset<T>> streams) {

        return streams.stream()
                .map(it -> StreamOffset.from(it.getKey(), it.getOffset().getOffset()))
                .toArray(StreamOffset[]::new);
    }

    private static io.lettuce.core.Consumer<ByteBuffer> toConsumer(Consumer consumer) {

        return io.lettuce.core.Consumer.from(
                ByteUtils.getByteBuffer(consumer.getGroup()), ByteUtils.getByteBuffer(consumer.getName()));
    }
