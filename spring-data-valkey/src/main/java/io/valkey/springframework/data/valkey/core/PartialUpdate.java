...
/**
 * {@link PartialUpdate} allows to issue individual property updates without the need of rewriting
 * the whole entity. It allows to define {@literal set}, {@literal delete} actions on existing
 * objects while taking care of updating potential expiration times of the entity itself as well as
 * index structures.
 *
...
public class PartialUpdate<T> {
     ...
     * @param value must not be {@literal null}. If you want to remove a value use {@link
     *     #del(String)}.
    ...
    public PartialUpdate<T> set(String path, Object value) {
        ...
        PartialUpdate<T> update =
                new PartialUpdate<>(
                        this.id, this.target, this.value, this.refreshTtl, this.propertyUpdates);
