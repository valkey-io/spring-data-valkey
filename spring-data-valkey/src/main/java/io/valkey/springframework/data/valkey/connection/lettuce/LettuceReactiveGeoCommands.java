...
import io.valkey.springframework.data.valkey.connection.ReactiveGeoCommands;
...
class LettuceReactiveGeoCommands implements ReactiveGeoCommands {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.geoadd(command.getKey(), values.toArray())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }

    @Override
    public Flux<CommandResponse<GeoDistCommand, Distance>> geoDist(
            Publisher<GeoDistCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Metric metric =
                                                    command.getMetric().isPresent()
                                                            ? command.getMetric().get()
                                                            : ValkeyGeoCommands.DistanceUnit.METERS;

                                            GeoArgs.Unit geoUnit = LettuceConverters.toGeoArgsUnit(metric);
                                            Converter<Double, Distance> distanceConverter =
                                                    LettuceConverters.distanceConverterForMetric(metric);

                                            Mono<Distance> result =
                                                    cmd.geodist(command.getKey(), command.getFrom(), command.getTo(), geoUnit)
    ...
    @Override
    public Flux<MultiValueResponse<GeoHashCommand, String>> geoHash(
            Publisher<GeoHashCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.geohash(
                                                            command.getKey(),
                                                            command.getMembers().stream().toArray(ByteBuffer[]::new))
                                                    .collectList()
                                                    .map(
                                                            value ->
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Mono<List<Value<GeoCoordinates>>> result =
                                                    cmd.geopos(
                                                                    command.getKey(),
                                                                    command.getMembers().stream().toArray(ByteBuffer[]::new))
                                                            .collectList();
                                            ...
                                            return result.map(
                                                    value ->
                                                            new MultiValueResponse<>(
                                                                    ...
                                                                    value.stream()
                                                                            .map(v -> v.getValueOrElse(null))
                                                                            .map(LettuceConverters::geoCoordinatesToPoint)
                                                                            .collect(Collectors.toList())));
                                        }));
    }

    @Override
    public Flux<CommandResponse<GeoRadiusCommand, Flux<GeoResult<GeoLocation<ByteBuffer>>>>>
            geoRadius(Publisher<GeoRadiusCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            GeoArgs geoArgs =
                                                    command.getArgs().isPresent()
                                                            ? LettuceConverters.toGeoArgs(command.getArgs().get())
                                                            : new GeoArgs();

                                            Flux<GeoResult<GeoLocation<ByteBuffer>>> result =
                                                    cmd.georadius(
                                                                    command.getKey(),
                                                                    command.getPoint().getX(),
                                                                    command.getPoint().getY(),
                                                                    command.getDistance().getValue(),
                                                                    LettuceConverters.toGeoArgsUnit(
                                                                            command.getDistance().getMetric()),
                                                                    geoArgs) //
    ...
    @Override
    public Flux<CommandResponse<GeoRadiusByMemberCommand, Flux<GeoResult<GeoLocation<ByteBuffer>>>>>
    ...
    @Override
    public Flux<CommandResponse<GeoSearchCommand, Flux<GeoResult<GeoLocation<ByteBuffer>>>>>
            geoSearch(Publisher<GeoSearchCommand> commands) {
                                ...
                                .map(
                                        command -> {
                                            ...
                                            GeoArgs geoArgs =
                                                    command
                                                            .getArgs()
                                                            .map(LettuceConverters::toGeoArgs)
                                                            .orElseGet(GeoArgs::new);
                                            GeoSearch.GeoRef<ByteBuffer> ref =
                                                    LettuceConverters.toGeoRef(command.getReference());
                                            GeoSearch.GeoPredicate predicate =
                                                    LettuceConverters.toGeoPredicate(command.getShape());

                                            Flux<GeoResult<GeoLocation<ByteBuffer>>> result =
                                                    cmd.geosearch(command.getKey(), ref, predicate, geoArgs)
    ...
    @Override
    public Flux<NumericResponse<GeoSearchStoreCommand, Long>> geoSearchStore(
                                ...
                                .flatMap(
                                        command -> {
                                            ...
                                            Boolean storeDist =
                                                    command
                                                            .getArgs()
                                                            .map(ValkeyGeoCommands.GeoSearchStoreCommandArgs::isStoreDistance)
                                                            .orElse(false);
                                            GeoSearch.GeoRef<ByteBuffer> ref =
                                                    LettuceConverters.toGeoRef(command.getReference());
                                            GeoSearch.GeoPredicate predicate =
