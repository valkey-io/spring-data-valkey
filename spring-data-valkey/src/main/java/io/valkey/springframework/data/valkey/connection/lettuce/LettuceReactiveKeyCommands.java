...
class LettuceReactiveKeyCommands implements ReactiveKeyCommands {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        (command) -> {
                                            ...
                                            return cmd.exists(command.getKey())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.type(command.getKey())
                                                    .map(LettuceConverters::toDataType)
                                                    .map(respValue -> new CommandResponse<>(command, respValue));
                                        }));
    }

    @Override
    public Flux<NumericResponse<Collection<ByteBuffer>, Long>> touch(
            Publisher<Collection<ByteBuffer>> keysCollection) {

        return connection.execute(
                cmd ->
                        Flux.from(keysCollection)
                                .concatMap(
                                        (keys) -> {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.rename(command.getKey(), command.getNewKey())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.renamenx(command.getKey(), command.getNewKey())
                                                    .map(value -> new BooleanResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        (command) -> {
                                            ...
                                            return cmd.del(command.getKey())
                                                    .map((value) -> new NumericResponse<>(command, value));
                                        }));
    }

    @Override
    public Flux<NumericResponse<List<ByteBuffer>, Long>> mDel(
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        (command) -> {
                                            ...
                                            return cmd.unlink(command.getKey())
                                                    .map((value) -> new NumericResponse<>(command, value));
                                        }));
    }

    @Override
    public Flux<NumericResponse<List<ByteBuffer>, Long>> mUnlink(
            Publisher<List<ByteBuffer>> keysCollection) {
                        ...
                        Flux.from(keysCollection)
                                .concatMap(
                                        (keys) -> {
                                            ...
                                            return cmd.unlink(keys.toArray(new ByteBuffer[keys.size()]))
                                                    .map((value) -> new NumericResponse<>(keys, value));
                                        }));
    }
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            if (command.getExpiration().isPersistent()) {
                                                return cmd.persist(command.getKey())
                                                        .map(value -> new BooleanResponse<>(command, value));
                                            }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.expireat(
                                                            command.getKey(),
                                                            command.getExpireAt().getEpochSecond(),
                                                            getExpireArgs(command.getOptions()))
                                                    .map(value -> new BooleanResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.pexpireat(
                                                            command.getKey(),
                                                            command.getExpireAt().toEpochMilli(),
                                                            getExpireArgs(command.getOptions()))
                                                    .map(value -> new BooleanResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.pttl(command.getKey())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.move(command.getKey(), command.getDatabase())
                                                    .map(value -> new BooleanResponse<>(command, value));
                                        }));
    }
    ...
    public Mono<ValueEncoding> encodingOf(ByteBuffer key) {
        ...
        return connection
                .execute(
                        cmd ->
                                cmd.objectEncoding(key)
                                        .map(ValueEncoding::of)
                                        .defaultIfEmpty(ValkeyValueEncoding.VACANT))
                .next();
    }
    ...
    private static ExpireArgs getExpireArgs(ExpirationOptions options) {
        ...
        return new ExpireArgs() {
            ...
            public <K, V> void build(CommandArgs<K, V> args) {
                super.build(args);
                if (ObjectUtils.nullSafeEquals(
                        options.getCondition(), ExpirationOptions.Condition.ALWAYS)) {
                    return;
                }
