...
/**
 * {@link IndexWriter} takes care of writing <a href="https://valkey.io/topics/indexes">secondary
 * index</a> structures to Valkey. Depending on the type of {@link IndexedData}, it uses Sets with
 * specific names to add actually referenced keys to. While doing so {@link IndexWriter} also keeps
 * track of all indexes associated with the root types key, which allows to remove the root key from
 * all indexes in case of deletion.
 *
...
class IndexWriter {
    ...
    private void createOrUpdateIndexes(
            Object key, @Nullable Iterable<IndexedData> indexValues, IndexWriteMode writeMode) {
    ...
    private byte[] toBytes(@Nullable Object source) {
        ...
        throw new InvalidDataAccessApiUsageException(
                ("Cannot convert %s to binary representation for index key generation; Are you missing a"
                        + " Converter; Did you register a non PathBasedValkeyIndexDefinition that might apply"
                        + " to a complex type")
