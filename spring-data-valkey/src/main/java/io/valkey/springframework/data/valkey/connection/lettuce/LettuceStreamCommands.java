...
class LettuceStreamCommands implements ValkeyStreamCommands {
    ...
    public Long xAck(byte[] key, String group, RecordId... recordIds) {
        ...
        return connection
                .invoke()
                .just(RedisStreamAsyncCommands::xack, key, LettuceConverters.toBytes(group), ids);
    }
    ...
    public RecordId xAdd(MapRecord<byte[], byte[], byte[]> record, XAddOptions options) {
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xadd, record.getStream(), args, record.getValue())
                .get(RecordId::of);
    }
    ...
    public List<RecordId> xClaimJustId(
        ...
        io.lettuce.core.Consumer<byte[]> from =
                io.lettuce.core.Consumer.from(
                        LettuceConverters.toBytes(group), LettuceConverters.toBytes(newOwner));
        XClaimArgs args = StreamConverters.toXClaimArgs(options).justid();

        return connection
                .invoke()
                .fromMany(RedisStreamAsyncCommands::xclaim, key, from, args, ids)
    ...
    public Long xDel(byte[] key, RecordId... recordIds) {
        ...
        return connection
                .invoke()
                .just(RedisStreamAsyncCommands::xdel, key, entryIdsToString(recordIds));
    }
    ...
    public String xGroupCreate(byte[] key, String groupName, ReadOffset readOffset, boolean mkSteam) {
        ...
        XReadArgs.StreamOffset<byte[]> streamOffset =
                XReadArgs.StreamOffset.from(key, readOffset.getOffset());

        return connection
                .invoke()
                .just(
                        RedisStreamAsyncCommands::xgroupCreate,
                        streamOffset,
                        LettuceConverters.toBytes(groupName),
    ...
    public Boolean xGroupDelConsumer(byte[] key, Consumer consumer) {
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xgroupDelconsumer, key, lettuceConsumer)
                .get(Objects::nonNull);
    }
    ...
    public Boolean xGroupDestroy(byte[] key, String groupName) {
        ...
        return connection
                .invoke()
                .just(RedisStreamAsyncCommands::xgroupDestroy, key, LettuceConverters.toBytes(groupName));
    }
    ...
    public XInfoStream xInfo(byte[] key) {
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xinfoStream, key)
                .get(XInfoStream::fromList);
    }
    ...
    public XInfoGroups xInfoGroups(byte[] key) {
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xinfoGroups, key)
                .get(XInfoGroups::fromList);
    }
    ...
    public XInfoConsumers xInfoConsumers(byte[] key, String groupName) {
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xinfoConsumers, key, LettuceConverters.toBytes(groupName))
    ...
    public PendingMessagesSummary xPending(byte[] key, String groupName) {
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xpending, key, group)
    ...
    public PendingMessages xPending(byte[] key, String groupName, XPendingOptions options) {
        ...
        io.lettuce.core.Range<String> range =
                RangeConverter.toRangeWithDefault(options.getRange(), "-", "+", Function.identity());
        io.lettuce.core.Limit limit =
                options.isLimited()
                        ? io.lettuce.core.Limit.from(options.getCount())
        ...
        if (options.hasConsumer()) {
            ...
            return connection
                    .invoke()
                    .from(
                            RedisStreamAsyncCommands::xpending,
                            key,
                            io.lettuce.core.Consumer.from(
                                    group, LettuceConverters.toBytes(options.getConsumerName())),
                            range,
                            limit)
        ...
        return connection
                .invoke()
                .from(RedisStreamAsyncCommands::xpending, key, group, range, limit)
                .get(it -> StreamConverters.toPendingMessages(groupName, options.getRange(), it));
    }
    ...
    public List<ByteRecord> xRange(byte[] key, Range<String> range, Limit limit) {
        ...
        return connection
                .invoke()
                .fromMany(RedisStreamAsyncCommands::xrange, key, lettuceRange, lettuceLimit)
                .toList(StreamConverters.byteRecordConverter());
    }
    ...
    public List<ByteRecord> xRead(StreamReadOptions readOptions, StreamOffset<byte[]>... streams) {
        ...
        if (readOptions.isBlocking()) {
            ...
            return connection
                    .invoke(getAsyncDedicatedConnection())
    ...
    public Long xTrim(byte[] key, long count, boolean approximateTrimming) {
        ...
        return connection
                .invoke()
                .just(RedisStreamAsyncCommands::xtrim, key, approximateTrimming, count);
    }
    ...
    private static XReadArgs.StreamOffset<byte[]>[] toStreamOffsets(StreamOffset<byte[]>[] streams) {
        ...
        return Arrays.stream(streams)
                .map(it -> XReadArgs.StreamOffset.from(it.getKey(), it.getOffset().getOffset()))
    ...
    private static io.lettuce.core.Consumer<byte[]> toConsumer(Consumer consumer) {
        ...
        return io.lettuce.core.Consumer.from(
                LettuceConverters.toBytes(consumer.getGroup()),
