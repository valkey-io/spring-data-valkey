...
import io.valkey.springframework.data.valkey.connection.lettuce.observability.ValkeyObservation.LowCardinalityCommandKeyNames;
...
record DefaultLettuceObservationConvention(boolean includeCommandArgsInSpanTags)
    ...
    public KeyValues getLowCardinalityKeyValues(LettuceObservationContext context) {
        ...
        KeyValues keyValues =
                KeyValues.of(
                        LowCardinalityCommandKeyNames.DATABASE_SYSTEM.withValue("valkey"), //
                        LowCardinalityCommandKeyNames.VALKEY_COMMAND.withValue(
                                context.getRequiredCommand().getType().toString()));
        ...
        if (ep instanceof SocketAddressEndpoint endpoint) {
            ...
            if (endpoint.socketAddress() instanceof InetSocketAddress inet) {
                keyValues =
                        keyValues.and(
                                KeyValues.of(
                                        LowCardinalityCommandKeyNames.NET_SOCK_PEER_ADDR.withValue(
                                                inet.getHostString()),
                ...
                keyValues =
                        keyValues.and(
                                KeyValues.of(
                                        LowCardinalityCommandKeyNames.NET_PEER_NAME.withValue(endpoint.toString()),
    ...
    public KeyValues getHighCardinalityKeyValues(LettuceObservationContext context) {
        ...
        if (includeCommandArgsInSpanTags) {
            ...
            if (command.getArgs() != null) {
                return KeyValues.of(
                        HighCardinalityCommandKeyNames.STATEMENT.withValue(
                                command.getType().toString() + " " + command.getArgs().toCommandString()));
            }
        }
