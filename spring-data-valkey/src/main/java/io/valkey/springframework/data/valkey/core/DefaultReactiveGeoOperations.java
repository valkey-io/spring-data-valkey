...
class DefaultReactiveGeoOperations<K, V> implements ReactiveGeoOperations<K, V> {
    ...
    public Mono<Long> add(K key, GeoLocation<V> location) {
        ...
        return createMono(
                geoCommands ->
                        geoCommands.geoAdd(
                                rawKey(key), new GeoLocation<>(rawValue(location.getName()), location.getPoint())));
    }
    ...
    public Mono<Long> add(K key, Map<V, Point> memberCoordinateMap) {
        ...
        return createMono(
                geoCommands -> {
                    Mono<List<GeoLocation<ByteBuffer>>> serializedList =
                            Flux.fromIterable(() -> memberCoordinateMap.entrySet().iterator())
    ...
    public Mono<Long> add(K key, Iterable<GeoLocation<V>> geoLocations) {
        ...
        return createMono(
                geoCommands -> {
                    Mono<List<GeoLocation<ByteBuffer>>> serializedList =
                            Flux.fromIterable(geoLocations)
                                    .map(
                                            location ->
                                        ...
                                        locationList ->
                                                locationList.stream()
                                                        .map(
                                                                location ->
                                                                        new GeoLocation<>(
                                                                                rawValue(location.getName()), location.getPoint()))
    ...
    public Mono<Distance> distance(K key, V member1, V member2) {
        ...
        return createMono(
                geoCommands -> geoCommands.geoDist(rawKey(key), rawValue(member1), rawValue(member2)));
    }
    ...
    public Mono<Distance> distance(K key, V member1, V member2, Metric metric) {
        ...
        return createMono(
                geoCommands ->
                        geoCommands.geoDist(rawKey(key), rawValue(member1), rawValue(member2), metric));
    }
    ...
    public Flux<GeoResult<GeoLocation<V>>> radius(K key, Circle within) {
        ...
        return createFlux(
                geoCommands -> geoCommands.geoRadius(rawKey(key), within).map(this::readGeoResult));
    }
    ...
    public Flux<GeoResult<GeoLocation<V>>> radius(K key, Circle within, GeoRadiusCommandArgs args) {
        ...
        return createFlux(
                geoCommands ->
                        geoCommands
                                .geoRadius(rawKey(key), within, args) //
    ...
    public Flux<GeoResult<GeoLocation<V>>> radius(K key, V member, double radius) {
        ...
        return createFlux(
                geoCommands ->
                        geoCommands
                                .geoRadiusByMember(rawKey(key), rawValue(member), new Distance(radius)) //
                                .map(this::readGeoResult));
    }
    ...
    public Flux<GeoResult<GeoLocation<V>>> radius(K key, V member, Distance distance) {
        ...
        return createFlux(
                geoCommands ->
                        geoCommands
                                .geoRadiusByMember(rawKey(key), rawValue(member), distance) //
                                .map(this::readGeoResult));
    }

    @Override
    public Flux<GeoResult<GeoLocation<V>>> radius(
            K key, V member, Distance distance, GeoRadiusCommandArgs args) {
        ...
        return createFlux(
                        geoCommands ->
                                geoCommands.geoRadiusByMember(rawKey(key), rawValue(member), distance, args))
                .map(this::readGeoResult);
    }
    ...
    @Override
    public Flux<GeoResult<GeoLocation<V>>> search(
            K key,
        ...
        return createFlux(
                geoCommands ->
                        geoCommands
                                .geoSearch(rawKey(key), rawReference, geoPredicate, args)
                                .map(this::readGeoResult));
    }

    @Override
    public Mono<Long> searchAndStore(
            K key,
            K destKey,
            GeoReference<V> reference,
            GeoShape geoPredicate,
        ...
        return createMono(
                geoCommands ->
                        geoCommands.geoSearchStore(
                                rawKey(destKey), rawKey(key), rawReference, geoPredicate, args));
    }
    ...
    private GeoResult<GeoLocation<V>> readGeoResult(GeoResult<GeoLocation<ByteBuffer>> source) {
        ...
        return new GeoResult<>(
                new GeoLocation<>(readValue(source.getContent().getName()), source.getContent().getPoint()),
                source.getDistance());
    }
}
