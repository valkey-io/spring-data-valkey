...
class LettuceReactiveHashCommands implements ReactiveHashCommands {
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            if (command.getFieldValueMap().size() == 1) {
                                                ...
                                                Entry<ByteBuffer, ByteBuffer> entry =
                                                        command.getFieldValueMap().entrySet().iterator().next();
                                                ...
                                                result =
                                                        cmd.hmset(command.getKey(), entries)
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            if (command.getFields().size() == 1) {
                                                ...
                                                result =
                                                        cmd.hget(command.getKey(), key.duplicate())
                                                                .map(value -> KeyValue.fromNullable(key, value))
                                                                .defaultIfEmpty(KeyValue.empty(key))
                                                                .map(Collections::singletonList);
                                            } else {
                                                result =
                                                        cmd.hmget(
                                                                        command.getKey(),
                                                                        command.getFields().stream().toArray(ByteBuffer[]::new))
                                                                .collectList();
                                            }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.hexists(command.getKey(), command.getField())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.hlen(command.getKey())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }

    @Override
    public Flux<CommandResponse<HRandFieldCommand, Flux<ByteBuffer>>> hRandField(
                                ...
                                .map(
                                        command -> {
                                            ...
                                            return new CommandResponse<>(
                                                    command, cmd.hrandfield(command.getKey(), command.getCount()));
                                        }));
    }

    @Override
    public Flux<CommandResponse<HRandFieldCommand, Flux<Entry<ByteBuffer, ByteBuffer>>>>
                        ...
                        Flux.from(commands)
                                .map(
                                        command -> {
                                            ...
                                            Flux<Entry<ByteBuffer, ByteBuffer>> flux =
                                                    cmd.hrandfieldWithvalues(command.getKey(), command.getCount())
                                                            .handle(
                                                                    (it, sink) -> {
                                                                        if (it.isEmpty()) {
                                                                            return;
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return Mono.just(
                                                    new CommandResponse<>(
                                                            command, result.map(LettuceReactiveHashCommands::toEntry)));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Flux<KeyValue<ByteBuffer, ByteBuffer>> result =
                                                    ScanStream.hscan(
                                                            cmd,
                                                            command.getKey(),
                                                            LettuceConverters.toScanArgs(command.getOptions()));

                                            Flux<Entry<ByteBuffer, ByteBuffer>> entryFlux =
                                                    result.map(
                                                            it ->
                                                                    new Entry<ByteBuffer, ByteBuffer>() {
                                                                        ...
                                                                        public ByteBuffer setValue(ByteBuffer value) {
                                                                            throw new UnsupportedOperationException(
                                                                                    "Cannot set value for entry in cursor.");
                                                                        }
                                                                    });
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.hstrlen(command.getKey(), command.getField())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            if (command.getExpiration().isPersistent()) {
                                                return cmd.hpersist(command.getKey(), fields)
                                                        .map(value -> new NumericResponse<>(command, value));
                                            }
                                                    ...
                                                    new ExpireArgs() {
                                                        ...
                                                        public <K, V> void build(CommandArgs<K, V> args) {
                                                            ...
                                                            if (ObjectUtils.nullSafeEquals(
                                                                    command.getOptions(), ExpirationOptions.none())) {
                                                                return;
                                                            }
                                            ...
                                            if (command.getExpiration().isUnixTimestamp()) {
                                                ...
                                                if (command.getExpiration().getTimeUnit().equals(TimeUnit.MILLISECONDS)) {
                                                    return cmd.hpexpireat(
                                                                    command.getKey(),
                                                                    command.getExpiration().getExpirationTimeInMilliseconds(),
                                                                    args,
                                                                    fields)
                                                            .map(value -> new NumericResponse<>(command, value));
                                                }
                                                return cmd.hexpireat(
                                                                command.getKey(),
                                                                command.getExpiration().getExpirationTimeInSeconds(),
                                                                args,
                                                                fields)
                                                        .map(value -> new NumericResponse<>(command, value));
                                            }
    ...
    @Override
    public Flux<NumericResponse<HashFieldsCommand, Long>> hPersist(
            Publisher<HashFieldsCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.hpersist(
                                                            command.getKey(), command.getFields().toArray(ByteBuffer[]::new))
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }

    @Override
    public Flux<NumericResponse<HashFieldsCommand, Long>> hTtl(
            Publisher<HashFieldsCommand> commands) {
    ...
    @Override
    public Flux<NumericResponse<HashFieldsCommand, Long>> hpTtl(
            Publisher<HashFieldsCommand> commands) {
