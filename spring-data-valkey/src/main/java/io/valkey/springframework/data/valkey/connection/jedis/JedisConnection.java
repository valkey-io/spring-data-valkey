 ...
 * href="https://github.com/redis/jedis">Jedis</a> library.
 *
...
public class JedisConnection extends AbstractValkeyConnection {
    ...
    private final JedisInvoker invoker =
            new JedisInvoker(
                    (directFunction, pipelineFunction, converter, nullDefault) ->
                            doInvoke(false, directFunction, pipelineFunction, converter, nullDefault));

    private final JedisInvoker statusInvoker =
            new JedisInvoker(
                    (directFunction, pipelineFunction, converter, nullDefault) ->
                            doInvoke(true, directFunction, pipelineFunction, converter, nullDefault));
    ...
    protected JedisConnection(
            Jedis jedis, @Nullable Pool<Jedis> pool, int dbIndex, @Nullable String clientName) {
    ...
    private Object doInvoke(
            boolean status,
            Function<Jedis, Object> directFunction,
                ...
                it -> {
                    if (isQueueing()) {
                        ...
                        Response<Object> response =
                                pipelineFunction.apply(JedisInvoker.createCommands(getRequiredTransaction()));
                        transaction(
                                status
                                        ? newStatusResult(response)
                                        : newJedisResult(response, converter, nullDefault));
                        return null;
                    }
                    ...
                    if (isPipelined()) {
                        ...
                        Response<Object> response =
                                pipelineFunction.apply(JedisInvoker.createCommands(getRequiredPipeline()));
                        pipeline(
                                status
                                        ? newStatusResult(response)
                                        : newJedisResult(response, converter, nullDefault));
    ...
    public List<Object> exec() {
        ...
        try {
            ...
            return !CollectionUtils.isEmpty(results)
                    ? new TransactionResultConverter<>(txResults, JedisExceptionConverter.INSTANCE)
                            .convert(results)
                    : results;
    ...
    /**
     * Obtain a {@link JedisInvoker} to call Jedis methods returning a status response on the current
     * {@link Jedis} instance. Status responses are not included in transactional and pipeline
     * results.
     *
     * @return the {@link JedisInvoker}.
     * @since 2.5
    ...
    <T, R> JedisResult<T, R> newJedisResult(
            Response<T> response, Converter<T, R> converter, Supplier<R> defaultValue) {
    ...
    /**
     * Specifies if pipelined results should be converted to the expected data type. If false, results
     * of {@link #closePipeline()} and {@link #exec()} will be of the type returned by the Jedis
     * driver
     *
    ...
    private void doExceptionThrowingOperationSafely(
            ExceptionThrowingOperation operation, String logMessage) {
