...
/**
 * Connection factory creating <a href="https://github.com/valkey-io/valkey-glide">Valkey Glide</a>
 * based connections. This is the central class for connecting to Valkey using Valkey-Glide.
 *
 * <p>This factory creates a new {@link ValkeyGlideConnection} on each call to {@link
 * #getConnection()}. The underlying client instance is shared among all connections.
 *
 * <p>This class implements the {@link org.springframework.beans.factory.InitializingBean}
 * interface, triggering the creation of the client instance on {@link #afterPropertiesSet()}. It
 * also implements {@link org.springframework.beans.factory.DisposableBean} for closing the client
 * on application shutdown.
 *
 * <p>The Valkey Glide connection factory can be used both with a standalone Valkey server and with
 * a Valkey cluster. For standalone mode, use the {@link #ValkeyGlideConnectionFactory()}
 * constructor or {@link #ValkeyGlideConnectionFactory(ValkeyGlideClientConfiguration)} constructor.
 * For cluster mode, ensure the {@link ValkeyGlideClientConfiguration} is configured for cluster
 * mode.
 *
...
public class ValkeyGlideConnectionFactory
     ...
     *
     * <p>This mapping is necessary because Glide requires pub/sub callbacks to be configured at
     * client creation time, before any subscriptions exist. When a client is created and added to the
     * pool, we also create a {@link DelegatingPubSubListener} and register it as the client's pub/sub
     * callback. The actual {@link MessageListener} is set on the {@link DelegatingPubSubListener}
     * later when {@code subscribe()} is called.
     *
     * <p>When a connection is obtained from the pool, we look up the corresponding {@link
     * DelegatingPubSubListener} for that client and pass it to the {@link ValkeyGlideConnection},
     * which can then configure it with the user's {@link MessageListener} during subscription.
     */
    ...
    /**
     * Constructs a new {@link ValkeyGlideConnectionFactory} instance with the given {@link
     * ValkeyStandaloneConfiguration} and {@link ValkeyGlideClientConfiguration}.
     *
    ...
    /**
     * Constructs a new {@link ValkeyGlideConnectionFactory} instance with the given {@link
     * ValkeyClusterConfiguration} and {@link ValkeyGlideClientConfiguration}.
     *
     * @param clusterConfiguration must not be {@literal null}
    ...
    /**
     * Constructs a new {@link ValkeyGlideConnectionFactory} instance using the given {@link
     * ValkeySentinelConfiguration}.
     *
     * @param sentinelConfiguration must not be {@literal null}.
     * @throws UnsupportedOperationException as Sentinel connections are not supported with
     *     Valkey-Glide.
     * @since 3.0
     */
    public ValkeyGlideConnectionFactory(ValkeySentinelConfiguration sentinelConfiguration) {
        throw new UnsupportedOperationException(
                "Sentinel connections not supported with Valkey-Glide!");
    }

    /**
     * Constructs a new {@link ValkeyGlideConnectionFactory} instance using the given {@link
     * ValkeySentinelConfiguration} and {@link ValkeyGlideClientConfiguration}.
     *
     * @param sentinelConfiguration must not be {@literal null}.
     * @param clientConfiguration must not be {@literal null}.
     * @throws UnsupportedOperationException as Sentinel connections are not supported with
     ...
     *
     * @param clientConfiguration must not be {@literal null}
     * @deprecated since 3.0, use {@link #ValkeyGlideConnectionFactory(ValkeyStandaloneConfiguration,
     *     ValkeyGlideClientConfiguration)} or {@link
     *     #ValkeyGlideConnectionFactory(ValkeyClusterConfiguration,
     *     ValkeyGlideClusterClientConfiguration)} instead
     */
    @Deprecated
    ...
    private boolean sameOtelConfig(
            @Nullable OpenTelemetryForGlide a, @Nullable OpenTelemetryForGlide b) {
    ...
    private GlideClient createGlideClient() {
        try {
            ...
            OpenTelemetryForGlide openTelemetryForGlide =
                    valkeyGlideConfiguration.getOpenTelemetryForGlide();
    ...
    private GlideClusterClient createGlideClusterClient() {
        try {
            ...
            ValkeyClusterConfiguration clusterConfig = (ValkeyClusterConfiguration) configuration;
    ...
    public void setExecutor(AsyncTaskExecutor executor) {
        if (executor != null) {
            logger.warn(
                    "AsyncTaskExecutor configuration ignored for Valkey-Glide. "
    ...
    public String getClientName() {
        // Valkey Glide supports client names via CLIENT SETNAME/GETNAME command but not in
        // configuration
        return null;
    }
