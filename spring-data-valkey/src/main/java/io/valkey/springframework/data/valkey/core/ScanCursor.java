...
/**
 * Valkey client agnostic {@link Cursor} implementation continuously loading additional results from
 * Valkey server until reaching its starting point {@code zero}. <br>
 * <strong>Note:</strong> Please note that the {@link ScanCursor} has to be initialized ({@link
 * #open()} prior to usage. Any failures during scanning will {@link #close() close} the cursor and
 * release any associated resources such as connections.
 *
...
public abstract class ScanCursor<T> implements Cursor<T> {
    ...
    /**
     * Performs the actual scan command using the native client implementation. The given {@literal
     * options} are never {@literal null}.
     *
     * @param cursorId
     * @param options
     * @return
    ...
    public final ScanCursor<T> open() {
        ...
        if (!isReady()) {
            throw new InvalidDataAccessApiUsageException(
                    "Cursor already " + state + "; Cannot (re)open it");
        }
