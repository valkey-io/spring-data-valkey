...
import static io.valkey.springframework.data.valkey.connection.lettuce.LettuceReactiveValkeyConnection.AsyncConnect.State.*;
...
class LettuceReactiveValkeyConnection implements ReactiveValkeyConnection {
    ...
    private final AsyncConnect<StatefulRedisPubSubConnection<ByteBuffer, ByteBuffer>>
            pubSubConnection;
     ...
     * @throws InvalidDataAccessResourceUsageException when {@code client} is not suitable for
     *     connection.
     */
    ...
    LettuceReactiveValkeyConnection(LettuceConnectionProvider connectionProvider) {
        ...
        this.pubSubConnection =
                new AsyncConnect(connectionProvider, StatefulRedisPubSubConnection.class);
    }

    /**
     * Creates new {@link LettuceReactiveValkeyConnection} given a shared {@link StatefulConnection
     * connection}.
     *
     ...
     */
    @SuppressWarnings("unchecked")
    LettuceReactiveValkeyConnection(
            StatefulConnection<ByteBuffer, ByteBuffer> sharedConnection,
    ...
    public <T> Flux<T> executeDedicated(LettuceReactiveCallback<T> callback) {
        return getDedicatedCommands()
                .flatMapMany(callback::doWithCommands)
    ...
    protected Mono<? extends RedisClusterReactiveCommands<ByteBuffer, ByteBuffer>> getCommands() {
        ...
        if (sharedConnection != null) {
            return sharedConnection.map(
                    LettuceReactiveValkeyConnection::getValkeyClusterReactiveCommands);
        }

        return getDedicatedCommands();
    }

    protected Mono<? extends RedisClusterReactiveCommands<ByteBuffer, ByteBuffer>>
            getDedicatedCommands() {
        return dedicatedConnection
                .getConnection()
                .map(LettuceReactiveValkeyConnection::getValkeyClusterReactiveCommands);
    }

    private static RedisClusterReactiveCommands<ByteBuffer, ByteBuffer>
            getValkeyClusterReactiveCommands(StatefulConnection<ByteBuffer, ByteBuffer> connection) {
    ...
    <T> Function<Throwable, Throwable> translateException() {
        ...
        return throwable -> {
            if (throwable instanceof RuntimeException) {
                ...
                DataAccessException convertedException =
                        LettuceExceptionConverter.INSTANCE.convert((RuntimeException) throwable);
    ...
    /**
     * Asynchronous connection utility. This utility has a lifecycle controlled by {@link
     * #getConnection()} and {@link #close()} methods:
     *
     * <ol>
     *   <li>Initial state: Not connected. Calling {@link #getConnection()} will transition to the
     *       next state.
     *   <li>Connection requested: First subscriber to {@link #getConnection()} initiates an
     *       asynchronous connect. Connection is accessible through the resulting {@link Mono} and
     *       will be cached. Connection will be closed if {@link AsyncConnect} gets closed before the
     *       connection publisher emits the actual connection.
     *   <li>Closing: Call to {@link #close()} initiates connection closing.
     *   <li>Closed: Connection is closed.
     * </ol>
     *
     * @author Mark Paluch
     * @author Christoph Strobl
    ...
    static class AsyncConnect<T extends io.lettuce.core.api.StatefulConnection<?, ?>> {
        ...
        static AtomicReferenceFieldUpdater<AsyncConnect, State> STATE =
                AtomicReferenceFieldUpdater.newUpdater(AsyncConnect.class, State.class, "state");
        ...
        AsyncConnect(LettuceConnectionProvider connectionProvider, Class<T> connectionType) {
            ...
            Mono<StatefulConnection> defer =
                    Mono.fromCompletionStage(() -> connectionProvider.getConnectionAsync(connectionType));

            this.connectionPublisher =
                    defer
                            .doOnNext(
                                    it -> {
                            ...
                            .handle(
                                    (connection, sink) -> {
                                        if (isClosing(STATE.get(this))) {
                                            sink.error(
                                                    new IllegalStateException("Unable to connect; Connection is closed"));
                                        } else {
        ...
        /**
         * Obtain a connection publisher. This method connects asynchronously by requesting a connection
         * from {@link LettuceConnectionProvider} with non-blocking synchronization if called
         * concurrently.
         *
