...
/**
 * Interface that specified a basic set of Valkey operations, implemented by {@link ValkeyTemplate}.
 * Not often used but a useful option for extensibility and testability (as it can be easily mocked
 * or stubbed).
 *
 * @author Costin Leau
...
public interface ValkeyOperations<K, V> {
    ...
    /**
     * Executes a Valkey session. Allows multiple operations to be executed in the same session
     * enabling 'transactional' capabilities through {@link #multi()} and {@link #watch(Collection)}
     * operations.
     *
     * @param <T> return type
    ...
    /**
     * Executes the given action object on a pipelined connection, returning the results. Note that
     * the callback <b>cannot</b> return a non-null value as it gets overwritten by the pipeline. This
     * method will use the default serializers to deserialize results
     *
     * @param action callback object to execute
     * @return pipeline results of the given {@link ValkeyCallback#doInValkey(ValkeyConnection)}
     *     invocation. Results are collected from {@link ValkeyConnection} calls, {@link
     *     ValkeyCallback#doInValkey(ValkeyConnection)} itself must return {@literal null}.
     */
    List<Object> executePipelined(ValkeyCallback<?> action);

    /**
     * Executes the given action object on a pipelined connection, returning the results using a
     * dedicated serializer. Note that the callback <b>cannot</b> return a non-null value as it gets
     * overwritten by the pipeline.
     *
     * @param action callback object to execute
     * @param resultSerializer The Serializer to use for individual values or Collections of values.
     *     If any returned values are hashes, this serializer will be used to deserialize both the key
     *     and value
     * @return pipeline results of the given {@link ValkeyCallback#doInValkey(ValkeyConnection)}
     *     invocation. Results are collected from {@link ValkeyConnection} calls, {@link
     *     ValkeyCallback#doInValkey(ValkeyConnection)} itself must return {@literal null}.
    ...
    /**
     * Executes the given Valkey session on a pipelined connection. Allows transactions to be
     * pipelined. Note that the callback <b>cannot</b> return a non-null value as it gets overwritten
     * by the pipeline.
     *
     * @param session Session callback
     * @return pipeline results of the given {@link SessionCallback#execute(ValkeyOperations)}
     *     invocation. Results are collected from {@link ValkeyOperations} calls, {@link
     *     SessionCallback#execute(ValkeyOperations)} itself must return {@literal null}.
     */
    List<Object> executePipelined(SessionCallback<?> session);

    /**
     * Executes the given Valkey session on a pipelined connection, returning the results using a
     * dedicated serializer. Allows transactions to be pipelined. Note that the callback <b>cannot</b>
     ...
     * @return The return value of the script or null if {@link ValkeyScript#getResultType()} is null,
    ...
    /**
     * Allocates and binds a new {@link ValkeyConnection} to the actual return type of the method. It
     * is up to the caller to free resources after use.
     *
     * @param callback must not be {@literal null}.
     * @return the {@link Object result} of the operation performed in the callback or {@literal
     *     null}.
     * @since 1.8
     */
    @Nullable
     ...
     * @return {@code true} when copied successfully or {@literal null} when used in pipeline /
     *     transaction.
     ...
     * @return The number of keys existing among the ones specified as arguments. Keys mentioned
    ...
    /**
     * Unlink the {@code key} from the keyspace. Unlike with {@link #delete(Object)} the actual memory
    ...
    /**
     * Unlink the {@code keys} from the keyspace. Unlike with {@link #delete(Collection)} the actual
     ...
     *
     * @param key must not be {@literal null}.
     * @return {@code true} when persisted successfully or {@literal null} when used in pipeline /
     *     transaction.
     * @see <a href="https://valkey.io/commands/persist">Valkey Documentation: PERSIST</a>
     */
    @Nullable
    ...
    /**
     * Create {@code key} using the {@code serializedValue}, previously obtained using {@link
     * #dump(Object)}.
     *
     * @param key must not be {@literal null}.
    ...
    @Nullable
    <T, S> List<T> sort(
            SortQuery<K> query, BulkMapper<T, S> bulkMapper, ValkeySerializer<S> resultSerializer);
     ...
     * If used along with {@link #watch(Object)} the operation will fail if any of watched keys has
     * been modified.
     *
    ...
    /**
     * Execute a transaction, using the provided {@link ValkeySerializer} to deserialize any results
     * that are byte[]s or Collections of byte[]s. If a result is a Map, the provided {@link
     * ValkeySerializer} will be used for both the keys and values. Other result types (Long, Boolean,
     * etc) are left as-is in the converted results. Tuple results are automatically converted to
     * TypedTuples.
     *
     * @param valueSerializer The {@link ValkeySerializer} to use for deserializing the results of
     *     transaction exec
     ...
     * @return the number of clients that received the message. {@literal null} when used in pipeline
     *     / transaction.
    ...
    /**
     * Returns the operations performed on simple values (or Strings in Valkey terminology) bound to
     * the given key.
     *
     * @param key Valkey key
    ...
    /**
     * Returns the operations performed on zset values (also known as sorted sets) bound to the given
     * key.
     *
     * @param key Valkey key
