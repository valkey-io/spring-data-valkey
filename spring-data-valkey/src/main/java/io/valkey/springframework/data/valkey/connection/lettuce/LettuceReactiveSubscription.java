...
class LettuceReactiveSubscription implements ReactiveSubscription {
    ...
    public Mono<Void> unsubscribe(ByteBuffer... channels) {
        ...
        return ObjectUtils.isEmpty(channels)
                ? Mono.empty()
                : channelState.unsubscribe(channels, commands::unsubscribe);
    }
    ...
    public Mono<Void> pUnsubscribe(ByteBuffer... patterns) {
        ...
        return ObjectUtils.isEmpty(patterns)
                ? Mono.empty()
                : patternState.unsubscribe(patterns, commands::pUnsubscribe);
    }
    ...
    public Flux<Message<ByteBuffer, ByteBuffer>> receive() {
        ...
        Flux<Message<ByteBuffer, ByteBuffer>> channelMessages =
                channelState.receive(
                        () ->
                                reactive
                                        ...
                                        .map(
                                                message ->
                                                        new ChannelMessage<>(message.getChannel(), message.getMessage())));

        Flux<Message<ByteBuffer, ByteBuffer>> patternMessages =
                patternState.receive(
                        () ->
                                reactive
                                        .observePatterns() //
                                        ...
                                        .map(
                                                message ->
                                                        new PatternMessage<>(
                                                                message.getPattern(), message.getChannel(), message.getMessage())));
                ...
                .then(
                        Mono.defer(
                                () -> {
                                    ...
                                    // this is to ensure completion of the futures and result processing. Since we're
                                    // unsubscribing first, we expect
    ...
    static class State {
        ...
        /**
         * Subscribe to {@code targets} using subscribe {@link Function} and register {@code targets}
         * after subscription.
         *
         * @param targets
         ...
         */
        Mono<Void> subscribe(
                ByteBuffer[] targets, Function<ByteBuffer[], Mono<Void>> subscribeFunction) {
        ...
        /**
         * Unsubscribe from to {@code targets} using unsubscribe {@link Function} and register {@code
         * targets} after subscription.
         *
         * @param targets
        ...
        Mono<Void> unsubscribe(
                ByteBuffer[] targets, Function<ByteBuffer[], Mono<Void>> unsubscribeFunction) {
        ...
        /**
         * Create a message stream from connect {@link Function}. Multiple calls to this method are
         * lock-free synchronized. The first successful caller creates the actual stream. Other
         * concurrent callers that do not pass the synchronization use the stream created by the first
         * successful caller.
         *
         * <p>The stream registers a disposal function upon subscription for external {@link
        ...
        <T> Flux<T> receive(Supplier<Flux<T>> connectFunction) {
            ...
            ConnectableFlux<T> connectableFlux =
                    connectFunction.get().onErrorMap(exceptionTranslator).publish();
            Flux<T> fluxToUse =
                    connectableFlux
                            .doOnSubscribe(
                                    subscription -> {
