...
public interface ValkeyKeyCommands {
    ...
    /**
     * Count how many of the given {@code keys} exist. Providing the very same {@code key} more than
     * once also counts multiple times.
     *
     * @param keys must not be {@literal null}.
     * @return the number of keys existing among the ones specified as arguments. {@literal null} when
    ...
    /**
     * Unlink the {@code keys} from the keyspace. Unlike with {@link #del(byte[]...)} the actual
     ...
     * @return empty {@link Set} if no match found. {@literal null} when used in pipeline /
     *     transaction.
     ...
     * @return {@literal null} when used in pipeline / transaction. {@literal true} if the timeout was
     *     set or {@literal false} if the timeout was not set; for example, the key doesn't exist, or
     *     the operation was skipped because of the provided arguments.
     * @since 3.5
     ...
     */
    @Nullable
    default Boolean applyExpiration(
            byte[] key,
    ...
    /**
     * Set time to live for given {@code key} using {@link Duration#toMillis() milliseconds}
     * precision.
     *
     * @param key must not be {@literal null}.
     * @param duration
    ...
    /**
     * Set the expiration for given {@code key} as a {@literal UNIX} timestamp in {@link
     * Instant#getEpochSecond() seconds} precision.
     *
     * @param key must not be {@literal null}.
     * @param unixTime
     * @return {@literal null} when used in pipeline / transaction. {@literal true} if the timeout was
    ...
    /**
     * Set the expiration for given {@code key} as a {@literal UNIX} timestamp in {@link
     * Instant#toEpochMilli() milliseconds} precision.
     *
     * @param key must not be {@literal null}.
     * @param unixTime
     * @return {@literal null} when used in pipeline / transaction. {@literal true} if the timeout was
     ...
     * @param key must not be {@literal null}.
     * @return {@link
     *     io.valkey.springframework.data.valkey.connection.ValueEncoding.ValkeyValueEncoding#VACANT}
     *     if key does not exist or {@literal null} when used in pipeline / transaction.
