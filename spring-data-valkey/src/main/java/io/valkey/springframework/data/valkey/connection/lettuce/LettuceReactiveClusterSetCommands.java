...
import io.valkey.springframework.data.valkey.connection.ReactiveClusterSetCommands;
 ...
 * @since 2.0
 */
class LettuceReactiveClusterSetCommands extends LettuceReactiveSetCommands
                                ...
                                Flux.from(commands)
                                        .concatMap(
                                                command -> {
                                                    ...
                                                    return sUnion(Mono.just(SUnionCommand.keys(command.getKeys())))
                                                            .next()
                                                            .flatMap(
                                                                    values -> {
                                                                        Mono<Long> result =
                                                                                values
                                                                                        ...
                                                                                        .flatMap(
                                                                                                it -> {
                                                                                                    ByteBuffer[] members =
                                                                                                            it.toArray(new ByteBuffer[0]);
                                                                                                    return cmd.sadd(command.getKey(), members);
                                                                                                });

                                                                        return result.map(
                                                                                value -> new NumericResponse<>(command, value));
                                                                    });
                                                }));
    }
                                ...
                                Flux.from(commands)
                                        .concatMap(
                                                command -> {
                                                    ...
                                                    Mono<List<ByteBuffer>> sourceSet =
                                                            cmd.smembers(command.getKeys().get(0)).distinct().collectList();

                                                    List<Mono<List<ByteBuffer>>> intersectingSets = new ArrayList<>();

                                                    for (int i = 1; i < command.getKeys().size(); i++) {
                                                        intersectingSets.add(
                                                                cmd.smembers(command.getKeys().get(i)).distinct().collectList());
                                                    }

                                                    Flux<List<ByteBuffer>> result =
                                                            Flux.zip(
                                                                    sourceSet,
                                                                    Flux.merge(intersectingSets).collectList(),
                                                    ...
                                                    return Mono.just(
                                                            new CommandResponse<>(
                                                                    command, result.concatMap(v -> Flux.fromStream(v.stream()))));
                                                }));
    }
                                ...
                                Flux.from(commands)
                                        .concatMap(
                                                command -> {
                                                    ...
                                                    return sInter(Mono.just(SInterCommand.keys(command.getKeys())))
                                                            .next()
                                                            .flatMap(
                                                                    values -> {
                                                                        Mono<Long> result =
                                ...
                                Flux.from(commands)
                                        .concatMap(
                                                command -> {
                                                    ...
                                                    return sDiff(Mono.just(SDiffCommand.keys(command.getKeys())))
                                                            .next()
                                                            .flatMap(
                                                                    values -> {
                                                                        Mono<Long> result =
                                ...
                                Flux.from(commands)
                                        .concatMap(
                                                command -> {
                                                    ...
                                                    Mono<Boolean> result =
                                                            cmd.exists(command.getKey())
                                                                    .flatMap(
                                                                            nrKeys ->
                                                                                    nrKeys == 0
                                                                                            ? Mono.empty()
                                                                                            : cmd.sismember(command.getKey(), command.getValue()))
                                                                    .flatMap(
                                                                            exists -> {
                                                                                if (!exists) {
                                                                                ...
                                                                                return cmd.sismember(
                                                                                                command.getDestination(), command.getValue())
                                                                                        .flatMap(
                                                                                                existsInTarget -> {
                                                                                                    Mono<Boolean> tmp =
                                                                                                            cmd.srem(command.getKey(), command.getValue())
