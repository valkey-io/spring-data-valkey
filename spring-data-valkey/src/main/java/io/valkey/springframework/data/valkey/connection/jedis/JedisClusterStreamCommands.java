...
import static io.valkey.springframework.data.valkey.connection.jedis.StreamConverters.*;
...
class JedisClusterStreamCommands implements ValkeyStreamCommands {
    ...
    public Long xAck(byte[] key, String group, RecordId... recordIds) {
        ...
        try {
            return connection
                    .getCluster()
                    .xack(key, JedisConverters.toBytes(group), entryIdsToBytes(Arrays.asList(recordIds)));
        } catch (Exception ex) {
        ...
        try {
            return RecordId.of(
                    JedisConverters.toString(
                            connection.getCluster().xadd(record.getStream(), record.getValue(), params)));
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
    ...
    public StreamInfo.XInfoConsumers xInfoConsumers(byte[] key, String groupName) {
        ...
        try {
            return StreamInfo.XInfoConsumers.fromList(
                    groupName,
    ...
    public PendingMessages xPending(byte[] key, String groupName, XPendingOptions options) {
        ...
        try {
            ...
            return StreamConverters.toPendingMessages(
                    groupName, range, BuilderFactory.STREAM_PENDING_ENTRY_LIST.build(response));
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
        ...
        try {
            return convertToByteRecord(
                    ...
                    connection
                            ...
                            .xrange(
                                    key,
                                    JedisConverters.toBytes(getLowerValue(range)),
                                    JedisConverters.toBytes(getUpperValue(range)),
                                    count));
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
