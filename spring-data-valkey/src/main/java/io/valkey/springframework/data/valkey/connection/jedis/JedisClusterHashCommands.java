...
class JedisClusterHashCommands implements ValkeyHashCommands {
    ...
    public Entry<byte[], byte[]> hRandFieldWithValues(byte[] key) {
        ...
        try {
            List<Entry<byte[], byte[]>> mapEntryList =
                    connection.getCluster().hrandfieldWithValues(key, 1);
    ...
    public Cursor<Entry<byte[], byte[]>> hScan(byte[] key, ScanOptions options) {
        ...
        return new ScanCursor<Entry<byte[], byte[]>>(options) {
            ...
            protected ScanIteration<Entry<byte[], byte[]>> doScan(
                    CursorId cursorId, ScanOptions options) {

                ScanParams params = JedisConverters.toScanParams(options);

                ScanResult<Entry<byte[], byte[]>> result =
                        connection.getCluster().hscan(key, JedisConverters.toBytes(cursorId), params);
    ...
    public List<Long> hExpire(
        ...
        try {
            ...
            return connection
                    .getCluster()
                    .hexpire(key, seconds, ExpiryOption.valueOf(condition.name()), fields);
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
    ...
    public List<Long> hpExpire(
        ...
        try {
            ...
            return connection
                    .getCluster()
                    .hpexpire(key, millis, ExpiryOption.valueOf(condition.name()), fields);
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
    ...
    public List<Long> hExpireAt(
        ...
        try {
            ...
            return connection
                    .getCluster()
                    .hexpireAt(key, unixTime, ExpiryOption.valueOf(condition.name()), fields);
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
    ...
    public List<Long> hpExpireAt(
        ...
        try {
            ...
            return connection
                    .getCluster()
                    .hpexpireAt(key, unixTimeInMillis, ExpiryOption.valueOf(condition.name()), fields);
        } catch (Exception ex) {
            throw convertJedisAccessException(ex);
        }
    }
