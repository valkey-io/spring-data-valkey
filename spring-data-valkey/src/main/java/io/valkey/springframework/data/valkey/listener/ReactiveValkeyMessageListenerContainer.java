...
/**
 * Container providing a stream of {@link ChannelMessage} for messages received via Valkey Pub/Sub
 * listeners. The stream is infinite and registers Valkey subscriptions. Handles the low level
 * details of listening, converting and message dispatching.
 *
 * <p>Note the container allocates a single connection when it is created and releases the
 * connection on {@link #destroy()}. Connections are allocated eagerly to not interfere with
 * non-blocking use during application operations. Using reactive infrastructure allows usage of a
 * single connection due to channel multiplexing.
 *
 * <p>This class is thread-safe and allows subscription by multiple concurrent threads.
 *
 * @author Mark Paluch
...
public class ReactiveValkeyMessageListenerContainer implements DisposableBean {
    ...
    private final SerializationPair<String> stringSerializationPair =
    ...
    /**
     * Create a new {@link ReactiveValkeyMessageListenerContainer} given {@link
    ...
    private Mono<Void> doDestroy() {
        ...
        return terminationSignals != null
                ? terminationSignals.then(connection.closeLater())
    ...
    public Collection<ReactiveSubscription> getActiveSubscriptions() {
        ...
        return subscriptions.entrySet().stream()
                .filter(entry -> entry.getValue().hasRegistration())
                .map(Map.Entry::getKey)
                .collect(Collectors.toList());
    }

    /**
     * Subscribe to one or more {@link ChannelTopic}s and receive a stream of {@link ChannelMessage}.
     * Messages and channel names are treated as {@link String}. The message stream subscribes lazily
     * to the Valkey channels and unsubscribes if the {@link org.reactivestreams.Subscription} is
    ...
    /**
     * Subscribe to one or more {@link ChannelTopic}s and receive a stream of {@link ChannelMessage}
     * once the returned {@link Mono} completes. Messages and channel names are treated as {@link
     * String}. The message stream subscribes lazily to the Valkey channels and unsubscribes if the
     * inner {@link org.reactivestreams.Subscription} is {@link
     * org.reactivestreams.Subscription#cancel() cancelled}.
     *
     * <p>The returned {@link Mono} completes once the connection has been subscribed to the given
     * {@link Topic topics}. Note that cancelling the returned {@link Mono} can leave the connection
     * in a subscribed state.
     *
     * @param channelTopics the channels to subscribe.
     * @return the message stream.
     * @throws InvalidDataAccessApiUsageException if {@code channelTopics} is empty.
    ...
    public Mono<Flux<Message<String, String>>> receiveLater(ChannelTopic... channelTopics) {
        ...
        return receiveLater(
                Arrays.asList(channelTopics), stringSerializationPair, stringSerializationPair);
    }

    /**
     * Subscribe to one or more {@link PatternTopic}s and receive a stream of {@link PatternMessage}.
     * Messages, pattern, and channel names are treated as {@link String}. The message stream
     * subscribes lazily to the Valkey channels and unsubscribes if the {@link
     * org.reactivestreams.Subscription} is {@link org.reactivestreams.Subscription#cancel()
     * cancelled}.
     *
    ...
    /**
     * Subscribe to one or more {@link PatternTopic}s and receive a stream of {@link PatternMessage}
     * once the returned {@link Mono} completes. Messages, pattern, and channel names are treated as
     * {@link String}. The message stream subscribes lazily to the Valkey channels and unsubscribes if
     * the inner {@link org.reactivestreams.Subscription} is {@link
     * org.reactivestreams.Subscription#cancel() cancelled}.
     *
     * <p>The returned {@link Mono} completes once the connection has been subscribed to the given
     * {@link Topic topics}. Note that cancelling the returned {@link Mono} can leave the connection
     ...
     */
    @SuppressWarnings("unchecked")
    public Mono<Flux<PatternMessage<String, String, String>>> receiveLater(
        ...
        return receiveLater(
                        Arrays.asList(patternTopics), stringSerializationPair, stringSerializationPair)
                .map(it -> it.map(m -> (PatternMessage<String, String, String>) m));
    }

    /**
     * Subscribe to one or more {@link Topic}s and receive a stream of {@link ChannelMessage}. The
     * stream may contain {@link PatternMessage} if subscribed to patterns. Messages, and channel
     * names are serialized/deserialized using the given {@code channelSerializer} and {@code
     * messageSerializer}. The message stream subscribes lazily to the Valkey channels and
     * unsubscribes if the {@link org.reactivestreams.Subscription} is {@link
     * org.reactivestreams.Subscription#cancel() cancelled}.
     *
     ...
     * @since 2.6
     */
    public Flux<Message<String, String>> receive(
            Iterable<? extends Topic> topics, SubscriptionListener subscriptionListener) {
     ...
     */
    public <C, B> Flux<Message<C, B>> receive(
            Iterable<? extends Topic> topics,
            SerializationPair<C> channelSerializer,
     ...
     * messageSerializer}. The message stream subscribes lazily to the Valkey channels and
     * unsubscribes if the {@link org.reactivestreams.Subscription} is {@link
     * org.reactivestreams.Subscription#cancel() cancelled}. {@link SubscriptionListener} is notified
     * upon subscription/unsubscription and can be used for synchronization.
     *
    ...
    public <C, B> Flux<Message<C, B>> receive(
        ...
        return doReceive(
                channelSerializer,
                messageSerializer,
                getRequiredConnection().pubSubCommands().createSubscription(subscriptionListener),
                patterns,
                channels);
    }

    private <C, B> Flux<Message<C, B>> doReceive(
            SerializationPair<C> channelSerializer,
            SerializationPair<B> messageSerializer,
            Mono<ReactiveSubscription> subscription,
            ByteBuffer[] patterns,
            ByteBuffer[] channels) {

        Flux<Message<ByteBuffer, ByteBuffer>> messageStream =
                subscription.flatMapMany(
                        it -> {
                                    ...
                                    .doOnCancel(
                                            () -> {
                                                ...
                                                if (subscribers.unregister()) {
                                                    ...
                                                    it.cancel()
                                                            .subscribe(
                                                                    v -> terminalSink.tryEmitEmpty(), terminalSink::tryEmitError);
                                                }
                                            })
        ...
        return messageStream.map(
                message ->
                        readMessage(channelSerializer.getReader(), messageSerializer.getReader(), message));
    }

    /**
     * Subscribe to one or more {@link Topic}s and receive a stream of {@link ChannelMessage}. The
     * returned {@link Mono} completes once the connection has been subscribed to the given {@link
     * Topic topics}. Note that cancelling the returned {@link Mono} can leave the connection in a
     * subscribed state.
     *
     * @param topics the channels to subscribe.
     * @param channelSerializer serialization pair to decode the channel/pattern name.
     ...
     * @since 2.6
     */
    public <C, B> Mono<Flux<Message<C, B>>> receiveLater(
            Iterable<? extends Topic> topics,
            SerializationPair<C> channelSerializer,
            SerializationPair<B> messageSerializer) {
        ...
        return Mono.defer(
                () -> {
                    SubscriptionReadyListener readyListener =
                            SubscriptionReadyListener.create(topics, stringSerializationPair);

                    return doReceiveLater(
                                    channelSerializer,
                                    messageSerializer,
                                    getRequiredConnection().pubSubCommands().createSubscription(readyListener),
                                    patterns,
                                    channels)
    ...
    private <C, B> Mono<Flux<Message<C, B>>> doReceiveLater(
            SerializationPair<C> channelSerializer,
            SerializationPair<B> messageSerializer,
            Mono<ReactiveSubscription> subscription,
            ByteBuffer[] patterns,
        ...
        return subscription.flatMap(
                it -> {
                    Mono<Void> subscribe =
                            subscribe(patterns, channels, it).doOnSuccess(v -> getSubscribers(it).registered());

                    Sinks.One<Message<ByteBuffer, ByteBuffer>> terminalSink = Sinks.one();

                    Flux<Message<C, B>> receiver =
                            it.receive()
                                    .doOnCancel(
                                            () -> {
                                                Subscribers subscribers = getSubscribers(it);
                                    ...
                                    .map(
                                            message ->
                                                    readMessage(
                                                            channelSerializer.getReader(),
                                                            messageSerializer.getReader(),
                                                            message));
    ...
    private static Mono<Void> subscribe(
            ByteBuffer[] patterns, ByteBuffer[] channels, ReactiveSubscription it) {

        Assert.isTrue(
                !ObjectUtils.isEmpty(channels) || !ObjectUtils.isEmpty(patterns),
    ...
    private <C, B> Message<C, B> readMessage(
            ValkeyElementReader<C> channelSerializer,
            ValkeyElementReader<B> messageSerializer,
            Message<ByteBuffer, ByteBuffer> message) {

        if (message instanceof PatternMessage<?, ?, ?>) {

            PatternMessage<ByteBuffer, ByteBuffer, ByteBuffer> patternMessage =
                    (PatternMessage<ByteBuffer, ByteBuffer, ByteBuffer>) message;
    ...
    static class Subscribers {
        ...
        private static final AtomicLongFieldUpdater<Subscribers> SUBSCRIBERS =
                AtomicLongFieldUpdater.newUpdater(Subscribers.class, "subscribers");
    ...
    static class SubscriptionReadyListener extends AtomicBoolean implements SubscriptionListener {
        ...
        public static SubscriptionReadyListener create(
                Iterable<? extends Topic> topics, SerializationPair<String> serializationPair) {
            ...
            for (Topic topic : topics) {
                wrappers.add(
                        new ByteArrayWrapper(
                                ByteUtils.getBytes(serializationPair.getWriter().write(topic.getTopic()))));
            }
