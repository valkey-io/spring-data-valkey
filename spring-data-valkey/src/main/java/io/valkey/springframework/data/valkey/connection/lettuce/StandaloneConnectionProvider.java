...
class StandaloneConnectionProvider
        implements LettuceConnectionProvider, TargetAware, ValkeyClientProvider {
    ...
    StandaloneConnectionProvider(
            RedisClient client, RedisCodec<?, ?> codec, @Nullable ReadFrom readFrom) {
    ...
    public <T extends StatefulConnection<?, ?>> T getConnection(Class<T> connectionType) {
        ...
        if (StatefulConnection.class.isAssignableFrom(connectionType)) {
            ...
            return connectionType.cast(
                    readFrom
                            .map(it -> this.masterReplicaConnection(valkeyURISupplier.get(), it))
    ...
    public <T extends StatefulConnection<?, ?>> CompletionStage<T> getConnectionAsync(
        ...
        if (connectionType.equals(StatefulRedisSentinelConnection.class)) {
            return client
                    .connectSentinelAsync(StringCodec.UTF8, redisURI)
        ...
        if (StatefulConnection.class.isAssignableFrom(connectionType)) {
            return readFrom
                    .map(it -> this.masterReplicaConnectionAsync(redisURI, it)) //
                    .orElseGet(() -> (CompletionStage) client.connectAsync(codec, redisURI)) //
                    .thenApply(connectionType::cast);
        }

        return LettuceFutureUtils.failed(
                new UnsupportedOperationException("Connection type " + connectionType + " not supported"));
    }
    ...
    private StatefulRedisConnection masterReplicaConnection(RedisURI redisUri, ReadFrom readFrom) {
        ...
        StatefulRedisMasterReplicaConnection<?, ?> connection =
                MasterReplica.connect(client, codec, redisUri);
        connection.setReadFrom(readFrom);

        return connection;
    }

    private CompletionStage<StatefulRedisConnection<?, ?>> masterReplicaConnectionAsync(
            RedisURI redisUri, ReadFrom readFrom) {
