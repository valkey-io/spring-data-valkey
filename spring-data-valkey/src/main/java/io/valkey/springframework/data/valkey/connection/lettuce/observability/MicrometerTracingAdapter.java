...
import io.valkey.springframework.data.valkey.connection.lettuce.observability.ValkeyObservation.HighCardinalityCommandKeyNames;
...
/**
 * {@link Tracing} adapter using Micrometer's {@link Observation}. This adapter integrates with
 * Micrometer to propagate observations into timers, distributed traces and any other registered
 * handlers. Observations include a set of tags capturing Valkey runtime information.
 *
 * <h3>Capturing full statements</h3>
 *
 * This adapter can capture full statements when enabling {@code includeCommandArgsInSpanTags}. You
 * should carefully consider the impact of this setting as all command arguments will be captured in
 * traces including these that may contain sensitive details.
 *
 * @author Mark Paluch
 * @author Yanming Zhou
 * @since 3.0
...
public class MicrometerTracingAdapter implements Tracing {
     ...
     * @param observationRegistry must not be {@literal null}.
     * @param serviceName service name to be used.
     * @param includeCommandArgsInSpanTags whether to attach the full command into the trace. Use this
     *     flag with caution as sensitive arguments will be captured in the observation spans and
     *     metric tags.
     */
    public MicrometerTracingAdapter(
        ...
        this.observationConvention =
                new DefaultLettuceObservationConvention(includeCommandArgsInSpanTags);
    ...
    public class MicrometerTracer extends Tracer {
        ...
        private Observation createObservation(@Nullable TraceContext parentContext) {
            ...
            return ValkeyObservation.VALKEY_COMMAND_OBSERVATION.observation(
                    observationRegistry,
                    () -> {
        ...
        public Span start(RedisCommand<?, ?, ?> command) {
            ...
            if (command instanceof CompleteableCommand<?> completeableCommand) {
                            ...
                            if (command.getOutput() != null) {
                                ...
                                if (error != null) {
                                    observation.highCardinalityKeyValue(
                                            HighCardinalityCommandKeyNames.ERROR.withValue(error));
                ...
                throw new IllegalArgumentException(
                        "Command "
                                + command
                                + " must implement CompleteableCommand to attach Span completion to command"
                                + " completion");
            }
    ...
    record MicrometerTraceContextProvider(ObservationRegistry registry)
        ...
        public Mono<TraceContext> getTraceContextLater() {
            ...
            return Mono.deferContextual(Mono::justOrEmpty)
                    .filter(
                            (it) -> {
                                return it.hasKey(TraceContext.class)
                                        || it.hasKey(Observation.class)
