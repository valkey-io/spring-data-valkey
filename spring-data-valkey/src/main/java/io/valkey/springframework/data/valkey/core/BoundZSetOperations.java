...
public interface BoundZSetOperations<K, V> extends BoundKeyOperations<K> {
    ...
    /**
     * Add {@code value} to a sorted set at the bound key, or update its {@code score} if it already
     * exists.
     *
    ...
    /**
     * Add {@code tuples} to a sorted set at the bound key, or update its {@code score} if it already
     * exists.
     *
    ...
    default Long lexCount(
            io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range range) {
    ...
    /**
     * Remove and return the value with its score having the lowest score from sorted set at the bound
     * key.
     *
    ...
    /**
     * Remove and return the value with its score having the highest score from sorted set at the
     * bound key.
     *
     * @return {@literal null} when the sorted set is empty or used in pipeline / transaction.
    ...
    /**
     * Remove and return the value with its score having the highest score from sorted set at the
     * bound key. <b>Blocks connection</b> until element available or {@code timeout} reached.
     *
     * @param timeout
     * @param unit must not be {@literal null}.
     * @return can be {@literal null}.
    ...
    /**
     * Remove elements in range between {@code start} and {@code end} from sorted set with the bound
     * key.
     *
     * @param start
     * @param end
    ...
    /**
     * Remove elements with scores between {@code min} and {@code max} from sorted set with the bound
     * key.
     *
     * @param min
     * @param max
    ...
    /**
     * Union sorted sets at the bound key and {@code otherKeys} and store result in destination {@code
     * destKey}.
     *
     ...
     */
    @Nullable
    Set<TypedTuple<V>> intersectWithScores(
            Collection<K> otherKeys, Aggregate aggregate, Weights weights);

    /**
     * Intersect sorted sets at the bound key and {@code otherKey} and store result in destination
     * {@code destKey}.
     *
     * @param otherKey must not be {@literal null}.
     * @param destKey must not be {@literal null}.
    ...
    /**
     * Intersect sorted sets at the bound key and {@code otherKeys} and store result in destination
     * {@code destKey}.
     *
     * @param otherKeys must not be {@literal null}.
     * @param destKey must not be {@literal null}.
    ...
    /**
     * Get all elements with lexicographical ordering with a value between {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMin()} and {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMax()}.
     *
     * @param range must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
     * @since 1.7
    ...
    /**
     * Get all elements with lexicographical ordering with a value between {@link
     * Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param range must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
     * @since 3.0
    ...
    /**
     * Get all elements {@literal n} elements, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with lexicographical ordering from {@literal ZSET} at
     * {@code key} with a value between {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMin()} and {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMax()}.
     *
     * @param range must not be {@literal null}.
    ...
    /**
     * Get all elements {@literal n} elements, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with lexicographical ordering having a value between
    ...
    /**
     * Get all elements with reverse lexicographical ordering from {@literal ZSET} with a value
     * between {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMin()} and {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMax()}.
     *
    ...
    @Nullable
    @Deprecated(since = "3.0", forRemoval = true)
    default Set<V> reverseRangeByLex(
            io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range range) {
        return reverseRangeByLex(range);
    }

    /**
     * Get all elements with reverse lexicographical ordering with a value between {@link
     * Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param range must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
    ...
    /**
     * Get all elements {@literal n} elements, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with reverse lexicographical ordering from {@literal
     * ZSET} having a value between {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMin()} and {@link
     * io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range#getMax()}.
     *
     * @param range must not be {@literal null}.
    ...
    /**
     * Get all elements {@literal n} elements, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with reverse lexicographical ordering having a value
    ...
    /**
     * Store all elements at {@code dstKey} with lexicographical ordering from {@literal ZSET} at the
     * bound key with a value between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
    ...
    /**
     * Store {@literal n} elements at {@code dstKey}, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with lexicographical ordering from {@literal ZSET} at the
     * bound key with a value between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Store all elements at {@code dstKey} with reverse lexicographical ordering from {@literal ZSET}
     * at the bound key with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
     ...
     * Store {@literal n} elements at {@code dstKey}, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with reverse lexicographical ordering from {@literal
     * ZSET} at the bound key with a value between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Store all elements at {@code dstKey} with ordering by score from {@literal ZSET} at the bound
     * key with a score between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @return the number of stored elements or {@literal null} when used in pipeline / transaction.
    ...
    /**
     * Store {@literal n} elements at {@code dstKey}, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with ordering by score from {@literal ZSET} at the bound
     * key with a score between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Store all elements at {@code dstKey} with reverse ordering by score from {@literal ZSET} at the
     * bound key with a score between {@link Range#getLowerBound()} and {@link Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
     * @return the number of stored elements or {@literal null} when used in pipeline / transaction.
    ...
    /**
     * Store {@literal n} elements at {@code dstKey}, where {@literal n = } {@link Limit#getCount()},
     * starting at {@link Limit#getOffset()} with reverse ordering by score from {@literal ZSET} at
     * the bound key with a score between {@link Range#getLowerBound()} and {@link
     * Range#getUpperBound()}.
     *
     * @param dstKey must not be {@literal null}.
     * @param range must not be {@literal null}.
