...
class DefaultZSetOperations<K, V> extends AbstractOperations<K, V> implements ZSetOperations<K, V> {
    ...
    public Set<TypedTuple<V>> reverseRangeWithScores(K key, long start, long end) {
        ...
        byte[] rawKey = rawKey(key);
        Set<Tuple> rawValues =
                execute(connection -> connection.zRevRangeWithScores(rawKey, start, end));
    ...
    public Set<V> rangeByLex(K key, Range<String> range, Limit limit) {
        ...
        byte[] rawKey = rawKey(key);
        Set<byte[]> rawValues =
                execute(connection -> connection.zRangeByLex(rawKey, serialize(range), limit));
    ...
    public Set<V> reverseRangeByLex(K key, Range<String> range, Limit limit) {
        ...
        byte[] rawKey = rawKey(key);
        Set<byte[]> rawValues =
                execute(connection -> connection.zRevRangeByLex(rawKey, serialize(range), limit));
    ...
    public Long rangeAndStoreByLex(K srcKey, K dstKey, Range<String> range, Limit limit) {
        ...
        return execute(
                connection -> connection.zRangeStoreByLex(rawDstKey, rawSrcKey, serialize(range), limit));
    }
    ...
    public Long reverseRangeAndStoreByLex(K srcKey, K dstKey, Range<String> range, Limit limit) {
        ...
        return execute(
                connection ->
                        connection.zRangeStoreRevByLex(rawDstKey, rawSrcKey, serialize(range), limit));
    }
    ...
    @Override
    public Long reverseRangeAndStoreByScore(
            K srcKey, K dstKey, Range<? extends Number> range, Limit limit) {

        byte[] rawDstKey = rawKey(dstKey);
        byte[] rawSrcKey = rawKey(srcKey);

        return execute(
                connection -> connection.zRangeStoreRevByScore(rawDstKey, rawSrcKey, range, limit));
    }
    ...
    public Set<V> rangeByScore(K key, double min, double max, long offset, long count) {
        ...
        byte[] rawKey = rawKey(key);
        Set<byte[]> rawValues =
                execute(connection -> connection.zRangeByScore(rawKey, min, max, offset, count));
    ...
    public Set<V> reverseRangeByScore(K key, double min, double max, long offset, long count) {
        ...
        byte[] rawKey = rawKey(key);
        Set<byte[]> rawValues =
                execute(connection -> connection.zRevRangeByScore(rawKey, min, max, offset, count));
    ...
    public Set<TypedTuple<V>> rangeByScoreWithScores(K key, double min, double max) {
        ...
        byte[] rawKey = rawKey(key);
        Set<Tuple> rawValues =
                execute(connection -> connection.zRangeByScoreWithScores(rawKey, min, max));
    ...
    public Set<TypedTuple<V>> rangeByScoreWithScores(
        ...
        byte[] rawKey = rawKey(key);
        Set<Tuple> rawValues =
                execute(connection -> connection.zRangeByScoreWithScores(rawKey, min, max, offset, count));
    ...
    public Set<TypedTuple<V>> reverseRangeByScoreWithScores(K key, double min, double max) {
        ...
        byte[] rawKey = rawKey(key);
        Set<Tuple> rawValues =
                execute(connection -> connection.zRevRangeByScoreWithScores(rawKey, min, max));

        return deserializeTupleValues(rawValues);
    }

    @Override
    public Set<TypedTuple<V>> reverseRangeByScoreWithScores(
            K key, double min, double max, long offset, long count) {
    ...
    public Set<TypedTuple<V>> intersectWithScores(
        ...
        byte[][] rawKeys = rawKeys(key, otherKeys);
        Set<Tuple> result =
                execute(connection -> connection.zInterWithScores(aggregate, weights, rawKeys));
    ...
    public Set<TypedTuple<V>> unionWithScores(
        ...
        byte[][] rawKeys = rawKeys(key, otherKeys);
        Set<Tuple> result =
                execute(connection -> connection.zUnionWithScores(aggregate, weights, rawKeys));
    ...
    public Cursor<TypedTuple<V>> scan(K key, ScanOptions options) {
        ...
        byte[] rawKey = rawKey(key);
        Cursor<Tuple> cursor =
                template.executeWithStickyConnection(connection -> connection.zScan(rawKey, options));
