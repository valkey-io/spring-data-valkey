...
import io.valkey.springframework.data.valkey.connection.BitFieldSubCommands.BitFieldGet;
...
public abstract class LettuceConverters extends Converters {
    ...
    public static ScriptOutputType toScriptOutputType(ReturnType returnType) {
        ...
        Assert.notNull(
                returnType, () -> "Return type " + returnType + " is not a supported script output type");
    ...
    /**
     * Convert a {@link io.valkey.springframework.data.valkey.connection.ValkeyZSetCommands.Range} to
     * a lettuce {@link Range}.
     *
     * @since 2.0
     */
    ...
    /**
     * Convert a {@link org.springframework.data.domain.Range} to a lettuce {@link Range} and reverse
     * boundaries.
     *
     * @since 2.0
     */
    ...
    public static <T> Range<T> toRevRange(org.springframework.data.domain.Range<T> range) {
        ...
        Range.Boundary upper =
                RangeConverter.convertBound(
                        range.getUpperBound(), false, null, it -> it.getBytes(StandardCharsets.UTF_8));

        Range.Boundary lower =
                RangeConverter.convertBound(
                        range.getLowerBound(), false, null, it -> it.getBytes(StandardCharsets.UTF_8));

        return Range.from(upper, lower);
    }

    /**
     * @param source List of Maps containing node details from SENTINEL REPLICAS or SENTINEL MASTERS.
    ...
    /**
     * @param sentinelConfiguration the sentinel configuration containing one or more sentinels and a
     *     master name. Must not be {@literal null}
     * @return A {@link RedisURI} containing Valkey Sentinel addresses of {@link
     *     ValkeySentinelConfiguration}
     * @since 1.5
     */
    public static RedisURI sentinelConfigurationToValkeyURI(
        ...
        for (ValkeyNode sentinel : sentinels) {
            ...
            RedisURI.Builder sentinelBuilder =
                    RedisURI.Builder.redis(sentinel.getHost(), sentinel.getPort());
     ...
     * @return a {@link ValkeyStandaloneConfiguration} representing the connection information in the
     *     Valkey URI.
     * @since 2.5.3
     */
     ...
     * @return a {@link ValkeySocketConfiguration} representing the connection information in the
     *     Valkey URI.
     * @since 2.5.3
     */
     ...
     * @return a {@link ValkeySentinelConfiguration} representing the Valkey Sentinel information in
     *     the Valkey URI.
     * @since 2.5.3
     */
    static ValkeySentinelConfiguration createValkeySentinelConfiguration(RedisURI redisURI) {
        ...
        for (RedisURI sentinelNodeValkeyUri : redisURI.getSentinels()) {
            ...
            ValkeyNode sentinelNode =
                    new ValkeyNode(sentinelNodeValkeyUri.getHost(), sentinelNodeValkeyUri.getPort());
    ...
    private static void applyAuthentication(
            RedisURI redisURI, ValkeyConfiguration.WithAuthentication valkeyConfiguration) {
    ...
    static Converter<List<byte[]>, Long> toTimeConverter(TimeUnit timeUnit) {
        ...
        return source -> {
            Assert.notEmpty(
                    source, "Received invalid result from server; Expected 2 items in collection");
            Assert.isTrue(
                    source.size() == 2,
                    "Received invalid nr of arguments from valkey server; Expected 2 received "
                            + source.size());
    ...
    public static GeoArgs.Unit toGeoArgsUnit(Metric metric) {
        ...
        Metric metricToUse =
                metric == null || ObjectUtils.nullSafeEquals(Metrics.NEUTRAL, metric)
                        ? DistanceUnit.METERS
                        : metric;
     ...
     * @return the converted {@link ScanArgs}. Returns {@literal null} if {@link ScanOptions} is
     *     {@literal null}.
     * @see 2.1
     */
    @Nullable
     ...
     * @since 1.8
     */
    public static Converter<Set<byte[]>, GeoResults<GeoLocation<byte[]>>>
     ...
     * @since 1.8
     */
    public static Converter<List<GeoWithin<byte[]>>, GeoResults<GeoLocation<byte[]>>>
     ...
     * @since 2.0.9
     */
    static <T extends Comparable<T>> Optional<T> getLowerBound(
            org.springframework.data.domain.Range<T> range) {
     ...
     * @since 2.0.9
     */
    static <T extends Comparable<T>> Optional<T> getUpperBound(
            org.springframework.data.domain.Range<T> range) {
    ...
    /**
     * Return the upper bound index from {@link Range} or {@literal -1} (minus one) if the upper range
     * is not bounded to point to the last element. To be used with index-based commands such as
     * {@code LRANGE}, {@code GETRANGE}.
     *
    ...
    static GeoSearch.GeoPredicate toGeoPredicate(GeoShape predicate) {
        ...
        if (predicate instanceof BoxShape boxPredicate) {
            ...
            return GeoSearch.byBox(
                    boundingBox.getWidth().getValue(),
                    boundingBox.getHeight().getValue(),
                    toGeoArgsUnit(boxPredicate.getMetric()));
        }

        throw new IllegalArgumentException(
                "Cannot convert %s to Lettuce GeoPredicate".formatted(predicate));
    }
    ...
    enum GeoResultsConverterFactory {
        ...
        private static class GeoResultsConverter
            ...
            public GeoResults<GeoLocation<byte[]>> convert(List<GeoWithin<byte[]>> source) {
                ...
                Converter<GeoWithin<byte[]>, GeoResult<GeoLocation<byte[]>>> converter =
                        GeoResultConverterFactory.INSTANCE.forMetric(metric);
    ...
    enum GeoResultConverterFactory {
        ...
        private static class GeoResultConverter
                implements Converter<GeoWithin<byte[]>, GeoResult<GeoLocation<byte[]>>> {
