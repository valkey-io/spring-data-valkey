...
import io.valkey.springframework.data.valkey.connection.lettuce.LettuceConverters;
...
class JedisClusterStringCommands implements ValkeyStringCommands {
    ...
    public List<byte[]> mGet(byte[]... keys) {
        ...
        return connection
                ...
                .executeMultiKeyCommand(
                        (JedisMultiKeyClusterCommandCallback<byte[]>) Jedis::get, Arrays.asList(keys))
        ...
        if (ClusterSlotHashUtil.isSameSlotForAllKeys(
            ...
            try {
                return JedisConverters.toBoolean(
                        connection.getCluster().msetnx(JedisConverters.toByteArrays(tuples)));
            } catch (Exception ex) {
                throw convertJedisAccessException(ex);
            }
        }
    ...
    public Long bitOp(BitOperation op, byte[] destination, byte[]... keys) {
        ...
        throw new InvalidDataAccessApiUsageException(
                "BITOP is only supported for same slot keys in cluster mode");
    }
