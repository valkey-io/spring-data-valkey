...
public class ValkeyNode implements NamedNode {
    ...
    /**
     * Parse a {@code hostAndPort} string into {@link ValkeyNode}. Supports IPv4, IPv6, and hostname
     * notations including the port. For example:
     *
     * <pre class="code">
    ...
    private static String[] getHostAndPortFromBracketedHost(String hostPortString) {
        ...
        if (!(colonIndex > -1 && closeBracketIndex > colonIndex)) {
            throw new IllegalArgumentException(
                    "Invalid bracketed host/port: %s".formatted(hostPortString));
        }
        ...
        if (closeBracketIndex + 1 == hostPortString.length()) {
            ...
            if (!(hostPortString.charAt(closeBracketIndex + 1) == ':')) {
                throw new IllegalArgumentException(
                        "Only a colon may follow a close bracket: %s".formatted(hostPortString));
            }
