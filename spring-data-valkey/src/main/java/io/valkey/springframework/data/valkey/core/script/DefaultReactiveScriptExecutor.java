...
/**
 * Default implementation of {@link ReactiveScriptExecutor}. Optimizes performance by attempting to
 * execute script first using {@code EVALSHA}, then falling back to {@code EVAL} if Valkey has not
 * yet cached the script.
 *
 * @author Mark Paluch
...
public class DefaultReactiveScriptExecutor<K> implements ReactiveScriptExecutor<K> {
    ...
    /**
     * Creates a new {@link DefaultReactiveScriptExecutor} given {@link
     * ReactiveValkeyConnectionFactory} and {@link ValkeySerializationContext}.
     *
     * @param connectionFactory must not be {@literal null}.
     * @param serializationContext must not be {@literal null}.
     */
    public DefaultReactiveScriptExecutor(
    ...
    protected <T> Flux<T> eval(
        ...
        Flux<T> result =
                connection.scriptingCommands().evalSha(script.getSha1(), returnType, numKeys, keysAndArgs);
                ...
                result.onErrorResume(
                        cause -> {
                            if (ScriptUtils.exceptionContainsNoScriptError(cause)) {
                                return connection
                                        .scriptingCommands()
                                        .eval(scriptBytes(script), returnType, numKeys, keysAndArgs);
                            }
    ...
    protected ByteBuffer[] keysAndArgs(ValkeyElementWriter argsWriter, List<K> keys, List<?> args) {
        ...
        return Stream.concat(
                        keys.stream().map(t -> keySerializer().getWriter().write(t)),
                        args.stream().map(t -> argsWriter.write(t)))
    ...
    protected ByteBuffer scriptBytes(ValkeyScript<?> script) {
        return serializationContext
                ...
                .getWriter()
                .write(script.getScriptAsString());
    }
    ...
    /**
     * Executes the given action object within a connection that is allocated eagerly and released
     * after {@link Flux} termination.
     *
     * @param <T> return type
    ...
    private <T> Flux<T> execute(ReactiveValkeyCallback<T> action) {
        ...
        return Flux.usingWhen(
                Mono.fromSupplier(factory::getReactiveConnection),
                action::doInValkey,
