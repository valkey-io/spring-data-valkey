...
import io.valkey.springframework.data.valkey.connection.ReactiveValkeyConnection.AbsentByteBufferResponse;
...
class LettuceReactiveStringCommands implements ReactiveStringCommands {
    ...
    @Override
    public Flux<MultiValueResponse<List<ByteBuffer>, ByteBuffer>> mGet(
            Publisher<List<ByteBuffer>> keyCollections) {

        return this.connection.execute(
                reactiveCommands ->
                        Flux.from(keyCollections)
                                .concatMap(
                                        (keys) -> {
                                            Assert.notNull(keys, "Keys must not be null");

                                            return reactiveCommands
                                                    .mget(keys.toArray(new ByteBuffer[0]))
                                                    .collectList()
                                                    .map(
                                                            value ->
                                                                    value.stream()
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        (command) -> {
                                            ...
                                            return reactiveCommands
                                                    .setGet(command.getKey(), command.getValue())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        (command) -> {
                                            ...
                                            return reactiveCommands
                                                    .getset(command.getKey(), command.getValue())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return reactiveCommands
                                                    .setnx(command.getKey(), command.getValue())
                                                    .map((value) -> new BooleanResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Assert.notNull(command.getValue(), "Value must not be null");
                                            Assert.isTrue(
                                                    command.getExpiration().isPresent(), "Expiration time must not be null");

                                            long expirationTimeInSeconds =
                                                    command.getExpiration().get().getExpirationTimeInSeconds();

                                            return reactiveCommands
                                                    .setex(command.getKey(), expirationTimeInSeconds, command.getValue())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            long expirationTimeInSeconds =
                                                    command.getExpiration().get().getExpirationTimeInMilliseconds();

                                            return reactiveCommands
                                                    .psetex(command.getKey(), expirationTimeInSeconds, command.getValue())
                                                    .map(LettuceConverters::stringToBoolean)
                                                    .map((value) -> new BooleanResponse<>(command, value));
                                        }));
    }
                ...
                reactiveCommands ->
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            Assert.notEmpty(
                                                    command.getKeyValuePairs(), "Pairs must not be null or empty");

                                            return reactiveCommands
                                                    .mset(command.getKeyValuePairs())
                                                    .map(LettuceConverters::stringToBoolean)
                                                    .map((value) -> new BooleanResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return reactiveCommands
                                                    .msetnx(command.getKeyValuePairs())
                                                    .map((value) -> new BooleanResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return reactiveCommands
                                                    .append(command.getKey(), command.getValue())
    ...
    @Override
    public Flux<NumericResponse<SetRangeCommand, Long>> setRange(
            Publisher<SetRangeCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return reactiveCommands
                                                    .setrange(command.getKey(), command.getOffset(), command.getValue())
                                                    .map((value) -> new NumericResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return reactiveCommands
                                                    .getbit(command.getKey(), command.getOffset())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return reactiveCommands
                                                    .setbit(command.getKey(), command.getOffset(), command.getValue() ? 1 : 0)
                                                    .map(LettuceConverters::toBoolean)
    ...
    @Override
    public Flux<NumericResponse<BitCountCommand, Long>> bitCount(
            Publisher<BitCountCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Mono<Long> bitcount =
                                                    !Range.unbounded().equals(range)
                                                            ? reactiveCommands.bitcount(
                                                                    command.getKey(),
                                                                    LettuceConverters.getLowerBoundIndex(range),
                                                                    LettuceConverters.getUpperBoundIndex(range))
    ...
    @Override
    public Flux<MultiValueResponse<BitFieldCommand, Long>> bitField(
            Publisher<BitFieldCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            BitFieldArgs args =
                                                    LettuceConverters.toBitFieldArgs(command.getSubCommands());

                                            return reactiveCommands
                                                    .bitfield(command.getKey(), args)
                                                    .collectList()
                                                    .map(
                                                            value ->
                                                                    new MultiValueResponse<>(
                                                                            command,
                                                                            value.stream()
                ...
                reactiveCommands ->
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            Assert.notNull(
                                                    command.getDestinationKey(), "DestinationKey must not be null");
                                            ...
                                            result =
                                                    switch (command.getBitOp()) {
                                                        ...
                                                        case NOT -> {
                                                            Assert.isTrue(
                                                                    sourceKeys.length == 1,
                                                                    "BITOP NOT does not allow more than 1 source key.");
                                                            yield reactiveCommands.bitopNot(destinationKey, sourceKeys[0]);
                                                        }
                                                        default ->
                                                                throw new IllegalArgumentException(
                                                                        "Unknown BITOP '%s'".formatted(command.getBitOp()));
                                                    };
                                ...
                                .flatMap(
                                        command -> {
                                            ...
                                            if (range.getLowerBound().isBounded()) {
                                                ...
                                                result =
                                                        reactiveCommands.bitpos(
                                                                command.getKey(), command.getBit(), getLowerValue(range));
