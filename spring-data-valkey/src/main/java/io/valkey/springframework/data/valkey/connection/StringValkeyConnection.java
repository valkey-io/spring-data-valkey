...
/**
 * Convenience extension of {@link ValkeyConnection} that accepts and returns {@link String}s
 * instead of byte arrays. Uses a {@link ValkeySerializer} underneath to perform the conversion.
 *
 * @author Costin Leau
...
public interface StringValkeyConnection extends ValkeyConnection {
    ...
    /**
     * 'Native' or 'raw' execution of the given command along-side the given arguments. The command is
     * executed as is, with as little 'interpretation' as possible - it is up to the caller to take
     * care of any processing of arguments or the result.
     *
    ...
    /**
     * Unlink the {@code keys} from the keyspace. Unlike with {@link #del(String...)} the actual
    ...
    /**
     * Set {@code value} for {@code key} applying timeouts from {@code expiration} if set and
     * inserting/updating values depending on {@code option}.
     *
     * @param key must not be {@literal null}.
     * @param value must not be {@literal null}.
    ...
    /**
     * Count the number of set bits (population counting) of value stored at {@code key} between
     ...
     * @return {@literal null} when used in pipeline / transaction. The position of the first bit set
     *     to 1 or 0 according to the request.
    ...
    /**
     * Return the position of the first bit set to given {@code bit} in a string. {@link
     * org.springframework.data.domain.Range} start and end can contain negative values in order to
     * index <strong>bytes</strong> starting from the end of the string, where {@literal -1} is the
     * last byte, {@literal -2} is the penultimate.
     *
     * @param key the key holding the actual String.
     * @param bit the bit value to look for.
     ...
     * @param key must not be {@literal null}.
     * @param element must not be {@literal null}.
     * @param rank specifies the "rank" of the first element to return, in case there are multiple
     *     matches. A rank of 1 means to return the first match, 2 to return the second match, and so
     *     forth.
    ...
    /**
     * Insert {@code value} {@link Position#BEFORE} or {@link Position#AFTER} existing {@code pivot}
     * for {@code key}.
     *
     * @param key must not be {@literal null}.
    ...
    /**
     * Atomically returns and removes the first/last element (head/tail depending on the {@code from}
     * argument) of the list stored at {@code sourceKey}, and pushes the element at the first/last
     * element (head/tail depending on the {@code to} argument) of the list stored at {@code
     * destinationKey}.
     *
     * @param sourceKey must not be {@literal null}.
    ...
    /**
     * Removes and returns first element from lists stored at {@code keys} (see: {@link
     * #lPop(byte[])}). <br>
    ...
    /**
     * Removes and returns last element from lists stored at {@code keys} (see: {@link
     * #rPop(byte[])}). <br>
     * <b>Blocks connection</b> until element available or {@code timeout} reached.
     *
     * @param timeout
    ...
    /**
     * Remove the last element from list at {@code srcKey}, append it to {@code dstKey} and return its
     * value (see {@link #rPopLPush(byte[], byte[])}). <br>
     * <b>Blocks connection</b> until element available or {@code timeout} reached.
     *
     * @param timeout
     * @param srcKey must not be {@literal null}.
    ...
    /**
     * Add the {@code value} to a sorted set at {@code key}, or update its {@code score} depending on
     * the given {@link ZAddArgs args}.
     *
     * @param key must not be {@literal null}.
    ...
    /**
     * Add {@code tuples} to a sorted set at {@code key}, or update its {@code score} depending on the
     * given {@link ZAddArgs args}.
     *
     * @param key must not be {@literal null}.
    ...
    /**
     * Get set of {@link Tuple}s in range from {@code start} to {@code end} where score is between
     ...
     */
    Set<StringTuple> zRangeByScoreWithScores(
    ...
    /**
     * Get set of {@link Tuple}s in range from {@code start} to {@code end} from sorted set ordered
     * from high to low.
     *
     * @param key must not be {@literal null}.
     * @param start
    ...
    /**
     * Get set of {@link Tuple} where score is between {@code min} and {@code max} from sorted set
    ...
    /**
     * Get set of {@link Tuple} in range from {@code start} to {@code end} where score is between
     ...
     */
    Set<StringTuple> zRevRangeByScoreWithScores(
            String key, double min, double max, long offset, long count);
    ...
    /**
     * Get all the elements in {@link Range} from the sorted set at {@literal key} in lexicographical
     * ordering.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Get all the elements in {@link Range} from the sorted set at {@literal key} in lexicographical
     * ordering. Result is limited via {@link io.valkey.springframework.data.valkey.connection.Limit}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
    ...
    /**
     * Get all the elements in {@link Range} from the sorted set at {@literal key} in reversed
     ...
     */
    default Set<String> zRevRangeByLex(
            String key, org.springframework.data.domain.Range<String> range) {
    ...
    /**
     * Get all the elements in {@link Range} from the sorted set at {@literal key} in reversed
     * lexicographical ordering. Result is limited via {@link
     * io.valkey.springframework.data.valkey.connection.Limit}.
     *
     * @param key must not be {@literal null}.
     * @param range must not be {@literal null}.
     ...
     */
    @Nullable
    default Long zRangeStoreByLex(
            String dstKey, String srcKey, org.springframework.data.domain.Range<String> range) {
        return zRangeStoreByLex(
                dstKey, srcKey, range, io.valkey.springframework.data.valkey.connection.Limit.unlimited());
    }
    ...
    /**
     * This command is like ZRANGE … REV , but stores the result in the {@literal dstKey} destination
     * key.
     *
     * @param dstKey must not be {@literal null}.
     * @param srcKey must not be {@literal null}.
     ...
     */
    @Nullable
    default Long zRangeStoreRevByLex(
            String dstKey, String srcKey, org.springframework.data.domain.Range<String> range) {
        return zRangeStoreRevByLex(
                dstKey, srcKey, range, io.valkey.springframework.data.valkey.connection.Limit.unlimited());
    }
     ...
     */
    @Nullable
    default Long zRangeStoreByScore(
            String dstKey, String srcKey, org.springframework.data.domain.Range<? extends Number> range) {
        return zRangeStoreByScore(
                dstKey, srcKey, range, io.valkey.springframework.data.valkey.connection.Limit.unlimited());
    }
     ...
     */
    @Nullable
    default Long zRangeStoreRevByScore(
            String dstKey, String srcKey, org.springframework.data.domain.Range<? extends Number> range) {
        return zRangeStoreRevByScore(
                dstKey, srcKey, range, io.valkey.springframework.data.valkey.connection.Limit.unlimited());
    }
     ...
     * key}. If the key or the field do not exist, {@code 0} is returned.
     *
     * @param key must not be {@literal null}.
     * @param field must not be {@literal null}.
     * @return {@literal null} when used in pipeline / transaction.
     ...
     *
     * @param key must not be {@literal null}.
     * @param seconds the amount of time after which the key will be expired in seconds, must not be
     *     {@literal null}.
     * @param fields must not be {@literal null}.
     ...
     *
     * @param key must not be {@literal null}.
     * @param millis the amount of time after which the key will be expired in milliseconds, must not
     *     be {@literal null}.
     * @param fields must not be {@literal null}.
     * @return a list of {@link Long} values for each of the fields provided: {@code 2} indicating the
     *     specific field is deleted already due to expiration, or provided expiry interval is 0;
     ...
     *     milliseconds; or a negative value to signal an error. The command returns {@code -1} if the
     *     key exists but has no associated expiration time. The command returns {@code -2} if the key
     ...
     * @param timeUnit must not be {@literal null}.
     * @param fields must not be {@literal null}.
     * @return a list of {@link Long} values for each of the fields provided: the time to live in the
     *     {@link TimeUnit} provided; or a negative value to signal an error. The command returns
     *     {@code -1} if the key exists but has no associated expiration time. The command returns
     *     {@code -2} if the key does not exist; {@literal null} when used in pipeline / transaction.
     * @see <a href="https://valkey.io/docs/latest/commands/hexpire/">Valkey Documentation: HTTL</a>
     * @since 3.5
     */
    @Nullable
    ...
    /**
     * Get the {@link Distance} between {@literal member1} and {@literal member2} in the given {@link
     * Metric}.
     *
     * @param key must not be {@literal null}.
     * @param member1 must not be {@literal null}.
    ...
    /**
     * Get the {@literal member}s within the boundaries of a given {@link Circle} applying {@link
    ...
    /**
     * Get the {@literal member}s within the circle defined by the {@literal members} coordinates and
     * given {@literal radius}.
     *
     * @param key must not be {@literal null}.
     * @param member must not be {@literal null}.
     * @param radius
     * @return never {@literal null}.
     * @since 1.8
     * @see <a href="https://valkey.io/commands/georadiusbymember">Valkey Documentation:
     *     GEORADIUSBYMEMBER</a>
    ...
    /**
     * Get the {@literal member}s within the circle defined by the {@literal members} coordinates and
     * given {@link Distance} and {@link GeoRadiusCommandArgs}.
     *
     * @param key must not be {@literal null}.
     * @param member must not be {@literal null}.
     * @param radius must not be {@literal null}.
     ...
     */
    GeoResults<GeoLocation<String>> geoRadiusByMember(
     ...
     * {@link GeoShape shape}. The query's center point is provided by {@link GeoReference}.
     *
     * @param key must not be {@literal null}.
     ...
     */
    @Nullable
    GeoResults<GeoLocation<String>> geoSearch(
     ...
     * {@link GeoShape shape} and store the result at {@code destKey}. The query's center point is
     * provided by {@link GeoReference}.
     *
     * @param key must not be {@literal null}.
     * @param reference must not be {@literal null}.
     * @param predicate must not be {@literal null}.
    ...
    /**
     * Subscribes the connection to the given channels. Once subscribed, a connection enters listening
     * mode and can only subscribe to other channels or unsubscribe. No other commands are accepted
     * until the connection is unsubscribed.
     *
     * <p>Note that this operation is blocking and the current thread starts waiting for new messages
     * immediately.
     *
    ...
    /**
     * Subscribes the connection to all channels matching the given patterns. Once subscribed, a
     * connection enters listening mode and can only subscribe to other channels or unsubscribe. No
     ...
     * @param name
     * @since 1.3
     * @see <a href="https://valkey.io/commands/client-setname">Valkey Documentation: CLIENT
     *     SETNAME</a>
    ...
    /**
     * Change the ownership of a pending message to the given new {@literal consumer} without
     * increasing the delivered count.
     *
     ...
     * @since 2.3
     */
    default List<StringRecord> xClaim(
            String key, String group, String newOwner, Duration minIdleTime, RecordId... recordIds) {
    ...
    /**
     * Obtain information about {@literal consumer groups} associated with the stream stored at the
    ...
    /**
     * Obtain information about every consumer in a specific {@literal consumer group} for the stream
     * stored at the specified {@literal key}.
     *
     * @param key the {@literal key} the stream is stored at.
     ...
     * @return a summary of pending messages within the given {@literal consumer group} or {@literal
    ...
    /**
     * Obtain detailed information about pending {@link PendingMessage messages} for a given {@link
     * org.springframework.data.domain.Range} within a {@literal consumer group}.
     *
     * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
     * @param groupName the name of the {@literal consumer group}. Must not be {@literal null}.
     ...
     * @return pending messages for the given {@literal consumer group} or {@literal null} when used
    ...
    /**
     * Obtain detailed information about pending {@link PendingMessage messages} applying given {@link
     * XPendingOptions options}.
     *
     * @param key the {@literal key} the stream is stored at. Must not be {@literal null}.
     * @param groupName the name of the {@literal consumer group}. Must not be {@literal null}.
     * @param options the options containing {@literal range}, {@literal consumer} and {@literal
     *     count}. Must not be {@literal null}.
     * @return pending messages matching given criteria or {@literal null} when used in pipeline /
     *     transaction.
     * @see <a href="https://valkey.io/commands/xpending">Valkey Documentation: xpending</a>
     * @since 2.3
     */
     ...
     */
    @Nullable
    default List<StringRecord> xRange(
            String key, org.springframework.data.domain.Range<String> range) {
        return xRange(key, range, io.valkey.springframework.data.valkey.connection.Limit.unlimited());
    }

    /**
     * Read records from a stream within a specific {@link Range} applying a {@link
     * io.valkey.springframework.data.valkey.connection.Limit}.
     *
     * @param key the stream key.
     * @param range must not be {@literal null}.
     ...
     * @return list ith members of the resulting stream. {@literal null} when used in pipeline /
     *     transaction.
     * @since 2.2
     ...
     */
    @Nullable
    default List<StringRecord> xReadAsString(
            StreamReadOptions readOptions, StreamOffset<String> stream) {
     ...
     */
    @Nullable
    default List<StringRecord> xReadGroupAsString(
            Consumer consumer, StreamOffset<String>... streams) {
    ...
    @Nullable
    default List<StringRecord> xReadGroupAsString(
            Consumer consumer, StreamReadOptions readOptions, StreamOffset<String> stream) {
     ...
     */
    @Nullable
    List<StringRecord> xReadGroupAsString(
     ...
     */
    @Nullable
    default List<StringRecord> xRevRange(
            String key, org.springframework.data.domain.Range<String> range) {
    ...
    /**
     * Read records from a stream within a specific {@link Range} applying a {@link
     * io.valkey.springframework.data.valkey.connection.Limit} in reverse order.
     *
     * @param key the stream key.
     * @param range must not be {@literal null}.
     * @param limit must not be {@literal null}.
