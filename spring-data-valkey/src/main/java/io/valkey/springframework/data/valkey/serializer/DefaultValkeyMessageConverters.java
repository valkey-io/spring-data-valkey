...
/**
 * Utility class that provides a {@link Builder} to create a composite {@link MessageConverter} for Valkey listener
 * endpoints.
 * <p>
 ...
 * @since 4.1
 */
class DefaultValkeyMessageConverters implements ValkeyMessageConverters {
	...
	static {
		ClassLoader classLoader = DefaultValkeyMessageConverters.class.getClassLoader();
	...
	public DefaultValkeyMessageConverters(MessageConverter messageConverter) {
	...
	static class DefaultBuilder implements ValkeyMessageConverters.Builder {
		...
		@Override
		public ValkeyMessageConverters.Builder defaultMimeType(MimeType defaultMimeType) {
			this.defaultMimeType = defaultMimeType;
			return this;
		}

		@Override
		public ValkeyMessageConverters.Builder registerDefaults(boolean registerDefaults) {
			this.registerDefaults = registerDefaults;
			return this;
		}

		@Override
		public ValkeyMessageConverters.Builder withStringConverter(MessageConverter stringMessageConverter) {
			this.stringMessageConverter = stringMessageConverter;
			return this;
		}

		@Override
		public ValkeyMessageConverters.Builder addCustomConverter(MessageConverter converter) {
			this.customConverters.add(converter);
			return this;
		}

		@Override
		public ValkeyMessageConverters build() {
			...
			return new DefaultValkeyMessageConverters(new CompositeMessageConverter(converters));
		}
		...
		private List<MessageConverter> getDefaultConverters() {
			...
			if (JACKSON_PRESENT) {
				defaultConverters
						.add(new SerializerMessageConverter(GenericJacksonJsonValkeySerializer.builder().build(), JSON_MIME_TYPES));
			} else if (JACKSON_2_PRESENT) {
				defaultConverters
						.add(new SerializerMessageConverter(
								GenericJackson2JsonValkeySerializer.builder().defaultTyping(false).build(), JSON_MIME_TYPES));
			}
			if (GSON_PRESENT) {
