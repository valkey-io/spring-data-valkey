...
class LettuceReactiveListCommands implements ReactiveListCommands {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.llen(command.getKey())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
    }

    @Override
    public Flux<CommandResponse<RangeCommand, Flux<ByteBuffer>>> lRange(
                                ...
                                .concatMap(
                                        command -> {
                                            ...
                                            if (command.getCount() != null) {
                                                values =
                                                        cmd.lpos(
                                                                command.getKey(), command.getElement(), command.getCount(), args);
                                            } else {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.lindex(command.getKey(), command.getIndex())
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.linsert(
                                                            command.getKey(),
                                                            Position.BEFORE.equals(command.getPosition()),
                                                            command.getPivot(),
                                                            command.getValue())
                                                    .map(value -> new NumericResponse<>(command, value));
                                        }));
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Assert.notNull(
                                                    command.getDestinationKey(), "Destination key must not be null");
                                            Assert.notNull(command.getTo(), "Destination direction must not be null");

                                            LMoveArgs lMoveArgs =
                                                    LettuceConverters.toLmoveArgs(command.getFrom(), command.getTo());
    ...
    public Flux<ByteBufferResponse<BLMoveCommand>> bLMove(Publisher<BLMoveCommand> commands) {
        ...
        return connection.executeDedicated(
                cmd ->
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            double timeout =
                                                    TimeoutUtils.toDoubleSeconds(
                                                            command.getTimeout().toMillis(), TimeUnit.MILLISECONDS);

                                            return cmd.blmove(
                                                            command.getKey(), command.getDestinationKey(), lMoveArgs, timeout)
                                                    .map(value -> new ByteBufferResponse<>(command, value));
                                        }));
    }
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Mono<ByteBuffer> popResult =
                                                    ObjectUtils.nullSafeEquals(Direction.RIGHT, command.getDirection())
    ...
    @Override
    public Flux<CommandResponse<PopCommand, Flux<ByteBuffer>>> popList(
            Publisher<PopCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            Mono<PopResult> mappedMono =
                                                    (ObjectUtils.nullSafeEquals(Direction.RIGHT, command.getDirection())
                                                                    ? cmd.brpop(
                                                                            timeout,
                                                                            command.getKeys().stream().toArray(ByteBuffer[]::new))
                                                                    : cmd.blpop(
                                                                            timeout,
                                                                            command.getKeys().stream().toArray(ByteBuffer[]::new)))
    ...
    @Override
    public Flux<ByteBufferResponse<BRPopLPushCommand>> bRPopLPush(
            Publisher<BRPopLPushCommand> commands) {
                        ...
                        Flux.from(commands)
                                .concatMap(
                                        command -> {
                                            ...
                                            return cmd.brpoplpush(
                                                            command.getTimeout().get(ChronoUnit.SECONDS),
                                                            command.getKey(),
