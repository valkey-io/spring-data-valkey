...
public class LettuceSentinelConnection implements ValkeySentinelConnection {
    ...
    /**
     * Creates a {@link LettuceSentinelConnection} with a dedicated client for a supplied {@link
     * ValkeyNode}.
     *
    ...
    /**
     * Creates a {@link LettuceSentinelConnection} with a client for the supplied {@code host} and
     * {@code port}.
     *
    ...
    /**
     * Creates a {@link LettuceSentinelConnection} with a client for the supplied {@code host} and
     * {@code port} and reuse existing {@link ClientResources}.
     *
     * @param host must not be {@literal null}.
     * @param port sentinel port.
    ...
    public LettuceSentinelConnection(String host, int port, ClientResources clientResources) {
        ...
        Assert.notNull(
                clientResources,
                "Cannot create LettuceSentinelConnection using 'null' as ClientResources.");
        Assert.notNull(host, "Cannot create LettuceSentinelConnection using 'null' as host.");
    ...
    protected LettuceSentinelConnection(StatefulRedisSentinelConnection<String, String> connection) {
        ...
        Assert.notNull(
                connection, "Cannot create LettuceSentinelConnection using 'null' as connection.");
        this.provider =
                new LettuceConnectionProvider() {
                    @Override
                    public <T extends StatefulConnection<?, ?>> T getConnection(Class<T> t) {
    ...
    public void failover(NamedNode master) {
        ...
        Assert.hasText(
                master.getName(), "Valkey master name must not be 'null' or empty for failover.");
     ...
     * @param masterName
     * @see
     *     io.valkey.springframework.data.valkey.connection.ValkeySentinelCommands#replicas(io.valkey.springframework.data.valkey.connection.NamedNode)
     * @return
     */
    public List<ValkeyServer> slaves(String masterName) {

        Assert.hasText(
                masterName, "Name of valkey master cannot be 'null' or empty when loading replicas.");
        try {
     ...
     * @param masterName
     * @see
     *     io.valkey.springframework.data.valkey.connection.ValkeySentinelCommands#remove(io.valkey.springframework.data.valkey.connection.NamedNode)
     */
    public void remove(String masterName) {

        Assert.hasText(
                masterName, "Name of valkey master cannot be 'null' or empty when trying to remove.");
    ...
    private static class DedicatedClientConnectionProvider implements LettuceConnectionProvider {
        ...
        public <T extends StatefulConnection<?, ?>> CompletableFuture<T> getConnectionAsync(
            ...
            return valkeyClient
                    .connectSentinelAsync(StringCodec.UTF8, uri)
        ...
        public CompletableFuture<Void> releaseAsync(StatefulConnection<?, ?> connection) {
            return connection
                    .closeAsync()
                    .exceptionally(LettuceFutureUtils.ignoreErrors())
