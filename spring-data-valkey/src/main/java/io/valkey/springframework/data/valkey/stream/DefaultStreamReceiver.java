...
import io.valkey.springframework.data.valkey.core.ReactiveStreamOperations;
...
class DefaultStreamReceiver<K, V extends Record<K, ?>> implements StreamReceiver<K, V> {
     ...
     * {@link io.valkey.springframework.data.valkey.stream.StreamReceiver.StreamReceiverOptions}.
     *
    ...
    DefaultStreamReceiver(
        ...
        ValkeySerializationContext<K, Object> serializationContext =
                ValkeySerializationContext.<K, Object>newSerializationContext(
                                options.getKeySerializer()) //
                        .key(options.getKeySerializer())
                        .hashKey(options.getHashKeySerializer())
                        .hashValue(options.getHashValueSerializer())
                        .build();
    ...
    public Flux<V> receive(StreamOffset<K> streamOffset) {
        ...
        ValkeySerializationContext.SerializationPair<K> keySerializer =
                template.getSerializationContext().getKeySerializationPair();
                    ...
                    return Flux.create(
                            sink ->
                                    new StreamSubscription(
                                                    ...
                                                    pollState,
                                                    readFunction,
                                                    receiverOptions.getResumeFunction())
                                            .arm());
                });
    }
    ...
    public Flux<V> receiveAutoAck(Consumer consumer, StreamOffset<K> streamOffset) {
        ...
        Function<ReadOffset, Flux<ByteBufferRecord>> readFunction =
                getConsumeReadFunction(streamOffset.getKey(), consumer, this.readOptions.autoAcknowledge());
    ...
    public Flux<V> receive(Consumer consumer, StreamOffset<K> streamOffset) {
        ...
        Function<ReadOffset, Flux<ByteBufferRecord>> readFunction =
                getConsumeReadFunction(streamOffset.getKey(), consumer, this.readOptions);
    ...
    @SuppressWarnings("unchecked")
    private Function<ReadOffset, Flux<ByteBufferRecord>> getConsumeReadFunction(
            K key, Consumer consumer, StreamReadOptions readOptions) {
    ...
    private Function<ByteBufferRecord, V> getDeserializer() {
        ...
        Function<ByteBufferRecord, MapRecord<K, Object, Object>> deserializer =
    ...
    class StreamSubscription {
        ...
        protected StreamSubscription(
            ...
            this.targetType =
                    TypeDescriptor.valueOf(
                            receiverOptions.hasHashMapper() ? receiverOptions.getTargetType() : MapRecord.class);
        }
            ...
            sink.onRequest(
                    toAdd -> {
                        ...
                        if (pollState.isSubscriptionActive()) {
                            ...
                            if (logger.isDebugEnabled()) {
                                logger.debug(
                                        "[stream: %s] onRequest(%d): Dropping, subscription canceled"
                                                .formatted(key, toAdd));
                            }
                        }
                    });
        ...
        private void scheduleIfRequired() {
            ...
            if (pollState.getRequested() > 0 && !overflow.isEmpty()) {
                if (logger.isDebugEnabled()) {
                    logger.info(
                            "[stream: %s] scheduleIfRequired(): Requested: %d Emit from buffer"
            ...
            if (pollState.getRequested() == 0) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "[stream: %s] scheduleIfRequired(): Subscriber has no demand; Suspending subscription"
                                    .formatted(key));
                }
                return;
            }
            ...
            if (pollState.activateSchedule()) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug(
                            "[stream: %s] scheduleIfRequired(): Activating subscription, offset %s"
                                    .formatted(key, readOffset));
                }

                Flux<ByteBufferRecord> poll =
                        readFunction
                                .apply(readOffset)
                                .onErrorResume(
                                        throwable ->
                                                Flux.from(resumeFunction.apply(throwable))
                                                        .then()
                                                        .cast(ByteBufferRecord.class));
                                ...
                                it -> {
                                    ...
                                    try {
                                        ...
                                        throw new ConversionFailedException(
                                                TypeDescriptor.forObject(it), targetType, it, ex);
                                    }
                                })
                        .onErrorResume(
                                throwable ->
        ...
        private void onStreamMessage(V message) {
            ...
            if (requested > 0) {
                ...
                if (requested == Long.MAX_VALUE) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "[stream: %s] onStreamMessage(%s): Emitting item, fast-path"
                    ...
                    if (pollState.decrementRequested()) {
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "[stream: %s] onStreamMessage(%s): Emitting item, slow-path"
                                            .formatted(key, message));
                        }
                        sink.next(message);
                    } else {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "[stream: %s] onStreamMessage(%s): Buffering overflow".formatted(key, message));
                        }
        ...
        private void emitBuffer() {
            ...
            while (!overflow.isEmpty()) {
                ...
                if (demand == Long.MAX_VALUE) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "[stream: %s] emitBuffer(%s): Emitting item from buffer, fast-path"
                                        .formatted(key, message));
                    }
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "[stream: %s] emitBuffer(%s): Emitting item from buffer, slow-path"
                                        .formatted(key, message));
                    }
    ...
    static class PollState {
         ...
         *
         * @return {@literal true} if there are still requests pending and the decrement has been
         *     successful.
         */
         ...
         *
         * @return {@literal true} if the schedule was activated by this call or {@literal false} if a
         *     different process activated the schedule.
         */
