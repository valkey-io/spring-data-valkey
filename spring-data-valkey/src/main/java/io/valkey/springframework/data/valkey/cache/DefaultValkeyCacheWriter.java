...
/**
 * {@link ValkeyCacheWriter} implementation capable of reading/writing binary data from/to Valkey in
 * {@literal standalone} and {@literal cluster} environments, and uses a given {@link
 * ValkeyConnectionFactory} to obtain the actual {@link ValkeyConnection}.
 *
 * <p>{@link DefaultValkeyCacheWriter} can be used in {@link
 * ValkeyCacheWriter#lockingValkeyCacheWriter(ValkeyConnectionFactory) locking} or {@link
 * ValkeyCacheWriter#nonLockingValkeyCacheWriter(ValkeyConnectionFactory) non-locking} mode. While
 * {@literal non-locking} aims for maximum performance it may result in overlapping, non-atomic,
 * command execution for operations spanning multiple Valkey interactions like {@code putIfAbsent}.
 * The {@literal locking} counterpart prevents command overlap by setting an explicit lock key and
 * checking against presence of this key which leads to additional requests and potential command
 * wait times.
 *
...
class DefaultValkeyCacheWriter implements ValkeyCacheWriter {
    ...
    private static final boolean REACTIVE_VALKEY_CONNECTION_FACTORY_PRESENT =
            ClassUtils.isPresent(
                    "io.valkey.springframework.data.valkey.connection.ReactiveValkeyConnectionFactory", null);
     ...
     * @param batchStrategy must not be {@literal null}.
     */
    DefaultValkeyCacheWriter(
            ValkeyConnectionFactory connectionFactory, Duration sleepTime, BatchStrategy batchStrategy) {
        this(
                connectionFactory,
                sleepTime,
    ...
    DefaultValkeyCacheWriter(
        ...
        if (REACTIVE_VALKEY_CONNECTION_FACTORY_PRESENT
                && this.connectionFactory instanceof ReactiveValkeyConnectionFactory) {
    ...
    @Nullable
    private byte[] doGet(
            ValkeyConnection connection, String name, byte[] key, @Nullable Duration ttl) {

        byte[] result =
                shouldExpireWithin(ttl)
    ...
    private void doPut(
            ValkeyConnection connection, String name, byte[] key, byte[] value, @Nullable Duration ttl) {
    ...
    public CompletableFuture<Void> store(
            String name, byte[] key, byte[] value, @Nullable Duration ttl) {
                        ...
                        if (shouldExpireWithin(ttl)) {
                            put =
                                    ObjectUtils.nullSafeEquals(
                                            connection
                                                    .stringCommands()
                                                    .set(key, value, Expiration.from(ttl), SetOption.ifAbsent()),
                                            true);
                        } else {
    ...
    @Override
    public ValkeyCacheWriter withStatisticsCollector(
            CacheStatisticsCollector cacheStatisticsCollector) {
        return new DefaultValkeyCacheWriter(
                connectionFactory, sleepTime, lockTtl, cacheStatisticsCollector, this.batchStrategy);
    }
    ...
    void doLock(
            ...
            @Nullable Object contextualValue,
            ValkeyConnection connection) {

        ValkeyStringCommands commands = connection.stringCommands();
        Expiration expiration =
                Expiration.from(this.lockTtl.getTimeToLive(contextualKey, contextualValue));
        byte[] cacheLockKey = createCacheLockKey(name);

        while (!ObjectUtils.nullSafeEquals(
                commands.set(cacheLockKey, new byte[0], expiration, SetOption.SET_IF_ABSENT), true)) {
    ...
    private void checkAndPotentiallyWaitUntilUnlocked(String name, ValkeyConnection connection) {
        ...
        try {
            ...
            throw new PessimisticLockingFailureException(
                    "Interrupted while waiting to unlock cache %s".formatted(name), ex);
        } finally {
    ...
    boolean doCheckLock(String name, ValkeyConnection connection) {
        return ObjectUtils.nullSafeEquals(
                connection.keyCommands().exists(createCacheLockKey(name)), true);
    }
    ...
    interface AsyncCacheWriter {
         ...
         * @return a future that completes either with a value if the value exists or completing with
         *     {@literal null} if the cache does not contain an entry.
         */
    ...
    /**
     * Delegate implementing {@link AsyncCacheWriter} to provide asynchronous cache retrieval and
     * storage operations using {@link ReactiveValkeyConnectionFactory}.
     *
     * @since 3.2
     */
    class AsynchronousCacheWriterDelegate implements AsyncCacheWriter {
        ...
        public CompletableFuture<byte[]> retrieve(String name, byte[] key, @Nullable Duration ttl) {
            ...
            return doWithConnection(
                    connection -> {
                        ...
                        Mono<?> cacheLockCheck =
                                isLockingCacheWriter() ? waitForLock(connection, name) : Mono.empty();
        ...
        private Mono<Boolean> doStoreWithLocking(
                String name,
                byte[] key,
                byte[] value,
                @Nullable Duration ttl,
            ...
            return Mono.usingWhen(
                    doLock(name, key, value, connection),
                    unused -> doStore(key, value, ttl, connection),
                    unused -> doUnlock(name, connection));
        }

        private Mono<Boolean> doStore(
                byte[] cacheKey,
                byte[] value,
                @Nullable Duration ttl,
                ReactiveValkeyConnection connection) {
        ...
        private Mono<Object> doLock(
            ...
            return connection
                    .stringCommands()
                    .set(key, value, expiration, SetOption.SET_IF_ABSENT) //
                    // Ensure we emit an object, otherwise, the Mono.usingWhen operator doesn't run the inner
                    // resource function.
        ...
        private Mono<Void> waitForLock(ReactiveValkeyConnection connection, String cacheName) {
            ...
            Mono<Boolean> exists =
                    connection.keyCommands().exists(ByteBuffer.wrap(cacheLockKey)).filter(it -> !it);

            return wait.doOnSubscribe(subscription -> lockWaitTimeNs.set(System.nanoTime())) //
                    .flatMap(it -> exists) //
                    .doFinally(
                            signalType ->
                                    statistics.incLockTime(cacheName, System.nanoTime() - lockWaitTimeNs.get())) //
                    .next() //
                    .then();
        }
        ...
        private <T> CompletableFuture<T> doWithConnection(
            ...
            return Mono.usingWhen(
                            Mono.fromSupplier(cf::getReactiveConnection), //
