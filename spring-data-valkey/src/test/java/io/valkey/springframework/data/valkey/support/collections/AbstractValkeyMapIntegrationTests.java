...
public abstract class AbstractValkeyMapIntegrationTests<K, V> {
    ...
    @SuppressWarnings("rawtypes")
    AbstractValkeyMapIntegrationTests(
            ObjectFactory<K> keyFactory, ObjectFactory<V> valueFactory, ValkeyTemplate template) {
    ...
    void testContainsValue() {
        ...
        assertThatExceptionOfType(UnsupportedOperationException.class)
                .isThrownBy(() -> map.containsValue(v1));
    }
    ...
    void testExpire() {
        ...
        BoundHashFieldExpirationOperations<K> ops =
                map.hashFieldExpiration(Collections.singletonList(k1));
        assertThat(ops.expire(Duration.ofSeconds(5))).satisfies(ExpireChanges::allOk);
        assertThat(ops.getTimeToLive())
                .satisfies(
                        expiration -> {
                            assertThat(expiration.expirationOf(k1).raw()).isBetween(1L, 5L);
                        });
        assertThat(ops.getTimeToLive(TimeUnit.MILLISECONDS))
                .satisfies(
                        expiration -> {
    ...
    void testIncrementDouble() {
        ...
        assertThat(twoDForm.format(map.increment(k1, 3.4)))
                .isEqualTo(twoDForm.format(Double.valueOf((String) v1) + 3.4));
    }
    ...
    void testRemoveNullValue() {
        assertThatExceptionOfType(NullPointerException.class)
                .isThrownBy(() -> map.remove(getKey(), null));
    }
    ...
    void testReplaceNullOldValue() {
        assertThatExceptionOfType(NullPointerException.class)
                .isThrownBy(() -> map.replace(getKey(), null, getValue()));
    }

    @ParameterizedValkeyTest
    void testReplaceNullNewValue() {
        assertThatExceptionOfType(NullPointerException.class)
                .isThrownBy(() -> map.replace(getKey(), getValue(), null));
    }
    ...
    void testReplaceNullValue() {
        assertThatExceptionOfType(NullPointerException.class)
                .isThrownBy(() -> map.replace(getKey(), null));
    }
