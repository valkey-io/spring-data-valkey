...
public class DefaultStreamOperationsIntegrationTests<K, HK, HV> {
    ...
    public DefaultStreamOperationsIntegrationTests(
    ...
    public static Collection<Object[]> testParams() {
        ...
        params.addAll(
                AbstractOperationsTestParams.testParams(
    ...
    void addShouldAddReadSimpleMessage() {
        ...
        List<ObjectRecord<K, HV>> messages =
                streamOps.range((Class<HV>) value.getClass(), key, Range.unbounded());
    ...
    void addMaxLenShouldLimitSimpleMessagesSize() {
        ...
        RecordId messageId =
                streamOps.add(StreamRecords.objectBacked(newValue).withStreamKey(key), options);
    ...
    void rangeShouldReportMessages() {
        ...
        List<MapRecord<K, HK, HV>> messages =
                streamOps.range(
                        key,
    ...
    void exclusiveRangeShouldReportMessages() {
        ...
        List<MapRecord<K, HK, HV>> messages =
                streamOps.range(
                        key,
                        Range.from(Bound.exclusive(messageId1.getValue()))
                                .to(Bound.inclusive(messageId2.getValue())));
        ...
        messages =
                streamOps.range(
                        key,
                        Range.from(Bound.inclusive(messageId1.getValue()))
                                .to(Bound.exclusive(messageId2.getValue())));
    ...
    void exclusiveReverseRangeShouldReportMessages() {
        ...
        List<MapRecord<K, HK, HV>> messages =
                streamOps.reverseRange(
                        key,
                        Range.from(Bound.exclusive(messageId1.getValue()))
                                .to(Bound.inclusive(messageId3.getValue())));

        assertThat(messages)
                .hasSize(2)
                .extracting(MapRecord::getId)
                .containsSequence(messageId3, messageId2);

        messages =
                streamOps.reverseRange(
                        key,
                        Range.from(Bound.inclusive(messageId1.getValue()))
                                .to(Bound.exclusive(messageId3.getValue())));

        assertThat(messages)
                .hasSize(2)
                .extracting(MapRecord::getId)
                .containsSequence(messageId2, messageId1);
    }
    ...
    void reverseRangeShouldConvertSimpleMessages() {
        ...
        List<ObjectRecord<K, HV>> messages =
                streamOps.reverseRange((Class<HV>) value.getClass(), key, Range.unbounded());
    ...
    void readShouldReadMessage() {
        ...
        List<MapRecord<K, HK, HV>> messages =
                streamOps.read(StreamOffset.create(key, ReadOffset.from("0-0")));
    ...
    void readShouldReadSimpleMessage() {
        ...
        List<ObjectRecord<K, HV>> messages =
                streamOps.read(
                        (Class<HV>) value.getClass(), StreamOffset.create(key, ReadOffset.from("0-0")));
    ...
    void readShouldReadMessages() {
        ...
        List<MapRecord<K, HK, HV>> messages =
                streamOps.read(
                        StreamReadOptions.empty().count(2), StreamOffset.create(key, ReadOffset.from("0-0")));
    ...
    void claimShouldReadMessageDetails() {
        ...
        streamOps.createGroup(key, ReadOffset.from("0-0"), "my-group");
        streamOps.read(
                Consumer.from("my-group", "name"), StreamOffset.create(key, ReadOffset.lastConsumed()));

        List<MapRecord<K, HK, HV>> messages =
                streamOps.claim(key, "my-group", "new-owner", Duration.ZERO, messageId);
