...
class IndexWriterUnitTests {
    ...
    void setUp() {
        ...
        converter =
                new MappingValkeyConverter(
                        new ValkeyMappingContext(), new PathIndexResolver(), referenceResolverMock);
    ...
    void addKeyToIndexShouldInvokeSaddCorrectly() {
        ...
        verify(connectionMock)
                .sAdd(
                        eq("persons:key-1:idx".getBytes(CHARSET)),
    ...
    void removeKeyFromExistingIndexesShouldThrowExecptionForNullIndexedData() {
        assertThatIllegalArgumentException()
                .isThrownBy(() -> writer.removeKeyFromExistingIndexes(KEY_BIN, null));
    }
    ...
    void addToIndexShouldThrowDataAccessExceptionWhenAddingDataThatConnotBeConverted() {
        assertThatExceptionOfType(InvalidDataAccessApiUsageException.class)
                .isThrownBy(
                        () ->
                                writer.addKeyToIndex(
                                        KEY_BIN,
                                        new SimpleIndexedPropertyValue(KEYSPACE, "firstname", new DummyObject())));
    }
    ...
    void addToIndexShouldUseRegisteredConverterWhenAddingData() {
        ...
        ((GenericConversionService) converter.getConversionService())
                .addConverter(
                        new Converter<DummyObject, byte[]>() {
        ...
        verify(connectionMock)
                .sAdd(eq(("persons:firstname:" + identityHexString).getBytes(CHARSET)), eq(KEY_BIN));
    }
    ...
    void createIndexShouldNotTryToRemoveExistingValues() {
        ...
        writer.createIndexes(
                ...
                Collections.<IndexedData>singleton(
                        new SimpleIndexedPropertyValue(KEYSPACE, "firstname", "Rand")));
    ...
    void removeGeoIndexShouldCallGeoRemove() {
        ...
        when(connectionMock.keys(any(byte[].class)))
                .thenReturn(new LinkedHashSet<>(Arrays.asList(indexKey1)));

        writer.removeKeyFromExistingIndexes(
                KEY_BIN, new GeoIndexedPropertyValue(KEYSPACE, "address.city", null));
