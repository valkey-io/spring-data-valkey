 ...
 *
 * <p>These tests validate the implementation of all ValkeyZSetCommands methods: - Basic sorted set
 * operations (zAdd, zRem, zCard, zRange, zScore) - Ranking operations (zRank, zRevRank) - Random
 * operations (zRandMember, zPopMin, zPopMax) - Blocking operations (bZPopMin, bZPopMax) - Range
 * operations (zRangeByScore, zRangeByLex, zRevRange) - Counting operations (zCount, zLexCount) -
 * Set algebra operations (zDiff, zInter, zUnion) - Store operations (zDiffStore, zInterStore,
 * zUnionStore) - Range store operations (zRangeStore variants) - Removal operations (zRemRange,
 * zRemRangeByScore, zRemRangeByLex) - Scanning operations (zScan) - Error handling and edge cases
 *
 * @author Ilia Kolominsky
 * @since 2.0
 */
public class ValkeyGlideConnectionZSetCommandsIntegrationTests
        extends AbstractValkeyGlideIntegrationTests {

    @Override
    protected String[] getTestKeyPatterns() {
        return new String[] {
            "test:zset:add:key", "test:zset:add:single:key", "test:zset:rem:key", "test:zset:mscore:key",
            "test:zset:incrby:key", "test:zset:range:key", "test:zset:rangebyscore:key",
                    "test:zset:rangebylex:key",
            "test:zset:rank:key", "test:zset:count:key", "test:zset:lexcount:key",
                    "test:zset:randmember:key",
            "test:zset:pop:key", "test:zset:diff:key1", "test:zset:diff:key2", "test:zset:diff:dest",
            "test:zset:inter:key1", "test:zset:inter:key2", "test:zset:inter:dest",
                    "test:zset:union:key1",
            "test:zset:union:key2", "test:zset:union:dest", "test:zset:rangestore:src",
                    "test:zset:rangestore:dest",
            "test:zset:rangestore:srclex", "test:zset:remrange:key", "test:zset:scan:key",
                    "test:zset:lex:bounds:key",
    ...
    void testZRemAndZScore() {
        ...
        try {
            ...
            Double scoreNonExistent =
                    connection.zSetCommands().zScore(key.getBytes(), "nonexistent".getBytes());
            ...
            Long remNonExistent =
                    connection.zSetCommands().zRem(key.getBytes(), "nonexistent".getBytes());
    ...
    void testZIncrBy() {
        ...
        try {
            ...
            Double newScore1 =
                    connection.zSetCommands().zIncrBy(key.getBytes(), 2.5, "member1".getBytes());
            ...
            Double newScore2 =
                    connection.zSetCommands().zIncrBy(key.getBytes(), 5.0, "member2".getBytes());
            assertThat(newScore2).isEqualTo(5.0);

            // Test negative increment
            Double newScore3 =
                    connection.zSetCommands().zIncrBy(key.getBytes(), -1.5, "member1".getBytes());
    ...
    void testZRangeAndZRevRange() {
        ...
        try {
            ...
            List<String> rangeList1 =
                    range1.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            ...
            List<String> rangeList2 =
                    range2.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            ...
            List<String> revRangeList =
                    revRange.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            assertThat(revRangeList).containsExactly("member5", "member4", "member3");

            // Test zRevRangeWithScores
            Set<Tuple> revRangeWithScores =
                    connection.zSetCommands().zRevRangeWithScores(key.getBytes(), 0, 2);
    ...
    void testZRangeByScore() {
        ...
        try {
            ...
            Set<byte[]> rangeResult2 =
                    connection.zSetCommands().zRangeByScore(key.getBytes(), "2.0", "4.0", 0, 10);
            assertThat(rangeResult2).hasSize(3);

            // Test zRangeByScore with limit
            Set<byte[]> rangeResult3 =
                    connection
                            .zSetCommands()
                            .zRangeByScore(
                                    key.getBytes(), range1.map(Number.class::cast), Limit.limit().count(2));
            assertThat(rangeResult3).hasSize(2);

            // Test zRangeByScoreWithScores
            Set<Tuple> rangeWithScores =
                    connection
                            .zSetCommands()
                            .zRangeByScoreWithScores(
                                    key.getBytes(), range1.map(Number.class::cast), Limit.unlimited());
            assertThat(rangeWithScores).hasSize(3);

            // Test zRevRangeByScore
            Set<byte[]> revRangeResult =
                    connection
                            .zSetCommands()
    ...
    void testZRangeByLex() {
        ...
        try {
            ...
            Set<byte[]> lexResult =
                    connection.zSetCommands().zRangeByLex(key.getBytes(), lexRange, Limit.unlimited());
            assertThat(lexResult).hasSize(3);

            List<String> lexResultList =
                    lexResult.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            assertThat(lexResultList).containsExactly("b", "c", "d");

            // Test zRevRangeByLex - should return results in reverse lexicographic order
            Set<byte[]> revLexResult =
                    connection.zSetCommands().zRevRangeByLex(key.getBytes(), lexRange, Limit.unlimited());
            assertThat(revLexResult).hasSize(3);

            List<String> revLexResultList =
                    revLexResult.stream().map(String::new).collect(java.util.stream.Collectors.toList());
    ...
    void testZRankAndZRevRank() {
        ...
        try {
            ...
            Long rankNonExistent =
                    connection.zSetCommands().zRank(key.getBytes(), "nonexistent".getBytes());
            ...
            Long revRankNonExistent =
                    connection.zSetCommands().zRevRank(key.getBytes(), "nonexistent".getBytes());
    ...
    void testZCount() {
        ...
        try {
            ...
            Range<Double> range1 = Range.closed(2.0, 4.0);
            Long count1 =
                    connection.zSetCommands().zCount(key.getBytes(), range1.map(Number.class::cast));
            assertThat(count1).isEqualTo(3L);

            // Test zCount with exclusive range
            Range<Double> range2 = Range.open(2.0, 4.0);
            Long count2 =
                    connection.zSetCommands().zCount(key.getBytes(), range2.map(Number.class::cast));
            ...
            Long count3 =
                    connection.zSetCommands().zCount(key.getBytes(), range3.map(Number.class::cast));
    ...
    void testZDiff() {
        ...
        try {
            ...
            Set<String> diffStrings =
                    diff.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(diffStrings).containsExactlyInAnyOrder("a", "c");

            // Test zDiffWithScores
            Set<Tuple> diffWithScores =
                    connection.zSetCommands().zDiffWithScores(key1.getBytes(), key2.getBytes());
    ...
    void testZInter() {
        ...
        try {
            ...
            Set<String> interStrings =
                    inter.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(interStrings).containsExactlyInAnyOrder("b", "c");

            // Test zInterWithScores
            Set<Tuple> interWithScores =
                    connection.zSetCommands().zInterWithScores(key1.getBytes(), key2.getBytes());
            assertThat(interWithScores).hasSize(2);

            // Test zInterWithScores with weights and aggregate
            Weights weights = Weights.of(1.0, 2.0);
            Set<Tuple> interWeighted =
                    connection
                            .zSetCommands()
            ...
            Long storeWeightedResult =
                    connection
                            .zSetCommands()
                            .zInterStore(
                                    destKey.getBytes(), Aggregate.MAX, weights, key1.getBytes(), key2.getBytes());
    ...
    void testZUnion() {
        ...
        try {
            ...
            Set<String> unionStrings =
                    union.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(unionStrings).containsExactlyInAnyOrder("a", "b", "c");

            // Test zUnionWithScores
            Set<Tuple> unionWithScores =
                    connection.zSetCommands().zUnionWithScores(key1.getBytes(), key2.getBytes());
            assertThat(unionWithScores).hasSize(3);

            // Test zUnionWithScores with weights and aggregate
            Weights weights = Weights.of(1.0, 2.0);
            Set<Tuple> unionWeighted =
                    connection
                            .zSetCommands()
            ...
            Long storeWeightedResult =
                    connection
                            .zSetCommands()
                            .zUnionStore(
                                    destKey.getBytes(), Aggregate.SUM, weights, key1.getBytes(), key2.getBytes());
    ...
    void testZRangeStore() {
        ...
        try {
            ...
            connection
                    .zSetCommands()
                    .zAdd(srcKey.getBytes(), tuples, ValkeyZSetCommands.ZAddArgs.empty());

            // Test zRangeStoreByScore
            Range<Double> scoreRange = Range.closed(2.0, 4.0);
            Long storeByScoreResult =
                    connection
                            .zSetCommands()
                            .zRangeStoreByScore(
                                    destKey.getBytes(),
            ...
            connection
                    .zSetCommands()
                    .zAdd(srcLexKey.getBytes(), lexTuples, ValkeyZSetCommands.ZAddArgs.empty());

            Range<byte[]> lexRange = Range.closed("b".getBytes(), "d".getBytes());
            Long storeByLexResult =
                    connection
                            .zSetCommands()
                            .zRangeStoreByLex(
                                    destKey.getBytes(), srcLexKey.getBytes(), lexRange, Limit.unlimited());
            assertThat(storeByLexResult).isEqualTo(3L);

            // Test zRangeStoreRevByLex
            Long storeRevByLexResult =
                    connection
                            .zSetCommands()
                            .zRangeStoreRevByLex(
                                    destKey.getBytes(), srcLexKey.getBytes(), lexRange, Limit.unlimited());
    ...
    void testLexicographicCountWithComplexBounds() {
        ...
        try {
            ...
            Range<byte[]> unboundedRange =
                    Range.from(Range.Bound.inclusive("banana".getBytes())).to(Range.Bound.unbounded());
    ...
    void testLexicographicRemovalWithEdgeCases() {
        ...
        try {
            ...
            Long exclusiveRemovalCount =
                    connection.zSetCommands().zRemRangeByLex(key.getBytes(), exclusiveRemovalRange);
            assertThat(exclusiveRemovalCount)
                    .isEqualTo(0L); // No elements between "alpha" and "omega" exclusively

            // Test removal with unbounded lower bound
            Range<byte[]> unboundedRemovalRange =
                    Range.<byte[]>from(Range.Bound.unbounded()).to(Range.Bound.inclusive("alpha".getBytes()));
            Long unboundedRemovalCount =
                    connection.zSetCommands().zRemRangeByLex(key.getBytes(), unboundedRemovalRange);
    ...
    void testLexicographicReverseOperationsWithBounds() {
        ...
        try {
            ...
            Set<byte[]> reverseResult =
                    connection.zSetCommands().zRevRangeByLex(key.getBytes(), reverseRange, Limit.unlimited());
            assertThat(reverseResult).hasSize(3);
            List<String> reverseList =
                    reverseResult.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            assertThat(reverseList).containsExactly("third", "second", "middle");

            // Test with limit
            Set<byte[]> limitedResult =
                    connection
                            .zSetCommands()
                            .zRevRangeByLex(key.getBytes(), reverseRange, Limit.limit().count(2));
            assertThat(limitedResult).hasSize(2);
            List<String> limitedList =
                    limitedResult.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            assertThat(limitedList).containsExactly("third", "second");

            // Test with offset and limit
            Set<byte[]> offsetResult =
                    connection
                            .zSetCommands()
            ...
            List<String> offsetList =
                    offsetResult.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            ...
            Long storeResult =
                    connection
                            .zSetCommands()
                            .zRangeStoreRevByLex(
                                    destKey.getBytes(), key.getBytes(), reverseRange, Limit.unlimited());
            assertThat(storeResult).isEqualTo(3L);

            Set<byte[]> storedData = connection.zSetCommands().zRange(destKey.getBytes(), 0, -1);
            List<String> storedList =
                    storedData.stream().map(String::new).collect(java.util.stream.Collectors.toList());
    ...
    void testBasicZSetOperationsInPipelineMode() {
        ...
        try {
            ...
            assertThat(
                            connection
                                    .zSetCommands()
                                    .zMScore(key.getBytes(), "member1".getBytes(), "member2".getBytes()))
                    .isNull();
            ...
            assertThat(connection.zSetCommands().zIncrBy(key.getBytes(), 1.5, "member2".getBytes()))
                    .isNull();
        ...
        try {
            ...
            assertThat(
                            connection
                                    .zSetCommands()
                                    .zRangeByScore(
                                            key.getBytes(), scoreRange.map(Number.class::cast), Limit.unlimited()))
                    .isNull();
            assertThat(
                            connection
                                    .zSetCommands()
            ...
            assertThat(
                    ...
                    .isNull();
            assertThat(
                            connection.zSetCommands().zCount(key.getBytes(), scoreRange.map(Number.class::cast)))
                    .isNull();
    ...
    void testSetAlgebraOperationsInPipelineMode() {
        ...
        try {
            ...
            assertThat(connection.zSetCommands().zDiffWithScores(key1.getBytes(), key2.getBytes()))
                    .isNull();
            assertThat(
                            connection
                                    .zSetCommands()
            ...
            assertThat(connection.zSetCommands().zInterWithScores(key1.getBytes(), key2.getBytes()))
                    .isNull();
            assertThat(
                            connection
                                    .zSetCommands()
            ...
            assertThat(connection.zSetCommands().zUnionWithScores(key1.getBytes(), key2.getBytes()))
                    .isNull();
            assertThat(
                            connection
                                    .zSetCommands()
    ...
    void testRangeStoreAndRemovalOperationsInPipelineMode() {
        ...
        try {
            ...
            connection
                    .zSetCommands()
                    .zAdd(remKey.getBytes(), tuples, ValkeyZSetCommands.ZAddArgs.empty());
            ...
            assertThat(
                            connection
                                    .zSetCommands()
                                    .zRemRangeByScore(remKey.getBytes(), scoreRange.map(Number.class::cast)))
                    .isNull();
            ...
            assertThat((Long) results.get(3))
                    .isEqualTo(0L); // zRemRangeByScore result (already removed by previous operation)
    ...
    void testWatchWithZSetCommandsTransaction() {
        ...
        try {
            ...
            connection
                    .zSetCommands()
                    .zAdd(watchKey.getBytes(), tuples, ValkeyZSetCommands.ZAddArgs.empty());
            ...
            connection
                    .zSetCommands()
                    .zAdd(otherKey.getBytes(), tuples, ValkeyZSetCommands.ZAddArgs.empty());
    ...
    void testZSetWithBinaryData() {
        ...
        try {
            ...
            Set<Tuple> rangeWithScores =
                    connection.zSetCommands().zRangeWithScores(key.getBytes(), 0, -1);
    ...
    void testZSetWithExtremeScoreValues() {
        ...
        try {
            ...
            tuples.add(
                    new DefaultTuple(
                            "max_precision".getBytes(), 1.7976931348623157E308)); // Near Double.MAX_VALUE
            ...
            Set<byte[]> range = connection.zSetCommands().zRange(key.getBytes(), 0, -1);
            List<String> orderedMembers =
                    range.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            ...
            Long count =
                    connection.zSetCommands().zCount(key.getBytes(), extremeRange.map(Number.class::cast));
