 ...
 *
 * <p>These tests validate the implementation of all ValkeyHyperLogLogCommands methods: - pfAdd: Add
 * elements to HyperLogLog - pfCount: Get cardinality estimate of HyperLogLog(s) - pfMerge: Merge
 * multiple HyperLogLogs into one
 *
 ...
 * @since 2.0
 */
public class ValkeyGlideConnectionHyperLogLogCommandsIntegrationTests
    ...
    protected String[] getTestKeyPatterns() {
        return new String[] {
            "test:hll:basic",
            "test:hll:count:single",
            "test:hll:count:multiple1",
            "test:hll:count:multiple2",
            "test:hll:merge:source1",
            "test:hll:merge:source2",
            "test:hll:merge:dest",
            "test:hll:empty",
            "test:hll:binary",
            "test:hll:duplicate",
            "test:hll:large",
            "test:hll:pipeline:basic",
            "test:hll:pipeline:count1",
            "test:hll:pipeline:count2",
            "test:hll:pipeline:merge:src1",
            "test:hll:pipeline:merge:src2",
            "test:hll:pipeline:merge:dst",
            "test:hll:transaction:basic",
            "test:hll:transaction:count1",
            "test:hll:transaction:count2",
            "test:hll:transaction:merge:src1",
            "test:hll:transaction:merge:src2",
            "test:hll:transaction:merge:dst",
            "test:hll:error:string",
            "non:existent:key"
        };
    }
    ...
    void testPfAddBasic() {
        ...
        try {
            ...
            Long addResult4 =
                    connection.hyperLogLogCommands().pfAdd(keyBytes, value1, "value4".getBytes());
    ...
    void testPfCountSingle() {
        ...
        try {
            ...
            assertThat(largeCount)
                    ...
                    .isLessThanOrEqualTo(110L); // Should be around 100
        } finally {
            cleanupKey(key);
        }
    }
    ...
    void testPfCountMultiple() {
        ...
        try {
            ...
            assertThat(unionCount)
                    .isGreaterThanOrEqualTo(4L)
                    .isLessThanOrEqualTo(6L); // Should be around 5
        } finally {
            cleanupKey(key1);
            cleanupKey(key2);
        }
    ...
    void testPfMerge() {
        ...
        try {
            ...
            assertThat(mergedCount)
                    .isGreaterThanOrEqualTo(4L)
                    .isLessThanOrEqualTo(6L); // Should be around 5 (elem3 overlaps)
            ...
            assertThat(finalCount)
                    ...
                    .isLessThanOrEqualTo(7L); // Should be around 6
        } finally {
            cleanupKey(source1);
            cleanupKey(source2);
            cleanupKey(dest);
    ...
    void testHyperLogLogWithBinaryData() {
        ...
        try {
            // Add binary values
            Long addResult1 =
                    connection.hyperLogLogCommands().pfAdd(keyBytes, binaryValue1, binaryValue2);
    ...
    void testHyperLogLogOperationsOnNonHLLTypes() {
        ...
        try {
            ...
            assertThatThrownBy(
                            () ->
                                    connection.hyperLogLogCommands().pfAdd(stringKey.getBytes(), "value".getBytes()))
    ...
    void testBasicHyperLogLogCommandsInPipelineMode() {
        ...
        try {
            ...
            assertThat((Long) results.get(2))
                    .isGreaterThanOrEqualTo(2L)
                    .isLessThanOrEqualTo(4L); // pfCount result
            assertThat(results.get(3)).isEqualTo(0L); // pfAdd with duplicate
            assertThat((Long) results.get(4))
                    .isGreaterThanOrEqualTo(2L)
                    .isLessThanOrEqualTo(4L); // pfCount result
    ...
    void testPfCountMultipleInPipelineMode() {
        ...
        try {
            ...
            assertThat(connection.hyperLogLogCommands().pfCount(key1Bytes, key2Bytes))
                    .isNull(); // union count
            ...
            assertThat((Long) results.get(0))
                    .isGreaterThanOrEqualTo(1L)
                    .isLessThanOrEqualTo(3L); // key1 count
            assertThat((Long) results.get(1))
                    .isGreaterThanOrEqualTo(1L)
                    .isLessThanOrEqualTo(3L); // key2 count
            assertThat((Long) results.get(2))
                    .isGreaterThanOrEqualTo(2L)
                    .isLessThanOrEqualTo(4L); // union count
    ...
    void testPfMergeInPipelineMode() {
        ...
        try {
            ...
            connection
                    .hyperLogLogCommands()
                    .pfMerge(destBytes, source1Bytes, source2Bytes); // void method, no assertion
            assertThat(connection.hyperLogLogCommands().pfCount(destBytes)).isNull();
            assertThat(connection.hyperLogLogCommands().pfCount(source1Bytes))
                    .isNull(); // verify unchanged
            assertThat(connection.hyperLogLogCommands().pfCount(source2Bytes))
                    .isNull(); // verify unchanged
            ...
            assertThat((Long) results.get(1))
                    .isGreaterThanOrEqualTo(2L)
                    .isLessThanOrEqualTo(4L); // merged count
            assertThat((Long) results.get(2))
                    .isGreaterThanOrEqualTo(1L)
                    .isLessThanOrEqualTo(3L); // source1 unchanged
            assertThat((Long) results.get(3))
                    .isGreaterThanOrEqualTo(1L)
                    .isLessThanOrEqualTo(3L); // source2 unchanged
    ...
    void testEmptyHyperLogLogOperationsInPipelineMode() {
        ...
        try {
            ...
            assertThat(connection.hyperLogLogCommands().pfCount(keyBytes)).isNull();
            assertThat(connection.hyperLogLogCommands().pfAdd(keyBytes, emptyValue))
                    .isNull(); // duplicate
            assertThat(connection.hyperLogLogCommands().pfCount(keyBytes)).isNull();
    ...
    void testHyperLogLogComprehensiveWorkflow() {
        ...
        try {
            ...
            connection
                    .hyperLogLogCommands()
                    .pfAdd(
                            hll1.getBytes(), "common1".getBytes(), "unique1a".getBytes(), "unique1b".getBytes());
            connection
                    .hyperLogLogCommands()
                    .pfAdd(
                            hll2.getBytes(), "common1".getBytes(), "common2".getBytes(), "unique2a".getBytes());
            connection
                    .hyperLogLogCommands()
                    .pfAdd(
                            hll3.getBytes(), "common2".getBytes(), "unique3a".getBytes(), "unique3b".getBytes());
            ...
            Long unionAll =
                    connection
                            .hyperLogLogCommands()
                            .pfCount(hll1.getBytes(), hll2.getBytes(), hll3.getBytes());
