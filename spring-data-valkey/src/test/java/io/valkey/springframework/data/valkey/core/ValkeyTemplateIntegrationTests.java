...
public class ValkeyTemplateIntegrationTests<K, V> {
    ...
    ValkeyTemplateIntegrationTests(
    ...
    public void testExec() {
        ...
        Set<TypedTuple<V>> tupleSet =
                new LinkedHashSet<>(Collections.singletonList(new DefaultTypedTuple<>(zsetValue, 1d)));

        assertThat(results)
                .usingElementComparator(CollectionAwareComparator.INSTANCE)
                .containsExactly(true, value1, 1L, list, 1L, set, true, tupleSet);
    }
    ...
    void testExecCustomSerializer() {
        ...
        Set<TypedTuple<Long>> tupleSet =
                new LinkedHashSet<>(Collections.singletonList(new DefaultTypedTuple<>(9L, 1d)));
        Set<Long> zSet = new LinkedHashSet<>(Collections.singletonList(9L));
        Map<Long, Long> map = new LinkedHashMap<>();
        map.put(10L, 11L);

        assertThat(results)
                .containsExactly(true, 5L, 1L, 1L, list, 1L, longSet, true, tupleSet, zSet, true, map);
    }
                ...
                valkeyTemplate.executePipelined(
                        (ValkeyCallback)
                                connection -> {
                                    ...
                                    connection.rPush(
                                            rawListKey, serialize(listValue, valkeyTemplate.getValueSerializer()));
                                    connection.rPush(
                                            rawListKey, serialize(listValue2, valkeyTemplate.getValueSerializer()));
                                    connection.lRange(rawListKey, 0, -1);
                                    return null;
                                });
        assertThat(results)
                .usingElementComparator(CollectionAwareComparator.INSTANCE)
                .containsExactly(true, value1, 1L, 2L, Arrays.asList(listValue, listValue2));
    }
        ...
        List<Object> results =
                valkeyTemplate.executePipelined(
                        (ValkeyCallback)
                                connection -> {
                                    connection.hGetAll(
                                            ((StringValkeySerializer) valkeyTemplate.getKeySerializer())
                                                    .serialize("foo"));
                                    return null;
                                });
    ...
    public void testExecutePipelinedNonNullValkeyCallback() {
        assertThatExceptionOfType(InvalidDataAccessApiUsageException.class)
                .isThrownBy(
                        () ->
                                valkeyTemplate.executePipelined(
                                        (ValkeyCallback<String>) connection -> "Hey There"));
    }
                        ...
                        () ->
                                valkeyTemplate.executePipelined(
                                        new SessionCallback<String>() {
                                            @SuppressWarnings("rawtypes")
                                            public String execute(ValkeyOperations operations)
    ...
    void testSortStore() {
        ...
        assumeThat(value1 instanceof Number).isTrue();
        valkeyTemplate.opsForList().rightPush(key1, value1);
        assertThat(valkeyTemplate.sort(SortQueryBuilder.sort(key1).build(), key2))
                .isEqualTo(Long.valueOf(1));
        assertThat(valkeyTemplate.boundListOps(key2).range(0, -1))
    ...
    public void testSortBulkMapper() {
        ...
        valkeyTemplate.opsForList().rightPush(key1, value1);
        List<String> results =
                valkeyTemplate.sort(SortQueryBuilder.sort(key1).get("#").build(), tuple -> "FOO");
    ...
    void testBoundExpireWithConditionsAndGetExpireSeconds() {
        ...
        assertThat(
                        exp.expire(
                                Expiration.from(Duration.ofSeconds(1)), ExpirationOptions.builder().gt().build()))
                .isEqualTo(ExpireChanges.ExpiryChangeState.CONDITION_NOT_MET);
        assertThat(
                        exp.expire(
                                Expiration.from(Duration.ofSeconds(10)), ExpirationOptions.builder().gt().build()))
    ...
    void testExpireAtMillisNotSupported() {
        ...
        StringValkeyTemplate template2 =
                new StringValkeyTemplate(valkeyTemplate.getConnectionFactory());
    ...
    void testExecuteScriptCustomSerializers() {
        ...
        assertThat(
                        valkeyTemplate.execute(
                                script,
                                valkeyTemplate.getValueSerializer(),
