 ...
 *
 * <p>These tests verify that the connection factory works correctly with a Valkey cluster, ensuring
 * that the cluster adapter properly handles routing and result aggregation.
 *
 * <p><strong>Important Notes:</strong>
 *
 * <ul>
 *   <li>All test keys use hash tags (e.g., {@code {test}:key}) to ensure multi-key operations work
 *       correctly in cluster mode by routing to the same slot.
 *   <li>Transactions ({@code MULTI/EXEC}) are not supported in cluster mode and are omitted.
 *   <li>Pipeline operations have limited support in cluster mode and are omitted.
 * </ul>
 *
...
public class ValkeyGlideClusterConnectionFactoryIntegrationTests {
    ...
    private ValkeyGlideConnectionFactory createClusterConnectionFactory() {
        ...
        return new ValkeyGlideConnectionFactory(
                clusterConfig, new DefaultValkeyGlideClientConfiguration());
    }
    ...
    /**
     * Note: Transactions (MULTI/EXEC) are NOT supported in cluster mode. This test is intentionally
     * omitted as cluster mode does not support transactions that span multiple keys, and even
     * single-key transactions have limited support.
     */
