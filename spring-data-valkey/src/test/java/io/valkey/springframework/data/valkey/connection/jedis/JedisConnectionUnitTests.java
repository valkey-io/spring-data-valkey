...
class JedisConnectionUnitTests {
    ...
    public class BasicUnitTests extends AbstractConnectionUnitTestBase<Connection> {
        ...
        void shutdownNosaveShouldBeSentCorrectly() {
            ...
            assertThatExceptionOfType(JedisException.class)
                    .isThrownBy(() -> connection.shutdown(ShutdownOption.NOSAVE));
        ...
        void shutdownSaveShouldBeSentCorrectly() {
            ...
            assertThatExceptionOfType(JedisException.class)
                    .isThrownBy(() -> connection.shutdown(ShutdownOption.SAVE));
        ...
        void restoreShouldThrowExceptionWhenTtlInMillisExceedsIntegerRange() {
            assertThatIllegalArgumentException()
                    .isThrownBy(
                            () ->
                                    connection.restore(
                                            "foo".getBytes(), (long) Integer.MAX_VALUE + 1L, "bar".getBytes()));
        }
        ...
        public void scanShouldKeepTheConnectionOpen() {
            ...
            doReturn(new ScanResult<>("0", Collections.<String>emptyList()))
                    .when(jedisSpy)
                    .scan(any(byte[].class), any(ScanParams.class));
        ...
        void scanShouldOperateUponUnsigned64BitCursorId() {
            ...
            doReturn(new ScanResult<>(cursorId, List.of("spring".getBytes())))
                    .when(jedisSpy)
                    .scan(any(byte[].class), any(ScanParams.class));
        ...
        public void sScanShouldKeepTheConnectionOpen() {
            ...
            doReturn(new ScanResult<>("0", Collections.<String>emptyList()))
                    .when(jedisSpy)
                    .sscan(any(byte[].class), any(byte[].class), any(ScanParams.class));
        ...
        void zScanShouldOperateUponUnsigned64BitCursorId() {
            ...
            doReturn(
                            new ScanResult<>(
                                    cursorId, List.of(new redis.clients.jedis.resps.Tuple("spring", 1D))))
                    .when(jedisSpy)
                    .zscan(any(byte[].class), any(byte[].class), any(ScanParams.class));
        ...
        void hScanShouldOperateUponUnsigned64BitCursorId() {
            ...
            doReturn(
                            new ScanResult<>(
                                    cursorId, List.of(Map.entry("spring".getBytes(), "data".getBytes()))))
                    .when(jedisSpy)
                    .hscan(any(byte[].class), any(byte[].class), any(ScanParams.class));

            Cursor<Entry<byte[], byte[]>> cursor =
                    connection.hashCommands().hScan("spring".getBytes(), ScanOptions.NONE);
            cursor.next(); // initial value
            assertThat(cursor.getCursorId()).isEqualTo(Long.parseUnsignedLong(cursorId));
