...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideClusterConnection} cluster
 * functionality using the ValkeyClusterCommands interface directly.
 *
 * <p>These tests validate the implementation of all ValkeyClusterCommands methods: - Cluster
 * topology operations (clusterGetNodes, clusterGetReplicas, clusterGetMasterReplicaMap) - Slot
 * routing operations (clusterGetSlotForKey, clusterGetNodeForSlot, clusterGetNodeForKey) - Cluster
 * information (clusterGetClusterInfo) - Slot management (clusterAddSlots, clusterDeleteSlots,
 * clusterCountKeysInSlot, clusterGetKeysInSlot) - Node management (clusterMeet, clusterForget,
 * clusterSetSlot, clusterReplicate)
 *
 * <p>Note: These tests cover DIRECT MODE only, as cluster mode does not support pipelining or
 * transactions in Valkey-Glide.
 *
 ...
 *       <ul>
 *         <li>7379: slots 0-5460 (5461 slots)
 *         <li>7380: slots 5461-10922 (5462 slots)
 *         <li>7381: slots 10923-16383 (5461 slots)
 *       </ul>
 * </ul>
 *
...
public class ValkeyGlideClusterConnectionCommandsIntegrationTests
    ...
    void testClusterGetNodes() {
        ...
        List<ValkeyClusterNode> nodeList =
                (List<ValkeyClusterNode>)
        ...
        assertThat(nodesByPort)
                .containsKeys(
                        MASTER_NODE_1_PORT, MASTER_NODE_2_PORT, MASTER_NODE_3_PORT, REPLICAOF_NODE_1_PORT);
    ...
    void testClusterGetMasterReplicaMap() {
        ...
        for (Map.Entry<ValkeyClusterNode, Collection<ValkeyClusterNode>> entry :
                masterReplicaMap.entrySet()) {
    ...
    void testClusterGetNodeForKey() {
        ...
        ValkeyClusterNode node1 =
                clusterConnection.clusterCommands().clusterGetNodeForKey(key1.getBytes());
        ValkeyClusterNode node2 =
                clusterConnection.clusterCommands().clusterGetNodeForKey(key2.getBytes());
        ValkeyClusterNode node3 =
                clusterConnection.clusterCommands().clusterGetNodeForKey(key3.getBytes());
        ...
        try {
            ...
            assertThatThrownBy(
                            () ->
                                    clusterConnection
                                            .clusterCommands()
                                            .clusterDeleteSlots(master2, slot1, slot2, slot3))
            ...
            // Final verification
            ValkeyClusterNode nodeForSlot1 =
                    clusterConnection.clusterCommands().clusterGetNodeForSlot(slot1);
    ...
    void testClusterAddSlotsWithSlotRange() {
        ...
        try {
            ...
            // Final verification
            ValkeyClusterNode nodeForSlot =
                    clusterConnection.clusterCommands().clusterGetNodeForSlot(rangeStart);
    ...
    // Valkey-Glide uses Jedis approach of taking the id of the node parameter, which is the
    // destination node and is incorrect by the protocol.
    ...
    void testClusterSetSlot() {
        ...
        try {
            ...
            clusterConnection
                    .clusterCommands()
                    .clusterSetSlot(master1, slotToMigrate, AddSlots.MIGRATING);

            // Step 2: Mark slot as IMPORTING on destination node (master2)
            clusterConnection
                    .clusterCommands()
                    .clusterSetSlot(master2, slotToMigrate, AddSlots.IMPORTING);
            ...
            ValkeyClusterNode nodeForSlot =
                    clusterConnection.clusterCommands().clusterGetNodeForSlot(slotToMigrate);
...
""")
    void testClusterSetSlotWithActualMigration() {
        ...
        try {
            ...
            clusterConnection
                    .clusterCommands()
                    .clusterSetSlot(destNode, slotToMigrate, AddSlots.IMPORTING);
            ...
            clusterConnection
                    .clusterCommands()
                    .clusterSetSlot(sourceNode, slotToMigrate, AddSlots.MIGRATING);
    ...
    void testClusterMeet() {
        ...
        List<ValkeyClusterNode> nodeList =
                java.util.stream.StreamSupport.stream(nodesAfter.spliterator(), false)
                        .collect(Collectors.toList());
    ...
    void testClusterForget() {
        ...
        ValkeyClusterNode nonExistentNode =
                ValkeyClusterNode.newValkeyClusterNode()
    ...
    /**
     * Finds a key whose slot falls within the specified range. Uses a simple iteration approach to
     * find a suitable key.
     */
