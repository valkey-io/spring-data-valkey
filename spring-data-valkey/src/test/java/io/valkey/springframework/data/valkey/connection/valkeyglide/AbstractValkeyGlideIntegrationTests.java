...
/**
 * Abstract base class for Valkey Glide integration tests that provides common setup, teardown, and
 * utility methods to reduce code duplication across test classes.
 *
 * <p>This class handles: - Connection factory creation and management - Server availability
 * validation - Test lifecycle management (setup/teardown) - Common utility methods for key cleanup
 * with proper error handling - Configuration property access
 *
 * <p>Subclasses should: - Override {@link #getTestKeyPatterns()} to define their specific test key
 * patterns - Implement their command-specific test methods - Call {@link #cleanupKey(String)} for
 * individual key cleanup in tests
 *
...
public abstract class AbstractValkeyGlideIntegrationTests {
    ...
    protected static final Logger logger =
            LoggerFactory.getLogger(AbstractValkeyGlideIntegrationTests.class);
    ...
    /**
     * Cleans up a single test key with strict error handling. This method will throw an exception if
     * cleanup fails, which can be useful when cleanup failures indicate a serious problem that should
     * stop the test.
     *
     * @param key the key to delete
    ...
    /**
     * Common test key patterns that are frequently used across different test classes. Subclasses can
     * use this in combination with their specific patterns.
     *
     * @return array of common test key patterns
     */
    protected String[] getCommonTestKeyPatterns() {
        return new String[] {
            "non:existent:key", "test:common:key1", "test:common:key2", "test:common:key3"
        };
    }
}
