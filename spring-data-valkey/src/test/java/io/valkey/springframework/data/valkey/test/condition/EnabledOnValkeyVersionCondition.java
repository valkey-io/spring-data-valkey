...
class EnabledOnValkeyVersionCondition implements ExecutionCondition {
    ...
    private static final ConditionEvaluationResult ENABLED_BY_DEFAULT =
            enabled("@EnabledOnVersion is not present");
    private static final ExtensionContext.Namespace NAMESPACE =
            ExtensionContext.Namespace.create(ValkeyConditions.class);
    ...
    public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
        ...
        Optional<EnabledOnValkeyVersion> optional =
                AnnotationUtils.findAnnotation(context.getElement(), EnabledOnValkeyVersion.class);
        ...
        ValkeyConditions conditions =
                store.getOrComputeIfAbsent(
                        ValkeyConditions.class,
                        ignore -> {
                            try (StatefulRedisConnection connection =
                                    lettuceExtension.resolve(context, StatefulRedisConnection.class)) {
        ...
        if (requiredVersionMet) {
            return enabled(
                    "Enabled on version %s; actual version: %s"
                            .formatted(requiredVersion, conditions.getValkeyVersion()));
        }

        return disabled(
                "Disabled; version %s not available on Valkey version %s"
                        .formatted(requiredVersion, conditions.getValkeyVersion()));
    }
}
