...
class ScanCursorUnitTests {
    ...
    void cursorShouldReturnNullWhenNoNextElementAvailable() {
        assertThatExceptionOfType(NoSuchElementException.class)
                .isThrownBy(() -> initCursor(new LinkedList<>()).next());
    }
    ...
    void repoeningCursorShouldHappenAtLastPosition() {
        ...
        assertThatExceptionOfType(InvalidDataAccessApiUsageException.class)
                .isThrownBy(() -> initCursor(values).open());
    }
    ...
    void streamLimitShouldApplyLimitation() {
        ...
        assertThat(cursor.stream().limit(2).collect(Collectors.toList()))
                .hasSize(2)
    ...
    void shouldCloseCursorOnScanFailure() {
        ...
        KeyBoundCursor<String> cursor =
                new KeyBoundCursor<String>("foo".getBytes(), Cursor.CursorId.initial(), null) {
                    @Override
                    protected ScanIteration<String> doScan(
