...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideConnection} key functionality
 * using the ValkeyKeyCommands interface directly.
 *
 * <p>These tests validate the implementation of all ValkeyKeyCommands methods in all 3 invocation
 * modes: - Immediate mode: Direct command execution with results - Pipeline mode: Commands return
 * null, results collected in closePipeline() - Transaction mode: Commands return null, results
 * collected in exec()
 *
...
public class ValkeyGlideConnectionKeyCommandsIntegrationTests
    ...
    protected String[] getTestKeyPatterns() {
        return new String[] {
            "test:key:exists:key1",
            "test:key:exists:key2",
            "test:key:exists:key3",
            "test:key:copy:source",
            "test:key:copy:target",
            "test:key:copy:existing",
            ...
            "test:key:scan:item1",
            "test:key:scan:item2",
            "test:key:scan:other1",
            "test:key:rename:old",
            "test:key:rename:new",
            "test:key:renamenx:old",
            "test:key:renamenx:new",
            "test:key:renamenx:existing",
            "test:key:expire",
            "test:key:pexpire",
            "test:key:expireat",
            "test:key:pexpireat",
            ...
            "test:key:restore",
            "test:key:move",
            "test:key:unlink:key1",
            "test:key:unlink:key2",
            "test:key:encoding",
            "test:key:idletime",
            "test:key:refcount",
            "test:key:pipeline:key1",
            "test:key:pipeline:key2",
            "test:key:pipeline:key3",
            "test:key:transaction:key1",
            "test:key:transaction:key2",
            "test:key:transaction:key3",
            "non:existent:key",
            "new:key"
        };
    }
    ...
    void testExistsAndDel() {
        ...
        try {
            ...
            Long existsMultiple1 =
                    connection.keyCommands().exists(key1.getBytes(), key2.getBytes(), key3.getBytes());
            ...
            Long existsMultiple2 =
                    connection.keyCommands().exists(key1.getBytes(), key2.getBytes(), key3.getBytes());
            assertThat(existsMultiple2).isEqualTo(2L); // 2 keys exist

            // Test with duplicate keys
            Long existsDuplicate =
                    connection.keyCommands().exists(key1.getBytes(), key1.getBytes(), key2.getBytes());
    ...
    void testUnlink() {
        ...
        try {
            ...
            Long unlinkResult2 =
                    connection.keyCommands().unlink(key2.getBytes(), "non:existent:key".getBytes());
    ...
    void testCopy() {
        ...
        try {
            ...
            Boolean copyNonExistent =
                    connection.keyCommands().copy(sourceKey.getBytes(), targetKey.getBytes(), false);
            ...
            Boolean copyResult2 =
                    connection.keyCommands().copy(sourceKey.getBytes(), existingTargetKey.getBytes(), false);
            ...
            Boolean copyResult3 =
                    connection.keyCommands().copy(sourceKey.getBytes(), existingTargetKey.getBytes(), true);
    ...
    void testTouch() {
        ...
        try {
            ...
            Long touchResult2 =
                    connection.keyCommands().touch(key1.getBytes(), key2.getBytes(), key3.getBytes());
    ...
    void testKeys() {
        ...
        try {
            ...
            Set<String> matchedKeyStrings =
                    matchedKeys.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testRenameNX() {
        ...
        try {
            ...
            Boolean renameResult1 =
                    connection.keyCommands().renameNX(oldKey.getBytes(), newKey.getBytes());
            ...
            Boolean renameResult2 =
                    connection.keyCommands().renameNX(newKey.getBytes(), existingKey.getBytes());
    ...
    void testExpirationConditions() {
        ...
        try {
            ...
            connection.stringCommands().set(key.getBytes(), value);
            Boolean expireNX1 =
                    connection.keyCommands().expire(key.getBytes(), 10, ExpirationOptions.Condition.NX);
            assertThat(expireNX1).isTrue(); // Should succeed, key had no expiration

            Boolean expireNX2 =
                    connection.keyCommands().expire(key.getBytes(), 20, ExpirationOptions.Condition.NX);
            ...
            Boolean expireXX1 =
                    connection.keyCommands().expire(key.getBytes(), 30, ExpirationOptions.Condition.XX);
            ...
            Boolean expireGT1 =
                    connection.keyCommands().expire(key.getBytes(), 20, ExpirationOptions.Condition.GT);
            assertThat(expireGT1).isFalse(); // Should fail, 20s < 30s (current)

            Boolean expireGT2 =
                    connection.keyCommands().expire(key.getBytes(), 40, ExpirationOptions.Condition.GT);
            ...
            Boolean expireLT1 =
                    connection.keyCommands().expire(key.getBytes(), 50, ExpirationOptions.Condition.LT);
            assertThat(expireLT1).isFalse(); // Should fail, 50s > 40s (current)

            Boolean expireLT2 =
                    connection.keyCommands().expire(key.getBytes(), 35, ExpirationOptions.Condition.LT);
            ...
            Boolean expireXX2 =
                    connection.keyCommands().expire(key.getBytes(), 40, ExpirationOptions.Condition.XX);
    ...
    void testPExpireAt() {
        ...
        try {
            ...
            Boolean pExpireAtResult =
                    connection.keyCommands().pExpireAt(key.getBytes(), futureTimestampMillis);
    ...
    void testTtlTimeUnitSpecialValues() {
        ...
        try {
            ...
            Long ttlNonExistentSeconds =
                    connection.keyCommands().ttl("non:existent:key".getBytes(), TimeUnit.SECONDS);
            assertThat(ttlNonExistentSeconds).isEqualTo(-2L);

            Long ttlNonExistentMillis =
                    connection.keyCommands().ttl("non:existent:key".getBytes(), TimeUnit.MILLISECONDS);
            assertThat(ttlNonExistentMillis).isEqualTo(-2L);

            Long ttlNonExistentMinutes =
                    connection.keyCommands().ttl("non:existent:key".getBytes(), TimeUnit.MINUTES);
            ...
            Long ttlNoExpirationMillis =
                    connection.keyCommands().ttl(key.getBytes(), TimeUnit.MILLISECONDS);
            ...
            Long ttlWithExpirationSeconds =
                    connection.keyCommands().ttl(key.getBytes(), TimeUnit.SECONDS);
            assertThat(ttlWithExpirationSeconds).isGreaterThan(0L).isLessThanOrEqualTo(60L);

            Long ttlWithExpirationMillis =
                    connection.keyCommands().ttl(key.getBytes(), TimeUnit.MILLISECONDS);
    ...
    void testPTtlTimeUnitSpecialValues() {
        ...
        try {
            ...
            // Should return -2 (key doesn't exist) without conversion
            Long pTtlNonExistentSeconds =
                    connection.keyCommands().pTtl("non:existent:key".getBytes(), TimeUnit.SECONDS);
            assertThat(pTtlNonExistentSeconds).isEqualTo(-2L);

            Long pTtlNonExistentMillis =
                    connection.keyCommands().pTtl("non:existent:key".getBytes(), TimeUnit.MILLISECONDS);
            assertThat(pTtlNonExistentMillis).isEqualTo(-2L);

            Long pTtlNonExistentMinutes =
                    connection.keyCommands().pTtl("non:existent:key".getBytes(), TimeUnit.MINUTES);
            ...
            Long pTtlNoExpirationSeconds =
                    connection.keyCommands().pTtl(key.getBytes(), TimeUnit.SECONDS);
            assertThat(pTtlNoExpirationSeconds).isEqualTo(-1L);

            Long pTtlNoExpirationMillis =
                    connection.keyCommands().pTtl(key.getBytes(), TimeUnit.MILLISECONDS);
            assertThat(pTtlNoExpirationMillis).isEqualTo(-1L);

            Long pTtlNoExpirationMinutes =
                    connection.keyCommands().pTtl(key.getBytes(), TimeUnit.MINUTES);
    ...
    void testSortComprehensive() {
        ...
        try {
            ...
            Long storeResult =
                    connection.keyCommands().sort(listKey.getBytes(), params4, storeKey.getBytes());
    ...
    void testObjectIntrospection() {
        ...
        try {
            ...
            ValueEncoding nonExistentEncoding =
                    connection.keyCommands().encodingOf("non:existent:key".getBytes());
            ...
            Duration nonExistentIdletime =
                    connection.keyCommands().idletime("non:existent:key".getBytes());
    ...
    void testAdvancedKeyOperationsPipeline() {
        ...
        try {
            ...
            Boolean renameNXResult =
                    connection.keyCommands().renameNX(renameKey.getBytes(), newRenameKey.getBytes());
            ...
            Boolean expireAtResult =
                    connection.keyCommands().expireAt(newRenameKey.getBytes(), futureTimestamp);
            assertThat(expireAtResult).isNull();

            long futureTimestampMillis = System.currentTimeMillis() + 30000;
            Boolean pExpireAtResult =
                    connection.keyCommands().pExpireAt(newRenameKey.getBytes(), futureTimestampMillis);
            assertThat(pExpireAtResult).isNull();

            Long ttlTimeUnitResult =
                    connection.keyCommands().ttl(newRenameKey.getBytes(), TimeUnit.MILLISECONDS);
            ...
            Long sortStoreResult =
                    connection.keyCommands().sort(listKey.getBytes(), sortParams, storeKey.getBytes());
    ...
    void testTtlTimeUnitConversionPipeline() {
        ...
        try {
            ...
            assertThat(actualTtlMilliseconds)
                    .isGreaterThanOrEqualTo(86390000L)
                    .isLessThanOrEqualTo(86400000L);
            ...
            if (actualTtlHours != null && actualTtlHours > 0) {
                ...
                assertThat(hoursToMillisecondsRatio)
                        .isBetween(3500000.0, 3700000.0); // ~3600000 ms per hour
            }
    ...
    void testPTtlTimeUnitConversionPipeline() {
        ...
        try {
            ...
            assertThat(actualPTtlMilliseconds)
                    .isGreaterThanOrEqualTo(86390000L)
                    .isLessThanOrEqualTo(86400000L);
            ...
            if (actualPTtlHours != null && actualPTtlHours > 0) {
                ...
                assertThat(hoursToMillisecondsRatio)
                        .isBetween(3500000.0, 3800000.0); // ~3600000 ms per hour
            }
    ...
    void testKeyOperationsErrorHandling() {
        ...
        assertThatThrownBy(
                        () ->
                                connection
                                        .keyCommands()
                                        .rename("non:existent:key".getBytes(), "new:key".getBytes()))
    ...
    void testAdvancedExpirationConditions() {
        ...
        try {
            ...
            Boolean pExpireAtNX2 =
                    connection
                            .keyCommands()
                            .pExpireAt(
                                    key.getBytes(), futureTimestampMillis + 10000, ExpirationOptions.Condition.NX);
            ...
            Boolean pExpireAtXX1 =
                    connection
                            .keyCommands()
                            .pExpireAt(
                                    key.getBytes(), futureTimestampMillis + 20000, ExpirationOptions.Condition.XX);
            assertThat(pExpireAtXX1).isTrue(); // Should succeed, key has expiration

            // Remove expiration
            connection.keyCommands().persist(key.getBytes());

            Boolean pExpireAtXX2 =
                    connection
                            .keyCommands()
                            .pExpireAt(
                                    key.getBytes(), futureTimestampMillis + 30000, ExpirationOptions.Condition.XX);
            ...
            connection.keyCommands().expire(key.getBytes(), 60); // 60 seconds
            Boolean expireAtXXAfterExpire =
                    connection
                            .keyCommands()
                            .expireAt(key.getBytes(), futureTimestamp + 60, ExpirationOptions.Condition.XX);
            assertThat(expireAtXXAfterExpire)
                    .isTrue(); // Should succeed, key has expiration from expire()
            ...
            Boolean pExpireAtXXAfterPExpire =
                    connection
                            .keyCommands()
                            .pExpireAt(
                                    key.getBytes(), futureTimestampMillis + 120000, ExpirationOptions.Condition.XX);
            assertThat(pExpireAtXXAfterPExpire)
                    .isTrue(); // Should succeed, key has expiration from pExpire()

            // ========== Test conditions on non-existent key ==========
            Boolean expireAtNonExistent =
                    connection
                            .keyCommands()
                            .expireAt(
                                    "non:existent:key".getBytes(), futureTimestamp, ExpirationOptions.Condition.NX);
            assertThat(expireAtNonExistent).isFalse(); // Should fail, key doesn't exist

            Boolean pExpireAtNonExistent =
                    connection
                            .keyCommands()
                            .pExpireAt(
                                    "non:existent:key".getBytes(),
                                    futureTimestampMillis,
    ...
    void testExpirationOperationsEdgeCases() {
        ...
        try {
            ...
            Boolean expireNonExistent =
                    connection.keyCommands().expire("non:existent:key".getBytes(), 10);
            assertThat(expireNonExistent).isFalse();
