...
public class ReactiveValkeyTemplateIntegrationTests<K, V> {
    ...
    void copy() {
        ...
        try (ReactiveValkeyClusterConnection connection =
                valkeyTemplate.getConnectionFactory().getReactiveClusterConnection()) {
    ...
    void countExistingKeysIfValidKeyExists() {
        ...
        ops.set(key, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
        ops.set(key2, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
        ops.set(key3, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();

        valkeyTemplate
                .countExistingKeys(Arrays.asList(key, key2, key3))
                .as(StepVerifier::create)
                .expectNext(3L)
                .verifyComplete();
    }
    ...
    void rename() {
        ...
        valkeyTemplate
                .opsForValue()
                .set(oldName, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void renameNx() {
        ...
        valkeyTemplate
                .opsForValue()
                .set(existing, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();

        valkeyTemplate
                .renameIfAbsent(oldName, newName)
                .as(StepVerifier::create) //
                .expectNext(true) //
                .expectComplete() //
                .verify();
        ...
        valkeyTemplate
                .renameIfAbsent(newName, existing)
                .as(StepVerifier::create)
                .expectNext(false) //
                .expectComplete() //
                .verify();
    ...
    void unlink() {
        ...
        valkeyTemplate
                .opsForValue()
                .set(single, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void unlinkMany() {
        ...
        valkeyTemplate
                .opsForValue()
                .set(key1, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
        valkeyTemplate
                .opsForValue()
                .set(key2, valueFactory.instance())
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void executeScript() {
        ...
        Flux<V> execute =
                valkeyTemplate.execute(
                        new DefaultValkeyScript<>(
                                "return redis.call('get', KEYS[1])", (Class<V>) value.getClass()),
    ...
    void executeScriptWithElementReaderAndWriter() {
        ...
        SerializationPair json =
                SerializationPair.fromSerializer(new Jackson2JsonValkeySerializer<>(Person.class));
        ValkeyElementReader<String> resultReader =
        ...
        valkeyTemplate
                .execute(
                        new DefaultValkeyScript<>("return redis.call('set', KEYS[1], ARGV[1])", String.class),
                        Collections.singletonList(key),
        ...
        Flux<Person> execute =
                valkeyTemplate.execute(
                        new DefaultValkeyScript<>("return redis.call('get', KEYS[1])", Person.class),
                        Collections.singletonList(key),
    ...
    void expire() {
        ...
        valkeyTemplate
                .expire(key, Duration.ofSeconds(10))
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void expireWithCondition() {
        ...
        valkeyTemplate
                .expire(key, Expiration.seconds(10), ExpirationOptions.none())
                .as(StepVerifier::create)
                .expectNext(ExpireChanges.ExpiryChangeState.OK)
                .verifyComplete();
        valkeyTemplate
                .expire(key, Expiration.seconds(20), ExpirationOptions.builder().lt().build())
                .as(StepVerifier::create)
                .expectNext(ExpireChanges.ExpiryChangeState.CONDITION_NOT_MET)
                .verifyComplete();

        valkeyTemplate
                .getExpire(key)
                .as(StepVerifier::create) //
                .consumeNextWith(actual -> assertThat(actual).isGreaterThan(Duration.ofSeconds(5)))
                .verifyComplete();
    }
    ...
    void preciseExpire() {
        ...
        valkeyTemplate
                .expire(key, Duration.ofMillis(10_001))
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void preciseExpireWithCondition() {
        ...
        valkeyTemplate
                .expire(key, Expiration.milliseconds(10000), ExpirationOptions.none())
                .as(StepVerifier::create)
                .expectNext(ExpireChanges.ExpiryChangeState.OK)
                .verifyComplete();
        valkeyTemplate
                .expire(key, Expiration.milliseconds(20000), ExpirationOptions.builder().lt().build())
                .as(StepVerifier::create)
                .expectNext(ExpireChanges.ExpiryChangeState.CONDITION_NOT_MET)
                .verifyComplete();
    ...
    void expireAt() {
        ...
        Instant expireAt =
                Instant.ofEpochSecond(Instant.now().plus(Duration.ofSeconds(10)).getEpochSecond());
    ...
    void preciseExpireAt() {
        ...
        Instant expireAt =
                Instant.ofEpochSecond(Instant.now().plus(Duration.ofSeconds(10)).getEpochSecond(), 5);
    ...
    void shouldApplyCustomSerializationContextToValues() {
        ...
        ValkeySerializationContext<Object, Object> objectSerializers =
                ValkeySerializationContext.newSerializationContext()
                        .key(jdkSerializer) //
                        .value(jdkSerializer) //
        ...
        ReactiveValueOperations<Object, Object> valueOperations =
                valkeyTemplate.opsForValue(objectSerializers);
    ...
    void shouldApplyCustomSerializationContextToHash() {
        ...
        ValkeySerializationContext<K, V> serializationContext =
                valkeyTemplate.getSerializationContext();

        K key = keyFactory.instance();
        String hashField = "foo";
        Person hashValue = new PersonObjectFactory().instance();

        ValkeySerializationContext<K, V> objectSerializers =
                ValkeySerializationContext.<K, V>newSerializationContext()
        ...
        ReactiveHashOperations<K, String, Object> hashOperations =
                valkeyTemplate.opsForHash(objectSerializers);

        hashOperations
                .put(key, hashField, hashValue)
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void listenToLaterChannelShouldReceiveChannelMessagesCorrectly() {
        ...
        valkeyTemplate
                .listenToChannelLater(channel) //
                .doOnNext(it -> valkeyTemplate.convertAndSend(channel, message).subscribe())
