...
public class DefaultHashOperationsIntegrationTests<K, HK, HV> {
    ...
    public DefaultHashOperationsIntegrationTests(
            ValkeyTemplate<K, ?> valkeyTemplate,
    ...
    public static Collection<Object[]> testParams() {
        ...
        return Arrays.asList(
                new Object[][] {
                    {stringTemplate, stringFactory, stringFactory, stringFactory},
    ...
    void testHScanReadsValuesFully() throws IOException {
        ...
        long count = 0;
        try (Cursor<Map.Entry<HK, HV>> it =
                hashOps.scan(key, ScanOptions.scanOptions().count(1).build())) {
    ...
    void testExpireAndGetExpireMillis() {
        ...
        assertThat(valkeyTemplate.opsForHash().getTimeToLive(key, List.of(key1)))
                .satisfies(
                        expirations -> {
                            assertThat(expirations.missing()).isEmpty();
                            assertThat(expirations.timeUnit()).isEqualTo(TimeUnit.SECONDS);
                            assertThat(expirations.expirationOf(key1))
                                    .extracting(TimeToLive::raw, InstanceOfAssertFactories.LONG)
                                    .isBetween(0L, 1L);
        ...
        assertThat(valkeyTemplate.opsForHash().expire(key, Duration.ofSeconds(5), List.of(key1, key2)))
                .satisfies(
                        changes -> {
                            ...
                            assertThat(changes.stateChanges())
                                    .map(ExpiryChangeState::value)
                                    .containsExactly(1L, 1L);
                        });

        assertThat(
                        valkeyTemplate.opsForHash().getTimeToLive(key, TimeUnit.SECONDS, List.of(key1, key2)))
                .satisfies(
                        expirations -> {
                            assertThat(expirations.missing()).isEmpty();
                            assertThat(expirations.timeUnit()).isEqualTo(TimeUnit.SECONDS);
                            assertThat(expirations.expirationOf(key1))
                                    .extracting(TimeToLive::raw, InstanceOfAssertFactories.LONG)
                                    .isBetween(0L, 5L);
                            assertThat(expirations.ttlOf(key1))
    ...
    void testBoundExpireAndGetExpireSeconds() {
        ...
        assertThat(exp.expire(Duration.ofSeconds(5)))
                .satisfies(
                        changes -> {
                            assertThat(changes.allOk()).isTrue();
                            assertThat(changes.stateOf(key1)).isEqualTo(ExpiryChangeState.OK);
        ...
        assertThat(exp.getTimeToLive(TimeUnit.SECONDS))
                .satisfies(
                        expirations -> {
                            assertThat(expirations.missing()).isEmpty();
                            assertThat(expirations.timeUnit()).isEqualTo(TimeUnit.SECONDS);
    ...
    void testExpireAtAndGetExpireMillis() {
        ...
        assertThat(
                        valkeyTemplate
                                .opsForHash()
                                .expireAt(key, Instant.now().plusMillis(500), List.of(key1, key2)))
                .satisfies(ExpireChanges::allOk);

        assertThat(
                        valkeyTemplate
                                .opsForHash()
                                .getTimeToLive(key, TimeUnit.MILLISECONDS, List.of(key1, key2)))
                .satisfies(
                        expirations -> {
                            assertThat(expirations.missing()).isEmpty();
        ...
        assertThatExceptionOfType(IllegalArgumentException.class)
                .isThrownBy(
                        () ->
                                valkeyTemplate
                                        .opsForHash()
                                        .getTimeToLive(key, TimeUnit.NANOSECONDS, List.of(key1)));
    }
    ...
    void testPersistAndGetExpireMillis() {
        ...
        assertThat(
                        valkeyTemplate
                                .opsForHash()
                                .expireAt(key, Instant.now().plusMillis(800), List.of(key1, key2)))
                .satisfies(ExpireChanges::allOk);

        assertThat(valkeyTemplate.opsForHash().persist(key, List.of(key2)))
                .satisfies(ExpireChanges::allOk);

        assertThat(valkeyTemplate.opsForHash().getTimeToLive(key, List.of(key1, key2)))
                .satisfies(
                        expirations -> {
