...
class ParameterizedTestNameFormatter {
    ...
    String format(int invocationIndex, Object... arguments) {
        try {
            ...
            throw new JUnitException(
                    "The display name pattern defined for the parameterized test is invalid;"
    ...
    private String prepareMessageFormatPattern(int invocationIndex, Object[] arguments) {
        ...
        if (result.contains(ARGUMENTS_WITH_NAMES_PLACEHOLDER)) {
            result =
                    result.replace(ARGUMENTS_WITH_NAMES_PLACEHOLDER, argumentsWithNamesPattern(arguments));
        }
    ...
    private String argumentsWithNamesPattern(Object[] arguments) {
        return IntStream.range(0, arguments.length) //
                .mapToObj(
                        index ->
                                methodContext.getParameterName(index).map(name -> name + "=").orElse("")
                                        + "{"
                                        + index
                                        + "}") //
                .collect(joining(", "));
    ...
    private Object[] makeReadable(MessageFormat format, Object[] arguments) {
        ...
        Object[] result =
                Arrays.copyOf(arguments, Math.min(arguments.length, formats.length), Object[].class);
