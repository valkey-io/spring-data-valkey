...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideConnection} server functionality
 * using the ValkeyServerCommands interface directly.
 *
 * <p>These tests validate the implementation of all ValkeyServerCommands methods: - Background
 * operations (bgReWriteAof, bgSave, lastSave, save) - Database operations (dbSize, flushDb,
 * flushAll) - Server information (info, time) - Configuration management (getConfig, setConfig,
 * resetConfigStats, rewriteConfig) - Client management (killClient, setClientName, getClientName,
 * getClientList) - Replication commands (replicaOf, replicaOfNoOne) - Data migration (migrate) -
 * Server control (shutdown) - tested with caution - Error handling and validation
 *
 * @author Ilia Kolominsky
 * @since 2.0
 */
public class ValkeyGlideConnectionServerCommandsIntegrationTests
    ...
    void testBgSave() {
        try {
            ...
            if (e.getMessage() != null
                    && e.getMessage().contains("Background save already in progress")) {
    ...
    void testLastSave() {
        ...
        if (lastSaveTime != null) {
            ...
            assertThat(lastSaveTime)
                    .isLessThanOrEqualTo(currentTimeSeconds + 60); // Allow 1 minute clock skew
        } else {
    ...
    void testInfo() {
        ...
        assertThat(serverInfo.containsKey("valkey_version") || serverInfo.containsKey("redis_version"))
                .isTrue();
        ...
        try {
            ...
            while (cause != null) {
                ...
                if (causeMessage != null
                        && causeMessage.contains("The server is running without a config file")) {
    ...
    void testGetClientList() {
        ...
        // Client name might be null or empty string - we'll skip detailed validation since it's
        // optional
    ...
    void testKillClient() {
        ...
        Throwable exception =
                catchThrowable(() -> connection.serverCommands().killClient("192.168.1.1", 12345));

        assertThat(exception).isNotNull();

        // Check the entire exception chain for the "No such client" message
        // The actual message format is "An error was signalled by the server: - ResponseError: No such
        // client"
        boolean foundMessage = false;
        Throwable cause = exception;
        while (cause != null) {
            if (cause.getMessage() != null
                    && cause.getMessage().contains("ResponseError: No such client")) {
                foundMessage = true;
                break;
            }
            cause = cause.getCause();
    ...
    void testBackgroundOperationsInPipelineMode() {
        try {
            ...
            // Note: bgReWriteAof, bgSave, save might fail due to server configuration, so we test
            // lastSave
    ...
    void testClientManagementInPipelineMode() {
        try {
            ...
            connection
                    .serverCommands()
                    .setClientName("test-pipeline-client".getBytes()); // void method, no assertion
    ...
    void testClientManagementInTransactionMode() {
        try {
            ...
            connection
                    .serverCommands()
                    .setClientName("test-transaction-client".getBytes()); // void method, no assertion
            assertThat(connection.serverCommands().getClientName()).isNull();
            assertThat(connection.serverCommands().getClientList()).isNull();
    ...
    void testEdgeCases() {
        ...
        try {
            ...
            Properties nonExistentConfig =
                    connection.serverCommands().getConfig("nonexistent_config_param");
