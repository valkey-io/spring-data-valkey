...
/**
 * JUnit 5 {@link Extension} using Lettuce providing parameter resolution for connection resources
 * and that reacts to callbacks. The following resource types are supported by this extension:
 *
 * <ul>
 *   <li>{@link ClientResources} (singleton)
 *   <li>{@link RedisClient} (singleton)
 *   <li>{@link RedisClusterClient} (singleton)
 *   <li>{@link StatefulRedisConnection}
 *   <li>{@link StatefulRedisPubSubConnection}
 *   <li>{@link StatefulRedisClusterConnection}
 * </ul>
 *
 * <pre class="code">
 ...
 * ExtensionContext.Store} for reuse across multiple tests. Client and {@link ClientResources} are
 * allocated through default {@link RedisClient} respective {@link ValkeyClientSupplier} so shutdown
 * is managed by the actual suppliers. Singleton connection resources are closed after the test
 * class (test container) is finished.
 *
 * @author Mark Paluch
...
public class LettuceExtension implements ParameterResolver, AfterAllCallback, AfterEachCallback {
    ...
    private static final ExtensionContext.Namespace NAMESPACE =
            ExtensionContext.Namespace.create(LettuceExtension.class);

    private static final ClusterClientOptions DEFAULT_OPTIONS =
    ...
    private static final Set<Class<?>> CLOSE_AFTER_EACH =
            new HashSet<>(
                    Arrays.asList(
                            StatefulRedisConnection.class,
                            StatefulRedisPubSubConnection.class,
    ...
    private static final List<Supplier<?>> SUPPLIERS =
            Arrays.asList(
                    ClientResourcesSupplier.INSTANCE,
                    ValkeyClusterClientSupplier.INSTANCE,
                    ValkeyClientSupplier.INSTANCE,
                    StatefulValkeyConnectionSupplier.INSTANCE,
                    StatefulValkeyPubSubConnectionSupplier.INSTANCE,
                    StatefulValkeyClusterConnectionSupplier.INSTANCE);

    private static final List<Function<?, ?>> RESOURCE_FUNCTIONS =
            Collections.singletonList(ValkeyCommandsFunction.INSTANCE);
    ...
    private Object doGetInstance(Type parameterizedType) {
        ...
        return resourceFunction
                .map(it -> it.function.apply(findSupplier(it.dependsOn.getType()).get()))
    ...
    private static Optional<ResourceFunction> findFunction(Type type) {
        ...
        return RESOURCE_FUNCTIONS.stream()
                .map(
                        it -> {
                            ResolvableType dependsOn =
                                    ResolvableType.forType(it.getClass()).as(Function.class).getGeneric(0);
                            ResolvableType providedType =
                                    ResolvableType.forType(it.getClass()).as(Function.class).getGeneric(1);

                            return new ResourceFunction(dependsOn, providedType, it);
                        })
                .filter(it -> requested.isAssignableFrom(it.provides))
                .findFirst();
    }
    ...
    enum ValkeyClientSupplier implements Supplier<RedisClient> {
        ...
        final Lazy<RedisClient> lazy =
                Lazy.of(
                        () -> {
                            RedisClient client =
                                    RedisClient.create(
                                            LettuceTestClientResources.getSharedClientResources(),
    ...
    enum ValkeyClusterClientSupplier implements Supplier<RedisClusterClient> {
        ...
        final Lazy<RedisClusterClient> lazy =
                Lazy.of(
                        () -> {
    ...
    enum StatefulValkeyConnectionSupplier
            implements Supplier<StatefulRedisConnection<String, String>> {
        INSTANCE;
    ...
    enum StatefulValkeyPubSubConnectionSupplier
            implements Supplier<StatefulRedisPubSubConnection<String, String>> {
        INSTANCE;
    ...
    enum StatefulValkeyClusterConnectionSupplier
            implements Supplier<StatefulRedisClusterConnection<String, String>> {
        INSTANCE;
