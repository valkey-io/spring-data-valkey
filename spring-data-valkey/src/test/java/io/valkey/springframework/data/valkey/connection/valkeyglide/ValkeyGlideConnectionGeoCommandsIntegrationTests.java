 ...
 *
 * <p>These tests validate the implementation of all ValkeyGeoCommands methods: - Basic geo
 * operations (geoAdd, geoRemove, geoPos, geoHash) - Distance calculations (geoDist) - Radius
 * searches (geoRadius, geoRadiusByMember) - Advanced search operations (geoSearch, geoSearchStore)
 * - Various argument combinations and edge cases - Error handling and validation
 *
 * @author Ilia Kolominsky
 * @since 2.0
 */
public class ValkeyGlideConnectionGeoCommandsIntegrationTests
    ...
    protected String[] getTestKeyPatterns() {
        return new String[] {
            "test:geo:add:key",
            "test:geo:add:map:key",
            "test:geo:add:iterable:key",
            "test:geo:dist:key",
            "test:geo:hash:key",
            "test:geo:pos:key",
            "test:geo:radius:key",
            "test:geo:radius:member:key",
            "test:geo:remove:key",
            "test:geo:search:key",
            "test:geo:search:store:key",
            "test:geo:search:dest:key",
            "test:geo:mixed:key",
            "test:geo:empty:key",
            "test:geo:validation:key",
            "test:geo:large:key",
            "test:geo:edge:cases:key",
            "test:geo:precision:key",
            // Pipeline mode test keys
            "test:pipeline:geo1",
            "test:pipeline:geo2",
            "test:pipeline:geo3",
            "test:pipeline:geosearch",
            "test:pipeline:geodest",
            "test:pipeline:mixed:1",
        ...
        try {
            ...
            List<Point> positions =
                    connection
                            .geoCommands()
                            .geoPos(
                                    key.getBytes(), "Palermo".getBytes(), "Catania".getBytes(), "NewYork".getBytes());
        ...
        try {
            ...
            List<Point> positions =
                    connection
                            .geoCommands()
                            .geoPos(
                                    key.getBytes(), "Palermo".getBytes(), "Catania".getBytes(), "Rome".getBytes());
            assertThat(positions).hasSize(3);
            assertThat(positions.get(0)).isNotNull();
            assertThat(positions.get(1)).isNotNull();
            assertThat(positions.get(2)).isNotNull();
    ...
    void testGeoRemove() {
        ...
        try {
            ...
            Long result2 =
                    connection
                            .geoCommands()
                            .geoRemove(key.getBytes(), "Catania".getBytes(), "Rome".getBytes());
    ...
    void testGeoPos() {
        ...
        try {
            ...
            List<Point> emptyPositions =
                    connection.geoCommands().geoPos(key.getBytes(), "NonExistent".getBytes());
    ...
    void testGeoHash() {
        ...
        try {
            ...
            List<String> emptyHashes =
                    connection.geoCommands().geoHash(key.getBytes(), "NonExistent".getBytes());
            ...
            List<String> singleHash =
                    connection.geoCommands().geoHash(key.getBytes(), "Palermo".getBytes());
    ...
    void testGeoDist() {
        ...
        try {
            ...
            assertThat(distTransAtlantic.getValue())
                    .isGreaterThan(1000); // Should be thousands of kilometers
        } finally {
            cleanupKey(key);
        }
    }
    ...
    void testGeoRadius() {
        ...
        try {
            ...
            GeoResults<GeoLocation<byte[]>> results1 =
            ...
            GeoResults<GeoLocation<byte[]>> results2 =
                    connection.geoCommands().geoRadius(key.getBytes(), searchArea, args);
    ...
    void testGeoSearch() {
        ...
        try {
            ...
            GeoResults<GeoLocation<byte[]>> results2 =
                    connection.geoCommands().geoSearch(key.getBytes(), coordRef, radiusShape, args);
            assertThat(results2).isNotNull();
            assertThat(results2.getContent()).isNotEmpty();
            ...
            GeoResults<GeoLocation<byte[]>> results3 =
                    connection.geoCommands().geoSearch(key.getBytes(), memberRef, boxShape, args);
    ...
    void testGeoSearchStore() {
        ...
        try {
            ...
            GeoSearchStoreCommandArgs args =
                    GeoSearchStoreCommandArgs.newGeoSearchStoreArgs().sortAscending().limit(10);
    ...
    void testEmptyKeyOperations() {
        ...
        try {
            ...
            GeoResults<GeoLocation<byte[]>> emptyRadius =
                    connection.geoCommands().geoRadius(key.getBytes(), searchArea);
            assertThat(emptyRadius).isNotNull();
            assertThat(emptyRadius.getContent()).isEmpty();

            Long emptyRemove =
                    connection.geoCommands().geoRemove(key.getBytes(), "NonExistent".getBytes());
        ...
        try {
            ...
            Distance dist =
                    connection
                            .geoCommands()
                            .geoDist(
                                    key.getBytes(), "Palermo".getBytes(), "Rome".getBytes(), DistanceUnit.KILOMETERS);
            ...
            List<Point> remainingPositions =
                    connection.geoCommands().geoPos(key.getBytes(), "Palermo".getBytes(), "Rome".getBytes());
    ...
    void testParameterValidation() {
        ...
        try {
            ...
            assertThatThrownBy(
                            () -> connection.geoCommands().geoAdd(key.getBytes(), null, "member".getBytes()))
                    .isInstanceOf(IllegalArgumentException.class);
            ...
            assertThatThrownBy(
                            () -> connection.geoCommands().geoPos(key.getBytes(), "member1".getBytes(), null))
                    .isInstanceOf(IllegalArgumentException.class);
    ...
    void testLargeDataset() {
        ...
        try {
            ...
            List<Point> positions =
                    connection.geoCommands().geoPos(key.getBytes(), "location50".getBytes());
            assertThat(positions).hasSize(1);
            assertThat(positions.get(0)).isNotNull();

            // Test radius search on large dataset
            Circle largeSearchArea =
                    new Circle(new Point(0, 0), new Distance(5000, DistanceUnit.KILOMETERS));
            GeoResults<GeoLocation<byte[]>> largeResults =
                    connection.geoCommands().geoRadius(key.getBytes(), largeSearchArea);
        ...
        try {
            ...
            Distance poleDistance =
                    connection
                            ...
                            .geoDist(
                                    key.getBytes(),
                                    "NearNorthPole".getBytes(),
                                    "NearSouthPole".getBytes(),
                                    DistanceUnit.KILOMETERS);
            ...
            GeoResults<GeoLocation<byte[]>> poleResults =
                    connection.geoCommands().geoRadius(key.getBytes(), poleSearch);
            assertThat(poleResults).isNotNull();
            assertThat(poleResults.getContent()).isNotEmpty();

            // Test distance across date line
            Distance dateLineDistance =
                    connection
                            .geoCommands()
                            .geoDist(
                                    key.getBytes(),
                                    "DateLine".getBytes(),
                                    "AntiMeridian".getBytes(),
                                    DistanceUnit.KILOMETERS);
    ...
    void testCoordinatePrecision() {
        ...
        try {
            ...
            List<Point> retrievedPositions =
                    connection.geoCommands().geoPos(key.getBytes(), "HighPrecision".getBytes());
            assertThat(retrievedPositions).hasSize(1);
            Point retrieved = retrievedPositions.get(0);
            assertThat(retrieved.getX())
                    .isCloseTo(highPrecision.getX(), within(0.01)); // More realistic tolerance
            ...
            Distance closeDistance =
                    connection
                            .geoCommands()
                            .geoDist(
                                    key.getBytes(), "Close1".getBytes(), "Close2".getBytes(), DistanceUnit.METERS);
            ...
            List<Point> closePositions =
                    connection.geoCommands().geoPos(key.getBytes(), "Close1".getBytes(), "Close2".getBytes());
            ...
            // Test with coordinates that have meaningful differences (larger difference for geohash
            // precision)
            Point precise1 = new Point(13.361389, 38.115556);
            Point precise2 =
                    new Point(13.362389, 38.115556); // Longitude differs by 0.001 degrees (~100m)
    ...
    void testAllGeoAddVariantsInPipelineMode() {
        ...
        try {
            ...
            List<Point> pos2 =
                    connection.geoCommands().geoPos(key2.getBytes(), "Rome".getBytes(), "Milan".getBytes());
    ...
    void testGeoRadiusVariantsInPipelineMode() {
        ...
        try {
            ...
            GeoResults<GeoLocation<byte[]>> result1 =
                    connection.geoCommands().geoRadius(key.getBytes(), searchArea);
            assertThat(result1).isNull(); // Should be null during pipeline
            ...
            GeoRadiusCommandArgs args =
                    GeoRadiusCommandArgs.newGeoRadiusArgs()
                            ...
                            .sortAscending()
                            .limit(10);
            GeoResults<GeoLocation<byte[]>> result2 =
                    connection.geoCommands().geoRadius(key.getBytes(), searchArea, args);
            assertThat(result2).isNull(); // Should be null during pipeline
    ...
    void testGeoRemoveInPipelineMode() {
        ...
        try {
            ...
            Long result2 =
                    connection
                            .geoCommands()
                            .geoRemove(key.getBytes(), "Palermo".getBytes(), "Rome".getBytes());
            assertThat(result2).isNull(); // Should be null during pipeline
        ...
        try {
            ...
            Long result =
                    connection
                            .geoCommands()
                            .geoSearchStore(
                                    destKey.getBytes(), sourceKey.getBytes(), memberRef, radiusShape, args);
            assertThat(result).isNull(); // Should be null during pipeline
    ...
    void testGeoOperationsInPipelineMode() {
        ...
        try {
            ...
            connection
                    .geoCommands()
                    .geoDist(
                            key1.getBytes(), "Palermo".getBytes(), "Catania".getBytes(), DistanceUnit.KILOMETERS);
            ...
            List<Point> removedPos =
                    connection.geoCommands().geoPos(key2.getBytes(), "NewYork".getBytes());
    ...
    void testMixedGeoOperationsInPipelineMode() {
        ...
        try {
            ...
            connection
                    .geoCommands()
                    .geoHash((baseKey + ":1").getBytes(), "Palermo".getBytes(), "Catania".getBytes());
            connection
                    .geoCommands()
    ...
    void testGeoSearchOperationsInTransactionMode() {
        ...
        try {
            ...
            connection
                    .geoCommands()
                    .geoRadiusByMember(key.getBytes(), "Palermo".getBytes(), radius, radiusArgs);

            // Test geoSearchStore if available
            GeoReference<byte[]> memberRef = GeoReference.fromMember("Palermo".getBytes());
            GeoShape radiusShape = GeoShape.byRadius(new Distance(300, DistanceUnit.KILOMETERS));
            GeoSearchStoreCommandArgs storeArgs =
                    GeoSearchStoreCommandArgs.newGeoSearchStoreArgs().limit(10);
            connection
                    .geoCommands()
            ...
            for (int i = 0; i < 2; i++) {
                if (results.get(i) != null) {
                    ...
                    @SuppressWarnings("unchecked")
                    GeoResults<GeoLocation<byte[]>> geoResults =
                            (GeoResults<GeoLocation<byte[]>>) results.get(i);
                    assertThat(geoResults.getContent()).isNotEmpty();
                }
            }
    ...
    void testTransactionWithWatchedGeoKeys() {
        ...
        try {
            ...
            List<Point> positions =
                    connection.geoCommands().geoPos(valueKey.getBytes(), "Catania".getBytes());
            assertThat(positions.get(0)).isNotNull();
        } finally {
    ...
    void testMixedGeoOperationsInTransaction() {
        ...
        try {
            ...
            List<Point> finalPos2 =
                    connection.geoCommands().geoPos((baseKey + ":2").getBytes(), "Rome".getBytes());
    ...
    void testGeoOperationsWithComplexTransactionScenario() {
        ...
        try {
            ...
            connection
                    .geoCommands()
                    .geoSearchStore(
                            targetKey.getBytes(), sourceKey.getBytes(), milanRef, searchArea, storeArgs);
            ...
            List<Point> targetPositions =
                    connection.geoCommands().geoPos(targetKey.getBytes(), "Milan".getBytes());
            assertThat(targetPositions.get(0)).isNotNull();

            assertThat(connection.stringCommands().get(logKey.getBytes()))
                    .isEqualTo("geo_search_completed".getBytes());
        } finally {
            cleanupKey(sourceKey);
