...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideConnection} stream functionality
 * using the ValkeyStreamCommands interface directly.
 *
 * <p>These tests systematically validate all ValkeyStreamCommands methods in three modes: 1.
 * IMMEDIATE MODE - Direct command execution 2. PIPELINE MODE - Batched command execution 3.
 * TRANSACTION MODE - Atomic command execution
 *
 * <p>Stream Commands Coverage: - Basic operations: xAdd, xDel, xLen, xTrim - Range queries: xRange,
 * xRevRange - Stream reading: xRead, xReadGroup - Consumer groups: xGroupCreate, xGroupDestroy,
 * xGroupDelConsumer - Acknowledgements: xAck - Message claiming: xClaim, xClaimJustId - Pending
 * messages: xPending (summary and detailed) - Stream information: xInfo, xInfoGroups,
 * xInfoConsumers - Error handling and edge cases
 *
 * @author Ilia Kolominsky
 * @since 2.0
 */
public class ValkeyGlideConnectionStreamCommandsIntegrationTests
    ...
    void testStreamBasicOperations() {
        ...
        try {
            // Test adding records
            Map<byte[], byte[]> fields1 =
                    Map.of(
                            "field1".getBytes(), "value1".getBytes(),
                            "field2".getBytes(), "value2".getBytes());

            MapRecord<byte[], byte[], byte[]> record1 =
                    StreamRecords.newRecord().in(streamKey.getBytes()).ofMap(fields1);
            ...
            MapRecord<byte[], byte[], byte[]> record2 =
                    StreamRecords.newRecord().in(streamKey.getBytes()).ofMap(fields2);
    ...
    void testStreamWithOptions() {
        ...
        try {
            ...
            MapRecord<byte[], byte[], byte[]> record =
                    StreamRecords.newRecord().in(streamKey.getBytes()).ofMap(fields);
            ...
            ValkeyStreamCommands.XAddOptions approxOptions =
                    ValkeyStreamCommands.XAddOptions.maxlen(10).approximateTrimming(true);
    ...
    void testStreamRangeOperations() {
        ...
        try {
            ...
            // Test reverse range
            List<ByteRecord> reverseRecords =
                    connection
                            .streamCommands()
    ...
    void testStreamRangeBoundaryOperations() {
        ...
        try {
            ...
            List<ByteRecord> inclusiveRange =
                    connection
                            .streamCommands()
                            .xRange(
                                    streamKey.getBytes(),
                                    Range.from(Range.Bound.inclusive(firstId.getValue()))
                                            .to(Range.Bound.inclusive(middleId.getValue())),
                                    Limit.unlimited());
            ...
            List<ByteRecord> exclusiveLowerRange =
                    connection
                            .streamCommands()
                            .xRange(
                                    streamKey.getBytes(),
                                    Range.from(Range.Bound.exclusive(firstId.getValue()))
                                            .to(Range.Bound.inclusive(middleId.getValue())),
                                    Limit.unlimited());
            ...
            List<ByteRecord> exclusiveUpperRange =
                    connection
                            .streamCommands()
                            .xRange(
                                    streamKey.getBytes(),
                                    Range.from(Range.Bound.inclusive(firstId.getValue()))
                                            .to(Range.Bound.exclusive(middleId.getValue())),
                                    Limit.unlimited());
            ...
            List<ByteRecord> bothExclusiveRange =
                    connection
                            .streamCommands()
                            .xRange(
                                    streamKey.getBytes(),
            ...
            List<ByteRecord> inclusiveRevRange =
                    connection
                            .streamCommands()
                            .xRevRange(
                                    streamKey.getBytes(),
                                    Range.from(Range.Bound.inclusive(firstId.getValue()))
                                            .to(Range.Bound.inclusive(lastId.getValue())),
                                    Limit.unlimited());
            assertThat(inclusiveRevRange).hasSize(3);
            assertThat(inclusiveRevRange.get(0).getId())
                    .isEqualTo(lastId); // Last record first in reverse
            assertThat(inclusiveRevRange.get(1).getId()).isEqualTo(middleId); // Middle record
            assertThat(inclusiveRevRange.get(2).getId())
                    .isEqualTo(firstId); // First record last in reverse

            // Exclusive reverse range - exclude first record
            List<ByteRecord> exclusiveRevRange =
                    connection
                            .streamCommands()
                            .xRevRange(
                                    streamKey.getBytes(),
            ...
            List<ByteRecord> exclusiveUpperRevRange =
                    connection
                            .streamCommands()
                            .xRevRange(
                                    streamKey.getBytes(),
                                    Range.from(Range.Bound.inclusive(firstId.getValue()))
                                            .to(Range.Bound.exclusive(lastId.getValue())),
                                    Limit.unlimited());
            assertThat(exclusiveUpperRevRange).hasSize(2);
            assertThat(exclusiveUpperRevRange.get(0).getId())
                    .isEqualTo(middleId); // Middle record first in reverse
            assertThat(exclusiveUpperRevRange.get(1).getId())
                    .isEqualTo(firstId); // First record second in reverse
    ...
    void testConsumerGroupOperations() {
        ...
        try {
            ...
            Boolean notDestroyed =
                    connection.streamCommands().xGroupDestroy(streamKey.getBytes(), "non-existent");
    ...
    void testConsumerManagement() {
        ...
        try {
            ...
            if (!records.isEmpty()) {
                ...
                Boolean deleted =
                        connection.streamCommands().xGroupDelConsumer(streamKey.getBytes(), consumer);
            ...
            Boolean notDeleted =
                    connection.streamCommands().xGroupDelConsumer(streamKey.getBytes(), nonExistentConsumer);
    ...
    void testStreamGroupReading() {
        ...
        try {
            ...
            if (!records.isEmpty()) {
                ...
                Long ackCount2 =
                        connection.streamCommands().xAck(streamKey.getBytes(), groupName, recordId);
        ...
        try {
            ...
            if (!records.isEmpty()) {
                ...
                PendingMessages pending =
                        connection.streamCommands().xPending(streamKey.getBytes(), groupName, consumerName);
                ...
                PendingMessages pendingWithOptions =
                        connection
                                .streamCommands()
                                .xPending(
                                        streamKey.getBytes(),
    ...
    void testErrorHandling() {
        ...
        try {
            ...
            assertThatThrownBy(
                            () -> connection.streamCommands().xGroupCreate(null, "group", ReadOffset.latest()))
                    .isInstanceOf(IllegalArgumentException.class);

            assertThatThrownBy(
                            () ->
                                    connection.streamCommands().xAck(streamKey.getBytes(), null, RecordId.of("1-0")))
                    .isInstanceOf(IllegalArgumentException.class);
            ...
            List<ByteRecord> revRangeRecords =
                    connection
                            .streamCommands()
            ...
            assertThatThrownBy(
                            () ->
                                    connection
                                            .streamCommands()
                                            .xClaim(
                                                    "non:existent".getBytes(),
                                                    "group",
                                                    "consumer",
                                                    ValkeyStreamCommands.XClaimOptions.minIdle(Duration.ofMillis(0))
                                                            .ids(RecordId.of("0-0"))))
                    .isInstanceOf(ValkeySystemException.class)
                    .hasMessageContaining("NOGROUP");
    ...
    void testStreamRangeOperationsPipeline() {
        ...
        try {
            ...
            List<ByteRecord> rangeResult =
                    connection
                            .streamCommands()
                            .xRange(streamKey.getBytes(), Range.unbounded(), Limit.unlimited());
            assertThat(rangeResult).isNull();

            List<ByteRecord> revRangeResult =
                    connection
                            .streamCommands()
    ...
    void testComplexStreamScenario() {
        ...
        try {
            ...
            PendingMessagesSummary summaryBefore =
                    connection.streamCommands().xPending(streamKey.getBytes(), groupName);
            ...
            Long ackCount =
                    connection
                            .streamCommands()
                            .xAck(streamKey.getBytes(), groupName, records1.get(0).getId());
            assertThat(ackCount).isEqualTo(1L);

            // Verify pending messages after acknowledgment - consumer1: 1 pending, consumer2: 2 pending,
            // total: 3 pending
            PendingMessagesSummary summary =
                    connection.streamCommands().xPending(streamKey.getBytes(), groupName);
    ...
    void testMultipleStreamsReading() {
        ...
        try {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                connection
                        .streamCommands()
                        .xAdd(StreamRecords.newRecord().in(stream1.getBytes()).ofMap(fields1));

                connection
                        .streamCommands()
                        .xAdd(StreamRecords.newRecord().in(stream2.getBytes()).ofMap(fields2));
            }
