...
public class ReactiveValkeyMessageListenerContainerIntegrationTests {
     ...
     */
    public ReactiveValkeyMessageListenerContainerIntegrationTests(
            LettuceConnectionFactory connectionFactory, String label) {
    ...
    void shouldReceiveChannelMessages() {
        ...
        ReactiveValkeyMessageListenerContainer container =
                new ReactiveValkeyMessageListenerContainer(connectionFactory);

        container
                .receiveLater(Topic.channel(CHANNEL1)) //
    ...
    void receiveChannelShouldNotifySubscriptionListener() throws Exception {
        ...
        container
                .receive(Collections.singletonList(Topic.channel(CHANNEL1)), listener) //
                .as(StepVerifier::create) //
    ...
    void shouldReceivePatternMessages() {
        ...
        container
                .receiveLater(PatternTopic.of(PATTERN1)) //
                .doOnNext(it -> doPublish(CHANNEL1.getBytes(), MESSAGE.getBytes()))
                .flatMapMany(Function.identity()) //
                .as(StepVerifier::create) //
                .assertNext(
    ...
    void receivePatternShouldNotifySubscriptionListener() throws Exception {
        ...
        container
                .receive(Collections.singletonList(Topic.pattern(PATTERN1)), listener) //
    ...
    void shouldPublishAndReceiveMessage() throws Exception {
        ...
        Disposable subscription =
                container
                        .receiveLater(PatternTopic.of(PATTERN1))
                        .doOnNext(it -> subscribed.complete(null))
                        .flatMapMany(Function.identity())
        ...
        template
                .convertAndSend(CHANNEL1, MESSAGE)
                .as(StepVerifier::create) //
                .expectNextCount(1) //
                .verifyComplete();
    ...
    void multipleListenShouldTrackSubscriptions() throws Exception {
        ...
        Flux<? extends ReactiveSubscription.Message<String, String>> c1 =
                container.receiveLater(Topic.channel(CHANNEL1)).block();
        Flux<? extends ReactiveSubscription.Message<String, String>> c1p1 =
                container
                        .receiveLater(
        ...
        BlockingQueue<ReactiveSubscription.Message<String, String>> c1Collector =
                new LinkedBlockingDeque<>();
        BlockingQueue<ReactiveSubscription.Message<String, String>> c2Collector =
                new LinkedBlockingDeque<>();
    ...
    private void doPublish(byte[] channel, byte[] message) {
        reactiveConnection
                .pubSubCommands()
                .publish(ByteBuffer.wrap(channel), ByteBuffer.wrap(message))
                .subscribe();
    }

    private static Runnable awaitSubscription(
            Supplier<Collection<ReactiveSubscription>> activeSubscriptions) {
