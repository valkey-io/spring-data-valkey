...
public class DefaultReactiveHashOperationsIntegrationTests<K, HK, HV> {
    ...
    public DefaultReactiveHashOperationsIntegrationTests(
            ReactiveValkeyTemplate<K, ?> valkeyTemplate,
            ObjectFactory<K> keyFactory,
            ObjectFactory<HK> hashKeyFactory,
            ObjectFactory<HV> hashValueFactory) {
    ...
    public static Collection<Object[]> testParams() {
        ...
        lettuceConnectionFactory.setClientResources(
        ...
        ValkeySerializationContext<String, String> serializationContext =
                ValkeySerializationContext.fromSerializer(StringValkeySerializer.UTF_8);
        ReactiveValkeyTemplate<String, String> stringTemplate =
                new ReactiveValkeyTemplate<>(lettuceConnectionFactory, serializationContext);

        ReactiveValkeyTemplate<byte[], byte[]> rawTemplate =
                new ReactiveValkeyTemplate(lettuceConnectionFactory, ValkeySerializationContext.raw());

        return Arrays.asList(
                new Object[][] {
                    {stringTemplate, stringFactory, stringFactory, stringFactory, "String"},
    ...
    void hasKey() {
        ...
        hashOperations
                .hasKey(key, hashKeyFactory.instance()) //
                .as(StepVerifier::create) //
                .expectNext(false) //
                .verifyComplete();
    }
    ...
    void getAbsentKey() {
        ...
        hashOperations
                .get(keyFactory.instance(), hashKeyFactory.instance())
                .as(StepVerifier::create) //
                .verifyComplete();
    }
    ...
    void multiGet() {
        ...
        assumeThat(
                        hashKeyFactory instanceof StringObjectFactory
                                && hashValueFactory instanceof StringObjectFactory)
                .isTrue();
        ...
        hashOperations
                .multiGet(key, Arrays.asList(hashkey1, hashkey2))
                .as(StepVerifier::create) //
                .consumeNextWith(
                        actual -> {
    ...
    void multiGetAbsentKeys() {
        ...
        hashOperations
                .multiGet(
                        keyFactory.instance(),
                        Arrays.asList(hashKeyFactory.instance(), hashKeyFactory.instance()))
                .as(StepVerifier::create) //
                .consumeNextWith(
                        actual -> {
    ...
    void randomField() {
        ...
        hashOperations
                .put(key, hashkey1, hashvalue1) //
                .as(StepVerifier::create) //
                .expectNext(true) //
                .verifyComplete();

        hashOperations
                .put(key, hashkey2, hashvalue2) //
                .as(StepVerifier::create) //
                .expectNext(true) //
                .verifyComplete();
    ...
    void putIfAbsent() {
        ...
        hashOperations
                .putIfAbsent(key, hashkey, hashvalue) //
                .as(StepVerifier::create) //
                .expectNext(true) //
                .verifyComplete();

        hashOperations
                .putIfAbsent(key, hashkey, hashvalue2) //
                .as(StepVerifier::create) //
                .expectNext(false) //
                .verifyComplete();
    }
    ...
    void testExpireAndGetExpireMillis() {
        ...
        hashOperations
                .expire(key, Duration.ofMillis(1500), List.of(key1)) //
                .as(StepVerifier::create) //
                .assertNext(
                        changes -> {
        ...
        hashOperations
                .getTimeToLive(key, List.of(key1)) //
                .as(StepVerifier::create) //
                .assertNext(
                        it -> {
    ...
    void testExpireWithOptions() {
        ...
        hashOperations
                .expire(
                        key,
                        io.valkey.springframework.data.valkey.core.types.Expiration.seconds(20),
        ...
        hashOperations
                .expire(
                        key,
                        io.valkey.springframework.data.valkey.core.types.Expiration.seconds(60),
                        ExpirationOptions.none(),
                        List.of(key2))
                .as(StepVerifier::create) //
                .assertNext(
        ...
        hashOperations
                .expire(
                        key,
                        io.valkey.springframework.data.valkey.core.types.Expiration.seconds(30),
    ...
    void testExpireAndGetExpireSeconds() {
        ...
        hashOperations
                .expire(key, Duration.ofSeconds(5), List.of(key1, key2)) //
                .as(StepVerifier::create) //
                .assertNext(
                        changes -> {
                            assertThat(changes.allOk()).isTrue();
        ...
        hashOperations
                .getTimeToLive(key, TimeUnit.SECONDS, List.of(key1, key2)) //
                .as(StepVerifier::create) //
                .assertNext(
                        it -> {
    ...
    void testExpireAtAndGetExpireMillis() {
        ...
        valkeyTemplate
                .opsForHash()
                .expireAt(key, Instant.now().plusMillis(1500), List.of(key1, key2))
                .as(StepVerifier::create) //
                .assertNext(
                        changes -> {
                            assertThat(changes.allOk()).isTrue();
        ...
        valkeyTemplate
                .opsForHash()
                .getTimeToLive(key, List.of(key1, key2))
                .as(StepVerifier::create) //
                .assertNext(
                        it -> {
                            assertThat(it.expirationOf(key1).raw()).isBetween(0L, 2L);
