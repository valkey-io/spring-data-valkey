...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideConnection} set functionality
 * using the ValkeySetCommands interface directly.
 *
 * <p>These tests validate the implementation of all ValkeySetCommands methods: - Basic set
 * operations (sAdd, sRem, sCard, sMembers) - Set membership operations (sIsMember, sMIsMember) -
 * Random operations (sRandMember, sPop) - Set movement operations (sMove) - Set algebra operations
 * (sDiff, sInter, sUnion) - Set store operations (sDiffStore, sInterStore, sUnionStore) - Set
 * scanning operations (sScan) - Error handling and edge cases
 *
...
public class ValkeyGlideConnectionSetCommandsIntegrationTests
    ...
    protected String[] getTestKeyPatterns() {
        return new String[] {
            "test:set:add:key", "test:set:members:key", "test:set:mismember:key",
            ...
            "test:set:diff:key3", "test:set:diffstore:key1", "test:set:diffstore:key2",
                    "test:set:diffstore:dest",
            "test:set:inter:key1", "test:set:inter:key2", "test:set:inter:key3",
                    "test:set:interstore:key1",
            "test:set:interstore:key2", "test:set:interstore:dest", "test:set:union:key1",
                    "test:set:union:key2",
            "test:set:union:key3", "test:set:unionstore:key1", "test:set:unionstore:key2",
                    "test:set:unionstore:dest",
            "test:set:scan:key", "test:set:edge:key", "test:set:algebra:edge:key1",
    ...
    void testSMembersAndSIsMember() {
        ...
        try {
            ...
            Set<String> memberStrings =
                    members.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSMIsMember() {
        ...
        try {
            ...
            List<Boolean> results =
                    connection.setCommands().sMIsMember(key.getBytes(), value1, value2, value3, nonMember);
            ...
            List<Boolean> emptyResults =
                    connection.setCommands().sMIsMember(emptyKey.getBytes(), value1, value2);
    ...
    void testSRandMember() {
        ...
        try {
            ...
            List<String> randomMemberStrs =
                    randomMembers.stream().map(String::new).collect(java.util.stream.Collectors.toList());
            ...
            assertThat(negativeCount)
                    .hasSize(5); // Should return exactly 5 elements (with possible duplicates)
        } finally {
            cleanupKey(key);
        }
    }
    ...
    void testSMove() {
        ...
        try {
            ...
            Boolean moveResult1 =
                    connection.setCommands().sMove(srcKey.getBytes(), destKey.getBytes(), value1);
            ...
            Boolean moveResult2 =
                    connection.setCommands().sMove(srcKey.getBytes(), destKey.getBytes(), nonMember);
            assertThat(moveResult2).isFalse();

            // Test moving from non-existing set
            String nonExistentKey = "test:set:move:nonexistent";
            Boolean moveResult3 =
                    connection.setCommands().sMove(nonExistentKey.getBytes(), destKey.getBytes(), value2);
    ...
    void testSDiff() {
        ...
        try {
            ...
            connection
                    .setCommands()
                    .sAdd(key2.getBytes(), "b".getBytes(), "c".getBytes(), "e".getBytes());
            // key3: {d, f}
            ...
            Set<String> diff1Strings =
                    diff1.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(diff1Strings).containsExactlyInAnyOrder("a", "d");

            // Test diff of key1 - key2 - key3 (should be {a})
            Set<byte[]> diff2 =
                    connection.setCommands().sDiff(key1.getBytes(), key2.getBytes(), key3.getBytes());
            Set<String> diff2Strings =
                    diff2.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            ...
            Set<byte[]> diff3 =
                    connection.setCommands().sDiff(key1.getBytes(), nonExistentKey.getBytes());
            Set<String> diff3Strings =
                    diff3.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSDiffStore() {
        ...
        try {
            ...
            connection
                    .setCommands()
                    .sAdd(key1.getBytes(), "a".getBytes(), "b".getBytes(), "c".getBytes());
            connection
                    .setCommands()
                    .sAdd(key2.getBytes(), "b".getBytes(), "c".getBytes(), "d".getBytes());

            // Test sDiffStore
            Long storeResult =
                    connection.setCommands().sDiffStore(destKey.getBytes(), key1.getBytes(), key2.getBytes());
            ...
            Set<String> storedDiffStrings =
                    storedDiff.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSInter() {
        ...
        try {
            ...
            connection
                    .setCommands()
                    .sAdd(key3.getBytes(), "c".getBytes(), "d".getBytes(), "e".getBytes());
            ...
            Set<String> inter1Strings =
                    inter1.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(inter1Strings).containsExactlyInAnyOrder("b", "c");

            // Test intersection of all three (should be {c})
            Set<byte[]> inter2 =
                    connection.setCommands().sInter(key1.getBytes(), key2.getBytes(), key3.getBytes());
            Set<String> inter2Strings =
                    inter2.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            ...
            Set<byte[]> inter3 =
                    connection.setCommands().sInter(key1.getBytes(), nonExistentKey.getBytes());
    ...
    void testSInterStore() {
        ...
        try {
            ...
            Set<String> storedInterStrings =
                    storedInter.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSUnion() {
        ...
        try {
            ...
            Set<String> union1Strings =
                    union1.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(union1Strings).containsExactlyInAnyOrder("a", "b", "c");

            // Test union of all three (should be {a, b, c, d})
            Set<byte[]> union2 =
                    connection.setCommands().sUnion(key1.getBytes(), key2.getBytes(), key3.getBytes());
            Set<String> union2Strings =
                    union2.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            ...
            Set<byte[]> union3 =
                    connection.setCommands().sUnion(key1.getBytes(), nonExistentKey.getBytes());
            Set<String> union3Strings =
                    union3.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSUnionStore() {
        ...
        try {
            ...
            Set<String> storedUnionStrings =
                    storedUnion.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSetOperationsInPipelineMode() {
        ...
        try {
            ...
            Set<String> memberStrings =
                    membersResult.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            ...
            Set<String> diffStrings =
                    diffResult.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSetOperationsWithMultipleValuesPipeline() {
        ...
        try {
            ...
            Long sRemResult =
                    connection.setCommands().sRem(key.getBytes(), "a".getBytes(), "b".getBytes());
            assertThat(sRemResult).isNull();
    ...
    void testSetOperationsInTransactionMode() {
        ...
        try {
            ...
            Set<String> interStrings =
                    interResult.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
    ...
    void testSetAlgebraOperationsTransaction() {
        ...
        try {
            ...
            Set<String> unionStrings =
                    unionResult.stream().map(String::new).collect(java.util.stream.Collectors.toSet());
            assertThat(unionStrings)
                    .containsExactlyInAnyOrder("a", "b", "c", "d"); // {a,b} ∪ {b,c} ∪ {c,d}
    ...
    void testSetMoveOperationTransaction() {
        ...
        try {
            ...
            connection
                    .setCommands()
                    .sAdd(srcKey.getBytes(), "a".getBytes(), "b".getBytes(), "c".getBytes());
            ...
            Boolean sMoveResult1 =
                    connection.setCommands().sMove(srcKey.getBytes(), destKey.getBytes(), "a".getBytes());
            assertThat(sMoveResult1).isNull();

            Boolean sMoveResult2 =
                    connection
                            .setCommands()
    ...
    void testMixedSetOperationsPipelineIndexCorrelation() {
        ...
        try {
            ...
            Long addResult =
                    connection.setCommands().sAdd(key1.getBytes(), "c".getBytes()); // Index 0: Long
            Boolean memberResult =
                    connection.setCommands().sIsMember(key1.getBytes(), "b".getBytes()); // Index 1: Boolean
            Set<byte[]> membersResult =
                    connection.setCommands().sMembers(key1.getBytes()); // Index 2: Set<byte[]>
            Long cardResult = connection.setCommands().sCard(key1.getBytes()); // Index 3: Long
            List<Boolean> multiMemberResult =
                    connection
                            .setCommands()
                            .sMIsMember(
                                    key1.getBytes(), "a".getBytes(), "d".getBytes()); // Index 4: List<Boolean>
            byte[] popResult = connection.setCommands().sPop(key1.getBytes()); // Index 5: byte[]
            Long remResult =
                    connection.setCommands().sRem(key1.getBytes(), "b".getBytes()); // Index 6: Long
    ...
    void testComplexSetOperationsTransactionIndexCorrelation() {
        ...
        try {
            ...
            for (int i = 1; i <= 3; i++) {
                connection
                        .setCommands()
                        .sAdd((baseKey + i).getBytes(), ("member" + i).getBytes(), "common".getBytes());
            }
            ...
            Long add1 =
                    connection.setCommands().sAdd((baseKey + "1").getBytes(), "new1".getBytes()); // Index 0
            Long add2 =
                    connection.setCommands().sAdd((baseKey + "2").getBytes(), "new2".getBytes()); // Index 1
            Long add3 =
                    connection.setCommands().sAdd((baseKey + "3").getBytes(), "new3".getBytes()); // Index 2

            Set<byte[]> diff12 =
                    connection
                            .setCommands()
                            .sDiff((baseKey + "1").getBytes(), (baseKey + "2").getBytes()); // Index 3
            Set<byte[]> inter123 =
                    connection
                            .setCommands()
                            .sInter(
                                    (baseKey + "1").getBytes(),
                                    (baseKey + "2").getBytes(),
                                    (baseKey + "3").getBytes()); // Index 4
            Set<byte[]> union12 =
                    connection
                            .setCommands()
                            .sUnion((baseKey + "1").getBytes(), (baseKey + "2").getBytes()); // Index 5

            Long diffStore =
                    connection
                            .setCommands()
                            .sDiffStore(
                                    (baseKey + "diff").getBytes(),
                                    (baseKey + "1").getBytes(),
                                    (baseKey + "3").getBytes()); // Index 6
            Long unionStore =
                    connection
                            .setCommands()
                            .sUnionStore(
                                    (baseKey + "union").getBytes(),
                                    (baseKey + "1").getBytes(),
                                    (baseKey + "2").getBytes(),
            ...
            assertThat(unionResult)
                    .hasSizeGreaterThanOrEqualTo(4); // At least: member1, member2, common, new1, new2
    ...
    void testSetPopAndRandomOperationsTransaction() {
        ...
        try {
            ...
            connection
                    .setCommands()
                    .sAdd(popKey.getBytes(), "pop1".getBytes(), "pop2".getBytes(), "pop3".getBytes());
            connection
                    .setCommands()
            ...
            List<byte[]> sRandMemberMultiResult =
                    connection.setCommands().sRandMember(randKey.getBytes(), 2);
            assertThat(sRandMemberMultiResult).isNull();
