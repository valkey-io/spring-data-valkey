...
public class StreamReceiverIntegrationTests {
    ...
    public StreamReceiverIntegrationTests(LettuceConnectionFactory connectionFactory) {
        ...
        ValkeySerializationContext<String, String> serializationContext =
                ValkeySerializationContext.<String, String>newSerializationContext(
                        ...
                        .hashKey(SerializationPair.raw())
                        .hashValue(SerializationPair.raw())
                        .build();

        this.reactiveValkeyTemplate =
                new ReactiveValkeyTemplate<>(connectionFactory, serializationContext);
    }
    ...
    void shouldReceiveMapRecords() {
        ...
        Flux<MapRecord<String, String, String>> messages =
                receiver.receive(StreamOffset.create("my-stream", ReadOffset.from("0-0")));

        messages
                .as(StepVerifier::create) //
                .then(
                        () ->
                                reactiveValkeyTemplate
                                        .opsForStream()
                                        .add("my-stream", Collections.singletonMap("key", "value"))
                                        .subscribe())
                .consumeNextWith(
                        it -> {
    ...
    void shouldReceiveSimpleObjectHashRecords() {
        ...
        StreamReceiverOptions<String, ObjectRecord<String, String>> receiverOptions =
                StreamReceiverOptions.builder().targetType(String.class).build();

        StreamReceiver<String, ObjectRecord<String, String>> receiver =
                StreamReceiver.create(connectionFactory, receiverOptions);

        Flux<ObjectRecord<String, String>> messages =
        ...
        messages
                ...
                .then(
                        () ->
                                reactiveValkeyTemplate
                                        .opsForStream()
                                        .add(ObjectRecord.create("my-stream", "foobar"))
                                        .subscribe())
                .consumeNextWith(
                        it -> {
                            assertThat(it.getStream()).isEqualTo("my-stream");
    ...
    void shouldReceiveObjectHashRecords() {
        ...
        StreamReceiverOptions<String, ObjectRecord<String, LoginEvent>> receiverOptions =
                StreamReceiverOptions.builder().targetType(LoginEvent.class).build();

        StreamReceiver<String, ObjectRecord<String, LoginEvent>> receiver =
                StreamReceiver.create(connectionFactory, receiverOptions);

        Flux<ObjectRecord<String, LoginEvent>> messages =
                receiver.receive(StreamOffset.fromStart("my-logins"));
        ...
        messages
                ...
                .then(
                        () ->
                                reactiveValkeyTemplate
                                        .opsForStream()
                                        .add(ObjectRecord.create("my-logins", new LoginEvent("Walter", "White")))
                                        .subscribe())
                .consumeNextWith(
                        it -> {
    ...
    void shouldReceiveCustomHashValueRecords() {
        ...
        StreamReceiverOptions<String, MapRecord<String, String, Integer>> receiverOptions =
                StreamReceiverOptions.builder()
                        .<String, Integer>hashValueSerializer(serializationPair)
                        .build();

        StreamReceiver<String, MapRecord<String, String, Integer>> receiver =
                StreamReceiver.create(connectionFactory, receiverOptions);

        Flux<MapRecord<String, String, Integer>> messages =
                receiver.receive(StreamOffset.fromStart("my-stream"));
        ...
        messages
                ...
                .then(
                        () ->
                                reactiveValkeyTemplate
                                        .opsForStream()
                                        .add("my-stream", Collections.singletonMap("Jesse", "Pinkman"))
                                        .subscribe())
                .consumeNextWith(
                        it -> {
                            assertThat(it.getStream()).isEqualTo("my-stream");
    ...
    void latestModeLosesMessages() {
        ...
        // XADD/XREAD highly timing-dependent as this tests require a poll subscription to receive
        // messages using $ offset.

        StreamReceiverOptions<String, MapRecord<String, String, String>> options =
                StreamReceiverOptions.builder().pollTimeout(Duration.ofSeconds(4)).build();
        StreamReceiver<String, MapRecord<String, String, String>> receiver =
                StreamReceiver.create(connectionFactory, options);

        Flux<MapRecord<String, String, String>> messages =
                receiver.receive(StreamOffset.create("my-stream", ReadOffset.latest()));
        ...
        messages
                ...
                .then(
                        () -> {
                            reactiveValkeyTemplate
                                    .opsForStream()
                                    .add("my-stream", Collections.singletonMap("key", "value1"))
                                    .subscribe();
                        }) //
                .expectNextCount(1) //
                .then(
                        () -> {
                            reactiveValkeyTemplate
                                    .opsForStream()
                                    .add("my-stream", Collections.singletonMap("key", "value2"))
                                    .subscribe();
                        }) //
                .thenRequest(1) //
                .thenAwait(Duration.ofMillis(500)) //
                .then(
                        () -> {
                            reactiveValkeyTemplate
                                    .opsForStream()
                                    .add("my-stream", Collections.singletonMap("key", "value3"))
                                    .subscribe();
                        })
                .consumeNextWith(
                        it -> {
    ...
    void shouldReceiveAsConsumerGroupMessages() {
        ...
        Flux<MapRecord<String, String, String>> messages =
                receiver.receive(
                        Consumer.from("my-group", "my-consumer-id"),
    ...
    void shouldStopReceivingOnError() {
        ...
        StreamReceiverOptions<String, MapRecord<String, String, String>> options =
                StreamReceiverOptions.builder().pollTimeout(Duration.ofMillis(100)).build();
    ...
    void shouldResumeFromError() {
        ...
        StreamReceiverOptions<String, ObjectRecord<String, Long>> options =
                StreamReceiverOptions.builder()
        ...
        StreamReceiver<String, ObjectRecord<String, Long>> receiver =
                StreamReceiver.create(connectionFactory, options);

        Flux<ObjectRecord<String, Long>> messages =
                receiver.receive(StreamOffset.fromStart("my-stream"));
        ...
        assertThat(((ConversionFailedException) ref.get()).getValue())
                .isInstanceOf(ByteBufferRecord.class);
    }
