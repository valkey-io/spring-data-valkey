...
public class DefaultReactiveStreamOperationsIntegrationTests<K, HK, HV> {
    ...
    public DefaultReactiveStreamOperationsIntegrationTests(Fixture<K, HV> fixture) {
        ...
        if (fixture.getSerializer() != null) {
            context =
                    ValkeySerializationContext.newSerializationContext()
                            ...
                            .hashKey(
                                    keyFactory instanceof PersonObjectFactory
                                            ? ValkeySerializer.java()
        ...
        this.streamOperations =
                fixture.getSerializer() != null
                        ? valkeyTemplate.opsForStream(context)
    ...
    void addShouldAddReadSimpleMessage() {
        ...
        assumeTrue(
                !(serializer instanceof Jackson2JsonValkeySerializer)
                        && !(serializer instanceof GenericJackson2JsonValkeySerializer)
                        && !(serializer instanceof JdkSerializationValkeySerializer)
                        && !(serializer instanceof OxmSerializer));
        ...
        streamOperations
                .range((Class<HV>) value.getClass(), key, Range.unbounded())
                .as(StepVerifier::create) //
                .consumeNextWith(
                        it -> {
    ...
    void addShouldAddReadSimpleMessageWithRawSerializer() {
        ...
        SerializationPair<K> keySerializer =
                valkeyTemplate.getSerializationContext().getKeySerializationPair();

        ValkeySerializationContext<K, String> serializationContext =
                ValkeySerializationContext.<K, String>newSerializationContext(StringValkeySerializer.UTF_8)
                        .key(keySerializer)
                        .hashValue(SerializationPair.raw())
                        .hashKey(SerializationPair.raw())
                        .build();

        ReactiveValkeyTemplate<K, String> raw =
                new ReactiveValkeyTemplate<>(valkeyTemplate.getConnectionFactory(), serializationContext);
    ...
    void addMaxLenShouldLimitMessagesSize() {
        ...
        RecordId messageId =
                streamOperations.add(key, Collections.singletonMap(hashKey, newValue), options).block();
    ...
    void addMaxLenShouldLimitSimpleMessagesSize() {
        ...
        RecordId messageId =
                streamOperations
                        .add(StreamRecords.objectBacked(newValue).withStreamKey(key), options)
                        .block();
    ...
    void rangeShouldReportMessages() {
        ...
        streamOperations
                .range(
                        key,
                        Range.from(Bound.inclusive(messageId1.getValue()))
                                .to(Bound.inclusive(messageId2.getValue())),
                        Limit.limit().count(1)) //
                .as(StepVerifier::create) //
                .consumeNextWith(
                        actual -> {
    ...
    void reverseRangeShouldConvertSimpleMessages() {
        ...
        streamOperations
                .reverseRange((Class<HV>) value.getClass(), key, Range.unbounded())
                .as(StepVerifier::create)
                .consumeNextWith(it -> assertThat(it.getId()).isEqualTo(messageId2))
                .consumeNextWith(it -> assertThat(it.getId()).isEqualTo(messageId1))
                .verifyComplete();
    }
    ...
    void readShouldReadMessage() {
        ...
        streamOperations
                .read(StreamOffset.create(key, ReadOffset.from("0-0"))) //
                .as(StepVerifier::create) //
                .consumeNextWith(
                        actual -> {
                            assertThat(actual.getId()).isEqualTo(messageId);
    ...
    void sizeShouldReportStreamSize() {
        ...
        streamOperations
                .add(key, Collections.singletonMap(hashKey, value))
                .as(StepVerifier::create)
                .expectNextCount(1)
                .verifyComplete();
        streamOperations
    ...
    void pendingShouldReadMessageDetails() {
        ...
        streamOperations
                .pending(key, "my-group", Range.unbounded(), 10L)
                .as(StepVerifier::create)
                .assertNext(
                        pending -> {
    ...
    void claimShouldReadMessageDetails() {
        ...
        streamOperations
                .claim(key, "my-group", "name", Duration.ZERO, messageId)
                .as(StepVerifier::create)
                .assertNext(
                        claimed -> {
