...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideConnection} hash functionality
 * using the ValkeyHashCommands interface directly.
 *
 * <p>These tests validate the implementation of all ValkeyHashCommands methods: - Basic hash
 * operations (hSet, hSetNX, hGet, hMGet, hMSet) - Increment/decrement operations (hIncrBy for long
 * and double) - Existence and deletion operations (hExists, hDel) - Structural operations (hLen,
 * hKeys, hVals, hGetAll) - Random field operations (hRandField variants) - Scanning operations
 * (hScan) - Field length operations (hStrLen) - Field expiration operations (hExpire, hpExpire,
 * hExpireAt, hpExpireAt, hPersist, hTtl, hpTtl)
 *
...
public class ValkeyGlideConnectionHashCommandsIntegrationTests
    ...
    void testHSetAndHGet() {
        ...
        try {
            ...
            byte[] nonExistentKeyValue =
                    connection.hashCommands().hGet("non:existent:key".getBytes(), field1);
    ...
    void testHMGetAndHMSet() {
        ...
        try {
            ...
            List<byte[]> values =
                    connection.hashCommands().hMGet(keyBytes, field1, field2, field3, field4);
            ...
            List<byte[]> nonExistentValues =
                    connection.hashCommands().hMGet("non:existent:key".getBytes(), field1);
    ...
    void testHExistsAndHDel() {
        ...
        try {
            ...
            Boolean existsNonKey =
                    connection.hashCommands().hExists("non:existent:key".getBytes(), field1);
    ...
    void testHLenHKeysHValsHGetAll() {
        ...
        try {
            ...
            // Since byte arrays are compared by reference, not content, we need to check the content
            // differently
    ...
    void testHRandFieldWithValues() {
        ...
        try {
            // Test on non-existent key
            Map.Entry<byte[], byte[]> nonExistentEntry =
                    connection.hashCommands().hRandFieldWithValues("non:existent:key".getBytes());
            ...
            Map.Entry<byte[], byte[]> randomEntry =
                    connection.hashCommands().hRandFieldWithValues(keyBytes);
            ...
            List<Map.Entry<byte[], byte[]>> randomEntries =
                    connection.hashCommands().hRandFieldWithValues(keyBytes, 2);
    ...
    void testHScan() {
        ...
        try {
            ...
            Cursor<Map.Entry<byte[], byte[]>> cursor =
                    connection.hashCommands().hScan(keyBytes, ScanOptions.NONE);
            ...
            Cursor<Map.Entry<byte[], byte[]>> patternCursor =
                    connection.hashCommands().hScan(keyBytes, patternOptions);
            ...
            Cursor<Map.Entry<byte[], byte[]>> countCursor =
                    connection.hashCommands().hScan(keyBytes, countOptions);
    ...
    void testHStrLen() {
        ...
        try {
            ...
            Long nonExistentLen =
                    connection.hashCommands().hStrLen("non:existent:key".getBytes(), field1);
            ...
            Long nonExistentFieldLen =
                    connection.hashCommands().hStrLen(keyBytes, "nonexistent".getBytes());
        ...
        try {
            ...
            expireResults =
                    connection
                            .hashCommands()
                            .hExpire(
                                    "non:existent:key".getBytes(), 10L, ExpirationOptions.Condition.ALWAYS, field1);
            assertThat(expireResults).hasSize(1);
            ...
            expireResults =
                    connection.hashCommands().hExpire(keyBytes, 10L, ExpirationOptions.Condition.NX, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(0);

            // Test hExpire (seconds) - Test XX
            expireResults =
                    connection.hashCommands().hExpire(keyBytes, 10L, ExpirationOptions.Condition.XX, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(1);

            // Test hExpire (seconds) - Test GT by setting a shorter expiry first
            expireResults =
                    connection.hashCommands().hExpire(keyBytes, 1L, ExpirationOptions.Condition.GT, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(0);

            // Test hExpire (seconds) - Test LT by setting a longer expiry
            expireResults =
                    connection.hashCommands().hExpire(keyBytes, 11L, ExpirationOptions.Condition.LT, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(0);
        } finally {
            cleanupKey(key);
    ...
    void testHpExpire() {
        ...
        try {
            ...
            expireResults =
                    connection
                            .hashCommands()
                            .hpExpire(keyBytes, 1000L, ExpirationOptions.Condition.GT, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(0);
        ...
        try {
            ...
            expireResults =
                    connection
                            .hashCommands()
                            .hExpireAt(
                                    keyBytes, nextExpirationTimestampMillis, ExpirationOptions.Condition.LT, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(0);
        } finally {
            cleanupKey(key);
        ...
        try {
            ...
            List<Long> expireResults =
                    connection
                            .hashCommands()
                            .hpExpireAt(
                                    keyBytes, expirationTimestamp, ExpirationOptions.Condition.ALWAYS, field1);
            assertThat(expireResults).hasSize(1);
            assertThat(expireResults.get(0)).isEqualTo(1);
    ...
    void testHPersist() {
        ...
        try {
            ...
            persistResults =
                    connection.hashCommands().hPersist(keyBytes, field1, field2, "nonexistent".getBytes());
            ...
            List<Long> nonExistentResults =
                    connection.hashCommands().hPersist(keyBytes, "nonexistent".getBytes());
            assertThat(nonExistentResults).hasSize(1);
            assertThat(nonExistentResults.get(0)).isEqualTo(-2);

            // Test on non-existent key
            List<Long> nonExistentKeyResults =
                    connection.hashCommands().hPersist("non:existent:key".getBytes(), field1);
    ...
    void testHTtl() {
        ...
        try {
            ...
            connection.hashCommands().hExpire(keyBytes, 10L, ExpirationOptions.Condition.ALWAYS, field1);
            ttlResults =
                    connection.hashCommands().hTtl(keyBytes, field1, field2, "nonexistent".getBytes());
            ...
            List<Long> ttlMillisResults =
                    connection.hashCommands().hTtl(keyBytes, TimeUnit.MILLISECONDS, field1, field2);
            assertThat(ttlMillisResults).hasSize(2);
            assertThat(ttlMillisResults.get(0))
                    .isGreaterThan(5000); // field1 should definitely have >5s left
            assertThat(ttlMillisResults.get(1))
                    .isGreaterThan(5000); // field2 should definitely have >5s left
    ...
    void testHpTtl() {
        ...
        try {
            ...
            connection.hashCommands().hExpire(keyBytes, 10L, ExpirationOptions.Condition.ALWAYS, field1);
            ttlResults =
                    connection.hashCommands().hpTtl(keyBytes, field1, field2, "nonexistent".getBytes());
            assertThat(ttlResults).hasSize(3);
        ...
        try {
            ...
            assertThatThrownBy(
                            () ->
                                    connection
                                            .hashCommands()
                                            .hSet(stringKey.getBytes(), "field".getBytes(), "value".getBytes()))
    ...
    void testHashRandomFieldCommandsInPipelineMode() {
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Map.Entry<byte[], byte[]>> multipleEntries =
                    (List<Map.Entry<byte[], byte[]>>) results.get(3);
    ...
    void testHashExpirationCommandsInPipelineMode() {
        ...
        try {
            ...
            assertThat(
                            connection
                                    .hashCommands()
                                    .hpExpire(keyBytes, 10000L, ExpirationOptions.Condition.ALWAYS, field2))
                    .isNull();
