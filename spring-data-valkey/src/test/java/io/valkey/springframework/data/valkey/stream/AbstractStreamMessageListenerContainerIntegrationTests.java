...
abstract class AbstractStreamMessageListenerContainerIntegrationTests {
    ...
    private final StreamMessageListenerContainerOptions<String, MapRecord<String, String, String>>
            containerOptions =
                    StreamMessageListenerContainerOptions.builder()
                            .pollTimeout(Duration.ofMillis(100))
                            .build();

    AbstractStreamMessageListenerContainerIntegrationTests(
    ...
    void shouldReceiveMapMessages() throws InterruptedException {
        ...
        Subscription subscription =
                container.receive(StreamOffset.create("my-stream", ReadOffset.from("0-0")), queue::add);
    ...
    void shouldReceiveSimpleObjectHashRecords() throws InterruptedException {
        ...
        assertThat(queue.poll(1, TimeUnit.SECONDS))
                .isNotNull()
    ...
    void shouldReceiveObjectHashRecords() throws InterruptedException {
        ...
        StreamMessageListenerContainerOptions<String, ObjectRecord<String, LoginEvent>>
                containerOptions =
                        StreamMessageListenerContainerOptions.builder()
                                .pollTimeout(Duration.ofMillis(100))
                                .targetType(LoginEvent.class)
                                .build();

        StreamMessageListenerContainer<String, ObjectRecord<String, LoginEvent>> container =
                StreamMessageListenerContainer.create(connectionFactory, containerOptions);
        ...
        valkeyTemplate
                .opsForStream()
                .add(ObjectRecord.create("my-stream", new LoginEvent("Walter", "White")));
    ...
    void errorShouldStopListening() throws InterruptedException {
        ...
        StreamReadRequest<String> readRequest =
                StreamReadRequest.builder(StreamOffset.create("my-stream", ReadOffset.lastConsumed()))
                        .errorHandler(failures::add)
    ...
    void deserializationShouldContinueStreamRead() throws InterruptedException {
        ...
        StreamMessageListenerContainerOptions<String, ObjectRecord<String, Long>> containerOptions =
                StreamMessageListenerContainerOptions.builder()
                        .batchSize(1)
                        .pollTimeout(Duration.ofMillis(100))
        ...
        StreamMessageListenerContainer<String, ObjectRecord<String, Long>> container =
                StreamMessageListenerContainer.create(connectionFactory, containerOptions);

        StreamReadRequest<String> readRequest =
                StreamReadRequest.builder(StreamOffset.create("my-stream", ReadOffset.from("0-0"))) //
                        .errorHandler(failures::add) //
                        .cancelOnError(t -> false) //
                        .build();
        ...
        assertThat(((ConversionFailedException) conversionFailure).getValue())
                .isInstanceOf(ByteRecord.class);
    ...
    private int getNumberOfPending(String stream, String group) {
            ...
            String value =
                    ((List)
                                    lettuce.execute(
                                            ...
                                            new byte[][] {stream.getBytes(), group.getBytes()}))
                            .get(0)
                            .toString();
        ...
        String value =
                ((List)
                                connectionFactory
                                        .getConnection()
                                        .execute("XPENDING", stream.getBytes(), group.getBytes()))
                        .get(0)
                        .toString();
        return NumberUtils.parseNumber(value, Integer.class);
    }
