...
/**
 * Transactional integration tests for {@link StringValkeyTemplate}.
 *
...
class TransactionalStringValkeyTemplateTests {

	private ValkeyConnectionFactory valkeyConnectionFactory;
	private StringValkeyTemplate stringTemplate;

	TransactionalStringValkeyTemplateTests(ValkeyConnectionFactory valkeyConnectionFactory) {
		this.valkeyConnectionFactory = valkeyConnectionFactory;

		if (valkeyConnectionFactory instanceof Lifecycle lifecycleBean) {
			lifecycleBean.start();
		}
	}
	...
	void beforeEach() {
		...
		stringTemplate = new StringValkeyTemplate(valkeyConnectionFactory);

		// explicitly enable transaction support
		stringTemplate.setEnableTransactionSupport(true);
		stringTemplate.afterPropertiesSet();

		stringTemplate.execute((ValkeyCallback) con -> {
	...
	@AfterEach
	void afterEach() {
		valkeyConnectionFactory.getConnection().serverCommands().flushAll();
	}
