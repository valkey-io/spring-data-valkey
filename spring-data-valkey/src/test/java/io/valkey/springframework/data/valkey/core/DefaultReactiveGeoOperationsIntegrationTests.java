...
public class DefaultReactiveGeoOperationsIntegrationTests<K, V> {
    ...
    void geoAddLocation() {
        ...
        geoOperations
                .add(key, new GeoLocation<>(value, POINT_PALERMO)) //
                .as(StepVerifier::create) //
                .expectNext(1L) //
                .expectComplete() //
                .verify();
    ...
    void geoAddIterableOfLocations() {
        ...
        List<GeoLocation<V>> geoLocations =
                Arrays.asList(
                        new GeoLocation<>(valueFactory.instance(), POINT_ARIGENTO),
    ...
    void geoAddPublisherOfLocations() {
        ...
        List<GeoLocation<V>> batch1 =
                Arrays.asList(
                        new GeoLocation<>(valueFactory.instance(), POINT_ARIGENTO),
                        new GeoLocation<>(valueFactory.instance(), POINT_PALERMO));

        List<GeoLocation<V>> batch2 =
                Arrays.asList(new GeoLocation<>(valueFactory.instance(), POINT_CATANIA));
                ...
                .consumeNextWith(
                        actual -> {
                            assertThat(actual.getValue())
                                    .isCloseTo(DISTANCE_PALERMO_CATANIA_METERS, offset(0.005));
    ...
    void geoDistShouldReturnDistanceInKilometersCorrectly() {
        ...
        geoOperations
                ...
                .consumeNextWith(
                        actual -> {
                            assertThat(actual.getValue())
                                    .isCloseTo(DISTANCE_PALERMO_CATANIA_KILOMETERS, offset(0.005));
    ...
    void geoRadiusByMemberWithin100KMShouldReturnLocations() {
        ...
        geoOperations
                .radius(key, member3, new Distance(100D, KILOMETERS))
                .as(StepVerifier::create) //
                .consumeNextWith(
                        actual -> {
                            assertThat(actual.getContent().getName()).isEqualTo(member3);
    ...
    void geoSearchShouldReturnLocationsWithDistance() {
        ...
        geoOperations
                .search(
                        key,
                        GeoReference.fromMember(member3),
                        GeoShape.byRadius(new Distance(100D, KILOMETERS)),
                        newGeoRadiusArgs().includeDistance().sortDescending())
                .as(StepVerifier::create) //
                .consumeNextWith(
                        actual -> {
    ...
    void geoSearchAndStoreShouldStoreItems() {
        ...
        geoOperations
                .searchAndStore(
                        key, destKey, GeoReference.fromMember(member3), new Distance(100D, KILOMETERS))
                .as(StepVerifier::create) //
                .expectNext(2L) //
                .verifyComplete();
