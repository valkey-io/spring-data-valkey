...
public class ValkeyGlideConnectionFactoryIntegrationTests {
    ...
    void testHashOperations() {
        ...
        try {
            ...
            assertThat(randomKeys)
                    .allMatch(key -> List.of("field1", "field2", "field3", "field4").contains(key));
                            ...
                            connection -> {
                                try (var cursor =
                                        connection
                                                ...
                                                .hScan(
                                                        key8.getBytes(),
                                                        io.valkey.springframework.data.valkey.core.ScanOptions.scanOptions()
                                                                .count(10)
                                                                .build())) {
                                    int count = 0;
                ...
                template.execute(
                        (ValkeyCallback<List<Long>>)
                                connection -> {
                                    ...
                                    assertThat(result)
                                            .containsExactly(1L, 1L); // Both fields should have expiration set
                                    return result;
                                });
                                ...
                                connection -> {
                                    List<Long> result =
                                            connection
                                                    .hashCommands()
                                                    .hpExpireAt(
                                                            key9.getBytes(), futureTimestampMillis, "expirefield2".getBytes());
                                    assertThat(result).containsExactly(1L); // Field should have expiration set
                                    return result;
                                });
                        ...
                        (ValkeyCallback<List<Long>>)
                                connection -> {
                                    ...
                                    assertThat(result.get(0))
                                            .isGreaterThan(0L); // expirefield1 should have TTL in milliseconds
                                    assertThat(result.get(1))
                                            .isEqualTo(-1L); // persistfield should have no expiration
                                    return result;
                                });
                                    ...
                                    List<Long> result =
                                            connection
                                                    ...
                                                    .hPersist(
                                                            ...
                                                            "expirefield2".getBytes());
                                    assertThat(result)
                                            .containsExactly(1L, 1L); // Both fields should have expiration removed
                                    return result;
                                });
                                    ...
                                    List<Long> result =
                                            connection
                                                    ...
                                                    .hTtl(
                                                            ...
                                                            "expirefield2".getBytes());
                                    assertThat(result)
                                            .containsExactly(-1L, -1L); // Both fields should have no expiration
                                    return result;
                                });
            }
        } finally {
    ...
    void testListOperations() {
        ...
        try {
            ...
            assertThat(leftPushAllResult1)
                    .containsExactly("third", "second", "first"); // Reverse order due to left push
            ...
            assertThat(leftPushAllResult2)
                    .containsExactly("c", "b", "a"); // Reverse order due to left push
                    ...
                    (ValkeyCallback<Long>)
                            connection ->
                                    connection
                                            ...
                                            .lInsert(
                                                    key4.getBytes(),
                                                    ValkeyListCommands.Position.BEFORE,
                                                    "pivot".getBytes(),
            ...
            String movedValue1 =
                    template
                            .opsForList()
                            .move(
                                    io.valkey.springframework.data.valkey.core.ListOperations.MoveFrom.fromTail(
                                            moveHighKey1),
                                    io.valkey.springframework.data.valkey.core.ListOperations.MoveTo.toHead(
                                            moveHighKey2));
            assertThat(movedValue1).isEqualTo("high3");
            assertThat(template.opsForList().range(moveHighKey1, 0, -1))
                    .containsExactly("high1", "high2");
            ...
            String movedValue2 =
                    template
                            .opsForList()
                            .move(
                                    io.valkey.springframework.data.valkey.core.ListOperations.MoveFrom.fromHead(
                                            moveHighKey1),
                                    io.valkey.springframework.data.valkey.core.ListOperations.MoveTo.toTail(
                                            moveHighKey2));
            assertThat(movedValue2).isEqualTo("high1");
            assertThat(template.opsForList().range(moveHighKey1, 0, -1)).containsExactly("high2");
            assertThat(template.opsForList().range(moveHighKey2, 0, -1))
                    .containsExactly("high3", "high1");
            ...
            Long removedCount =
                    template.opsForList().remove(key7, 2, "remove"); // Remove first 2 occurrences
            ...
            List<String> blockedPop =
                    template.execute(
                            (ValkeyCallback<List<String>>)
                                    connection -> {
            ...
            List<String> blockedRightPop =
                    template.execute(
                            (ValkeyCallback<List<String>>)
                                    connection -> {
                                        List<byte[]> result =
                                                connection.listCommands().bRPop(1, "temp_key2".getBytes());
                                        if (result != null && result.size() >= 2) {
                                            return List.of(new String(result.get(0)), new String(result.get(1)));
                                        }
                                        return null;
            ...
            String blockMoved =
                    template.execute(
                            (ValkeyCallback<String>)
                                    connection -> {
                                        byte[] result =
                                                connection
                                                        .listCommands()
                                                        .bRPopLPush(1, blockSrc.getBytes(), blockDst.getBytes());
    ...
    void testSetOperations() {
        ...
        try {
            ...
            if (isServerVersionAtLeast(6, 2)) {
                template.execute(
                        (ValkeyCallback<List<Boolean>>)
                                connection -> {
                                    Map<Object, Boolean> membershipMap =
                                            template.opsForSet().isMember(key1, "value1", "nonexistent", "value3");
            ...
            assertThat(template.opsForSet().members(destKey))
                    .containsExactlyInAnyOrder("a", "b", "c", "d");
            template.delete(destKey);

            // SUNIONSTORE with multiple keys
            Long unionStoreMultiResult =
                    template.opsForSet().unionAndStore(key2, List.of(key3, key4), destKey);
            ...
            Long intersectStoreMultiResult =
                    template.opsForSet().intersectAndStore(key2, List.of(key3, key4), destKey);
            ...
            Long diffStoreMultiResult =
                    template.opsForSet().differenceAndStore(key2, List.of(key3, key4), destKey);
            ...
            assertThat(randomMembers)
                    .allMatch(
                            member -> List.of("rand1", "rand2", "rand3", "rand4", "rand5").contains(member));
                    ...
                    (ValkeyCallback<Void>)
                            connection -> {
                                try (var cursor =
                                    ...
                                    assertThat(scannedMembers)
                                            .containsExactlyInAnyOrder("scan1", "scan2", "scan3", "scan4", "scan5");
                                }
                                return null;
                            });
            ...
            try (io.valkey.springframework.data.valkey.core.Cursor<String> cursor =
                ...
                assertThat(scannedHighMembers)
                        .containsExactlyInAnyOrder("high1", "high2", "high3", "high4", "high5");
            }
    ...
    void testStringOperations() {
        ...
        try {
            ...
            List<String> multiRetrieved =
                    template.opsForValue().multiGet(List.of(multi1Key, multi2Key, multi3Key));
            assertThat(multiRetrieved).containsExactly("value1", "value2", "value3");

            // MSETNX operation (set multiple if none exist)
            Map<String, String> msetnxValues =
                    Map.of(
            ...
            Boolean getBitResult =
                    template.execute(
                            (ValkeyCallback<Boolean>)
                                    connection -> connection.stringCommands().getBit(bitKey.getBytes(), 1));
            assertThat(getBitResult).isTrue(); // Second bit of 'a' is 1

            Boolean setBitResult =
                    template.execute(
                            (ValkeyCallback<Boolean>)
                                    connection -> connection.stringCommands().setBit(bitKey.getBytes(), 0, true));
            assertThat(setBitResult).isFalse(); // Original first bit was 0

            Long bitCountResult =
                    template.execute(
                            (ValkeyCallback<Long>)
                                    connection -> connection.stringCommands().bitCount(bitKey.getBytes()));
                            ...
                            (ValkeyCallback<Long>)
                                    connection ->
                                            connection
                                                    .stringCommands()
                                                    .bitOp(
                                                            ValkeyStringCommands.BitOperation.AND,
                                                            bitDestKey.getBytes(),
                                                            bitKey1.getBytes(),
                                                            bitKey2.getBytes()));
    ...
    /**
     * Checks if the server version is at least the specified major.minor version. Uses the INFO
     * server command to get server version information.
     */
                ...
                (ValkeyCallback<Boolean>)
                        connection -> {
                            ...
                            String versionString =
                                    serverInfo.getProperty("valkey_version", serverInfo.getProperty("redis_version"));
