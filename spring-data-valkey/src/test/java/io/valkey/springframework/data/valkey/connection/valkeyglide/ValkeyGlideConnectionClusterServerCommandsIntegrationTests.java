...
/**
 * Comprehensive low-level integration tests for {@link ValkeyGlideClusterConnection} cluster server
 * commands functionality using the ValkeyClusterServerCommands interface directly.
 *
 ...
 *
 * <ul>
 *   <li>Background operations (bgReWriteAof, bgSave, lastSave, save) on specific nodes
 *   <li>Database operations (dbSize, flushDb, flushAll) on specific nodes
 *   <li>Server information (info, time) from specific nodes
 *   <li>Configuration management (getConfig, setConfig, resetConfigStats, rewriteConfig) on
 *       specific nodes
 *   <li>Client management (getClientList) from specific nodes
 * </ul>
 *
 * <h3>Cluster-Wide Aggregation</h3>
 *
 * <ul>
 *   <li>Verify cluster-wide info() returns node-prefixed properties from all primaries
 *   <li>Verify cluster-wide dbSize() returns sum of all node sizes
 *   <li>Verify cluster-wide lastSave() returns most recent timestamp
 *   <li>Verify cluster-wide operations execute on all primaries
 * </ul>
 *
 * <p><strong>Note:</strong> Since ValkeyClusterServerCommands extends ValkeyServerCommands and we
 * already have ValkeyGlideConnectionServerCommandsIntegrationTests covering the base interface,
 * these tests focus on the NEW methods specific to cluster mode (those with ValkeyClusterNode
 * parameter) and verifying proper aggregation behavior of inherited methods.
 *
 * <p><strong>Reference Implementation:</strong> Uses JedisClusterServerCommands as the reference
 * for expected routing and aggregation behavior.
 *
 * <p><strong>Cluster Topology:</strong> Tests use ClusterTestVariables constants for cluster
 * configuration:
 *
 * <ul>
 *   <li>3 Master nodes: 7379, 7380, 7381
 *   <li>1 Replica node: 7382 (replica of 7379)
 * </ul>
 *
...
public class ValkeyGlideConnectionClusterServerCommandsIntegrationTests
    ...
    ValkeyClusterNode allNodes[] =
            new ValkeyClusterNode[] {CLUSTER_NODE_1, CLUSTER_NODE_2, CLUSTER_NODE_3, REPLICA_OF_NODE_1};

    ValkeyClusterNode allPrimaries[] =
            new ValkeyClusterNode[] {CLUSTER_NODE_1, CLUSTER_NODE_2, CLUSTER_NODE_3};

    @Override
    protected String[] getTestKeyPatterns() {
        return new String[] {
            "test:cluster:server:key1",
            "test:cluster:server:key2",
            "test:cluster:server:key3",
            "{user}:cluster:server:key1",
            "{user}:cluster:server:key2",
            "test:cluster:server:flush:key",
    ...
    void testDbSizeOnSpecificNode() {
        ...
        try {
            ...
            ValkeyClusterNode targetNode =
                    clusterConnection.clusterCommands().clusterGetNodeForKey(key.getBytes());
    ...
    void testFlushDbOnSpecificNode() {
        ...
        try {
            ...
            ValkeyClusterNode targetNode =
                    clusterConnection.clusterCommands().clusterGetNodeForKey(key1.getBytes());
            assertThat(targetNode).isNotNull();
    ...
    void testInfoWithSectionOnSpecificNode() {
        for (ValkeyClusterNode node : allNodes) {
            ...
            assertThat(
                            serverInfo.containsKey("valkey_version") || serverInfo.containsKey("redis_version"))
                    .isTrue();
    ...
    void testInfoClusterWideAggregation() {
        ...
        boolean hasNode1Props =
                allInfo.stringPropertyNames().stream().anyMatch(key -> key.startsWith(node1Prefix));
        boolean hasNode2Props =
                allInfo.stringPropertyNames().stream().anyMatch(key -> key.startsWith(node2Prefix));
        boolean hasNode3Props =
                allInfo.stringPropertyNames().stream().anyMatch(key -> key.startsWith(node3Prefix));
    ...
    void testInfoWithSectionClusterWideAggregation() {
        ...
        boolean hasNode1Version =
                serverInfo.stringPropertyNames().stream()
                        .anyMatch(
                                key ->
                                        key.startsWith(node1Prefix)
                                                && (key.contains("valkey_version") || key.contains("redis_version")));
        boolean hasNode2Version =
                serverInfo.stringPropertyNames().stream()
                        .anyMatch(
                                key ->
                                        key.startsWith(node2Prefix)
                                                && (key.contains("valkey_version") || key.contains("redis_version")));
        boolean hasNode3Version =
                serverInfo.stringPropertyNames().stream()
                        .anyMatch(
                                key ->
                                        key.startsWith(node3Prefix)
                                                && (key.contains("valkey_version") || key.contains("redis_version")));
    ...
    void testLastSaveClusterWideAggregation() {
        // Get cluster-wide lastSave (should return most recent timestamp), should be non-null due to
        // prior saves
    ...
    void testMigrateBasic() {
        ...
        try {
            ...
            ValkeyClusterNode sourceNode =
                    clusterConnection.clusterCommands().clusterGetNodeForKey(key.getBytes());
            assertThat(sourceNode).isNotNull();

            // Find a different random master node as target
            final ValkeyClusterNode targetNode =
                    Arrays.stream(allPrimaries).filter(n -> !n.equals(sourceNode)).findFirst().orElseThrow();
    ...
    void testMigrateParameterValidation() {
        ...
        // Test null key
        assertThatThrownBy(
                        () -> clusterConnection.serverCommands().migrate(null, targetNode, 0, null, 5000))
                .isInstanceOf(IllegalArgumentException.class)
                .hasMessageContaining("Key must not be null");

        // Test null target
        assertThatThrownBy(
                        () -> clusterConnection.serverCommands().migrate(key.getBytes(), null, 0, null, 5000))
                .isInstanceOf(IllegalArgumentException.class)
    ...
    void testNodeSpecificOperationsWithInvalidNode() {
        ...
        ValkeyClusterNode invalidNode =
                ValkeyClusterNode.newValkeyClusterNode().listeningAt("192.168.255.255", 9999).build();
    ...
    void testNodeParameterValidation() {
        ...
        assertThatThrownBy(() -> commands.dbSize(nullNode))
                .isInstanceOf(IllegalArgumentException.class);
    ...
    /**
     * Lazy-initialized Backend version detection flag. True if Backend 7.0+, false if Backend 6.x.
     */
    ...
    private boolean isBackend7OrNewer() {
        if (isBackend7OrNewer == null) {
            ...
            String version =
                    info.getProperty("redis_version", info.getProperty("valkey_version", "0.0.0"));
            // Parse major version
    ...
    /**
     * Gets the appropriate command name format based on Backend version. Backend 7.0+: returns
     * "command|subcommand" format (e.g., "config|get") Backend 6.x: returns parent command only
     * (e.g., "config")
     *
    ...
    private long getCommandCallCount(ValkeyClusterNode node, String commandName) {
        ...
        throw new IllegalStateException(
                String.format(
                        "Failed to parse 'calls' field from command stats for '%s'. Value: %s",
