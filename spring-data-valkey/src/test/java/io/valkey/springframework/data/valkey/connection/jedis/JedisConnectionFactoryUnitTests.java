...
class JedisConnectionFactoryUnitTests {
    ...
    private static final ValkeySentinelConfiguration SINGLE_SENTINEL_CONFIG =
            new ValkeySentinelConfiguration().master("mymaster").sentinel("127.0.0.1", 26379);

    private static final ValkeyClusterConfiguration CLUSTER_CONFIG =
    ...
    void shouldInitJedisPoolWhenNoSentinelConfigPresent() {
        ...
        connectionFactory =
                initSpyedConnectionFactory((ValkeySentinelConfiguration) null, new JedisPoolConfig());
        connectionFactory.afterPropertiesSet();
        connectionFactory.start();

        verify(connectionFactory, times(1)).createValkeyPool();
        verify(connectionFactory, never())
                .createValkeySentinelPool(any(ValkeySentinelConfiguration.class));
    }
    ...
    void shouldInitConnectionCorrectlyWhenClusterConfigPresent() {
        ...
        verify(connectionFactory, times(1))
                .createCluster(eq(CLUSTER_CONFIG), any(GenericObjectPoolConfig.class));
        verify(connectionFactory, never()).createValkeyPool();
    }
    ...
    void shouldReadStandalonePassword() {
        ...
        connectionFactory =
                new JedisConnectionFactory(envConfig, JedisClientConfiguration.defaultConfiguration());
    ...
    void shouldApplyClientConfiguration() throws NoSuchAlgorithmException {
        ...
        connectionFactory =
                new JedisConnectionFactory(new ValkeyStandaloneConfiguration(), configuration);
        ...
        assertThat(connectionFactory.getTimeout())
                .isEqualTo((int) Duration.of(5, ChronoUnit.DAYS).toMillis());
    ...
    void shouldReturnStandaloneConfiguration() {
        ...
        connectionFactory =
                new JedisConnectionFactory(configuration, JedisClientConfiguration.defaultConfiguration());
    ...
    void shouldDenyChangesToImmutableClientConfiguration() throws NoSuchAlgorithmException {
        ...
        connectionFactory =
                new JedisConnectionFactory(
                        new ValkeyStandaloneConfiguration(), JedisClientConfiguration.defaultConfiguration());
    ...
    void clientConfigurationAppliesCustomizer() {
        ...
        JedisClientConfig resp3Config =
                apply(
                        JedisClientConfiguration.builder()
                                .customize(DefaultJedisClientConfig.Builder::resp3)
                                .build());
        ...
        JedisClientConfig resp2Config =
                apply(
                        JedisClientConfiguration.builder()
                                .customize(it -> it.protocol(RedisProtocol.RESP2))
                                .build());
    ...
    private JedisConnectionFactory initSpyedConnectionFactory(
            ValkeySentinelConfiguration sentinelConfiguration, @Nullable JedisPoolConfig poolConfig) {

        Pool<Jedis> poolMock = mock(Pool.class);
        // we have to use a spy here as jedis would start connecting to valkey sentinels when the pool
        // is created.
        JedisConnectionFactory connectionFactorySpy =
                spy(new JedisConnectionFactory(sentinelConfiguration, poolConfig));

        doReturn(poolMock)
                .when(connectionFactorySpy)
                .createValkeySentinelPool(any(ValkeySentinelConfiguration.class));
    ...
    private JedisConnectionFactory initSpyedConnectionFactory(
            ValkeyClusterConfiguration clusterConfiguration, @Nullable JedisPoolConfig poolConfig) {

        JedisCluster clusterMock = mock(JedisCluster.class);

        JedisConnectionFactory connectionFactorySpy =
                spy(new JedisConnectionFactory(clusterConfiguration, poolConfig));

        doReturn(clusterMock)
                .when(connectionFactorySpy)
                .createCluster(any(ValkeyClusterConfiguration.class), any(GenericObjectPoolConfig.class));
