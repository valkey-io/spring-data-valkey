...
class JedisConnectionFactoryUnitTests {
	...
	void shouldInitRedisClusterConnectionCorrectlyWhenClusterConfigPresent() {
		...
		doReturn(clusterMock).when(connectionFactory).createCluster(any(ValkeyClusterConfiguration.class),
				any(GenericObjectPoolConfig.class));

		doReturn(null).when(connectionFactory).createValkeyPool();
	...
	void shouldDetectSentinelConfiguration() {
		...
		assertThat(connectionFactory.isValkeyClusterAware()).isFalse();
	}
	...
	void shouldDetectClusterConfiguration() {
		...
		assertThat(connectionFactory.isValkeySentinelAware()).isFalse();
