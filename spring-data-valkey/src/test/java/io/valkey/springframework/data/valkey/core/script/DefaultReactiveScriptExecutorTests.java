...
public class DefaultReactiveScriptExecutorTests {
    ...
    static void setUp() {
        ...
        stringScriptExecutor =
                new DefaultReactiveScriptExecutor<>(connectionFactory, ValkeySerializationContext.string());
    }
    ...
    void shouldReturnLong() {
        ...
        script.setLocation(
                new ClassPathResource("io/valkey/springframework/data/valkey/core/script/increment.lua"));
        script.setResultType(Long.class);

        stringScriptExecutor
                .execute(script, Collections.singletonList("mykey"))
    ...
    void shouldReturnBoolean() {
        ...
        ValkeySerializationContextBuilder<String, Long> builder =
                ValkeySerializationContext.newSerializationContext(StringValkeySerializer.UTF_8);
        builder.value(new GenericToStringSerializer<>(Long.class));

        DefaultValkeyScript<Boolean> script = new DefaultValkeyScript<>();
        script.setLocation(
                new ClassPathResource("io/valkey/springframework/data/valkey/core/script/cas.lua"));
        script.setResultType(Boolean.class);

        ReactiveScriptExecutor<String> scriptExecutor =
                new DefaultReactiveScriptExecutor<>(connectionFactory, builder.build());

        stringTemplate.opsForValue().set("counter", "0");

        scriptExecutor
                .execute(script, Collections.singletonList("counter"), Arrays.asList(0, 3))
                .as(StepVerifier::create)
                .expectNext(true)
                .verifyComplete();
    ...
    void shouldApplyCustomArgsSerializer() {
        ...
        script.setLocation(
                new ClassPathResource("io/valkey/springframework/data/valkey/core/script/bulkpop.lua"));
        script.setResultType(List.class);

        stringTemplate.boundListOps("mylist").leftPushAll("a", "b", "c", "d");

        Flux<List<String>> mylist =
                stringScriptExecutor.execute(
                        script,
                        Collections.singletonList("mylist"),
                        Collections.singletonList(1L),
                        ValkeyElementWriter.from(new GenericToStringSerializer<>(Long.class)),
    ...
    void testExecuteMixedListResult() {
        ...
        script.setLocation(
                new ClassPathResource(
                        "io/valkey/springframework/data/valkey/core/script/popandlength.lua"));
        script.setResultType(List.class);

        stringScriptExecutor
                .execute(script, Collections.singletonList("mylist"))
                .as(StepVerifier::create)
                .expectNext(Arrays.asList(null, 0L))
                .verifyComplete();
    ...
    void shouldReturnStatusValue() {
        ...
        StepVerifier.create(
                        scriptExecutor.execute(
                                script, Collections.singletonList("foo"), Collections.singletonList(3L)))
                .expectNext("OK")
                .verifyComplete();
    ...
    void shouldApplyCustomResultSerializer() {
        ...
        Jackson2JsonValkeySerializer<Person> personSerializer =
        ...
        script.setScriptSource(
                new StaticScriptSource("redis.call('SET',KEYS[1], ARGV[1])\nreturn 'FOO'"));
        script.setResultType(String.class);
        ...
        Flux<String> result =
                stringScriptExecutor.execute(
                        ...
                        ValkeyElementWriter.from(personSerializer),
