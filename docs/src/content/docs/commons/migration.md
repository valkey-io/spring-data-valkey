...
Spring Data Valkey is a fork of Spring Data Redis, currently aligned with upstream version 4.1.0, created to provide first-class support for Valkey database. To migrate from Spring Data Redis to Spring Data Valkey, see the comprehensive [Migration Guide](https://github.com/valkey-io/spring-data-valkey/blob/main/MIGRATION.md) in the Spring Data Valkey repository.

The migration guide covers:

- **Dependency Changes** - Updated Maven/Gradle dependencies for Spring Boot and vanilla Spring
- **Package Name Changes** - Spring Data packages: `org.springframework.data.redis.*` → `io.valkey.springframework.data.valkey.*`, Spring Boot packages: `org.springframework.boot.*.redis.*` → `io.valkey.springframework.boot.*.valkey.*`
