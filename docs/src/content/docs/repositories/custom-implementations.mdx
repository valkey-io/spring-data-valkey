...
description: Guide to creating custom repository implementations including individual customizations, fragments, and base repository customization
---
...
Fragments are the base repository, functional aspects (such as [Querydsl](/repositories/core-extensions#querydsl)), and custom interfaces along with their implementations.
...
If your implementation fragment bean needs special wiring, you can declare the bean and name it according to the conventions described in the [preceding section](#resolution-of-ambiguity).
...
As already mentioned in the [Configuration](#configuration) section, the infrastructure only auto-detects fragments within the repository base-package.
...
*Example 4. Expose Repository Metadata*

<Tabs>
<TabItem label="Marker Interface">
...
<TabItem label="Bean Post Processor">
...
The approach described in the [preceding section](#manual-wiring) requires customization of each repository interfaces when you want to customize the base repository behavior so that all repositories are affected.
...
*Example 5. Configuring a custom repository base class*
